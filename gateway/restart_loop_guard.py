...
Defenses 1 and 2 (the ``_LUCIFEX_GATEWAY`` guard on ``lucifex gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.lucifex.gateway")``,
...
State lives in ``<LUCIFEX_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_lucifex_home() / "gateway" / "restart_loop.json"
