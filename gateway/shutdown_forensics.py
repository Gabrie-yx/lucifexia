...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if lucifex_home_str:
            takeover_path = Path(lucifex_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(lucifex_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded lucifex-agent but never re-ran ``lucifex setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../lucifex-gateway.service"
    ...
    # that's the common case for lucifex.
    ...
    # constant in lucifex_cli/gateway.py.
    headroom = 30.0
