...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    has_gateway_entry = (
        ...
        or any(t.rsplit("/", 1)[-1] in ("hermes", "lucifex.exe") for t in tokens)
    )
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    ...
    ``profiles``).  The root/default home (``~/.hermes`` or ``$LUCIFEX_HOME``)
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # services share the same default ~/.hermes (LUCIFEX_HOME not set
