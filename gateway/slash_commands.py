...
Module-level run.py helpers a handler needs (``_lucifex_home``,
...
from lucifex_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: lucifex gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Lucifex commands.
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from lucifex_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`lucifex gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _lucifex_home
        ...
        try:
            ...
            atomic_json_write(
                _lucifex_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _lucifex_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Lucifex Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from lucifex_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _lucifex_home, _load_gateway_config
        import yaml
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _lucifex_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from lucifex_cli.context_switch_guard import (
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Lucifex default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_lucifex_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from lucifex_cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _lucifex_home / "config.yaml"
            if _cfg_path.exists():
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        from lucifex_constants import VALID_REASONING_EFFORTS
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _lucifex_home
        from lucifex_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.lucifex/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``lucifex skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _lucifex_home
        from lucifex_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `lucifex skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.lucifex/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _lucifex_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _lucifex_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from lucifex_cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `lucifex bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``lucifex debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Lucifex Agent to the latest version.

        Spawns ``lucifex update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``lucifex update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _lucifex_home, _resolve_lucifex_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from lucifex_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Lucifex Agent')}"
        ...
        if not lucifex_cmd:
            return t("gateway.update.lucifex_cmd_not_found")
        ...
        # Spawn `lucifex update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `lucifex update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # lucifex_cmd is a list of argv parts we can pass directly
                ...
                lucifex_cmd_str = " ".join(shlex.quote(part) for part in lucifex_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {lucifex_cmd_str} update --gateway"
