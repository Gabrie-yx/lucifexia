...
There are two ways to add a platform to the Lucifex gateway:
...
**zero changes to core Lucifex code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lucifex gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lucifex_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lucifex core.
...
And add it to the `lucifex-gateway` composite:

```python
"lucifex-gateway": {
    "includes": [..., "lucifex-your-platform"]
}
```
...
## 12. Status Display (`lucifex_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lucifex_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lucifex_cli/ toolsets.py \
  --include="*.py" -l | sort -u
