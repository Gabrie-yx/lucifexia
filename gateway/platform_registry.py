...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``lucifex setup`` display).
    ...
    # built-ins).  Used by ``lucifex gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `lucifex`
        # invocation -- including plain `lucifex chat`, which never touches any
        ...
        # `lucifex setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``lucifex setup``/``gateway status``, channel
