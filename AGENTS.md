...
`$HOME/.hermes/lucifex-agent/venv` (for worktrees that share a venv with the
main checkout).
...
The dashboard embeds the real `hermes --tui` — **not** a rewrite.  See `lucifex_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `lucifex_cli/web_server.py`.
...
**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_lucifex_home()` for the base directory — never `Path.home() / ".hermes"`. This ensures each profile gets its own state.
...
| `load_config()` | `hermes tools`, `hermes setup`, most CLI subcommands | `lucifex_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
`PluginManager` discovers plugins from `~/.lucifex/plugins/`, `./.hermes/plugins/`,
...
- **CLI:** `lucifex_cli/curator.py` wires `hermes curator <verb>` where
...
- **CLI:** `lucifex_cli/kanban.py` wires `hermes kanban` with verbs
...
2. **Use `display_lucifex_home()` for user-facing messages.** Import from `lucifex_constants`.
   This returns `~/.hermes` for default or `~/.lucifex/profiles/<name>` for profiles.
   ```python
   # GOOD
...
4. **Tests that mock `Path.home()` must also set `LUCIFEX_HOME`** — since code now uses
   `get_lucifex_home()` (reads env var), not `Path.home() / ".hermes"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"LUCIFEX_HOME": str(tmp_path / ".hermes")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not LUCIFEX_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".hermes" / "profiles"`, NOT `get_lucifex_home() / "profiles"`.
