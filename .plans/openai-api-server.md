# OpenAI-Compatible API Server for Lucifex Agent
...
SSE streaming. By exposing this endpoint, lucifex-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  lucifex-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.lucifex/.env`
2. Run `lucifex gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with lucifex-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns lucifex-agent as a model) |
...
Authorization: Bearer lucifex-api-key-here
...
But lucifex-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "lucifex-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured lucifex-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of lucifex-agent's core prompt
...
Once implemented, lucifex-agent works as a drop-in backend for:
