...
When running Lucifex inside Docker, the default `network_mode: host` gives the
...
The Lucifex [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ lucifex-agent │   │ lucifex-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   LUCIFEX_UID=$(id -u) LUCIFEX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=lucifex,lucifex-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://lucifex-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Lucifex trust model and vulnerability reporting
