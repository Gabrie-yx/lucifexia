...
Lucifex middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Lucifex'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Lucifex calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Lucifex retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Lucifex guardrails,
...
- `telemetry_schema_version`: currently `lucifex.observer.v1`
- `middleware_schema_version`: currently `lucifex.middleware.v1`
...
Lucifex stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Lucifex runs
them as a nested chain in registration order. Middleware failures are fail-open:
Lucifex logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Lucifex applies middleware in this order:
...
For each tool call, Lucifex applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Lucifex pre-execution path against the effective arguments:
...
```bash
lucifex plugins enable <plugin-name>
```

For isolated local testing, use one `LUCIFEX_HOME` for plugin enablement and the
agent run:

```bash
export LUCIFEX_HOME=/tmp/lucifex-middleware-test
mkdir -p "$LUCIFEX_HOME"
lucifex plugins enable <plugin-name>
lucifex chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run lucifex plugins enable <plugin-name>
uv run lucifex chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["lucifex_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/lucifex-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Lucifex expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Lucifex treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Lucifex preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Lucifex does not convert downstream
