# Lucifex Observer Hooks

Lucifex observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Lucifex' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Lucifex catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Lucifex only builds
