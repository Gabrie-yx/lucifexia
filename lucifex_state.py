#!/usr/bin/env python3
"""
SQLite State Store for Lucifex Agent.
...
DEFAULT_DB_PATH = get_lucifex_home() / "state.db"
...
# lucifex_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``lucifex_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see lucifex_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_lucifex_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple lucifex processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``lucifex_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `lucifex update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._lucifex_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._lucifex_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _backfill_gateway_metadata_from_sessions_json(
        ...
        """
        sessions_file = get_lucifex_home() / "sessions" / "sessions.json"
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: lucifex_state already depends on agent.* (see
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Lucifex surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``lucifex sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Lucifex, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Lucifex session.

        A Lucifex session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Lucifex session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``LUCIFEX_DISABLE_FTS_TRIGRAM`` or not yet created), so
