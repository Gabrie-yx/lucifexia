...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``lucifex://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``lucifex://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"lucifex://blueprint/{quote(blueprint.key)}{qs}"
