...
``lucifex gateway restart`` (or ``launchctl kickstart ai.lucifex.gateway``
or ``systemctl restart lucifex-gateway``).  When the cron fires, the
...
``lucifex cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``lucifex gateway``, ``launchctl ... lucifex-gateway``,
``systemctl ... lucifex-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_LUCIFEX_GATEWAY=1``, and
``lucifex gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `lucifex gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:lucifex\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a lucifex-gateway label. macOS launchd
    # labels look like `ai.lucifex.gateway` / `lucifex-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated lucifex services (e.g.
    # `launchctl unload ai.lucifex.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\blucifex[.\-]?gateway)"
    # Branch C: systemctl ops on a lucifex-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\blucifex[.\-]?gateway)"
    # Branch D: pkill / kill targeting the lucifex gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    if raw.is_absolute():
        return raw
    return get_lucifex_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `lucifex gateway restart` from a shell outside "
