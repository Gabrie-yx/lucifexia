...
# into one shared `jobs.json` and run them under whatever LUCIFEX_HOME the
...
# `_get_lucifex_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
LUCIFEX_DIR = get_lucifex_home().resolve()
...
CRON_DIR = LUCIFEX_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``lucifex cron status`` process share it
...
# threshold in `lucifex cron status` (lucifex_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_lucifex_home()
       (context-local override, then the LUCIFEX_HOME env var) — so a test
       or embedder that re-points LUCIFEX_HOME after this module was
...
# the cron inactivity timeout is disabled (LUCIFEX_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. LUCIFEX_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``LUCIFEX_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("LUCIFEX_CRON_TIMEOUT", "").strip()
...
def _jobs_lock():
    ...
    and standalone ``lucifex`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Lucifex timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `lucifex_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                lucifex_tz = _lucifex_now().tzinfo
                dt = dt.replace(tzinfo=lucifex_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _lucifex_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Lucifex configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Lucifex timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _lucifex_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Lucifex/system local time
    ...
    if kind == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall lucifex-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `lucifex cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``lucifex cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `lucifex cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _lucifex_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_lucifex_home() / "config.yaml"
...
def create_job(
    ...
    # covered, not just `lucifex cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _lucifex_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _lucifex_now().isoformat(),
        },
    )
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_owner:
                return False
            claim["at"] = _lucifex_now().isoformat()
            save_jobs(jobs)
            return True
    return False
...
def _machine_id() -> str:
    ...
    Uses ``LUCIFEX_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("LUCIFEX_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from LUCIFEX_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one LUCIFEX_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from LUCIFEX_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`lucifex_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``lucifex_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _lucifex_now().strftime("%Y-%m-%d_%H-%M-%S")
