...
LUCIFEX_HOME_DIR="${LUCIFEX_HOME:-$HOME/.lucifex}"
LUCIFEX_CFG="${LUCIFEX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Lucifex config has twozero_td MCP entry ──
if [[ ! -f "$LUCIFEX_CFG" ]]; then
    echo -e " ${FAIL} Lucifex config not found at ${LUCIFEX_CFG}"
    manual_steps+=("Create ${LUCIFEX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LUCIFEX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Lucifex config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Lucifex config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$LUCIFEX_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LUCIFEX_CFG} manually"); }
    manual_steps+=("Restart Lucifex session to pick up config change")
fi
