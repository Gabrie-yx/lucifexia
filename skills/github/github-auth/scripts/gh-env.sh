#!/usr/bin/env bash
# GitHub environment detection helper for Lucifex Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _lucifex_env="${LUCIFEX_HOME:-$HOME/.lucifex}/.env"; [ -f "$_lucifex_env" ] && grep -q "^GITHUB_TOKEN=" "$_lucifex_env" 2>/dev/null; then
