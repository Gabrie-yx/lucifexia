...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hermes_env="${LUCIFEX_HOME:-$HOME/.hermes}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
...
```bash
source "${LUCIFEX_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/gh-env.sh"
