...
Lucifex Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Lucifex works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.
...
This skill is a concise operating guide, not the complete source of truth for every Lucifex feature. If a Lucifex feature, command, or setting is not mentioned here, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets:

- CLI commands: `lucifex --help`, `lucifex <command> --help`, and `lucifex_cli/main.py`
...
# Other surfaces
lucifex desktop                 # launch the native desktop app (alias: lucifex gui)
lucifex dashboard               # web admin panel + embedded chat
lucifex proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
Credentials (OAuth + API keys, with pooling) are managed under `lucifex auth` — see the Credentials & Pools section below.
...
lucifex auth add [PROVIDER]  Add OAuth or API-key credential
...
lucifex secrets bitwarden …  External secret store (Bitwarden Secrets Manager)
lucifex memory setup/status/off  Memory provider config
lucifex send                 Send a one-off message through a gateway platform
...
For the full, authoritative command list run `lucifex --help` (and `lucifex <command> --help`). Plugin- and provider-supplied subcommands (e.g. `lucifex photon setup` for iMessage) only appear once their plugin is installed/active.
...
20+ providers supported. Set via `lucifex model` or `lucifex setup`.
...
Enable/disable via `lucifex tools` (interactive) or `lucifex tools enable/disable NAME`.
...
Lucifex injects project-level instructions into the system prompt by reading context files from the working directory. The discovery order is **first match wins** — only one project context source is loaded per session.

| File (in priority order) | Discovery | Use when |
|---|---|---|
| `.lucifex.md` / `LUCIFEX.md` | Walks parents up to the git root, stops at git root | You want hierarchical project rules (root + per-package overrides) |
| `AGENTS.md` / `agents.md` | **Cwd only** — subdirectory and parent copies are ignored | You want portable agent instructions that work the same in Lucifex, Claude Code, Codex, etc. |
...
- **Use `.lucifex.md`** when you want Lucifex-specific behavior that lives above the cwd (root + subtree), or when you want rules to inherit from a parent directory. The parent walk stops at the git root, so a home-level `.lucifex.md` won't leak into every project (a git repo's root is the boundary).
...
- **Don't put project rules in `~/.lucifex/AGENTS.md`** (or any other home-level location). When Lucifex runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$LUCIFEX_HOME`, identity-only) or install a skill via `lucifex skills install`.
...
`lucifex --ignore-rules` skips auto-injection of all project context files (`.lucifex.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`) **and** `SOUL.md` identity, plus user config, plugins, and MCP servers. Use it to isolate whether a problem is your setup or Lucifex itself.

### Example: a small `.lucifex.md`

```markdown
# My Project

Lucifex: when working in this repo, follow these rules.
...
That file at `/home/me/projects/myrepo/.lucifex.md` is auto-loaded when Lucifex runs in any subdirectory of `/home/me/projects/myrepo`, but not when it runs in `/home/me/other-project`.

## Security & Privacy Toggles

Common "why is Lucifex doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `lucifex` invocation) because they're read once at startup.
...
By default (`approvals.mode: manual`), Lucifex prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `lucifex curator run --consolidate`. Routine background curation costs
  zero tokens.
...
- **Desktop app** (`lucifex desktop` / `lucifex gui`) — native Electron app
...
- **Web dashboard** (`lucifex dashboard`) — full admin panel: configure
  ...
  an embedded `lucifex --tui` chat. Secured behind an OAuth/token gate.
- **OpenAI-compatible proxy** (`lucifex proxy`) — exposes a
...
- **Automation Blueprints** — pick a named automation and Lucifex asks for
...
`lucifex config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Lucifex's env scrubber dropping
...
Lucifex-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
2. `lucifex auth` — re-authenticate OAuth providers (or `lucifex auth add <provider>`)
...
`_LUCIFEX_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_lucifex_home()` for paths,
never hardcode `~/.lucifex`. For custom/local-only tools, write a plugin in
`~/.lucifex/plugins/` instead of editing core — see the developer docs.
