...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Lucifex agent runs by POSTing events to a URL.
...
Add to `${LUCIFEX_HOME:-~/.lucifex}/.env`:
```bash
...
All management is via the `lucifex webhook` CLI command:

### Create a subscription
```bash
lucifex webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.lucifex/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.lucifex/scripts/` (path traversal is blocked).

```bash
lucifex webhook subscribe todoist-lucifex \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-lucifex-label.py" \
...
### GitHub: PR reviews
```bash
lucifex webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
lucifex webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
lucifex webhook subscribe alerts \
...
- Subscriptions persist to `~/.lucifex/webhook_subscriptions.json`

## How It Works

1. `lucifex webhook subscribe` writes to `~/.lucifex/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.lucifex/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `lucifex webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `lucifex webhook test <name>` to verify the route works.
