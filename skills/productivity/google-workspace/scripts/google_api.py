#!/usr/bin/env python3
"""Google Workspace API CLI for Lucifex Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Lucifex-facing JSON contract and falls back to the Python client
...
from _lucifex_home import get_lucifex_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LUCIFEX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Lucifex Agent")
