#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Lucifex Agent.
...
# Ensure sibling modules (_lucifex_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _lucifex_home import display_lucifex_home, get_lucifex_home

LUCIFEX_HOME = get_lucifex_home()
TOKEN_PATH = LUCIFEX_HOME / "google_token.json"
CLIENT_SECRET_PATH = LUCIFEX_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = LUCIFEX_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Lucifex profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Lucifex Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Lucifex Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'lucifex-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Lucifex home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_lucifex_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Lucifex")
