...
3. Store in `${LUCIFEX_HOME:-~/.lucifex}/.env`:
   ```
...
Add those exports to your shell profile (or to `${LUCIFEX_HOME:-~/.lucifex}/.env`) so every session inherits them.
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it via Lucifex' MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
