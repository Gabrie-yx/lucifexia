---
name: sdd-skill
description: Spec-Driven Development (SDD) — guide users through GitHub's Spec-Kit methodology for executable specifications covering greenfield (new project) and brownfield (existing codebase) workflows. Auto-generates structured 10-point summaries after each artifact, tracks feature status, and handles natural-language feature management (add/move/reorder/skip). Adapted from SpillwaveSolutions/sdd-skill v2.1.0.
version: 2.1.0
triggers:
...
> **Lucifex Adaptation Note** — Originally built for Claude Code (`~/.claude/skills/sdd/`).
> Installed path on this machine:
> `C:\Users\gabri\AppData\Local\LUCIFEX\skills\software-development\sdd-skill\`
> Scripts are located at `./scripts/` relative to this SKILL.md.
> The `specify` CLI is the GitHub Spec-Kit installer — install it on demand via `uv tool install specify-cli --from git+https://github.com/github/spec-kit.git` (see [Installation Guide](references/sdd_install.md)). Until then, treat the `/speckit.*` slash commands as **methodological prompts** (the artifact names and steps) — guide the user through each phase manually rather than relying on a slash command handler.
