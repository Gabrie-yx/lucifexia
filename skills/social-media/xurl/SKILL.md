...
> **Docker HOME pitfall:** In the official lucifex Docker layout, `/opt/data` is `LUCIFEX_HOME`, bulucifex tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` lucifexucifex-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
- lucifex adaptation: reformatted folucifex skill conventions; safety guardrails preserved verbatim.
