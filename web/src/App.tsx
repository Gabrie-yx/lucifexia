...
const SIDEBAR_COLLAPSED_KEY = "lucifexex-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="lucifexex-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="lucifexex-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdatelucifexex = status?.can_updatlucifexifex === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'lucifexex update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "lucifexex update";
    return (
      t.status.updatelucifexexConfirmMessage ??
      `This will run 'lucifexex update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updatelucifexexConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdatelucifexex) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updatelucifexex,
      runningLabel: t.status.updatinglucifexex,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updatelucifexexConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updatelucifexexConfirmTitle ?? `${t.status.updalucifexifex}?`}
    />
    </>
  );
}
