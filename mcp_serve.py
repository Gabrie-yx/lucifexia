"""
Lucifex MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Lucifex-specific extra)

Usage:
    lucifex mcp serve
    lucifex mcp serve --verbose
...
logger = logging.getLogger("lucifex.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LUCIFEX_HOME."""
    try:
        from lucifex_constants import get_lucifex_home
        return get_lucifex_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LUCIFEX_HOME", Path.home() / ".lucifex")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from lucifex_constants import get_lucifex_home
        directory_file = get_lucifex_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LUCIFEX_HOME", Path.home() / ".lucifex")
...
class EventBridge:
    ...
    This is the Lucifex equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from lucifex_constants import get_lucifex_home
            db_file = get_lucifex_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LUCIFEX_HOME", Path.home() / ".lucifex")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Lucifex MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "lucifex",
        instructions=(
            "Lucifex Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Lucifex MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
