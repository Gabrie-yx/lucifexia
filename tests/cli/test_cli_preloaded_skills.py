...
def _make_real_cli(**kwargs):
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return cli_mod.LucifexCLI(**kwargs)
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "LucifexCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None: ("skill prompt", ["lucifex-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="lucifex-agent-dev,github-auth", list_tools=True)

    cli_obj = created["cli"]
    assert cli_obj.system_prompt == "base prompt\n\nskill prompt"
    assert cli_obj.preloaded_skills == ["lucifex-agent-dev", "github-auth"]


def test_main_raises_for_unknown_preloaded_skill(monkeypatch):
    import cli as cli_mod

    monkeypatch.setattr(cli_mod, "LucifexCLI", lambda **kwargs: _DummyCLI(**kwargs))
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["lucifex-agent-dev", "github-auth"]
