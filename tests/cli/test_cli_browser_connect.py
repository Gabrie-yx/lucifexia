...
class TestChromeDebugLaunch:
    ...
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == r"C:\Chrome\chrome.exe"), \
             patch("lucifex_cli.browser_connect._wait_for_browser_debug_ready_or_exit", return_value="ready"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert LucifexCLI._try_launch_chrome_debug(9333, "Windows") is True
    ...
    def test_windows_launch_falls_back_to_common_install_dirs(self, monkeypatch):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", return_value=None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == installed), \
             patch("lucifex_cli.browser_connect._wait_for_browser_debug_ready_or_exit", return_value="ready"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert LucifexCLI._try_launch_chrome_debug(9222, "Windows") is True

        _assert_chrome_debug_cmd(captured["cmd"], installed, 9222)

    def test_manual_command_uses_detected_linux_browser(self):
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: "/usr/bin/chromium" if name == "chromium" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == "/usr/bin/chromium"):
    ...
    def test_linux_candidates_prefer_chrome_before_brave_when_both_exist(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=fake_which), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
    ...
    def test_linux_candidates_prefer_chrome_install_path_before_brave_on_path(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave-browser" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_windows_candidates_prefer_chrome_install_path_before_brave_on_path(self, monkeypatch):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave.exe" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {chrome, brave}):
    ...
    def test_linux_candidates_include_arch_brave_install_path(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", return_value=None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_brave_binary_name(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: brave if name == "brave" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == brave):
            candidates = get_chrome_debug_candidates("Linux")
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_linux_candidates_include_official_brave_and_edge_stable_paths(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", return_value=None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {brave, edge}):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_launch_tries_next_browser_when_first_candidate_fails(self):
        ...
        with patch("lucifex_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("lucifex_cli.browser_connect._wait_for_browser_debug_ready_or_exit", return_value="ready"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert LucifexCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_wait_for_browser_debug_ready_or_exit_detects_early_exit(self, monkeypatch):
        ...
        monkeypatch.setattr("lucifex_cli.browser_connect.time.sleep", lambda _seconds: None)
        with patch("lucifex_cli.browser_connect.is_browser_debug_ready", return_value=False):
    ...
    def test_launch_tries_next_browser_when_first_candidate_exits_before_debug_ready(self):
        ...
        with patch("lucifex_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("lucifex_cli.browser_connect._wait_for_browser_debug_ready_or_exit", side_effect=["exited", "ready"]), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert LucifexCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_launch_result_hints_singleton_forward_on_clean_exit(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "lucifex_cli.browser_connect.chrome_debug_data_dir", lambda: str(tmp_path)
        )
        with patch("lucifex_cli.browser_connect.get_chrome_debug_candidates", return_value=[chrome]), \
             patch("lucifex_cli.browser_connect.is_browser_debug_ready", return_value=False), \
    ...
    def test_launch_result_no_hint_when_no_candidates(self):
        with patch("lucifex_cli.browser_connect.get_chrome_debug_candidates", return_value=[]):
            result = launch_chrome_debug(9222, "Linux")
    ...
    def test_manual_command_uses_wsl_windows_chrome_when_available(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", return_value=None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_manual_command_uses_windows_quoting_on_windows(self):
        ...
        with patch("lucifex_cli.browser_connect.shutil.which", side_effect=lambda name: chrome if name == "chrome.exe" else None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
    ...
    def test_manual_command_returns_none_when_linux_browser_missing(self):
        with patch("lucifex_cli.browser_connect.shutil.which", return_value=None), \
             patch("lucifex_cli.browser_connect.os.path.isfile", return_value=False):
    ...
    def test_connect_context_note_allows_expected_browser_use(self, monkeypatch):
        ...
        """
        cli = LucifexCLI.__new__(LucifexCLI)
        ...
        with patch(
                 "lucifex_cli.cli_commands_mixin.discover_local_cdp_url",
                 return_value="http://127.0.0.1:9222",
             ), \
             patch("lucifex_cli.cli_commands_mixin.is_browser_debug_ready", return_value=True), \
