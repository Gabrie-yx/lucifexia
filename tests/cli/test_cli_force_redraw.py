...
@pytest.fixture
def bare_cli():
    """A LucifexCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(LucifexCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate LucifexCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
