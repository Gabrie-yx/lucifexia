...
Regression: the old implementation wrote ``lucifex_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``lucifex sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.lucifex/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_lucifex_home(lucifex_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.lucifex/sessions/saved/, not CWD."""
    ...
    assert "lucifex --resume 20260101_120000_abc123" in out, out
