...
minimal ``LucifexCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def lucifex_home(tmp_path, monkeypatch):
    """Isolated LUCIFEX_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".lucifex"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("LUCIFEX_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves LUCIFEX_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal LucifexCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, lucifex_home):
        ...
        with patch("lucifex_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, lucifex_home):
        ...
        with patch("lucifex_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, lucifex_home):
    ...
    def test_no_assistant_message_skipped(self, lucifex_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, lucifex_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, lucifex_home):
        ...
        src = inspect.getsource(LucifexCLI.chat)
