...
Uses a throwaway LUCIFEX_HOME so it never touches ~/.hermes. No real names.
"""
...
def _fresh_home():
    """Point LUCIFEX_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_lucifex_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["LUCIFEX_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER LUCIFEX_HOME is set.
...
# a different get_lucifex_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"LUCIFEX_HOME (throwaway): {HOME}")
