...
def test_root_session_no_compression(db):
    ...
    assert prov["currentLucifexSessionId"] == "root1"
    assert prov["rootLucifexSessionId"] == "root1"
    assert prov["parentLucifexSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_lucifex_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentLucifexSessionId"] == "old"
    assert prov["rootLucifexSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousLucifexSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootLucifexSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootLucifexSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_lucifex_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousLucifexSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"lucifex"}
    assert "sessionProvenance" in meta["lucifex"]
    assert meta["lucifex"]["sessionProvenance"]["currentLucifexSessionId"] == "root1"
