...
Older versions of ``install.sh`` created ``$command_link_dir/lucifex`` as a
symlink to the pip-generated entry point at ``$LUCIFEX_BIN`` (i.e.
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/lucifex\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/lucifex"')
    cat_idx = block.find('cat > "$command_link_dir/lucifex" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/lucifex before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          ...
          local_bin/lucifex       <- symlink → ../venv/bin/lucifex  (old install)
    ...
    requires that, after the run:
      ...
      * ``venv/bin/lucifex`` still contains its original pip-script body
      * ``local_bin/lucifex`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "lucifex"
    ...
    shim_path = command_link_dir / "lucifex"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/lucifex was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/lucifex must be replaced with a regular file, not "
