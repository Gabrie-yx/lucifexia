...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Lucifex is at the active session limit (1/1). "
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["LUCIFEX_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("LUCIFEX_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_VOICE", "1")
    monkeypatch.delenv("LUCIFEX_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("LUCIFEX_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # _get_platform_tools (a non-configurable platform toolset in lucifex-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("LUCIFEX_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown LUCIFEX_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/lucifex-test "}}
    )

    assert server._resolve_model() == "nous/lucifex-test"
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `lucifex --tui -m <model>` sets LUCIFEX_MODEL/LUCIFEX_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("LUCIFEX_MODEL", "one-shot/model")
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("LUCIFEX_MODEL", "seed/model")
    monkeypatch.setenv("LUCIFEX_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("LUCIFEX_MODEL", "nous/lucifex-test")
    monkeypatch.setenv("LUCIFEX_TUI_PROVIDER", "nous")
    monkeypatch.delenv("LUCIFEX_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/lucifex-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    assert server._resolve_startup_runtime() == ("nous/lucifex-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("LUCIFEX_MODEL", "sonnet")
    monkeypatch.delenv("LUCIFEX_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("LUCIFEX_INFERENCE_PROVIDER", raising=False)
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("LUCIFEX_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("LUCIFEX_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["LUCIFEX_GATEWAY_SESSION"] == "1"
    assert server.os.environ["LUCIFEX_EXEC_ASK"] == "1"
    assert server.os.environ["LUCIFEX_INTERACTIVE"] == "1"
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing LUCIFEX_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("LUCIFEX_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "lucifex_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["LUCIFEX_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "LUCIFEX_TUI_PROVIDER" not in os.environ
        assert "LUCIFEX_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("LUCIFEX_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("LUCIFEX_MODEL", raising=False)
    monkeypatch.delenv("LUCIFEX_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["LUCIFEX_TUI_PROVIDER"] == "openai-codex"
        assert "LUCIFEX_MODEL" not in os.environ
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("LUCIFEX_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("LUCIFEX_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["LUCIFEX_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["LUCIFEX_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".lucifex" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.lucifex/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".lucifex" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.lucifex/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".lucifex" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.lucifex/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Lucifex TUI Status" in out
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `lucifex model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `lucifex model` and the classic CLI /model picker.
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``lucifex --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_session_save_writes_under_lucifex_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Lucifex profile
    ...
    Regression: the gateway handler wrote ``lucifex_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Lucifex.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("lucifex_conversation_*.json"))
    ...
    assert payload["model"] == "lucifex-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Lucifex."
