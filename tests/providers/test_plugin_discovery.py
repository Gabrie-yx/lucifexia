...
Verifies that:
 ...
 2. User plugins at $LUCIFEX_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_lucifex_user_provider")
        ):
            del sys.modules[mod]
...
def test_user_plugin_overrides_bundled(tmp_path, monkeypatch):
    ...
    user_gmi = lucifex_home / "plugins" / "model-providers" / "gmi"
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    user_plugin = lucifex_home / "plugins" / "test-model-provider"
