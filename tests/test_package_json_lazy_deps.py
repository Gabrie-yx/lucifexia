...
The root ``package.json`` is installed by ``lucifex update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``lucifex tools`` → Browser Automation → Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``lucifex update`` for
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "`lucifex tools` → Browser Automation → Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `lucifex setup` / `lucifex update` without an explicit "
        "post_setup step."
    )
