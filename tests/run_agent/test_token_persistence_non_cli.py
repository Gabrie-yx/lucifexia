...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    lucifex_state = ModuleType("lucifex_state")
    lucifex_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "lucifex_state", lucifex_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "lucifexex"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "lucifexex"
