...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=lucifex_tz) which shifted
        absolute time when system-local tz != Lucifex tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_lucifex_tz(self):
        """Already-aware datetimes should be normalized to Lucifex tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Lucifex tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Lucifex tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Lucifex is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Lucifex tz
        ...
        # Use a Lucifex timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _lucifex_now's wall time — this would
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Lucifex timezone"
        )
