"""
Tests for timezone support (lucifex_time module + integration points).
...
def _reset_lucifex_time_cache():
    """Reset the lucifex_time module cache (replacement for removed reset_cache)."""
    lucifex_time._cached_tz = None
    lucifex_time._cached_tz_name = None
    lucifex_time._cache_resolved = False


# =========================================================================
# lucifex_time.now() — core helper
# =========================================================================

class TestlucifexexTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_lucifex_time_cache()

    def teardown_method(self):
        _reset_lucifex_time_cache()
        os.environ.pop("lucifex_timeZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["lucifex_timeZONE"] = "Asia/Kolkata"
        result = lucifex_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["lucifex_timeZONE"] = "UTC"
        result = lucifex_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["lucifex_timeZONE"] = "America/New_York"
        result = lucifex_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["lucifex_timeZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="lucifex_time"):
            result = lucifex_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["lucifex_timeZONE"] = "Europe/London"
        tz = lucifex_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["lucifex_timeZONE"] = "Not/A/Timezone"
        tz = lucifex_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When lucifex_timeZONE is set, child process sees TZ env var.
        ...
        #   2) lucifex_timeZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("lucifex_timeZONE=" + os.environ.get("lucifex_timeZONE", "NOT_SET"))'
        )
        ...
        assert "lucifex_timeZONE=NOT_SET" in result["output"], (
            "lucifex_timeZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When lucifex_timeZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("lucifex_timeZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=lucifexex_tz) which shifted
        absolute time when system-local tz != lucifexex tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_lucifexex_tz(self):
        """Already-aware datetimes should be normalized to lucifexex tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in lucifexex tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of lucifexex tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), lucifexex is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when lucifexex tz
        ...
        # Use a lucifexex timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _lucifexex_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["lucifex_timeZONE"] = "Pacific/Midway"  # UTC-11
        _reset_lucifex_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of lucifexex timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["lucifex_timeZONE"] = "US/Eastern"
        _reset_lucifex_time_cache()
