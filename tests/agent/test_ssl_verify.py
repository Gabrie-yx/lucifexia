...
_CA_ENV_VARS = ("LUCIFEX_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE")
...
def test_lucifex_ca_bundle_returns_ssl_context(clean_ca_env, monkeypatch):
    monkeypatch.setenv("LUCIFEX_CA_BUNDLE", certifi.where())
...
def test_missing_ca_bundle_falls_back_to_true(clean_ca_env, monkeypatch):
    monkeypatch.setenv("LUCIFEX_CA_BUNDLE", "/nonexistent/root-ca.pem")
