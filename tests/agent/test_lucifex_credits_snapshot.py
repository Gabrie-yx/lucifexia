"""Tests for build_lucifex_credits_snapshot (L6-A, magnitudes-only)."""

from __future__ import annotations

from agent.account_usage import build_lucifex_credits_snapshot
...
def test_healthy():
    ...
    snap = build_lucifex_credits_snapshot(info)
...
def test_money_rule_no_percent():
    info = _account(
        ...
        subscription=LucifexportalSubscriptionInfo(plan="Pro"),
    )
    snap = build_lucifex_credits_snapshot(info)
    assert snap is not None
...
def test_logged_out():
    ...
    assert build_lucifex_credits_snapshot(info) is None
