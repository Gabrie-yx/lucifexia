...
""",
        ...
        from lucifex_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        from lucifex_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from lucifex_cli.plugins import PluginContext, PluginManifest, PluginManager
