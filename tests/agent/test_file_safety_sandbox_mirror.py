...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.lucifex/…`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Lucifex process never reads
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".lucifex"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".lucifex"
            / Path(inner)
        )
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain Lucifex path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target

        target = tmp_path / ".lucifex" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
        target.write_text("# real SOUL\n")

        assert classify_sandbox_mirror_target(str(target)) is None

    def test_sandboxes_segment_without_home_lucifex_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Lucifex-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_lucifex_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-lucifex`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.lucifex/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestGetSandboxMirrorWarning:
    ...
    def test_mirror_warning_names_mirror_root_and_inner_path(self, tmp_path):
        ...
        assert "sandboxes/docker/default/home/.lucifex" in warn
    ...
    def test_warning_is_defense_in_depth_not_boundary(self, tmp_path):
        ...
        target = (
            tmp_path
            / "sandboxes" / "docker" / "t" / "home" / ".lucifex"
            / "profiles" / "g" / "SOUL.md"
        )
        target.parent.mkdir(parents=True)
        target.write_text("x")
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_lucifex_root_path", lambda: tmp_path)
