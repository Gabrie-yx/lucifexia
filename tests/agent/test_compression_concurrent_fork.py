...
Damien's incident (Discord, 2026-05-28): a long Lucifex session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``lucifex_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "lucifex_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "lucifex_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``lucifex_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = lucifex_state.SessionDB
    monkeypatch.setattr(lucifex_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(lucifex_state, "SessionDB", real_session_db_type)
