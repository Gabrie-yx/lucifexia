"""Tests for agent.portal_tags — Lucifex portal request tag contract."""

from __future__ import annotations


def test_lucifex_client_tag_includes_current_version():
    """The client tag must reflect lucifex_cli.__version__ verbatim."""
    from lucifex_cli import __version__
    from agent.portal_tags import lucifex_client_tag

    assert lucifex_client_tag() == f"client=lucifex-client-v{__version__}"


def test_lucifex_client_tag_format():
    """The client tag has the exact shape Lucifex portal expects."""
    from agent.portal_tags import lucifex_client_tag

    tag = lucifex_client_tag()
    assert tag.startswith("client=lucifex-client-v")
    # No spaces, no commas — single tag value
    assert " " not in tag
    assert "," not in tag


def test_lucifex_portal_tags_contains_product_and_client():
    """Every Lucifex portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import lucifex_client_tag, lucifex_portal_tags

    tags = lucifex_portal_tags()
    assert "product=lucifex-agent" in tags
    assert lucifex_client_tag() in tags
    assert len(tags) == 2


def test_lucifex_portal_tags_returns_fresh_list():
    """Callers mutate the returned list; we must not share state across calls."""
    from agent.portal_tags import lucifex_portal_tags

    a = lucifex_portal_tags()
    a.append("client=test-mutation")
    b = lucifex_portal_tags()
...
def test_lucifex_portal_tags_appends_conversation_when_session_id_given():
    """A session id adds a third, high-cardinality conversation tag."""
    from agent.portal_tags import conversation_tag, lucifex_portal_tags

    tags = lucifex_portal_tags(session_id="sess-42")
    assert "product=lucifex-agent" in tags
    assert conversation_tag("sess-42") in tags
    assert len(tags) == 3


def test_lucifex_portal_tags_omits_conversation_without_session_id():
    """Base tag set stays at two tags when no session id is available."""
    from agent.portal_tags import lucifex_portal_tags

    for empty in (None, ""):
        tags = lucifex_portal_tags(session_id=empty)
        assert len(tags) == 2
...
def test_ambient_context_tags_calls_without_explicit_session_id():
    """set_conversation_context makes bare lucifex_portal_tags() carry the tag.

    This is the mechanism auxiliary calls (compression, titles, vision, MoA
    slots) rely on — they call lucifex_portal_tags() with no argument.
    """
    ...
    assert not any(t.startswith("conversation=") for t in lucifex_portal_tags())
...
def test_ambient_context_wins_over_explicit_session_id():
    ...
    try:
        tags = lucifex_portal_tags(session_id="segment-2")
...
def test_ambient_context_set_none_clears():
    ...
    for empty in (None, ""):
        ...
        try:
            ...
            assert len(lucifex_portal_tags()) == 2
        finally:
            reset_conversation_context(token)
...
def test_ambient_context_propagates_via_thread_context_helper():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            plain = ex.submit(lucifex_portal_tags).result()
            propagated = ex.submit(
                propagate_context_to_thread(lucifex_portal_tags)
            ).result()
    finally:
        reset_conversation_context(token)
...
def test_auxiliary_client_nous_extra_body_uses_helper():
    ...
    assert NOUS_EXTRA_BODY == {"tags": lucifex_portal_tags()}
...
def test_nous_provider_profile_uses_helper():
    ...
    assert body["tags"] == lucifex_portal_tags()
