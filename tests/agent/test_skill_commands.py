...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("LUCIFEX_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"LUCIFEX_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"LUCIFEX_PLATFORM": "discord"}):
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        """``LUCIFEX_SESSION_PLATFORM`` from the gateway session context must
        also trigger a rescan, not just ``LUCIFEX_PLATFORM`` (#14536).

        Exercises the real ContextVar path: the gateway sets the active
        adapter via ``set_session_vars(platform=...)`` and the resolver
        reads it via ``get_session_env``. Setting ``LUCIFEX_SESSION_PLATFORM``
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(self, tmp_path, monkeypatch):
        ...
        def _disabled_skills():
            if os.getenv("LUCIFEX_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
        ...
        with (
            ...
            monkeypatch.setenv("LUCIFEX_PLATFORM", "telegram")
            telegram_commands = dict(get_skill_commands())
            assert "/telegram-only" not in telegram_commands

            # Drop back to no platform scope — bare CLI / cron / RL rollouts.
            monkeypatch.delenv("LUCIFEX_PLATFORM", raising=False)
    ...
    def test_get_skill_commands_does_not_rescan_when_platform_unchanged(self, tmp_path):
        ...
        with (
            ...
            patch.object(sc_mod, "_skill_commands", {}),
            patch.object(sc_mod, "_skill_commands_platform", None),
            patch.dict(os.environ, {"LUCIFEX_PLATFORM": "telegram"}),
        ):
            _make_skill(tmp_path, "shared")
            # Prime the cache.
            get_skill_commands()
    ...
    def test_skill_collides_with_core_command_is_skipped(self, tmp_path):
        """A skill whose auto-generated /command collides with a core Lucifex
...
class TestBuildPreloadedSkillsPrompt:
    ...
    def test_skips_disabled_skill(self, tmp_path, monkeypatch):
        ...
        LUCIFEX_TUI_SKILLS preloading (mirrors the bundle gate, #59156)."""
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
...
class TestTemplateVarSubstitution:
    """``${LUCIFEX_SKILL_DIR}`` and ``${LUCIFEX_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${LUCIFEX_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${LUCIFEX_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_substitutes_session_id_when_available(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            _make_skill(
                tmp_path,
                "sess-templated",
                body="Session: ${LUCIFEX_SESSION_ID}",
            )
            scan_skill_commands()
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${LUCIFEX_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${LUCIFEX_SKILL_DIR}/scripts/foo.js" in msg
