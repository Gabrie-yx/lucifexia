...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Lucifex passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("LUCIFEX_HOME", "/users/alice/.lucifex/profiles/backend-worker")
        monkeypatch.setenv("LUCIFEX_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "LUCIFEX_KANBAN_DB",
            "/users/alice/.lucifex/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.lucifex/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through lucifex_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Lucifex secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "LUCIFEX_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "LUCIFEX_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through lucifex_subprocess_env must not
