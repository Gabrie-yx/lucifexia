...
def clear_kanban_env(monkeypatch):
    for var in ("LUCIFEX_KANBAN_TASK", "LUCIFEX_KANBAN_STOP_NUDGE"):
...
def test_enabled_with_kanban_task(clear_kanban_env):
    clear_kanban_env.setenv("LUCIFEX_KANBAN_TASK", "t_abc")
    assert kanban_stop_nudge_enabled() is True


def test_env_can_disable(clear_kanban_env):
    clear_kanban_env.setenv("LUCIFEX_KANBAN_TASK", "t_abc")
    clear_kanban_env.setenv("LUCIFEX_KANBAN_STOP_NUDGE", "0")
    assert kanban_stop_nudge_enabled() is False
    assert build_kanban_stop_nudge(messages=[]) is None


def test_nudge_when_no_terminal_tool(clear_kanban_env):
    clear_kanban_env.setenv("LUCIFEX_KANBAN_TASK", "t_46be8aa5")
    messages = [
...
# handles it.  See also tests/lucifex_cli/test_kanban_core_functionality.py
