...
to a generic "Your API key was rejected... run lucifex setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # (Lucifex portal has no API key path — auth_type=oauth_device_code only).
    assert "Lucifex portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "lucifex setup".
    assert "lucifex portal" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because Lucifex portal doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
