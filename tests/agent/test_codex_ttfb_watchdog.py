...
def test_ttfb_kills_when_no_stream_event(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", "1")
...
def test_ttfb_default_tolerates_slow_first_event(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", raising=False)
    monkeypatch.delenv("LUCIFEX_CODEX_TTFB_MAX_SECONDS", raising=False)
...
def test_ttfb_high_env_is_capped_for_openai_codex(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", "90")
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_MAX_SECONDS", "1")
...
def test_event_idle_kills_after_first_event_then_silence(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", "10")
    monkeypatch.setenv("LUCIFEX_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "1")
...
def test_ttfb_disabled_via_env_zero(tmp_path, monkeypatch):
    """Setting LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS=0 disables the TTFB watchdog;
    a no-event stall then falls through to the (here, 60s) stale timeout, so a
    short hang is NOT killed by TTFB."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", "0")
...
def test_large_codex_request_strict_ttfb_env_still_reconnects(tmp_path, monkeypatch):
    ...
    with LUCIFEX_CODEX_TTFB_STRICT=1."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_TIMEOUT_SECONDS", "1")
    monkeypatch.setenv("LUCIFEX_CODEX_TTFB_STRICT", "1")
...
def test_large_codex_request_hard_ceiling_reclaims_silent_stall(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    # Real default TTFB threshold (no LUCIFEX_CODEX_TTFB_* override) → for a
    # >10k-token request the no-byte TTFB watchdog is auto-disabled.
    monkeypatch.setenv("LUCIFEX_CODEX_HARD_TIMEOUT_SECONDS", "3")
...
def test_large_codex_request_hard_ceiling_disabled_restores_legacy(tmp_path, monkeypatch):
    """Setting LUCIFEX_CODEX_HARD_TIMEOUT_SECONDS=0 disables the ceiling entirely,
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_HARD_TIMEOUT_SECONDS", "0")
...
def test_large_codex_request_hard_ceiling_caps_raised_stale_floor(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("LUCIFEX_CODEX_HARD_TIMEOUT_SECONDS", "4")
