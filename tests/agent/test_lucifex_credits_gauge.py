"""Tests for the Nous-credits subscription % gauge in build_lucifex_credits_snapshot.
...
from lucifex_cli.lucifex_account import (
    ...
    LucifexportalSubscriptionInfo,
    _subscription_from_payload,
)
from agent.account_usage import build_lucifex_credits_snapshot, render_account_usage_lines
...
def test_gauge_present_with_monthly_credits():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(
...
def test_gauge_90pct():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=220, credits_remaining=22.0),
    ))
...
def test_gauge_debt_clamps_to_100():
    snap = build_lucifex_credits_snapshot(_acct(
        ...
        subscription=LucifexportalSubscriptionInfo(monthly_credits=220, credits_remaining=-5.0),
...
def test_gauge_at_cap_is_zero_used():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=220, credits_remaining=220.0),
    ))
...
def test_no_monthly_credits_falls_back_to_magnitudes():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(plan="Ultra", credits_remaining=-0.79),
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
...
def test_inf_cap_no_window():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=float("inf"), credits_remaining=10.0),
        paid_service_access_info=NousPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_rollover_balance_exceeds_cap_no_window():
    ...
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=220, credits_remaining=300, rollover_credits=80),
...
def test_bool_monthly_credits_no_window():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=True, credits_remaining=1.0),
        paid_service_access_info=NousPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_zero_monthly_credits_no_divzero():
    snap = build_lucifex_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=LucifexportalSubscriptionInfo(monthly_credits=0, credits_remaining=0.0),
        paid_service_access_info=NousPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_failopen_none_and_logged_out():
    assert build_lucifex_credits_snapshot(None) is None
    assert build_lucifex_credits_snapshot(_acct(logged_in=False)) is None
