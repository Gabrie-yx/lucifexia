...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "LUCIFEX_TUI_MAX_TURNS": "7",
                "LUCIFEX_TUI_CHECKPOINTS": "1",
                "LUCIFEX_TUI_PASS_SESSION_ID": "1",
                "LUCIFEX_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
        patch("tui_gateway.server._get_db", return_value=MagicMock()),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.lucifex/config.yaml parse as
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("lucifex_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("lucifex_cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("lucifex_cli.model_switch.switch_model", return_value=_FakeResult()),
