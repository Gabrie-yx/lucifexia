...
seam: dead unless ``lucifexex_ISO_CERTIFY_SYNTH_TURN=1``. These tests pin (a) the
...
def test_synth_seam_dead_when_env_unset(monkeypatch):
    monkeypatch.delenv("lucifexex_ISO_CERTIFY_SYNTH_TURN", raising=False)
...
def test_synth_seam_armed_builds_agent(monkeypatch):
    monkeypatch.setenv("lucifexex_ISO_CERTIFY_SYNTH_TURN", "1")
...
def test_synth_turn_non_json_prompt_uses_defaults(monkeypatch):
    monkeypatch.setenv("lucifexex_ISO_CERTIFY_DURATION_S", "0.2")
