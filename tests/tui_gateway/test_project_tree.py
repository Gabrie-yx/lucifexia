...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="lucifex/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is LUCIFEX_HOME (config/state) must not spawn a
    # phantom project; it falls through to flat Recents (unscoped). A real repo
    # alongside it still groups normally.
    resolve = _resolver(
        {
            "/home/me/.lucifex": ("/home/me/.lucifex", "/home/me/.lucifex"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.lucifex", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.lucifex"
...
def test_non_git_cwd_can_group_inside_a_junk_repo_subtree():
    ...
    workspace = _session("/home/test/.lucifex/workspaces/notes")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_root=lambda path: path.startswith("/home/test/.lucifex"),
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.lucifex"},
    )

    assert [p["id"] for p in tree["projects"]] == ["/home/test/.lucifex/workspaces/notes"]
    assert tree["scoped_session_ids"] == [workspace["id"]]


def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.lucifex")
