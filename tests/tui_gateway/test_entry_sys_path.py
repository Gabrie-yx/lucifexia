...
Lucifex's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    Lucifex root to the front so ``from utils import ...`` resolves to Lucifex."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/lucifex", "/usr/lib"]
        lucifex_bootstrap.harden_import_path(src_root="/opt/lucifex")
        assert sys.path[0] == "/opt/lucifex"
        assert sys.path.index("/opt/lucifex") < sys.path.index(
