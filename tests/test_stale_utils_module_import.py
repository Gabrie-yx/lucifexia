...
2. ``lucifexex update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``lucifex_cli/main.py`` (the ``lucifexex update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``lucifex_constants`` after the
