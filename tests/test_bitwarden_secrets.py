...
binary download are exercised manually by `lucifex secrets bitwarden
...
def lucifex_home(tmp_path, monkeypatch):
    """Point Lucifex at an isolated home directory."""
    ...
    if hasattr(lucifex_constants, "_LUCIFEX_HOME_CACHE"):
        lucifex_constants._LUCIFEX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(lucifex_home, monkeypatch):
...
def test_install_bws_happy_path(lucifex_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(lucifex_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(lucifex_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from lucifex_cli.env_loader import _apply_external_secret_sources
