...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and lucifex-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # lucifex-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_lucifex_home():
    prev = os.environ.get("LUCIFEX_HOME")
    with tempfile.TemporaryDirectory() as d:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("LUCIFEX_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
