...
SESSION_REVOKED / REFRESH_DEAD), the LUCIFEX_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, LUCIFEX_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable lucifex_constants).
        with patch.object(mod, "_LUCIFEX_HOME", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
...
def test_LUCIFEX_HOME_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path / "custom-home"))
    # Block the lucifex_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"lucifex_constants": None}):
        home = mod._LUCIFEX_HOME()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "lucifexex Agent"  # human credited first
