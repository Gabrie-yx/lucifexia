...
SCRIPT_PATH = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_lucifexex.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_lucifexex", SCRIPT_PATH)
...
"""
    ...
    assert "Tyler Williams > Active Projects: lucifexex Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `lucifexex cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # lucifexex_config_mod  — print statements in the post-migration summary that
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "lucifexex_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``lucifexex``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "lucifexex prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told lucifexex to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "lucifex config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "lucifexex uses tools well"
    # All-lowercase matches → lowercase ``lucifexex``; this preserves the
    # real filesystem path ``~/.lucifexex``lucifexifex home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "lucifexex should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "lucifexex remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "lucifexex prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "lucifexex was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "lucifexex uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "lucifexex said to aslucifexifex ablucifexucifex settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.lucifexex`` (the realucifexifex home), not the broken
    ``.lucifexex``.
    ...
    assert mod.rebrand_text("use .openclaw directory") == "use .lucifexex directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.lucifexex'"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "lucifexex"
    target_root.mkdir()
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are lucifexex" in result
