...
"""
    ...
    assert "Tyler Williams > Active Projects: Lucifex Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `lucifex cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # lucifex_config_mod  — print statements in the post-migration summary that
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "lucifex_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Lucifex``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Lucifex prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Lucifex to use dark mode"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Lucifex remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "lucifex prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Lucifex was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "lucifex uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Lucifex said to ask Lucifex about Lucifex settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.lucifex`` (the real Lucifex home), not the broken
    ``.Lucifex``.
    ...
    assert mod.rebrand_text("use .openclaw directory") == "use .lucifex directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.lucifex'"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Lucifex" in result
