...
These guard the contract:
  ...
  * OpenRouter and Lucifex portal are NEVER merged — they keep their curated
...
appear in ``/model`` without a Lucifex release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see lucifex_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("lucifex_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
                "lucifex_cli.auth.resolve_api_key_provider_credentials",
                return_value={
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from lucifex_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("lucifex_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("lucifex_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("lucifex_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "lucifex_cli.models._merge_with_models_dev",
