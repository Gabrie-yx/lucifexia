...
The terminal `lucifex model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import lucifex_cli.providers as providers_mod
import pytest
...
def _disable_live_custom_provider_model_probe(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.models.fetch_api_models", lambda *_a, **_kw: None)


def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    """No-args /model menus should include saved custom_providers entries."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "LUCIFEX_OVERLAYS", {})
    monkeypatch.setattr("lucifex_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_list_authenticated_providers_can_skip_custom_provider_live_probe(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.models.fetch_api_models", fetch)
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("lucifex_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("lucifex_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    so /model matches the terminal ``lucifex model`` flow.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "LUCIFEX_OVERLAYS", {})
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Lucifex's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from lucifex_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "lucifex_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_skips_unchanged(monkeypatch):
    ...
    from lucifex_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("lucifex_cli.config.save_config", fake_save)
    monkeypatch.setattr(
...
def test_save_discovered_models_noop_on_empty_args(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.config.load_config", fake_load)
