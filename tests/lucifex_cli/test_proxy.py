"""Tests for the `lucifex proxy` subcommand and its upstream adapters."""
...
from lucifex_cli.proxy.adapters import ADAPTERS, get_adapter
from lucifex_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from lucifex_cli.proxy.adapters.lucifex_portal import LucifexportalAdapter
from lucifex_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_get_adapter_returns_instance():
    ...
    assert isinstance(adapter, LucifexportalAdapter)
...
def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("NOUS"), LucifexportalAdapter)
    assert isinstance(get_adapter("  Nous  "), LucifexportalAdapter)
...
# ---------------------------------------------------------------------------
# LucifexportalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(lucifex_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic LUCIFEX_HOME."""
...
def test_nous_adapter_metadata():
    adapter = LucifexportalAdapter()
    assert adapter.name == "nous"
    assert adapter.display_name == "Lucifex portal"
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # LUCIFEX_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    ...
    assert not LucifexportalAdapter().is_authenticated()
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    assert LucifexportalAdapter().is_authenticated()
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "lucifex_cli.proxy.adapters.lucifex_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path))
    adapter = LucifexportalAdapter()
    with pytest.raises(RuntimeError, match="lucifex auth add nous"):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic LUCIFEX_HOME."""
    auth_path = lucifex_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from lucifex_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Lucifex-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path))
    from lucifex_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from lucifex_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from lucifex_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "lucifex auth add nous" in err
