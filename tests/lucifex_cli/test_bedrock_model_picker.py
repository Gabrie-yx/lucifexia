...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (LUCIFEX_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via LUCIFEX_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in LUCIFEX_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from lucifex_cli.providers import LUCIFEX_OVERLAYS
        assert "bedrock" in LUCIFEX_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from lucifex_cli.providers import LUCIFEX_OVERLAYS
        assert LUCIFEX_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from lucifex_cli.providers import LUCIFEX_OVERLAYS
        assert LUCIFEX_OVERLAYS["bedrock"].auth_type == "aws_sdk"
