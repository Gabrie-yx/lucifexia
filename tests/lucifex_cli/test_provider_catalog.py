"""Tests for the unified provider catalog (lucifex_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``lucifex model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from lucifex_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_lucifex_model_provider():
    """PARITY CONTRACT: the catalog == the `lucifex model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `lucifex model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_lucifex_model():
    """The catalog must not invent providers `lucifex model` doesn't show."""
