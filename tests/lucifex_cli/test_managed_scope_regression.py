...
def lucifex_home(tmp_path, monkeypatch):
    ...
    # /etc/lucifex on the dev/CI box can't influence the test.
    monkeypatch.setenv("LUCIFEX_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(lucifex_home, monkeypatch):
    from lucifex_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(lucifex_home, monkeypatch):
    from lucifex_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(lucifex_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_lucifex_dotenv(lucifex_home=str(home))
