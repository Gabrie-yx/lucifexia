"""Tests for normalized Lucifex portal account entitlement helpers."""
...
from lucifex_cli.lucifex_account import (
    ...
    lucifex_portal_topup_url,
    reset_lucifex_portal_account_info_cache,
)
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "lucifex-cli",
            "product_id": "nous-lucifex-agent",
            "nous_client": "lucifex-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("lucifex_cli.auth.get_provider_auth_state", lambda provider: _state(token))

    info = get_lucifex_portal_account_info()
    ...
    assert info.product_id == "nous-lucifex-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("lucifex_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("lucifex_cli.lucifex_account._fetch_lucifex_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, LucifexportalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("lucifex_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_returns_none_for_paid_access():
    ...
    assert format_lucifex_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "cannot verify your Lucifex portal paid access" in message
    assert "Log in with `lucifex model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_lucifex_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_lucifex_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `lucifex model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a Lucifex portal account or organisation" in message
...
def test_topup_url_falls_back_to_legacy_when_slug_null():
    ...
    url = lucifex_portal_topup_url(info)
...
def test_topup_url_defaults_to_production_portal_for_none():
    url = lucifex_portal_topup_url(None)
