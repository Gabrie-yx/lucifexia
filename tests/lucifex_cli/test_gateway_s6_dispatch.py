"""Tests for the Phase 4 s6 dispatch helper in lucifex_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`lucifex gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from lucifex_cli import gateway as gw
    monkeypatch.setattr(
        "lucifex_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "lucifex_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "lucifex_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "lucifex_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`lucifex -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from lucifex_cli import gateway as gw
    from lucifex_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "lucifex_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "lucifex profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "lucifex_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "lucifex_cli.service_manager.detect_service_manager",
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("lucifex_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("lucifex_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "lucifex_cli.gateway._block_until_terminated",
    ...
    assert "LUCIFEX_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("lucifex_cli.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "lucifex_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("lucifex_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `lucifex gateway run --replace`, the
    LUCIFEX_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
...
def test_redirect_respects_no_supervise_env(
    ...
    """`LUCIFEX_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("LUCIFEX_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("LUCIFEX_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of LUCIFEX_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `LUCIFEX_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("LUCIFEX_GATEWAY_NO_SUPERVISE", falsy)
