"""Comprehensive tests for lucifex_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and LUCIFEX_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.lucifex/profiles)
    * LUCIFEX_HOME  -> tmp_path/.lucifex  (so get_lucifex_home() agrees)
    * Creates the bare-minimum ~/.lucifex directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["lucifex", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Lucifex install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_lucifex_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".lucifex" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".lucifex" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.lucifex must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".lucifex"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes lucifex-agent, .worktrees,
        ...
        (default_home / "lucifex-agent" / ".git").mkdir(parents=True)
        (default_home / "lucifex-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "lucifex-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "lucifex-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `lucifex profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `lucifex update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `lucifex update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".lucifex" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".lucifex" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("lucifex_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("lucifex_cli.profiles._cleanup_gateway_service"), \
             patch("lucifex_cli.profiles.time.sleep"), \
             patch("lucifex_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("lucifex_cli.profiles._cleanup_gateway_service"), \
             patch("lucifex_cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("lucifex_cli.profiles._cleanup_gateway_service"), \
             patch("lucifex_cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("lucifex_cli.profiles.time.sleep"), \
             patch("lucifex_cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "lucifex_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "lucifex_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "lucifex_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "lucifex_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".lucifex" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_lucifex_home_returns_default(self, profile_env):
        # LUCIFEX_HOME points to tmp_path/.lucifex which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom LUCIFEX_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("LUCIFEX_HOME", str(custom))
        # With Docker-aware roots, a custom LUCIFEX_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("lucifex")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("lucifex_cli.profiles.shutil.which", lambda name: "/opt/lucifex/bin/lucifex")
        from lucifex_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/lucifex/bin/lucifex -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "lucifex -p mybot" in content
        assert "%*" in content

    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from lucifex_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from lucifex_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "lucifex -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from lucifex_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from lucifex_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from lucifex_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".lucifex" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("lucifex_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".lucifex" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".lucifex" / "honcho.json"
        ...
        assert "lucifex.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["lucifex_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["lucifex_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "lucifex.ssi_health": {"workspace": "lucifex", "enabled": True}
            }
        }))
        ...
        assert "lucifex.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["lucifex_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["lucifex_heimdall"]["workspace"] == "lucifex"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "lucifex.ssi_health": {"aiPeer": "ssi_health"},
                "lucifex_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["lucifex.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["lucifex_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("lucifex-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".lucifex_history",
        ...
        excluded_prefixes = [
            "default/lucifex-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.lucifex_history",
    ...
    def test_export_default_uses_allowlist_for_unrelated_dirs(self, profile_env, tmp_path):
        """Unrelated directories under LUCIFEX_HOME are excluded by allow-list (#58394).

        Docker/custom deployments often set LUCIFEX_HOME to a working
        ...
        Lucifex artifacts end up in the archive. Replaces the old
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultLucifexHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_lucifex_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".lucifex" / "profiles"

    def test_default_lucifex_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_lucifex_home()
        assert home == tmp_path / ".lucifex"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (LUCIFEX_HOME outside ~/.lucifex), profiles go under LUCIFEX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_lucifex_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_lucifex_home() returns LUCIFEX_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (LUCIFEX_HOME under ~/.lucifex), profiles root is still ~/.lucifex/profiles."""
        native = tmp_path / ".lucifex"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under LUCIFEX_HOME."""
        from lucifex_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under LUCIFEX_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".lucifex" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from lucifex_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".lucifex"
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="lucifex gateway run --replace",
        ):
            assert _check_gateway_running(default_home) is True
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``lucifex -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".lucifex" / "profiles" / "coder"
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            "gateway.status._read_process_cmdline",
            return_value="lucifex -p coder gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_lucifex_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with LUCIFEX_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("LUCIFEX_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_lucifex_home()
