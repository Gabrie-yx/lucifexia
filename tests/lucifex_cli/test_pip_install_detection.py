...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("lucifex_cli.config.get_managed_system", return_value=None), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When LUCIFEX_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("lucifex_cli.config.get_managed_system", return_value="NixOS"), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=tmp_path):
        from lucifex_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from lucifex_cli.config import recommended_update_command_for_method
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $LUCIFEX_HOME.

    Models a host git install whose $LUCIFEX_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'lucifex update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("lucifex_cli.config.get_managed_system", return_value=None), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=home):
        from lucifex_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $LUCIFEX_HOME. With no code-scoped stamp, a host
    ...
    with patch("lucifex_cli.config.get_managed_system", return_value=None), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=home), \
         patch("lucifex_cli.config._running_in_container", return_value=False):
        from lucifex_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("lucifex_cli.config.get_managed_system", return_value=None), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=home), \
         patch("lucifex_cli.config._running_in_container", return_value=True):
        from lucifex_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $LUCIFEX_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $LUCIFEX_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``lucifex update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("lucifex_cli.config.get_managed_system", return_value=None), \
         patch("lucifex_cli.config.get_lucifex_home", return_value=tmp_path), \
         patch("lucifex_constants.is_container", return_value=True):
        from lucifex_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from lucifex_cli import banner

    hh = tmp_path / ".lucifex"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("lucifex_cli.config.get_lucifex_home", return_value=hh), \
         patch("lucifex_constants.get_lucifex_home", return_value=hh):
        buf = io.StringIO()
