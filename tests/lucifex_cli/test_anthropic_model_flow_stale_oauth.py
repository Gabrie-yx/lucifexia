"""Tests for Bug #12905 fix — stale OAuth token detection in lucifex model flow.

Bug 3: `lucifex model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.lucifex/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("lucifex_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from lucifex_cli.main import _model_flow_anthropic
        cfg = {}
