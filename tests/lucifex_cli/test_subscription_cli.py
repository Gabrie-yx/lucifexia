...
@pytest.fixture
def cli():
    obj = LucifexCLI.__new__(LucifexCLI)  # bypass __init__ (no full app needed)
...
def test_change_flow_schedules_a_downgrade(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus", "yes"), raising=False)
    monkeypatch.setattr(
...
def test_change_flow_upgrade_charges_now(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: st)
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("change", "ultra", "yes"), raising=False)
...
def test_change_menu_cancel_schedules_cancellation(cli, monkeypatch, capsys):
    cli._app = object()
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: _sub_state())
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("cancel_sub", "yes"), raising=False)
    seen = {}
...
def test_pending_change_menu_offers_undo(cli, monkeypatch, capsys):
    ...
    st = _sub_state(pending_downgrade_tier_name="Plus", pending_downgrade_at="2026-07-28")
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: st)
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("keep"), raising=False)
    called = {"n": 0}
...
def test_insufficient_scope_triggers_stepup_then_replays(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus", "yes", "yes"), raising=False)
    ...
    monkeypatch.setattr(auth, "step_up_lucifex_billing_scope", lambda **kw: True, raising=False)
...
def test_stepup_declined_grant_does_not_replay(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(auth, "step_up_lucifex_billing_scope", lambda **kw: False, raising=False)
...
def test_unknown_preview_effect_fails_safe(cli, monkeypatch, capsys):
    ...
    cli._app = object()
    monkeypatch.setattr(sv, "build_subscription_state", lambda *a, **kw: _sub_state())
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("change", "plus"), raising=False)
...
def test_upgrade_transport_failure_is_ambiguous_not_flat_failure(cli, monkeypatch, capsys):
    ...
    def _boom(**kw):
        raise nb.BillingError("Could not reach Lucifex portal", error="endpoint_unavailable")
...
def test_upgrade_transport_failure_still_ambiguous_after_narrowing(cli, monkeypatch, capsys):
    ...
    def _net(**kw):
        raise nb.BillingError("Could not reach Lucifex portal: timeout", error="network_error")
...
def test_upgrade_confirm_names_the_subscription_card(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted_modal("change", "ultra", "cancel"), raising=False)
    monkeypatch.setattr(nb, "post_subscription_preview", lambda **kw: {"effect": "charge_now", "targetTierName": "Ultra", "amountDueNowCents": 4630})
    import agent.billing_view as bv
