"""Tests for IRC gateway configuration via `lucifex setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lucifex install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lucifex install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lucifex")
            monkeypatch.setenv("IRC_NICKNAME", "lucifex-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lucifex setup gateway` experience with IRC present."""
