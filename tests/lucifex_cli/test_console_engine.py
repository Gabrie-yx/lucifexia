...
from lucifex_cli.console_engine import LucifexConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/lucifex-console-test.zip",
    "import /tmp/lucifex-console-test.zip",
...
def test_console_parses_bare_and_lucifex_prefixed_commands(_isolate_lucifex_home):
    engine = LucifexConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("lucifex config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    _isolate_lucifex_home,
):
    import lucifex_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'lucifex doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'lucifex setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = LucifexConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "lucifex doctor" not in result.output
    assert "lucifex setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'lucifex doctor' for detailed diagnostics"))
        print(osc_link("  Run 'lucifex setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    help_text = LucifexConsoleEngine().help_text()
    ...
    assert "Run `lucifex skills list`" not in help_text
    assert "Run `lucifex tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = LucifexConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(LucifexConsoleEngine().commands)
...
def test_hosted_console_registry_exposes_only_hosted_safe_surface():
    engine = LucifexConsoleEngine(context="hosted")
    hosted = {
...
def test_hosted_console_rejects_local_only_or_dangerous_commands(line):
    result = LucifexConsoleEngine(context="hosted").execute(line)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = LucifexConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = LucifexConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_lucifex_home):
    engine = LucifexConsoleEngine()
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_lucifex_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_lucifex_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_lucifex_home):
    ...
    assert code == 0
    assert "Lucifex Console" in stdout.getvalue()
    assert "lucifex>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_repl_refuses_non_interactive_confirmation(_isolate_lucifex_home):
...
def test_main_console_subcommand_smoke(_isolate_lucifex_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "lucifex_cli.main", "console"],
    ...
    assert "Lucifex Console" in result.stdout
