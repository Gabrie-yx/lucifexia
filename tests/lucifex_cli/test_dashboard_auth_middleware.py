...
from lucifex_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    ...
    assert any("lucifex_session_at" in c for c in set_cookies)
    assert any("lucifex_session_rt" in c for c in set_cookies)
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``lucifex_session_at`` / ``lucifex_session_rt``
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("lucifex_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("lucifex_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
...
class _UnreachableProvider(StubAuthProvider):
    ...
    provider first, which tries to reach Lucifex portal's JWKS; if that's
    ...
    def verify_session(self, *, access_token: str):
        from lucifex_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["lucifex_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
