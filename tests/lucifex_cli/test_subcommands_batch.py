...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="lucifex")
...
def test_config_get_unset_subcommands_parse():
    """`lucifex config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="lucifex")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `lucifex login` fails gracefully, not with argparse error ────
#
# `lucifex login` is a removed command; its handler (`login_command` in
# `lucifex_cli/auth.py`) prints a deprecation notice pointing at `lucifex auth` /
# `lucifex model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `lucifex --help`.
