"""Unit tests for lucifex_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/lucifex_cli/test_pty_bridge.py.  Drives
...
``lucifex_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from lucifex_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from lucifex_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo lucifex-ok"])
        try:
            output = _read_until(bridge, b"lucifex-ok")
            assert b"lucifex-ok" in output
        finally:
            bridge.close()
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from lucifex_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from lucifex_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('LUCIFEX_PTY_TEST=' + os.environ.get('LUCIFEX_PTY_TEST',''))",
            ],
            env={**os.environ, "LUCIFEX_PTY_TEST": "pty-env-works"},
        )
        try:
