...
from lucifex_cli import gateway as gateway_cli
...
def test_positive_watchdog_config_generates_notify_system_unit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda _user: ("lucifex", "lucifex", str(tmp_path)),
    )

    unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="lucifex")
...
def test_system_unit_reads_watchdog_from_target_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_HOME", str(caller_home))
    monkeypatch.setattr(
        gateway_cli,
        "_system_service_identity",
...
def test_managed_watchdog_override_controls_generated_unit(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
    monkeypatch.setenv("LUCIFEX_MANAGED_DIR", str(managed_home))
