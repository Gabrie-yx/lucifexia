"""Regression tests for the Lucifex portal env-override bypassing the host
...
``LUCIFEX_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` in its
...
``_LUCIFEX_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_lucifex_oauth_state`` and wiped the entire credential pool.
...
from lucifex_cli.auth import (
    ...
    _LUCIFEX_PORTAL_ALLOWED_HOSTS,
    _lucifex_portal_env_override,
)


class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("LUCIFEX_PORTAL_BASE_URL", raising=False)
        assert _lucifex_portal_env_override() is None

    def test_lucifex_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "LUCIFEX_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com/"
        )
        monkeypatch.delenv("LUCIFEX_PORTAL_BASE_URL", raising=False)
        assert (
            _lucifex_portal_env_override() == "https://portal.staging-nousresearch.com"
        )

    def test_lucifex_portal_base_url_used_as_fallback(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_PORTAL_BASE_URL", raising=False)
        monkeypatch.setenv(
            "LUCIFEX_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com"
        )
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        _LUCIFEX_PORTAL_ALLOWED_HOSTS, and the helper must return it anyway —
        gating happens only for network-provenance values."""
        monkeypatch.setenv(
            "LUCIFEX_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com"
        )
        assert "portal.staging-nousresearch.com" not in _LUCIFEX_PORTAL_ALLOWED_HOSTS
        assert (
            _lucifex_portal_env_override() == "https://portal.staging-nousresearch.com"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("lucifex_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior LUCIFEX_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        monkeypatch.setenv("LUCIFEX_PORTAL_BASE_URL", staging_portal)
    ...
    def test_env_override_wins_over_prod_state(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path))
        monkeypatch.setenv("LUCIFEX_PORTAL_BASE_URL", staging_portal)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_LUCIFEX_PORTAL_URL)
    ...
    def test_no_env_override_stored_staging_host_heals_to_prod(
        ...
        assert seen_portal_urls == [DEFAULT_LUCIFEX_PORTAL_URL]
