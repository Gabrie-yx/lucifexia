...
def _client():
    ...
    # Keep the state DB under the isolated LUCIFEX_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_lucifex_home):
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.lucifex/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `lucifex auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `lucifex auth add`, which clears every suppression for the
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from lucifex_cli.auth import _load_auth_store, read_credential_pool
        from lucifex_cli.config import save_env_value
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_lucifex_home):
        ...
        (get_lucifex_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/lucifex-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/lucifex-test.zip"],
            "name": "backup",
        }
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "lucifex_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("lucifex-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # lucifex-index source advertises availability + featured search.
            idx = _Src("lucifex-index")
        ...
        assert {"official", "github", "lucifex-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "lucifex_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/lucifex/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/lucifex/update`` runs ``lucifex update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import lucifex_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/lucifex/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/lucifex/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import lucifex_cli.debug as dbg
        ...
        monkeypatch.setattr("lucifex_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `lucifex tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_lucifex_action", _fake_spawn)
        r = self.client.post(
...
# ---------------------------------------------------------------------------
# _spawn_lucifex_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_lucifex_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _LUCIFEX_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_lucifex_action(["gateway", "restart"], "gateway-restart")

    assert "_LUCIFEX_GATEWAY" not in captured["env"]
    assert captured["env"]["LUCIFEX_NONINTERACTIVE"] == "1"
