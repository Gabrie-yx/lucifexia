"""Tests for lucifex backup and import commands."""
...
def _make_lucifex_tree(root: Path) -> None:
    ...
    # lucifex-agent repo (should be EXCLUDED)
    (root / "lucifex-agent").mkdir(exist_ok=True)
    (root / "lucifex-agent" / "run_agent.py").write_text("# big file\n")
    (root / "lucifex-agent" / ".git").mkdir()
    (root / "lucifex-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_lucifex_agent(self):
        from lucifex_cli.backup import _should_exclude
        assert _should_exclude(Path("lucifex-agent/run_agent.py"))
        assert _should_exclude(Path("lucifex-agent/.git/HEAD"))
    ...
    def test_includes_nested_lucifex_agent_in_skills(self):
        """skills/autonomous-ai-agents/lucifex-agent/ must NOT be excluded —
        only the root-level lucifex-agent/ repo is skipped."""
        from lucifex_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/lucifex-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/lucifex-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under LUCIFEX_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        lucifex_home.mkdir()
        _make_lucifex_tree(lucifex_home)

        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        # get_default_lucifex_root needs this
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = lucifex_home / "backups" / "pre-update-test.zip"
    ...
    def test_excludes_lucifex_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include lucifex-agent/ directory."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        _make_lucifex_tree(lucifex_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "lucifex-agent" in n]
            assert agent_files == [], f"lucifex-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under LUCIFEX_HOME must be
        ...
        venv_pkg = lucifex_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = lucifex_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_lucifex_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../lucifex-agent/ but NOT root lucifex-agent/."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        _make_lucifex_tree(lucifex_home)

        # Add a nested lucifex-agent directory inside skills (like the real layout)
        nested = lucifex_home / "skills" / "autonomous-ai-agents" / "lucifex-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Lucifex Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root lucifex-agent must be excluded
            root_agent = [n for n in names if n.startswith("lucifex-agent/")]
            assert root_agent == [], f"root lucifex-agent leaked: {root_agent}"
            # Nested skill lucifex-agent must be included
            assert "skills/autonomous-ai-agents/lucifex-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/lucifex-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/lucifex-backup-*.zip."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("lucifex-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside LUCIFEX_HOME."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        _make_lucifex_tree(lucifex_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(lucifex_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Lucifex backup."""
        from lucifex_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only lucifex_state.db (old wrong name) is rejected."""
        from lucifex_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["lucifex_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into lucifex home."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (lucifex_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (lucifex_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (lucifex_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (lucifex_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_lucifex_prefix(self, tmp_path, monkeypatch):
        """Import strips .lucifex/ prefix if all entries share it."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".lucifex/config.yaml": "model: test\n",
            ".lucifex/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (lucifex_home / "config.yaml").read_text() == "model: test\n"
        assert (lucifex_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_lucifex_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a lucifex backup."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (lucifex_home / "config.yaml").exists()
        # traversal file should NOT exist outside lucifex home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        disconnecting it from the Lucifex portal (NS-508). The live file wins.
        """
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        ...
        (lucifex_home / "gateway_state.json").write_text(live_state)
        ...
        assert (lucifex_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (lucifex_home / "config.yaml").exists()
        assert not (lucifex_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        (lucifex_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (lucifex_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (lucifex_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            lucifex_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (lucifex_home / "gateway.pid").write_text("4242")
        (lucifex_home / "processes.json").write_text('{"live": true}')
        ...
        assert (lucifex_home / "gateway.pid").read_text() == "4242"
        assert (lucifex_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (lucifex_home / "cron.pid").exists()
        assert not (lucifex_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (lucifex_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (lucifex_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (lucifex_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (lucifex_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".lucifex"
        src_home.mkdir(parents=True)
        _make_lucifex_tree(src_home)

        monkeypatch.setenv("LUCIFEX_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from lucifex_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".lucifex"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("LUCIFEX_HOME", str(dst_home))
        ...
        # lucifex-agent should NOT be present
        assert not (dst_home / "lucifex-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from lucifex_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without lucifex markers fails validation."""
        import io
        from lucifex_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_lucifex(self):
        """Detects .lucifex/ prefix wrapping all entries."""
        import io
        from lucifex_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".lucifex/config.yaml", "test")
            zf.writestr(".lucifex/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".lucifex/"
...
class TestBackupEdgeCases:
    def test_nonexistent_lucifex_home(self, tmp_path, monkeypatch):
        """Backup exits when lucifex home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".lucifex"
        monkeypatch.setenv("LUCIFEX_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("lucifex-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_lucifex_home(self, tmp_path, monkeypatch):
        """Backup handles empty lucifex home (no files to back up)."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        # Only excluded dirs, no actual files
        (lucifex_home / "__pycache__").mkdir()
        (lucifex_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = lucifex_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = lucifex_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_lucifex(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside lucifex root."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside lucifex home
        out_zip = lucifex_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = lucifex_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (lucifex_home / "config.yaml").exists()
        assert (lucifex_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (lucifex_home / "profiles" / "coder" / "config.yaml").exists()
        assert (lucifex_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "lucifex -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from lucifex_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def lucifex_home(self, tmp_path):
        """Create a fake LUCIFEX_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, lucifex_home):
    ...
    def test_label_in_id(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", lucifex_home=lucifex_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        db_copy = lucifex_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
    ...
    def test_copies_channel_aliases(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        copied = lucifex_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        with open(lucifex_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_list_snapshots(self, lucifex_home):
        ...
        id1 = create_quick_snapshot(label="first", lucifex_home=lucifex_home)
        id2 = create_quick_snapshot(label="second", lucifex_home=lucifex_home)

        snaps = list_quick_snapshots(lucifex_home=lucifex_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", lucifex_home=lucifex_home)
        snaps = list_quick_snapshots(limit=3, lucifex_home=lucifex_home)
        assert len(snaps) == 3

    def test_restore_config(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)

        (lucifex_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (lucifex_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, lucifex_home=lucifex_home)
        assert result is True
        assert "openrouter" in (lucifex_home / "config.yaml").read_text()

    def test_restore_state_db(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)

        conn = sqlite3.connect(str(lucifex_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, lucifex_home=lucifex_home)
    ...
    def test_restore_nonexistent(self, lucifex_home):
        from lucifex_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", lucifex_home=lucifex_home) is False

    def test_auto_prune(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", lucifex_home=lucifex_home)
        snaps = list_quick_snapshots(limit=100, lucifex_home=lucifex_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", lucifex_home=lucifex_home)
        deleted = prune_quick_snapshots(keep=3, lucifex_home=lucifex_home)
        assert deleted == 7
        assert len(list_quick_snapshots(lucifex_home=lucifex_home)) == 3

    def test_snapshot_includes_pairing_directories(self, lucifex_home):
        ...
        (lucifex_home / "platforms" / "pairing").mkdir(parents=True)
        (lucifex_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (lucifex_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (lucifex_home / "pairing").mkdir()
        (lucifex_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (lucifex_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, lucifex_home):
        ...
        assert restore_quick_snapshot(snap_id, lucifex_home=lucifex_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, lucifex_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (lucifex update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, lucifex_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, lucifex_home):
        ...
        escape_dst = lucifex_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped LUCIFEX_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from lucifex_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        copy = lucifex_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        copy = lucifex_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, lucifex_home):
        from lucifex_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(lucifex_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, lucifex_home):
        ...
        board_dir = lucifex_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        copy = (
            lucifex_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, lucifex_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(lucifex_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, lucifex_home):
        ...
        board = lucifex_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(lucifex_home=lucifex_home)
        snap = lucifex_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, lucifex_home, monkeypatch):
        ...
        import lucifex_cli.backup as bk
        from lucifex_cli.backup import create_quick_snapshot
        ...
        copy = lucifex_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def lucifex_home(self, tmp_path):
        ...
        root.mkdir()
        _make_lucifex_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, lucifex_home):
        from lucifex_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(lucifex_home=lucifex_home)
        assert out is not None
        assert out.exists()
        assert out.parent == lucifex_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, lucifex_home):
        """Pre-update backup should include the same user data that
        ``lucifex backup`` would, and should exclude the same directories."""
        from lucifex_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(lucifex_home=lucifex_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # lucifex-agent repo excluded
        assert not any(n.startswith("lucifex-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, lucifex_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from lucifex_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(lucifex_home=lucifex_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(lucifex_home=lucifex_home)
    ...
    def test_rotation_keeps_only_n(self, lucifex_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(lucifex_home=lucifex_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (lucifex_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, lucifex_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from lucifex_cli.backup import create_pre_update_backup

        (lucifex_home / "backups").mkdir(exist_ok=True)
        manual = lucifex_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(lucifex_home=lucifex_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from lucifex_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(lucifex_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, lucifex_home):
        ...
        """
        from lucifex_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(lucifex_home=lucifex_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, lucifex_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, lucifex_home):
        ...
        first = create_pre_update_backup(lucifex_home=lucifex_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(lucifex_home=lucifex_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(lucifex_home=lucifex_home, keep=0)
    ...
    def test_skips_symlinked_files(self, lucifex_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside LUCIFEX_HOME."""
        from lucifex_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def lucifex_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_lucifex_tree(root)
        # Point LUCIFEX_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("LUCIFEX_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for lucifex_cli.config + lucifex_constants so they pick up LUCIFEX_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("lucifex_cli.config") or mod == "lucifex_constants":
...
# ---------------------------------------------------------------------------
# Pre-migration backup (lucifex claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``lucifex claw migrate`` runs before mutating ~/.lucifex/."""
    ...
    def test_creates_backup_under_backups_dir(self, lucifex_home):
        from lucifex_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(lucifex_home=lucifex_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `lucifex
        # import` and the update-backup listing both pick them up.
        assert out.parent == lucifex_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, lucifex_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``lucifex backup`` / ``create_pre_update_backup`` — no drift."""
        from lucifex_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(lucifex_home=lucifex_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_lucifex_import(self, lucifex_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Lucifex
        backup — `lucifex import` should accept it."""
        from lucifex_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(lucifex_home=lucifex_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, lucifex_home):
        from lucifex_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(lucifex_home=lucifex_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(lucifex_home=lucifex_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, lucifex_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(lucifex_home=lucifex_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((lucifex_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_lucifex_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.lucifex yet — nothing to back up."""
        from lucifex_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(lucifex_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, lucifex_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from lucifex_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(lucifex_home=lucifex_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(lucifex_home=lucifex_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`lucifex update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, lucifex_home: Path, label="pre-update"):
        from lucifex_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, lucifex_home=lucifex_home, keep=5)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = lucifex_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, lucifex_home=lucifex_home) is None
        assert restore_cron_jobs_if_emptied("", lucifex_home=lucifex_home) is None
...
# location, NOT under LUCIFEX_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, lucifex_home: Path) -> None:
        lucifex_home.mkdir(parents=True, exist_ok=True)
        (lucifex_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (lucifex_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (lucifex_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        self._make_min_tree(lucifex_home)
        # External provider state living OUTSIDE LUCIFEX_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under LUCIFEX_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        lucifex_home = dst_home / ".lucifex"
        lucifex_home.mkdir()
        ...
        # External state did NOT leak into LUCIFEX_HOME.
        assert not (lucifex_home / "_external").exists()
