...
LUCIFEX_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
from lucifex_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Lucifex-Session-Token": _SESSION_TOKEN}
...
@pytest.fixture
def lucifex_home(monkeypatch, tmp_path):
    """Fresh LUCIFEX_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("LUCIFEX_HOME", str(home))
    from lucifex_cli.config import invalidate_env_cache
...
def test_delete_env_key_prunes_env_seeded_pool_entry(lucifex_home):
    _write_env(lucifex_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(lucifex_home, _zai_pool_fixture())
...
def test_delete_env_key_removes_provider_pool_key_when_emptied(lucifex_home):
...
def test_delete_survives_pool_reload(lucifex_home):
    """#59761: the pool loader must not resurrect the entry after 'restart'."""
    _write_env(lucifex_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(lucifex_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
...
def test_delete_clears_provider_models_cache(lucifex_home):
    _write_env(lucifex_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(lucifex_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = lucifex_home / "provider_models_cache.json"
...
def test_delete_unknown_key_404s(lucifex_home):
...
def test_delete_does_not_touch_other_providers(lucifex_home):
    _write_env(lucifex_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    other_key = "dk-" + "e" * 24
...
def test_update_rotates_config_yaml_model_mirror(lucifex_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(lucifex_home, OPENAI_API_KEY=old)
    _write_config(
        lucifex_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = lucifex_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
def test_update_rotates_custom_provider_mirror(lucifex_home):
    old = "sk-cp-" + "h" * 24
    new = "sk-cp-" + "i" * 24
    _write_env(lucifex_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_update_leaves_unrelated_config_keys_alone(lucifex_home):
    """A DIFFERENT key configured inline must not be rewritten by value-match."""
    old = "sk-un-" + "j" * 24
    unrelated = "sk-un-" + "k" * 24
    _write_env(lucifex_home, OPENAI_API_KEY=old)
    _write_config(lucifex_home, f"model:\n  provider: custom\n  api_key: {unrelated}\n")
...
def test_delete_scrubs_config_yaml_mirror(lucifex_home):
    old = "sk-dl-" + "m" * 24
    _write_env(lucifex_home, OPENAI_API_KEY=old)
    _write_config(lucifex_home, f"model:\n  provider: custom\n  api_key: {old}\n")
...
def test_delete_then_resave_round_trip(lucifex_home):
    _write_env(lucifex_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(lucifex_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    ...
    assert not is_source_suppressed("zai", "env:ZAI_API_KEY"), (
        "an explicit re-save must lift the suppression (like `lucifex auth add`)"
    )
