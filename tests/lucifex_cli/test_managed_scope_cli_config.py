...
lucifex_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `lucifex config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_lucifex_home = get_lucifex_home()`` at import time (module
    singleton), so monkeypatching LUCIFEX_HOME after import doesn't move it.
    ...
    this only matters for tests that swap LUCIFEX_HOME.)
    """
    import cli

    cli._lucifex_home = home
