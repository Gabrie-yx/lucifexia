...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LUCIFEX_HOME with .container-mode file."""
    lucifex_home = tmp_path / ".lucifex"
    lucifex_home.mkdir()
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
    monkeypatch.delenv("LUCIFEX_DEV", raising=False)

    container_mode = lucifex_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=lucifex-agent\n"
        "exec_user=lucifex\n"
        "lucifex_bin=/data/current-package/bin/lucifex\n"
    )
    return lucifex_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lucifex_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lucifex-agent"
    assert info["exec_user"] == "lucifex"
    assert info["lucifex_bin"] == "/data/current-package/bin/lucifex"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("lucifex_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_lucifex_dev(container_env, monkeypatch):
    """Returns None when LUCIFEX_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LUCIFEX_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_lucifex_dev_zero(container_env, monkeypatch):
    """LUCIFEX_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LUCIFEX_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        lucifex_home = Path(tmpdir) / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("lucifex_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_lucifex_home": lambda: lucifex_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LUCIFEX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=lucifex-custom\n"
        "exec_user=myuser\n"
        "lucifex_bin=/opt/lucifex/bin/lucifex\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "lucifex-custom"
    assert info["exec_user"] == "myuser"
    assert info["lucifex_bin"] == "/opt/lucifex/bin/lucifex"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lucifex_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "lucifex"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "lucifex-agent" in cmd
    assert "/data/current-package/bin/lucifex" in cmd
    assert "chat" in cmd
