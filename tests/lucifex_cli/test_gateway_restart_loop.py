...
Covers:
- Defense 1: gateway stop/restart refuse when _LUCIFEX_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lucifex  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "LUCIFEX GATEWAY RESTART",           # uppercase
    ])
    def test_lucifex_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.lucifex.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.lucifex.gateway.plist",
        "launchctl stop ai.lucifex.gateway",
        "systemctl restart lucifex-gateway",
        "systemctl stop lucifex-gateway.service",
        "systemctl start lucifex-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill lucifex gateway process",
        "pkill -f lucifex.*gateway",
        "pkill -f gateway.*lucifex",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lucifex cron list",
        "lucifex update",
        "lucifex config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `lucifex gateway start` is benign — starting a gateway from inside a
        ...
        "lucifex gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway lucifex
        # services must not be falsely blocked (the old `.*lucifex` matched any
        # lucifex token).
        "launchctl unload ai.lucifex.update-checker.plist",
        "launchctl restart ai.lucifex.daemon",
        "systemctl restart lucifex-meta.service",
        "systemctl restart lucifex-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_lucifex_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade lucifex then run lucifex gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.lucifex.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_lucifex_gateway_once.sh). The script must live under
        # LUCIFEX_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path / ".lucifex"))
        scripts_dir = tmp_path / ".lucifex" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify lucifex gateway stop/restart refuse when _LUCIFEX_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_LUCIFEX_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_LUCIFEX_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _LUCIFEX_GATEWAY=1.

    Issue #37453: systemctl --user restart lucifex-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart lucifex-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-lucifex systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _LUCIFEX_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart lucifex-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart lucifex-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run lucifex gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("lucifex gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfelucifex gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under LUCIFEX_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.lucifex.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    lucifex_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run lucifex gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path / ".lucifex"))
        (tmp_path / ".lucifex").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run lucifex gateway restart nightly",
        ))
