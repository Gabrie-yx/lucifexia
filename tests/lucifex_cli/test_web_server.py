"""Tests for lucifex_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_lucifex_home):
    """Drop the example-dashboard fixture into the per-test LUCIFEX_HOME
    ...
    isolated ``LUCIFEX_HOME``.

    The user-plugin source is preferred over a transient
    ``LUCIFEX_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, lucifex-achievements, model providers
    ...
    from lucifex_cli import web_server
    ...
    # plugin in exactly as a real operator would with `lucifex plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down LUCIFEX_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Lucifex vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Lucifex env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints LUCIFEX_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("LUCIFEX_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_lucifex_home):
        """Create a TestClient and isolate the state DB under the test LUCIFEX_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")

        import lucifex_state
        from lucifex_constants import get_lucifex_home
        from lucifex_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(lucifex_state, "DEFAULT_DB_PATH", get_lucifex_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "lucifex_home" in data
        assert "active_sessions" in data
        assert data["can_update_lucifex"] is True

    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        import lucifex_cli.web_server as web_server
        import lucifex_state
        ...
        fake_db_path.touch()
        monkeypatch.setattr(lucifex_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("lucifex_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(lucifex_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("lucifex_state.SessionDB", _boom)
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_lucifex"] is False
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in lucifex-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "lucifex"
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_lucifex_home() / "hindsight" / "config.json"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from lucifex_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide LUCIFEX_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from lucifex_constants import get_lucifex_home

        path = get_lucifex_home() / "honcho.json"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/lucifex"
        ...
        assert fields["workspace"]["placeholder"] == "lucifex"
        assert fields["aiPeer"]["placeholder"] == "lucifex"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_lucifex_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["lucifex"]["workspace"] == "myws"
        assert cfg["hosts"]["lucifex"]["peerName"] == "eri"
        assert cfg["hosts"]["lucifex"]["environment"] == "local"
        assert cfg["hosts"]["lucifex"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["lucifex"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_lucifex_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("lucifex", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_lucifex_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["lucifex"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_lucifex_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["lucifex"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["lucifex"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"lucifex": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert resp.status_code == 200
        assert not (get_lucifex_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["lucifex"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("LUCIFEX_HONCHO_HOST", "lucifex_work")

        path = self._seed_local_honcho({"hosts": {"lucifex.work": {"workspace": "w", "peerName": "eri"}}})
        ...
        assert set(hosts) == {"lucifex.work"}
        assert hosts["lucifex.work"] == {"workspace": "w", "peerName": "eri", "sessionStrategy": "per-repo"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"lucifex": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["lucifex"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["lucifex"]["workspace"] == "myws"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins LUCIFEX_HONCHO_HOST=lucifex; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("LUCIFEX_HONCHO_HOST", raising=False)
        from lucifex_constants import get_lucifex_home
        from lucifex_cli.profiles import get_profile_dir
        ...
        worker_cfg.write_text(json.dumps({"hosts": {"lucifex_worker": {"workspace": "kept"}}}), encoding="utf-8")
        ...
        own = json.loads((get_lucifex_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_lucifex_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from lucifex_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_lucifex_home() / "images"
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from lucifex_cli.web_server import app
    ...
    def test_update_lucifex_returns_docker_guidance_without_spawning(self, monkeypatch):
        import lucifex_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn lucifex update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_lucifex_action", fail_spawn)
        web_server._ACTION_PROCS.pop("lucifex-update", None)
        web_server._ACTION_RESULTS.pop("lucifex-update", None)

        resp = self.client.post("/api/lucifex/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "lucifex-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/lucifex-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/lucifex-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/lucifex-agent:latest" in line for line in status_data["lines"])

    def test_update_lucifex_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import lucifex_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn lucifex update")
    ...
    def test_update_lucifex_spawns_on_non_docker_install(self, monkeypatch):
        import lucifex_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_lucifex_action", fake_spawn)
        web_server._ACTION_PROCS.pop("lucifex-update", None)
        web_server._ACTION_RESULTS.pop("lucifex-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "lucifex-update"}
        assert calls == [(["update"], "lucifex-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("lucifex-update", None)
        web_server._ACTION_RESULTS.pop("lucifex-update", None)
        web_server._ACTION_PROCS["lucifex-update"] = proc

        resp = self.client.get("/api/actions/lucifex-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "lucifex-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["lucifex-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["lucifex-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/lucifex-update/status?lines=3")
    ...
    def test_config_schema_merges_custom_command_tts_provider(self):
        ...
        from lucifex_cli.config import load_config, save_config
        from lucifex_cli.web_server import CONFIG_SCHEMA
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from lucifex_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `lucifex model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from lucifex_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Lucifex provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `lucifex import` doesn't auto-abort at the overwrite prompt."""
        import lucifex_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_lucifex_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_lucifex_home() / "backups"
        assert archive.name.startswith("lucifex-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_lucifex_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import lucifex_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("LUCIFEX_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "lucifex-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_lucifex_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from lucifex_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from lucifex_cli.config import save_env_value
        from lucifex_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from lucifex_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Lucifex Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("LucifexDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "lucifex_pair123_bot",
                "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair123_bot",
                "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "lucifex_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "lucifex_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_lucifex_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "lucifex_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "lucifex_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_lucifex_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``lucifex gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "lucifex_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "lucifex_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "lucifex_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "lucifex_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/LucifexSetupBot/lucifex_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/lucifex_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/lucifex"})
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`lucifex serve` (LUCIFEX_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("LUCIFEX_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "lucifex-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from lucifex_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from lucifex_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``lucifex model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "lucifex-4"}
        cfg["auxiliary"] = {
    ...
    def test_set_model_main_preserves_base_url_for_named_custom_provider(self):
        ...
        save_config({
            "model": {"provider": "nous", "default": "lucifex-4"},
            "providers": {
                "axet-proxy": {
                    "name": "Axet Proxy",
                    "base_url": "http://127.0.0.1:8081/v1",
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `lucifex model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see lucifex_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from lucifex_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from lucifex_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_lucifex_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("lucifex://")
    ...
    def test_profiles_list_includes_default(self):
        from lucifex_constants import get_lucifex_home
        get_lucifex_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from lucifex_constants import get_lucifex_home
        import lucifex_cli.profiles as profiles_mod
        ...
        named = lucifex_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_lucifex_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_lucifex_for_default_profile(self):
        from lucifex_constants import get_lucifex_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "lucifex setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/lucifex/bin/lucifex")
        ...
        if is_windows:
            assert lines == ["@echo off", "lucifex -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/lucifex/bin/lucifex -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_lucifex_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_lucifex_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_lucifex_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_lucifex_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_lucifex_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_lucifex_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_lucifex_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_lucifex_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_lucifex_home() / "profiles" / "builder"
        token = set_lucifex_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_lucifex_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import lucifex_cli.skills_config as skills_config
        import lucifex_cli.web_server as web_server
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "lucifex"
        save_config(config)

        body = self.client.get("/api/tools/terminal/backends").json()
        ssh = next(r for r in body["backends"] if r["name"] == "ssh")
        assert ssh["status"] == "ready"
        assert "lucifex@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from lucifex_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from lucifex_cli.web_server import _denormalize_config_from_web
        from lucifex_cli.config import save_config
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from lucifex_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.lucifex/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_lucifex_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_lucifex_home_override(token)
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(lucifex_home, name="ocean"):
        themes_dir = lucifex_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="lucifex-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "lucifex-theme-bootstrap" not in resp.text
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_lucifex_home):
        ...
        monkeypatch.setattr(
            lucifex_state, "DEFAULT_DB_PATH", get_lucifex_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``lucifex_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in lucifex_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_lucifex_home):
        ...
        # Pin the SessionDB to the isolated LUCIFEX_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_lucifex_home, _install_example_plugin):
        ...
        installs it into the per-test ``LUCIFEX_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into LUCIFEX_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, lucifex-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (lucifex-achievements is loaded by default).
        resp = self.client.get("/api/plugins/lucifex-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        LUCIFEX_HOME override scopes a request to another profile."""
        from lucifex_constants import (
            reset_lucifex_home_override,
            set_lucifex_home_override,
        )
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(launch_home))
        from lucifex_cli import web_server
        token = set_lucifex_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``lucifex --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import lucifex_cli.main as main_mod
        ...
        assert env["LUCIFEX_TUI_DASHBOARD"] == "1"
        assert env["LUCIFEX_TUI_INLINE"] == "1"
        assert env["LUCIFEX_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("LUCIFEX_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("LUCIFEX_PYTHON", raising=False)
        monkeypatch.delenv("LUCIFEX_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env["LUCIFEX_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["LUCIFEX_PYTHON"] == sys.executable
        assert env["LUCIFEX_CWD"] == os.getcwd()
    ...
    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_PYTHON_SRC_ROOT", "/definitely/missing/lucifex-src")
        monkeypatch.setenv("LUCIFEX_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("LUCIFEX_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("LUCIFEX_CWD", str(tmp_path))
        monkeypatch.setenv("LUCIFEX_PYTHON", str(relative_python))
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(self, monkeypatch, tmp_path):
        ...
        assert env is not None
        assert env["LUCIFEX_CWD"] == str(tmp_path)

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_lucifex_home
    ):
        import lucifex_cli.main as main_mod

        config_path = Path(os.environ["LUCIFEX_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/lucifex-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/lucifex-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"lucifex-ws-ok" in buf:
                    break
            assert b"lucifex-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from lucifex_cli.pty_bridge import PtyUnavailableError
        ...
        import lucifex_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        LUCIFEX_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from lucifex_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import lucifex_cli.main as cli_main
    import lucifex_cli.web_server as ws
    ...
    assert env is not None
    gateway_url = env.get("LUCIFEX_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_lucifex_home, _install_example_plugin):
        ...
        the per-test ``LUCIFEX_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_lucifex_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("LUCIFEX_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under LUCIFEX_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_lucifex_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("LUCIFEX_DESKTOP", raising=False)
