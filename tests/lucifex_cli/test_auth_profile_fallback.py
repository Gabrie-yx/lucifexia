...
When ``LUCIFEX_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.lucifex/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.lucifex            (has its own auth.json fixture)
    * Profile     -> tmp_path/.lucifex/profiles/coder   (active, LUCIFEX_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Lucifex is not
# logged into Lucifex portal"`` even though the user has a valid global Nous
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from lucifex_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    lucifex_home = tmp_path / "classic"
    lucifex_home.mkdir()
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))

    _write(lucifex_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (LUCIFEX_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.lucifex as the "real home" — which is NOT equal
    # to the LUCIFEX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    lucifex_home = tmp_path / "classic"
    ...
    _write(lucifex_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from lucifex_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: LUCIFEX_HOME is set to a custom path that is NOT under
    # ~/.lucifex/profiles/ — get_default_lucifex_root() returns LUCIFEX_HOME
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_lucifex_home_override(profile_b)
        try:
