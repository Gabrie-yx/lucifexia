"""Tests for the per-profile Projects store (lucifex_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Lucifex Agent", folders=["/tmp/lucifex"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "lucifex-agent"
    assert proj.name == "Lucifex Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/lucifex"
    assert [f.path for f in proj.folders] == ["/tmp/lucifex"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "lucifex-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Lucifex Agent")
    pdb.create_project(conn, name="Lucifex Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["lucifex-agent", "lucifex-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' LUCIFEX_HOME.
...
def test_db_path_under_lucifex_home():
    # Resolves under LUCIFEX_HOME (set by the autouse isolation fixture).
