...
def main_mod(monkeypatch):
    import lucifex_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["lucifex", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["lucifex", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["lucifex", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["lucifex", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LUCIFEX_TUI", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["lucifex", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LUCIFEX_TUI", raising=False)
    monkeypatch.delenv("LUCIFEX_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("LUCIFEX_FAST_STARTUP_BANNER", raising=False)
    monkeypatch.setattr(sys, "argv", ["lucifex"])
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
    monkeypatch.setattr(
    ...
    assert os.environ["LUCIFEX_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["LUCIFEX_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LUCIFEX_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lucifex", "version"])
    monkeypatch.setattr(
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LUCIFEX_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lucifex", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Lucifex Agent v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("LUCIFEX_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lucifex", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("LUCIFEX_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("LUCIFEX_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lucifex", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_lucifex_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("LUCIFEX_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_run_and_exit_oneshot_hard_exits_when_oneshot_import_fails(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "lucifex_cli.oneshot" and "run_oneshot" in (fromlist or ()):
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import lucifex_cli.oneshot as oneshot
        from lucifex_cli.main import _exit_after_oneshot
...
def test_main_oneshot_path_bypasses_late_atexit_abort():
    ...
    program = textwrap.dedent(
        ...
        sys.modules["lucifex_cli.oneshot"] = types.SimpleNamespace(
...
def test_oneshot_run_agent_closes_agent_after_chat(monkeypatch):
    import lucifex_cli.oneshot as oneshot_mod
...
def test_oneshot_exit_code_when_failed_without_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "lucifex_cli.oneshot._run_agent",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from lucifex_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "lucifex -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """lucifex -z bypasses LucifexCLI, but recall still needs SessionDB."""
    from lucifex_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "lucifex_state", mod("lucifex_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.config",
        mod("lucifex_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.models",
        mod("lucifex_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.runtime_provider",
        mod(
            "lucifex_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "lucifex_cli.tools_config",
        mod("lucifex_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["LUCIFEX_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/lucifex-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["LUCIFEX_MODEL"] == "nous/lucifex-test"
    assert env["LUCIFEX_INFERENCE_MODEL"] == "nous/lucifex-test"
    assert env["LUCIFEX_TUI_PROVIDER"] == "nous"
    assert env["LUCIFEX_INFERENCE_PROVIDER"] == "nous"
    assert env["LUCIFEX_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["LUCIFEX_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("lucifex-tui-active-session-")
...
def test_launch_tui_worktree_validates_relative_python_against_final_cwd(
    ...
    monkeypatch.setenv("LUCIFEX_CWD", str(parent_cwd))
    ...
    assert captured["env"]["LUCIFEX_CWD"] == str(worktree)
    assert captured["env"]["LUCIFEX_PYTHON"] == str(relative_python)


def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_lucifex_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/lucifex-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("lucifex_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("LUCIFEX_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "LUCIFEX_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["LUCIFEX_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_lucifex_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("LUCIFEX_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "lucifex_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "lucifex --tui --resume 20260409_000001_abc123" in out
    assert 'lucifex --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "lucifex --tui --resume actual_session" in out
