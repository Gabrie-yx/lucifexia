"""Tests for `lucifex chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("LUCIFEX_SAFE_MODE", "LUCIFEX_IGNORE_USER_CONFIG", "LUCIFEX_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in lucifex_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("LUCIFEX_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under LUCIFEX_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_SAFE_MODE", raising=False)
        from lucifex_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under LUCIFEX_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `lucifex chat`."""
