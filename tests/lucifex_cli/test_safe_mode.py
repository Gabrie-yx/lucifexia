"""Tests for `lucifex chat --safe-mode` isolation."""
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["LUCIFEX_SAFE_MODE"] == "1"
        assert os.environ["LUCIFEX_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["LUCIFEX_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("lucifex_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "lucifex_cli.plugins", plugins)
