"""Tests for lucifex_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'lucifex' instead of 'lucifex-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import lucifex_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["lucifex"]}}

    with caplog.at_level(logging.WARNING, logger="lucifex_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "lucifex" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["lucifex-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["lucifex-cli", "bogus"]}}
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `lucifex tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "lucifex_cli.tools_config._xai_credentials_present", lambda: True
    )
...
# via the platform's native composite. The composite ``lucifex-discord``
...
def test_discord_composite_only_enables_discord_toolsets():
    """Layer 1: ``platform_toolsets.discord: [lucifex-discord]`` is an explicit
    opt-in to the full Discord bundle (which includes the ``discord`` and
    ``discord_admin`` tools). They must not be silently stripped."""
    config = {"platform_toolsets": {"discord": ["lucifex-discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled, "discord toolset missing from lucifex-discord composite"
...
def test_discord_composite_plus_configurable_enables_discord_toolsets():
    ...
    config = {"platform_toolsets": {"discord": ["lucifex-discord", "spotify"]}}
    enabled = _get_platform_tools(config, "discord")
...
def test_discord_composite_plus_partial_explicit_enables_sibling():
    """Layer 3: ``[lucifex-discord, discord]`` lists discord explicitly but
    discord_admin arrives only via the composite. Both must survive."""
    config = {"platform_toolsets": {"discord": ["lucifex-discord", "discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["lucifex-telegram", "discord"]}}
...
def test_discord_explicit_workaround_still_works():
    ...
    config = {
        "platform_toolsets": {"discord": ["lucifex-discord", "discord", "discord_admin"]}
    }
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "lucifex_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `lucifex tools`,
    ...
    # User explicitly opted into spotify but not x_search via `lucifex tools`.
    config = {"platform_toolsets": {"cli": ["lucifex-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[lucifex-cli, spotify]`` (composite + configurable) must keep the full
    ``lucifex-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``lucifex-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when lucifex-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["lucifex-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``lucifex-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["lucifex-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _LUCIFEX_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `lucifex tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("lucifex_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/lucifex-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (lucifex-cli, lucifex-telegram, etc.) must NOT
    ...
    ``lucifex tools``, but lucifex-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "lucifex-cli",  # <-- the culprit
    ...
    # lucifex-cli must NOT survive — it's a platform default, not an MCP server
    assert "lucifex-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_lucifex_telegram():
    """Same bug for Telegram — lucifex-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "lucifex-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "lucifex-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "lucifex-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "lucifex_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("lucifex_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "lucifex_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("lucifex_cli.tools_config.save_config", lambda config: None)
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    """Regression: `lucifex tools` → Configure all platforms → Web Search
    ...
    monkeypatch.setattr(
        "lucifex_cli.tools_config._get_enabled_platforms",
        lambda: ["cli", "telegram"],
    )
    ...
    monkeypatch.setattr("lucifex_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "lucifex_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "lucifex_cli.tools_config._toolset_needs_configuration_prompt",
        lambda ts_key, config, **kwargs: ts_key == "web",
    )
    monkeypatch.setattr(
        "lucifex_cli.tools_config._configure_toolset",
...
def test_configure_single_platform_configures_selected_tool_missing_provider(monkeypatch):
    """Regression (per-platform sibling of the global flow): `lucifex tools` →
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from lucifex_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """lucifex-gateway includes list should cover all messaging platforms."""
        from lucifex_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["lucifex-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from lucifex_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from lucifex_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/lucifex-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `lucifex tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's LUCIFEX_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"LUCIFEX_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"LUCIFEX_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from lucifex_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from lucifex_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from lucifex_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("lucifex_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("lucifex_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("lucifex_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`lucifex tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["lucifex-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "lucifex-_test_platform"},
    }

    with mock_patch("lucifex_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'lucifex-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `lucifex tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from lucifex_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `lucifex tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from lucifex_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `lucifex tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from lucifex_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("lucifex_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("lucifex_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("lucifex_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("lucifex_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from lucifex_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from lucifex_cli import tools_config
...
#    reported as added/removed by `lucifex tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `lucifex tools` printed ``- kanban``
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``lucifex model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path))
    import lucifex_cli.tools_config as tc
    ...
    seq = iter([1, 0, 1])
    with patch("lucifex_cli.model_switch.list_authenticated_providers",
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("lucifex_cli.tools_config.get_env_value", lambda key: "sk-x")
...
def test_provider_readiness_local_install_rows_track_module_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        "lucifex_cli.tools_config._module_installed", lambda name: False
    )
    assert provider_readiness_status(kitten, {}) == "needs_setup"
    assert provider_readiness_status(piper, {}) == "needs_setup"

    monkeypatch.setattr(
        "lucifex_cli.tools_config._module_installed", lambda name: True
    )
...
# `lucifex tools post-setup <key>` child. On Windows each console child (npm,
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    from lucifex_cli import _subprocess_compat
    from lucifex_cli.tools_config import _post_setup_no_window_flags
