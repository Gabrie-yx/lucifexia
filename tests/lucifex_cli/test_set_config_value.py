...
from lucifex_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_lucifex_home(tmp_path):
    """Point LUCIFEX_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"LUCIFEX_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_lucifex_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_lucifex_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_lucifex_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_lucifex_home):
        set_config_value(key, "secret-456")
        env_content = _read_env(_isolated_lucifex_home)
        assert f"{key}=secret-456" in env_content
        assert key not in _read_config(_isolated_lucifex_home)

    def test_case_insensitive(self, _isolated_lucifex_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_lucifex_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_lucifex_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_lucifex_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_lucifex_home)

    def test_nested_key(self, _isolated_lucifex_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_lucifex_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_lucifex_home)

    def test_terminal_image_goes_to_config(self, _isolated_lucifex_home):
    ...
    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_lucifex_home):
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_lucifex_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_lucifex_home):
    ...
    def test_zero_routes_to_config(self, _isolated_lucifex_home):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_lucifex_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_lucifex_home, capsys):
    ...
    def test_config_get_prints_structured_json(self, _isolated_lucifex_home, capsys):
        set_config_value("terminal.backend", "docker")
        capsys.readouterr()
    ...
    def test_config_get_dotted_token_yaml_key(self, _isolated_lucifex_home, capsys):
        (_isolated_lucifex_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_lucifex_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_lucifex_home)
        capsys.readouterr()
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_lucifex_home)) or {}
        assert reloaded == {}
        assert "TERMINAL_ENV=" not in _read_env(_isolated_lucifex_home)
        assert "Unset terminal.backend" in capsys.readouterr().out

    def test_config_unset_removes_env_key(self, _isolated_lucifex_home, capsys):
        set_config_value("OPENROUTER_API_KEY", "sk-test")
        assert "OPENROUTER_API_KEY=sk-test" in _read_env(_isolated_lucifex_home)
        capsys.readouterr()

        args = argparse.Namespace(config_command="unset", key="OPENROUTER_API_KEY")
        config_command(args)

        assert "OPENROUTER_API_KEY=" not in _read_env(_isolated_lucifex_home)
        assert "Unset OPENROUTER_API_KEY" in capsys.readouterr().out

    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_lucifex_home, capsys):
        (_isolated_lucifex_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_lucifex_home))
...
class TestListNavigation:
    """lucifex config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_lucifex_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_lucifex_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_lucifex_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_lucifex_home):
...
class TestStringTypedConfigValues:
    ...
    def test_string_typed_values_are_not_coerced(self, _isolated_lucifex_home, value):
        """Values stay strings when DEFAULT_CONFIG declares the leaf as a string."""
        set_config_value("approvals.mode", value)

        import yaml
        saved = yaml.safe_load(_read_config(_isolated_lucifex_home))
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_lucifex_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_lucifex_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from lucifex_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_lucifex_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_lucifex_home, capsys):
...
class TestSchemaValidation:
    """#34067: ``lucifex config set`` must not report bare success for
    ...
    a post-write notice warns that Lucifex may never read it and suggests the
    ...
    def test_unknown_top_level_key_written_with_notice(self, _isolated_lucifex_home, capsys):
        ...
        assert "totally_made_up_key" in _read_config(_isolated_lucifex_home)

    def test_unknown_subkey_written_with_notice(self, _isolated_lucifex_home, capsys):
    ...
    def test_platforms_container_is_accepted(self, _isolated_lucifex_home, capsys):
        ...
        content = _read_config(_isolated_lucifex_home)
        assert "enabled: true" in content
        assert "not a recognized config key" not in capsys.readouterr().out

    def test_gateway_platforms_nested_is_accepted(self, _isolated_lucifex_home, capsys):
    ...
    def test_unknown_approvals_subkey_warns_but_writes(self, _isolated_lucifex_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "not a recognized config key" in out
        assert "notarealkey" in _read_config(_isolated_lucifex_home)

    def test_known_approvals_subkey_is_accepted(self, _isolated_lucifex_home, capsys):
    ...
    def test_close_typo_suggests_correct_key(self, _isolated_lucifex_home, capsys):
    ...
    def test_typoed_subkey_suggests_sibling(self, _isolated_lucifex_home, capsys):
        """``agent.max_turn`` should suggest ``agent.max_turns``."""
        set_config_value("agent.max_turn", "100")
        out = capsys.readouterr().out
        assert "agent.max_turns" in out

    def test_force_suppresses_notice(self, _isolated_lucifex_home, capsys):
    ...
    def test_known_top_level_key_accepted(self, _isolated_lucifex_home):
        """Sanity check: real config keys still work."""
        set_config_value("terminal.backend", "docker")
        content = _read_config(_isolated_lucifex_home)
        assert "backend: docker" in content

    def test_known_platform_config_accepted(self, _isolated_lucifex_home):
    ...
    def test_open_dict_mcp_servers_accepts_any_subkey(self, _isolated_lucifex_home):
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from lucifex_cli.config import _validate_config_key
