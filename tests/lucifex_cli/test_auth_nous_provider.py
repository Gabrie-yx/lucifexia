...
from lucifex_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from lucifex_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("LUCIFEX_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_lucifex_ca_bundle_env_falls_back(self, monkeypatch):
        from lucifex_cli.auth import _resolve_verify

        monkeypatch.setenv("LUCIFEX_CA_BUNDLE", "/nonexistent/lucifex-ca.pem")
...
def test_resolve_nous_runtime_credentials_env_override_wins_live_not_persisted(
    ...
    monkeypatch.setattr("lucifex_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
    monkeypatch.setenv("LUCIFEX_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="lucifex_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return lucifex_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from lucifex_cli.auth import PROVIDER_REGISTRY, _login_nous

        lucifex_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `lucifex auth add nous`
    ...
    """
    from lucifex_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `lucifex auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    re-authenticate" string led users to report this as a Lucifex persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from lucifex_cli.auth import _refresh_access_token
    ...
    assert "lucifex auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect LUCIFEX_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("LUCIFEX_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without LUCIFEX_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.lucifex/shared/`` across CI runs.
    """
    from lucifex_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("LUCIFEX_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """LUCIFEX_SHARED_AUTH_DIR must redirect the path."""
    from lucifex_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("LUCIFEX_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from lucifex_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from lucifex_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from lucifex_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `lucifex auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from lucifex_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "lucifex-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different LUCIFEX_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("LUCIFEX_HOME", str(profile_a))
    ...
    # Profile B: fresh LUCIFEX_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("LUCIFEX_HOME", str(profile_b))
