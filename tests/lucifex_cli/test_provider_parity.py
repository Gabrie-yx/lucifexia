...
the SAME provider universe as ``lucifex model`` (the CLI/TUI picker).
...
def test_every_lucifex_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `lucifex model` universe."""
    ...
    assert not missing, (
        "providers shown in `lucifex model` but not configurable in the desktop "
