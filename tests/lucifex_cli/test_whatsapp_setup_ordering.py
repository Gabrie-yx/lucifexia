...
Before the fix, ``lucifex whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``lucifex gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    lucifex = home / ".lucifex"
    lucifex.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex))
...
def _env_value(lucifex_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from lucifex_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("lucifex_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``lucifex whatsapp`` with an existing paired session and
