...
def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@lucifex" / "ink" / "package.json"
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "lucifex-ink"
    ink_dir.mkdir(parents=True)
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    lucifex_cli/tui_dist/entry.js but never ships ui-tui/ (that directory only
...
def test_make_tui_argv_exits_with_recovery_hint_when_workspace_unrecoverable(
    ...
    assert "lucifex update --force" in err
...
def test_make_tui_argv_restores_missing_workspace_from_git(
    ...
    monkeypatch.delenv("LUCIFEX_TUI_DIR", raising=False)
    monkeypatch.delenv("LUCIFEX_QUIET", raising=False)
    monkeypatch.setattr(main_mod, "_ensure_tui_node", lambda: None)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (lucifex-ink etc.)
