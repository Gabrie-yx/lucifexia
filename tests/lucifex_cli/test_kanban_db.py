"""Tests for the Kanban DB layer (lucifex_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated LUCIFEX_HOME with an empty kanban DB."""
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("LUCIFEX_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("LUCIFEX_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("LUCIFEX_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("LUCIFEX_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """LUCIFEX_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import lucifex_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("LUCIFEX_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("LUCIFEX_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("LUCIFEX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("LUCIFEX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "LUCIFEX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from lucifex_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Lucifex profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="lucifex-task", assignee="daily")
...
def test_scratch_workspace_created_under_lucifex_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import lucifex_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Lucifex-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``LUCIFEX_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("LUCIFEX_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Lucifex' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Lucifex' DB/metadata/logs on task completion.
    """
...
# spawned with `lucifex -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's LUCIFEX_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's LUCIFEX_HOME."""

    def _set_home(self, monkeypatch, tmp_path, lucifex_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
        monkeypatch.delenv("LUCIFEX_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_lucifex(
        self, tmp_path, monkeypatch
    ):
        # Standard install: LUCIFEX_HOME == ~/.lucifex, no profile active.
        default_home = tmp_path / ".lucifex"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.lucifex/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.lucifex/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.lucifex/kanban.db.
        default_home = tmp_path / ".lucifex"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # LUCIFEX_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `lucifex -p coder`).
    ...
    def test_docker_custom_lucifex_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: LUCIFEX_HOME points outside ~/.lucifex.
        # `get_default_lucifex_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".lucifex"`.
        custom_root = tmp_path / "opt" / "lucifex"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: LUCIFEX_HOME=/opt/lucifex/profiles/coder;
        # `get_default_lucifex_root()` walks up to /opt/lucifex because
    ...
    def test_explicit_override_via_lucifex_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: LUCIFEX_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".lucifex"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(profile_home))
        monkeypatch.setenv("LUCIFEX_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(default_home))
        monkeypatch.setenv("LUCIFEX_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # LUCIFEX_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile LUCIFEX_HOME and reads.
        monkeypatch.setenv("LUCIFEX_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_lucifex_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LUCIFEX_KANBAN_DB pins the file path directly and beats both
        # LUCIFEX_KANBAN_HOME and the `get_default_lucifex_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(default_home))
        monkeypatch.setenv("LUCIFEX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LUCIFEX_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows LUCIFEX_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_lucifex_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LUCIFEX_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".lucifex"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(default_home))
        monkeypatch.setenv("LUCIFEX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LUCIFEX_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows LUCIFEX_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(default_home))
        monkeypatch.setenv("LUCIFEX_KANBAN_DB", "   ")
        monkeypatch.setenv("LUCIFEX_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject LUCIFEX_KANBAN_DB
        # and LUCIFEX_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites LUCIFEX_HOME.
        default_home = tmp_path / ".lucifex"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["LUCIFEX_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["LUCIFEX_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["LUCIFEX_KANBAN_TASK"] == "t_dispatch_env"
        assert env["LUCIFEX_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see lucifex_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("lucifex_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="lucifex_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lucifex kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_lucifex_argv()
#
# Workers spawned by the dispatcher must use a `lucifex` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["lucifex", ...]`
...
def test_resolve_lucifex_argv_prefers_path_shim(monkeypatch):
    """When `lucifex` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import lucifex_cli.kanban_db as kb

    monkeypatch.delenv("LUCIFEX_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/lucifex")
    argv = kb._resolve_lucifex_argv()
    assert argv == ["/usr/local/bin/lucifex"]


def test_resolve_lucifex_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import lucifex_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("LUCIFEX_BIN", ".\\lucifex.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lucifex_argv() == [os.path.abspath(".\\lucifex.exe")]


def test_resolve_lucifex_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "lucifex.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("LUCIFEX_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lucifex_argv() == [sys.executable, "-m", "lucifex_cli.main"]


def test_resolve_lucifex_argv_honors_lucifex_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like LUCIFEX_BIN lets service managers pin the executable."""
    import shutil
    import lucifex_cli.kanban_db as kb

    shim = tmp_path / "bin" / "lucifex"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("LUCIFEX_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_lucifex_argv() == [str(shim)]


def test_resolve_lucifex_argv_lucifex_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare LUCIFEX_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import lucifex_cli.kanban_db as kb

    cwd_lucifex = tmp_path / "lucifex"
    cwd_lucifex.write_text("wrong\n", encoding="utf-8")
    cwd_lucifex.chmod(cwd_lucifex.stat().st_mode | stat.S_IXUSR)
    path_lucifex = tmp_path / "bin" / "lucifex"
    path_lucifex.parent.mkdir()
    path_lucifex.write_text("right\n", encoding="utf-8")
    path_lucifex.chmod(path_lucifex.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_lucifex.parent))
    monkeypatch.setenv("LUCIFEX_BIN", "lucifex")

    assert kb._resolve_lucifex_argv() == [str(path_lucifex)]


def test_resolve_lucifex_argv_lucifex_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare LUCIFEX_BIN does not accept current-directory shadow executables."""
    import sys
    import lucifex_cli.kanban_db as kb

    (tmp_path / "lucifex.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_lucifex_argv_lucifex_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved LUCIFEX_BIN batch shim is not used as worker argv[0]."""
    import sys
    import lucifex_cli.kanban_db as kb
...
def test_resolve_lucifex_argv_lucifex_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved LUCIFEX_BIN command names do not delegate cwd search to Popen."""
    import sys
    import lucifex_cli.kanban_db as kb
...
def test_resolve_lucifex_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m lucifex_cli.main`.

    Pins the correct module name (NOT `lucifex` — there is no top-level
    `lucifex` package). Regression for #23198: the original PR shipped
    `python -m lucifex` which fails with `No module named lucifex` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_lucifex_argv()
    assert argv == [sys.executable, "-m", "lucifex_cli.main"]


def test_resolve_lucifex_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `lucifex_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m lucifex_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("LUCIFEX_BIN", None)
    ...
    assert "Lucifex Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="lucifex_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from lucifex_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("lucifex_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from lucifex_cli.kanban_db import connect, write_txn
    import lucifex_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from lucifex_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from lucifex_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("lucifex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lucifex_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.kanban_db.os.name", "nt")
    with patch("lucifex_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("lucifex_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("lucifex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "lucifex_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("lucifex_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "lucifex_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("lucifex_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("lucifex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lucifex_cli.kanban_db._record_worker_exit"):
            with patch("lucifex_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
