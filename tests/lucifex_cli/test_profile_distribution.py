"""Tests for lucifex_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/lucifex_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_lucifex_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Lucifex"):
                check_lucifex_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Lucifex distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_lucifex_requires(self, profile_env, monkeypatch):
        # Pin current Lucifex version to something well below the requirement
        import lucifex_cli
        monkeypatch.setattr(lucifex_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            lucifex_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "lucifex_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from lucifex_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from lucifex_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from lucifex_cli.profiles import create_profile, list_profiles, get_profile_dir
