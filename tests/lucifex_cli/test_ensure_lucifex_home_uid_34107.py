"""Regression tests for #34107 — Docker UID/GID handling in ensure_lucifex_home.

When Lucifex runs in Docker with ``LUCIFEX_UID=1000`` / ``LUCIFEX_GID=911``,
the entrypoint chowns the top-level ``LUCIFEX_HOME`` once at startup. But
...
The fix is a ``_chown_to_lucifex_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_lucifex_uid_gid
# ---------------------------------------------------------------------------


class TestResolveLucifexUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_UID", "1000")
        monkeypatch.setenv("LUCIFEX_GID", "911")
        from lucifex_cli.config import _resolve_lucifex_uid_gid
        uid, gid = _resolve_lucifex_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_UID", raising=False)
        monkeypatch.delenv("LUCIFEX_GID", raising=False)
        from lucifex_cli.config import _resolve_lucifex_uid_gid
        uid, gid = _resolve_lucifex_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_UID", "not-a-number")
        monkeypatch.setenv("LUCIFEX_GID", "911")
        from lucifex_cli.config import _resolve_lucifex_uid_gid
        uid, gid = _resolve_lucifex_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_UID", "")
        monkeypatch.setenv("LUCIFEX_GID", "")
        from lucifex_cli.config import _resolve_lucifex_uid_gid
        uid, gid = _resolve_lucifex_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_lucifex_uid
# ---------------------------------------------------------------------------


class TestChownToLucifexUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_lucifex_uid(d)  # must not raise
