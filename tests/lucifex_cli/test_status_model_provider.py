"""Tests for lucifex_cli.status model/provider display."""
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("lucifex_cli.status.managed_nous_tools_enabled", lambda: True)
    from lucifex_cli import status as status_mod
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "lucifex_cli.models.probe_lmstudio_models",
