...
def test_billing_overview_non_interactive_renders_text_not_modal(cli, monkeypatch, capsys):
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
    state = BillingState(
        logged_in=True,
        org_name="Acme",
        role="OWNER",
...
def test_buy_flow_no_card_guides_then_continues_after_recheck(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted("recheck", "cancel"), raising=False)
...
def test_buy_flow_no_card_back_abandons(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(LucifexCLI, "_prompt_text_input_modal", _scripted("cancel"), raising=False)
