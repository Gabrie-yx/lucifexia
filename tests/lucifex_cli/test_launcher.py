"""Tests for the top-level `./lucifex` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lucifex` should use `lucifex_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lucifex"
    called = []

    fake_main_module = types.ModuleType("lucifex_cli.main")

    def fake_main():
        called.append("lucifex_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lucifex_cli.main", fake_main_module)
    ...
    assert called == ["lucifex_cli.main"]
