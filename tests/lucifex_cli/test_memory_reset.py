"""Tests for the `lucifex memory reset` CLI command.
...
- Profile-scoped reset (uses LUCIFEX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LUCIFEX_HOME with memory files."""
    lucifex_home = tmp_path / ".lucifex"
    memories = lucifex_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nLucifex repo is at ~/.lucifex/lucifex-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `lucifex memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        (lucifex_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LUCIFEX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_lucifex_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
