...
from lucifex_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("lucifex_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import lucifex_cli.gateway as _gw_mod
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("lucifex_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("lucifex_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "lucifex_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "lucifex_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("lucifex_cli.setup.prompt_choice", fake_prompt_choice)
    ...
    from lucifex_cli.setup import setup_terminal_backend
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """LUCIFEX_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("LUCIFEX_NONINTERACTIVE", "1")
...
def test_prompt_yes_no_eof_returns_default_instead_of_exiting(monkeypatch):
    ...
    monkeypatch.delenv("LUCIFEX_NONINTERACTIVE", raising=False)
