"""Tests for lucifex_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $LUCIFEX_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Lucifex-managed node dir.
"""
...
def _make_lucifex_node(lucifex_home: Path) -> Path:
    """Create a fake $LUCIFEX_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = lucifex_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_lucifex_node(fake_home):
    lucifex_home = fake_home / ".lucifex"
    node_bin = _make_lucifex_node(lucifex_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(lucifex_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    lucifex_home = fake_home / ".lucifex"
    _make_lucifex_node(lucifex_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(lucifex_home) == []


def test_removes_dangling_symlink_into_lucifex_node(fake_home):
    """A link into the Lucifex node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Lucifex links that exist; ignores the ones that don't."""
    lucifex_home = fake_home / ".lucifex"
    node_bin = _make_lucifex_node(lucifex_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Lucifex-managed; node is a real user binary.
