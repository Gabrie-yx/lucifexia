...
Regression guard for the bug where `lucifex model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`lucifex_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from lucifex_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in LUCIFEX_OVERLAYS
        overlay = LUCIFEX_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `lucifex_cli/main.py` (cmd_model): read
