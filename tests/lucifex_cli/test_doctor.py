...
import lucifex_cli.doctor as doctor
import lucifex_cli.gateway as gateway_cli
from lucifex_cli import doctor as doctor_mod
from lucifex_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `lucifex doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "LUCIFEX_HOME", lucifex_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("LUCIFEX_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_lucifex_home(self, tmp_path, provider=""):
        """Create a minimal LUCIFEX_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_lucifex_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "LUCIFEX_HOME", home)
...
def test_run_doctor_accepts_lucifex_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp LUCIFEX_HOME.

        ``run_doctor`` reads the module-level ``LUCIFEX_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("LUCIFEX_HOME")`` alone
        leaves doctor probing the REAL ~/.lucifex. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_lucifex_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from lucifex_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("lucifex_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "lucifex_cli.doctor", raising=False)
        from lucifex_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from lucifex_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_lucifex_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Lucifex portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale LUCIFEX_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"LUCIFEX_MAX_ITERATIONS={ghost}\n")
        (lucifex_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "LUCIFEX_HOME", lucifex_home)
        monkeypatch.setattr(doctor_mod, "get_lucifex_home", lambda: lucifex_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("LUCIFEX_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("LUCIFEX_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale LUCIFEX_MAX_ITERATIONS" in out
        env_after = (lucifex_home / ".env").read_text(encoding="utf-8")
        assert "LUCIFEX_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`lucifex doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Lucifex install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "LUCIFEX_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "QQ_HOME_CHANNEL" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["LUCIFEX_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir(parents=True)
        (lucifex_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (lucifex_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, lucifex_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (lucifex_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: LUCIFEX_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: LUCIFEX_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"LUCIFEX_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: LUCIFEX_TOOL_PROGRESS_MODE" in out
