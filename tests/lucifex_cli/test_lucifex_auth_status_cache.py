"""Tests for the get_lucifex_auth_status() process-level cache.

The cache avoids re-validating Nous credentials on every menu paint —
`lucifex tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
...
also call invalidate_lucifex_auth_status_cache().
"""
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test LUCIFEX_HOME.
...
def test_get_lucifex_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    auth_mod.invalidate_lucifex_auth_status_cache()
    ...
    with patch.object(auth_mod, "_compute_lucifex_auth_status", side_effect=fake_compute):
        first = auth_mod.get_lucifex_auth_status()
        second = auth_mod.get_lucifex_auth_status()
        third = auth_mod.get_lucifex_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_lucifex_auth_status was called {call_count['n']}× — "
        "cache is not deduplicating within TTL."
    )
    # Each call returns a copy so callers can't mutate the cached dict.
    assert first == second == third
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_lucifex_auth_status()

    auth_mod.invalidate_lucifex_auth_status_cache()


def test_get_lucifex_auth_status_invalidates_on_auth_file_mtime(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_lucifex_auth_status", side_effect=fake_compute):
        auth_mod.get_lucifex_auth_status()
...
def test_get_lucifex_auth_status_cache_is_scoped_by_auth_file_path(tmp_path, monkeypatch):
...
def test_invalidate_lucifex_auth_status_cache_forces_recompute(tmp_path, monkeypatch):
...
def test_get_lucifex_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too — that's where the cost was.

    Teknium's case: ~31 cache misses per `lucifex tools` "All Platforms"
