...
board into `LUCIFEX_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`lucifex kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `lucifex kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `LUCIFEX_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("LUCIFEX_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["LUCIFEX_KANBAN_BOARD"] = prev


def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("lucifex_cli.main")

    import lucifex_cli.kanban_db as kdb
    monkeypatch.setattr(kdb, "get_current_board", lambda: "space")

    main_mod._pin_kanban_board_env()

    assert main_mod.os.environ.get("LUCIFEX_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("LUCIFEX_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("lucifex_cli.main")
    ...
    assert main_mod.os.environ.get("LUCIFEX_KANBAN_BOARD") == "preset"
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "LUCIFEX_KANBAN_BOARD" not in main_mod.os.environ
