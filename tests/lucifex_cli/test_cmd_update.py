...
from lucifex_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `lucifex update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".lucifex"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(lucifex_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            lucifex_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("LUCIFEX_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("LUCIFEX_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/lucifex-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "lucifex-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/lucifex-launcher-venv"
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $LUCIFEX_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("lucifex_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/lucifex-agent silently misses updates.
        """
        from lucifex_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/lucifex-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lucifex_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "lucifex_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("lucifex_cli.config.check_config_version", return_value=(1, 2)), patch(
            "lucifex_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("lucifex_cli.main.sys") as mock_sys:
            ...
            from lucifex_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lucifex_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "lucifex_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "lucifex_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "lucifex_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lucifex_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "lucifex_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "lucifex_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "lucifex_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("lucifex_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.lucifex"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.lucifex/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.lucifex/profiles/work"))
        ...
        with (
            patch("lucifex_cli.profiles.list_profiles", return_value=all_profiles),
            patch("lucifex_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("lucifex_cli.profiles.list_profiles", return_value=[default_p]),
            patch("lucifex_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``lucifex update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``lucifex update --check --branch <name>`` honors the branch override.
    ...
    @patch("lucifex_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("lucifex_cli.config.detect_install_method", return_value="pip")
    @patch("lucifex_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``lucifex update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from lucifex_cli.main import _update_via_zip
