...
from lucifex_cli.main, these tests mock the delegation point and verify
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside lucifex setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("lucifex_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("lucifex_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'lucifex setup' to configure" not in output
