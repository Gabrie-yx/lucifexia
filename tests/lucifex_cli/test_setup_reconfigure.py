...
- Bare `lucifex setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `lucifex setup --quick` runs the narrower "fill in missing items" flow.
- `lucifex setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("lucifex_cli.setup.ensure_lucifex_home", {}),
        ("lucifex_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("lucifex_cli.config.is_managed", {"return_value": False}),
        ("lucifex_cli.setup.load_config", {"return_value": {}}),
        ("lucifex_cli.setup.save_config", {}),
        ("lucifex_cli.setup.get_env_value", {"return_value": None}),
        ("lucifex_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("lucifex_cli.setup._print_setup_summary", {}),
        ("lucifex_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("lucifex_cli.setup.load_config", {"return_value": {}}),
        ("lucifex_cli.setup.save_config", {}),
        ("lucifex_cli.auth.get_active_provider", {"return_value": None}),
        ("lucifex_cli.setup.get_env_value", {"return_value": None}),
        ("lucifex_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `lucifex setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="lucifex_cli.setup.prompt_choice",
                quick="lucifex_cli.setup._run_quick_setup",
                model="lucifex_cli.setup.setup_model_provider",
                terminal="lucifex_cli.setup.setup_terminal_backend",
                agent="lucifex_cli.setup.setup_agent_settings",
                gateway="lucifex_cli.setup.setup_gateway",
                tools="lucifex_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`lucifex setup --reconfigure` behaves the same as bare `lucifex setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("lucifex_cli.setup.prompt_choice", {"return_value": 0}),
                first="lucifex_cli.setup._run_first_time_quick_setup",
            )
            from lucifex_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "lucifex_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["lucifex", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lucifex", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lucifex", "setup"])
        try:
            main()
        except SystemExit:
            pass
