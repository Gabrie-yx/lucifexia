...
# The curated list in lucifex_cli/models.py defines the floor; models.dev only
...
def test_opencode_go_appears_when_api_key_set():
    ...
    # models.dev is reachable) or "lucifex" (from LUCIFEX_OVERLAYS fallback when
    # the API is unavailable, e.g. in CI).
    assert opencode_go["source"] in {"built-in", "lucifex"}
