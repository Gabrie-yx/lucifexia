...
2. Spawn: _default_spawn sets the LUCIFEX_KANBAN_GOAL_MODE env vars only
...
def test_spawn_sets_goal_env_only_when_enabled(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("LUCIFEX_KANBAN_GOAL_MODE") == "1"
    assert env.get("LUCIFEX_KANBAN_GOAL_MAX_TURNS") == "5"
...
def test_spawn_no_goal_env_for_plain_task(kanban_home, monkeypatch):
    ...
    kb._default_spawn(task, str(kanban_home))
    env = captured["env"]
    assert "LUCIFEX_KANBAN_GOAL_MODE" not in env
    assert "LUCIFEX_KANBAN_GOAL_MAX_TURNS" not in env
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (lucifex kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """lucifex kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from lucifex_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("lucifex_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("lucifex_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("lucifex_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("lucifex_cli.kanban._worker_run_id_for", lambda _: None)
