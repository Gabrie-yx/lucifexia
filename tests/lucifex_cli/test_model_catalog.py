"""Tests for lucifex_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate LUCIFEX_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from lucifex_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "Lucifex portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from lucifex_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://lucifex-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/lucifex-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import lucifex_cli.model_catalog as model_catalog
        from lucifex_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in lucifex_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from lucifex_cli import model_catalog
        from lucifex_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from lucifex_cli import model_catalog
        from lucifex_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``lucifex model`` CLI — not the live
        ...
        # ``LUCIFEX_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` LUCIFEX_HOME directly instead.
        import importlib
        from lucifex_cli import model_catalog
        from lucifex_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from lucifex_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["LUCIFEX_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("lucifex_cli.models.check_nous_free_tier", return_value=False), patch(
                "lucifex_cli.models.union_with_portal_free_recommendations",
