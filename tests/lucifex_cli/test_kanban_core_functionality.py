...
Complements tests/lucifex_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/lucifex_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/lucifex_cli/test_kanban_db.py.
    monkeypatch.setenv("LUCIFEX_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``lucifex kanban create --max-retries N``)
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.lucifex/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("LUCIFEX_HOME", raising=False)
    profiles = tmp_path / ".lucifex" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom LUCIFEX_HOME root."""
...
def test_parse_duration_accepts_formats():
    from lucifex_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`lucifex kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: lucifex_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "lucifex_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty LUCIFEX_HOME must not error with
    ...
    env = {**os.environ, "LUCIFEX_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "lucifex_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh LUCIFEX_HOME must create the
...
def test_cli_show_json_carries_runs(kanban_home):
    """lucifex kanban show --json must include runs[] so scripts that
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    + "comment from worker" prefix so a misleading LUCIFEX_PROFILE name
    (e.g. "lucifex-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth — see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="lucifex-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**lucifex-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `lucifex-system` at " in ctx
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    lucifex subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("LUCIFEX_KANBAN_TASK") == tid
    assert env.get("LUCIFEX_PROFILE") == "some-profile"
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`lucifex kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`lucifex kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from lucifex_cli import kanban as kb_cli
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "lucifex gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "lucifex gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "lucifex gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "lucifex gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`lucifex kanban daemon` (no --force) is a deprecation stub."""
    from lucifex_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "lucifex gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `lucifex kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """LUCIFEX_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("LUCIFEX_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``lucifex_cli.kanban_db`` fresh and uses that single
