...
``~/.lucifex/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(lucifex_home):
...
def test_remove_export_prefixed_token(lucifex_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(lucifex_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
    ...
    env_text = lucifex_home.joinpath(".env").read_text(encoding="utf-8")
...
def test_update_export_prefixed_token_does_not_duplicate(lucifex_home):
...
def test_plain_line_save_and_remove_still_work(lucifex_home):
    """Sanity: the ordinary KEY= path is unchanged."""
    from lucifex_cli.config import load_env, remove_env_value, save_env_value
...
def test_export_line_with_comment_untouched(lucifex_home):
