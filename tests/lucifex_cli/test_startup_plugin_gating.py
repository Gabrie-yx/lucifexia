...
``lucifex_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``lucifex --help``, ``lucifex version``,
``lucifex logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``lucifex --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from lucifex_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["lucifex", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["lucifex", "-w", "--tui"], None),
        (["lucifex", "version"], "version"),
        (["lucifex", "--tui", "chat"], "chat"),
        (["lucifex", "-w", "logs"], "logs"),
        (["lucifex", "chat", "hello world"], "chat"),
        (["lucifex", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["lucifex", "-m", "gpt5", "chat"], "chat"),
        (["lucifex", "--model", "gpt5", "chat", "hi"], "chat"),
        (["lucifex", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["lucifex", "-z", "hello world"], None),
        (["lucifex", "-z", "hello", "chat"], "chat"),
        (["lucifex", "--model=gpt5", "chat"], "chat"),     # inline form
        (["lucifex", "--", "chat"], "chat"),               # -- terminator
        (["lucifex", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["lucifex", "some-plugin-cmd"], "some-plugin-cmd"),
        (["lucifex", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["lucifex"],                          # bare → chat
        ["lucifex", "--help"],                # top-level help
        ["lucifex", "-h"],
        ["lucifex", "version"],               # known built-in
        ["lucifex", "logs"],
        ["lucifex", "gateway", "run"],
        ["lucifex", "--tui"],
        ["lucifex", "-w", "--tui"],
        ["lucifex", "chat", "hi"],
        ["lucifex", "help"],                  # accepted built-in-ish
        ["lucifex", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["lucifex", "meet", "join"],          # potential google_meet plugin
        ["lucifex", "honcho", "status"],      # potential memory plugin
        ["lucifex", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``lucifex help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"lucifex_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
