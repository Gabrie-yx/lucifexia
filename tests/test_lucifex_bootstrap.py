"""Tests for lucifex_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lucifex entry point
(lucifex, lucifex-agent, lucifex-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lucifex entry point imports lucifex_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lucifex_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lucifex_bootstrap", None)
    import lucifex_bootstrap  # noqa: WPS433
    return lucifex_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lucifex CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lucifex entry point must import lucifex_bootstrap as its
    ...
    # Entry points that invoke Lucifex as a process.  Each one must
    # import lucifex_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lucifex_cli/main.py",   # lucifex CLI (console_script)
        "run_agent.py",          # lucifex-agent (console_script)
        "acp_adapter/entry.py",  # lucifex-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lucifex_bootstrap' and that
        ...
        half-finished ``lucifex update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lucifex_bootstrap`` as a top-level module) leaves lucifex
        ...
        """
        # Resolve relative to the lucifex-agent repo root.  Tests live
        # at tests/test_lucifex_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lucifex start even when lucifex_bootstrap hasn't been
        ...
        assert first_import_name == "lucifex_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lucifex_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lucifex_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Lucifex's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("LUCIFEX_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["LUCIFEX_PYTHON_SRC_ROOT"] = env
            elif "LUCIFEX_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["LUCIFEX_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/lucifex")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LUCIFEX_PYTHON_SRC_ROOT", None)
            else:
                os.environ["LUCIFEX_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/lucifex", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Lucifex root.  The guard must relocate Lucifex to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/lucifex"])
        assert result[0] == "/opt/lucifex"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Lucifex root.
        assert result.index("/opt/lucifex") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/lucifex", "/opt/lucifex", ""])
        assert result.count("/opt/lucifex") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["LUCIFEX_PYTHON_SRC_ROOT"] = "/env/lucifex"
            hb.harden_import_path()
            assert sys.path[0] == "/env/lucifex"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LUCIFEX_PYTHON_SRC_ROOT", None)
