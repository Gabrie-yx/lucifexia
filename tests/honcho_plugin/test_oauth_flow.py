    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"lucifex": {"saveMessages": True, "recallMode": "hybrid"}},
            }
...
def fake_as(monkeypatch):
    ...
    monkeypatch.setenv("HONCHO_OAUTH_CLIENT_ID", "lucifex-desktop")
    try:
        yield base
    finally:
        server.shutdown()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["lucifex"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["lucifex"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["lucifex"]["recallMode"] == "hybrid"
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "lucifex", now=saved["hosts"]["lucifex"]["oauth"]["expiresAt"] + 10
    )
    assert refreshed is True
    assert token == "hch-at-2"
    rotated = json.loads(config_path.read_text())["hosts"]["lucifex"]["oauth"]
...
def test_state_mismatch_is_rejected(fake_as, tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown or expired"):
        oauth_flow.complete_authorization(
            ...
            config_path=tmp_path / "honcho.json", host="lucifex",
        )
...
def test_source_tags_the_authorize_link(fake_as):
    ...
    assert "source=lucifex-cli" in url
    untagged, _ = oauth_flow.begin_authorization(endpoints)
    assert "source=" not in untagged


def test_client_id_defaults_to_lucifex_agent(monkeypatch):
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["lucifex"]["oauth"]["clientId"] == "lucifex-agent"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".lucifex" / "profiles" / "work" / "honcho.json"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"lucifex": {"saveMessages": False}}}))
...
def test_launcher_runs_flow_in_background_and_reports_connected(monkeypatch, reset_flow):
    ...
    st = oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="lucifex")
    assert st["state"] == "pending"  # returns immediately, before the flow finishes
    assert _wait_until(lambda: seen.get("source") == "lucifex-desktop")  # default source tag
    assert seen["host"] == "lucifex"
    gate.set()
...
def test_launcher_reports_error_on_flow_failure(monkeypatch, reset_flow):
    ...
    oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="lucifex")
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="lucifex")
    assert _wait_until(lambda: len(calls) == 1)  # first flow is running
    s2 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="lucifex")
    block.set()
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "lucifex")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"lucifex": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"lucifex": {"apiKey": "hch-v3-static"}}}))
    ...
    cfgfile.write_text(json.dumps({"hosts": {"lucifex": {
        ...
        "oauth": {"refreshToken": "hch-rt-x", "expiresAt": 9_999_999_999,
                  "clientId": "lucifex-desktop", "tokenEndpoint": "http://x/oauth/token"},
    }}}))
    s = oauth_flow.get_flow_status()
