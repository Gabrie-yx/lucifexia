...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing LUCIFEX_ prefixed keys.
        assert "LUCIFEX_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "LUCIFEX_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("LUCIFEX_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
...
# LUCIFEX_LANGFUSE_PUBLIC_KEY / SECRET_KEY at a template value like
...
# Lucifex logs.  The fix in `_get_langfuse()` validates the documented
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "LUCIFEX_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "LUCIFEX_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "LUCIFEX_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "LUCIFEX_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("LUCIFEX_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("LUCIFEX_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("LUCIFEX_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "LUCIFEX_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("LUCIFEX_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("LUCIFEX_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "LUCIFEX_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("LUCIFEX_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("LUCIFEX_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
    ...
    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical LUCIFEX_-prefixed env var and
        ...
        # LUCIFEX_-prefixed one — operators set the LUCIFEX_-prefixed one).
        assert "LUCIFEX_LANGFUSE_PUBLIC_KEY" in caplog.text
