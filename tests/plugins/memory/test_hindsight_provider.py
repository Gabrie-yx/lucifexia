...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Lucifex Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "lucifex"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["lucifex"]["bankId"] == "env-bank"
        assert cfg["banks"]["lucifex"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        assert not (user_home / ".hindsight" / "profiles" / "lucifex.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "lucifex.env"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Lucifex Agent and the User"
        ...
        assert item["metadata"]["source"] == "lucifex"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Lucifex Agent and the User"
...
class TestBankIdTemplate:
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="lucifex", profile="coder"
        )
        assert result == "lucifex"

    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "lucifex-{profile}", fallback="lucifex",
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "lucifex-{user}" becomes "lucifex-" -> trimmed to "lucifex"
        result = _resolve_bank_id_template(
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "lucifex-{unknown}", fallback="lucifex",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "lucifex"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "lucifex-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="lucifex",
        )
        assert p._bank_id == "lucifex-coder"
        assert p._bank_id_template == "lucifex-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "lucifex-" which collapses to "lucifex"
