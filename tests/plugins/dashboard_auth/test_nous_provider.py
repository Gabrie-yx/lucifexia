...
Covers four shapes from Phase 4 of ``.lucifex/plans/2026-05-21-dashboard-oauth-auth.md``:
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="lucifex-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        ...
        assert "LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        ...
        assert "lucifex-dashboard" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("LUCIFEX_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("LUCIFEX_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.nousresearch.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID``
    and ``LUCIFEX_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "LUCIFEX_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://lucifex.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://lucifex.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "lucifex_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["lucifex_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["lucifex_session_pkce"] != b.cookie_payload[
            "lucifex_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://lucifex.fly.dev/auth/callback")
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between LUCIFEX_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
        token = _mint_token(rsa_keypair, iss="https://evil.example")
