...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "lucifexex_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``lucifexex kanban boards switch`` last pinned.
    """
...
def test_board_auto_initializes_missing_db(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("lucifexex_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "lucifex_cli.web_server", stub)
    monkeypatch.setattr(lucifex_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "lucifex_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `lucifex gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
# tests/lucifex_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "lucifex_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see lucifex_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "lucifexex-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "lucifexex-kanban-card--failed" in js
    assert "lucifexex-kanban-card--failed" in css
