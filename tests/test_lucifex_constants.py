"""Tests for lucifex_constants module."""
...
from lucifex_constants import (
    ...
    find_lucifex_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_lucifex_root,
    get_lucifex_dir,
    get_lucifex_home,
    get_process_lucifex_home,
    heal_lucifex_managed_node,
    lucifex_managed_node_tree_present,
    iter_lucifex_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultLucifexRoot:
    """Tests for get_default_lucifex_root() — Docker/custom deployment awareness."""

    def test_no_lucifex_home_returns_native(self, tmp_path, monkeypatch):
        """When LUCIFEX_HOME is not set, returns ~/.lucifex."""
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_lucifex_root() == tmp_path / ".lucifex"

    def test_lucifex_home_is_native(self, tmp_path, monkeypatch):
        """When LUCIFEX_HOME = ~/.lucifex, returns ~/.lucifex."""
        native = tmp_path / ".lucifex"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(native))
        assert get_default_lucifex_root() == native

    def test_lucifex_home_is_profile(self, tmp_path, monkeypatch):
        """When LUCIFEX_HOME is a profile under ~/.lucifex, returns ~/.lucifex."""
        native = tmp_path / ".lucifex"
    ...
    def test_lucifex_home_is_docker(self, tmp_path, monkeypatch):
        """When LUCIFEX_HOME points outside ~/.lucifex (Docker), returns LUCIFEX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(docker_home))
        assert get_default_lucifex_root() == docker_home

    def test_lucifex_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LUCIFEX_HOME outside ~/.lucifex is treated as the root."""
        custom = tmp_path / "my-lucifex-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(custom))
        assert get_default_lucifex_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (LUCIFEX_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LUCIFEX_HOME", str(profile))
        assert get_default_lucifex_root() == docker_root

    def test_no_lucifex_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\lucifex, not ~/.lucifex."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lucifex_constants.sys, "platform", "win32")

        assert get_default_lucifex_root() == local_appdata / "lucifex"

    def test_no_lucifex_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/lucifex without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
        ...
        assert get_default_lucifex_root() == home / "AppData" / "Local" / "lucifex"


class TestGetLucifexHome:
    """Tests for get_lucifex_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When LUCIFEX_HOME is unset on Windows, use %LOCALAPPDATA%\\lucifex."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lucifex_constants.sys, "platform", "win32")
        monkeypatch.setattr(lucifex_constants, "_profile_fallback_warned", False)

        assert get_lucifex_home() == local_appdata / "lucifex"


class TestGetProcessLucifexHome:
    """Tests for get_process_lucifex_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_lucifex_home() == home

    def test_env_unset_returns_platform_default(self, tmp_path, monkeypatch):
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_process_lucifex_home() == tmp_path / ".lucifex"
    ...
    def test_ignores_context_local_override(self, tmp_path, monkeypatch):
        ...
        token = set_lucifex_home_override(profile_home)
        try:
            # get_lucifex_home() follows the override; the process-scoped
            # variant must not.
            assert get_lucifex_home() == profile_home
            assert get_process_lucifex_home() == launch_home
        finally:
            reset_lucifex_home_override(token)


class TestLucifexManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_lucifex_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lucifex_constants.sys, "platform", "win32")
        monkeypatch.setenv("LUCIFEX_HOME", str(home))
        monkeypatch.setattr(lucifex_constants, "node_tool_runnable", lambda path: True)

        assert find_lucifex_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(lucifex_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(lucifex_constants, "heal_lucifex_managed_node", lambda: False)
        monkeypatch.setattr(
            lucifex_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert lucifex_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_lucifex_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "lucifex"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_lucifex_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Lucifex-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lucifex_constants, "heal_lucifex_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_lucifex_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(lucifex_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(lucifex_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from lucifex_constants import resolve_per_model_reasoning_effort
...
class TestReasoningOverridesDefaultConfig:
    ...
    def test_load_config_preserves_user_reasoning_overrides(self, tmp_path, monkeypatch):
        ...
        import yaml
        from lucifex_cli.config import load_config, get_config_path
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".lucifex"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import lucifex_cli._subprocess_compat as subprocess_compat
...
class TestGetLucifexDir:
    """Tests for ``get_lucifex_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_lucifex_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_lucifex_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_lucifex_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_lucifex_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert lucifex_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert lucifex_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert lucifex_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert lucifex_constants.windows_path_to_wsl("/home/alex") is None
        assert lucifex_constants.windows_path_to_wsl("relative\\dir") is None

    def test_wsl_unc_to_posix_both_spellings(self):
        assert lucifex_constants.wsl_unc_path_to_posix(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        assert lucifex_constants.wsl_unc_path_to_posix(r"\\wsl$\Ubuntu\home\alex") == "/home/alex"
        # Forward-slash spelling and distro root.
        assert lucifex_constants.wsl_unc_path_to_posix("//wsl.localhost/Debian/srv/app") == "/srv/app"
        assert lucifex_constants.wsl_unc_path_to_posix("\\\\wsl.localhost\\Ubuntu\\") == "/"

    def test_wsl_unc_ignores_non_unc_paths(self):
        assert lucifex_constants.wsl_unc_path_to_posix(r"C:\Users\alex") is None
        assert lucifex_constants.wsl_unc_path_to_posix("/home/alex") is None

    def test_translate_is_noop_off_wsl(self, monkeypatch):
        monkeypatch.setattr(lucifex_constants, "is_wsl", lambda: False)
        assert lucifex_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == r"C:\Users\alex"

    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(lucifex_constants, "is_wsl", lambda: True)
        assert lucifex_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert lucifex_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert lucifex_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
