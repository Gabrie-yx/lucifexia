...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("lucifex_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/lucifex/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/lucifex/profiles/from-source")
    ...
    def test_routing_wins_over_active_profile(self, mock_runner, discord_source):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("lucifex_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/lucifex/profiles/routed")
                    ...
                    assert result == Path("/lucifex/profiles/routed")
    ...
    def test_active_profile_fallback(self, mock_runner, discord_source):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/lucifex/profiles/active")
                ...
                assert result == Path("/lucifex/profiles/active")
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value=None):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/lucifex")
                
                mock_runner._profile_name_for_source = MagicMock(return_value=None)
                
                result = mock_runner._resolve_profile_home_for_source(discord_source)
                
                assert result == Path("/lucifex")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/lucifex/profiles/nonexistent")
                with patch("lucifex_cli.profiles.profile_exists", return_value=False):
                    with patch("lucifex_constants.get_lucifex_home", return_value=Path("/lucifex")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global LUCIFEX_HOME
                            assert result == Path("/lucifex")
    ...
    def test_existing_profile_no_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/lucifex/profiles/existing")
                with patch("lucifex_cli.profiles.profile_exists", return_value=True):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
                        
                        assert result == Path("/lucifex/profiles/existing")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("lucifex_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("lucifex_constants.get_lucifex_home", return_value=Path("/lucifex")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("lucifex_cli.profiles.get_active_profile_name", return_value=None):
            with patch("lucifex_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("lucifex_constants.get_lucifex_home", return_value=Path("/lucifex")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
