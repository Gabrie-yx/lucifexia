...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_lucifex_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <LUCIFEX_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``lucifex pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <LUCIFEX_HOME>/profiles/yangyang/pairing/."""
        from lucifex_constants import get_lucifex_home
        monkeypatch.setattr("lucifex_constants.get_lucifex_home", lambda: tmp_path)
