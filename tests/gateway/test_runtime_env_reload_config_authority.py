...
LUCIFEX_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env LUCIFEX_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    lucifex_home = tmp_path / ".lucifex"
    lucifex_home.mkdir()
    (lucifex_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (lucifex_home / ".env").write_text("LUCIFEX_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["LUCIFEX_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("LUCIFEX_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["LUCIFEX_MAX_ITERATIONS"] = "200"
