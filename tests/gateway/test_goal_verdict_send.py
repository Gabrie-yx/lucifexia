...
async def test_goal_verdict_done_sent_via_adapter_send(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None)):
...
async def test_goal_verdict_continue_enqueues_continuation(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_budget_exhausted_sends_pause(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("continue", "keep going", False, None)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_survives_adapter_without_send(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("done", "ok", False, None)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
