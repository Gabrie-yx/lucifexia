...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test LUCIFEX_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None, False)):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None, False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(lucifex_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from lucifex_cli.goals import GoalManager, save_goal
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("continue", "keep going", False, None, False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(lucifex_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(lucifex_home):
    ...
    with patch("lucifex_cli.goals.judge_goal", return_value=("done", "ok", False, None, False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
