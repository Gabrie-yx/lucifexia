...
class TestBypassWithBotnameSuffix:
    ...
    async def test_stop_with_botname(self):
        """/stop@MyLucifexBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/stop@MyLucifexBot"))

        assert sk not in adapter._pending_messages, (
            "/stop@MyLucifexBot was queued instead of bypassing"
        )
        assert any("handled:stop" in r for r in adapter.sent_responses)

    @pytest.mark.asyncio
    async def test_new_with_botname(self):
        """/new@MyLucifexBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/new@MyLucifexBot"))
