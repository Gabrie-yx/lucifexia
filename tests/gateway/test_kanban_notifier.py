...
def test_kanban_notifier_dedupes_board_slugs_pointing_to_same_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LUCIFEX_KANBAN_DB", str(db_path))
    kb.init_db()
...
def test_kanban_db_path_is_test_isolated_from_real_home():
    lucifex_home = Path(kb.kanban_home())
    production_db = Path.home() / ".lucifex" / "kanban.db"
    ...
    assert kb.kanban_db_path().resolve().is_relative_to(lucifex_home.resolve())
    assert kb.kanban_db_path().resolve() != production_db.resolve()
