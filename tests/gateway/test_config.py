...
class TestLoadGatewayConfig:
    def test_shipped_template_does_not_enable_auto_reset(self, tmp_path, monkeypatch):
        ...
        docker/stage2-hook.sh, lucifex doctor) copy the template verbatim to
        ~/.lucifex/config.yaml, so whatever ``session_reset.mode`` the template
    ...
    def test_typing_status_text_from_toplevel_platform_block(self, tmp_path, monkeypatch):
        ...
        fallback) — the route a bare ``lucifex config set``-style YAML uses."""
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text(
    ...
    def test_multiplex_profiles_from_nested_gateway_section(self, tmp_path, monkeypatch):
        ...
        ``lucifex config set gateway.multiplex_profiles true``) must enable
    ...
    def test_profile_scoped_env_overrides_do_not_fall_back_to_default_profile_env(
        ...
        home_token = set_lucifex_home_override(str(secondary_home))
...
class TestMultiplexProfilesEnvOverride:
    ...
    def _load(self, tmp_path, monkeypatch, config_text=None):
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir(exist_ok=True)
        if config_text is not None:
            (lucifex_home / "config.yaml").write_text(config_text, encoding="utf-8")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
...
class TestMultiplexProfilesConfig:
    ...
    def test_multiplex_profiles_nested_under_gateway(self, tmp_path, monkeypatch):
        """gateway.multiplex_profiles (the form written by `lucifex config set
    ...
    def test_multiplex_profiles_default_false(self, tmp_path, monkeypatch):
        ...
        lucifex_home = tmp_path / ".lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("", encoding="utf-8")
        monkeypatch.setenv("LUCIFEX_HOME", str(lucifex_home))
