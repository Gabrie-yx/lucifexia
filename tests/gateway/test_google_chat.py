...
def adapter(tmp_path):
    ...
    ~/.lucifex/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.lucifex/...)
...
class TestOnPubsubMessage:
    ...
    def test_relay_flat_bot_sender_is_filtered_end_to_end(self, adapter):
        ...
        envelope = {
            ...
            "sender_display_name": "lucifexexBot",
            "sender_type": "BOT",
            "text": "reply from bot",
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the lucifexex host. The relay flattens Chat
...
class TestSend:
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "lucifexex_clarify"
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_default_status_text(self, adapter):
        ...
        await adapter.send_typing("spaces/S")
        body = adapter._create_message.await_args.args[1]
        assert body["text"] == "lucifexex is thinking…"
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'lucifexex is thinking…' stuck in chat. Race fix via
    ...
    async def test_orphan_typing_cards_reaped_on_completion(self, adapter):
        ...
        'lucifexex is thinking…' messages."""
        from plugins.platforms.google_chat.adapter import _TYPING_CONSUMED_SENTINEL
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/lucifexex-chat"
            ),
        ...
        monkeypatch.setattr("lucifex_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("lucifex_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("lucifex_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "lucifex_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "lucifex_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "lucifex_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "lucifex_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
