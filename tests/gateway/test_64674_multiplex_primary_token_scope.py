...
class TestLoadGatewayConfigForRunner:
    ...
    def test_scoped_reload_picks_up_default_profile_token(self, tmp_path, monkeypatch):
        ...
        from gateway import run as run_mod
        import lucifex_constants as hc
        ...
        monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
        # Point both lucifex_constants and gateway.run at our temp home.
        monkeypatch.setattr(hc, "get_lucifex_home", lambda: home)
        monkeypatch.setattr(run_mod, "get_lucifex_home", lambda: home)
        monkeypatch.setattr(run_mod, "_LUCIFEX_HOME", home)
