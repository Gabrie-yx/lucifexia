...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.lucifex."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /lucifex, /btw, /stop,
        ...
        for expected in ("/lucifex", "/btw", "/stop", "/model", "/help"):
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "lucifex_feedback" in created_apps[0].registered_actions
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@lucifex`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@lucifex"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@lucifex")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /lucifex subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /lucifex btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_lucifex_prefix_still_works(self, adapter):
        """Backward compat: /lucifex btw foo must still route to /btw foo.

        Old workspace manifests only declared /lucifex as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_lucifex_freeform_question(self, adapter):
        """/lucifex <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/lucifex",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_lucifex_slash_stashes_context(self, adapter):
        """Legacy /lucifex <subcommand> also stashes context."""
        command = {
            "command": "/lucifex",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_lucifex_question_does_not_stash_context(self, adapter):
        """Free-form /lucifex <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/lucifex",
