...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        lucifex_home = tmp_path / "lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_lucifex_home", lucifex_home)
        monkeypatch.setattr(gateway_run, "_env_path", lucifex_home / ".env")
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        lucifex_home = tmp_path / "lucifex"
        lucifex_home.mkdir()
        (lucifex_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_lucifex_home", lucifex_home)
