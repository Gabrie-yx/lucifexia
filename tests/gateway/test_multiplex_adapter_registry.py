...
class TestCredentialFingerprint:
    ...
    def test_direct_token_takes_precedence_over_config(self):
        ...
        import hashlib
        expected = hashlib.sha256(b"lucifex-mux:from-direct").hexdigest()[:16]
        assert fp == expected
...
def _install_secondary_reconnect_context(monkeypatch, runner, adapter, scoped_homes=None):
    ...
    monkeypatch.setattr(
        "lucifex_cli.profiles.get_profile_dir", lambda name: Path("/profiles") / name
    )
    monkeypatch.setattr(
...
class TestSecondaryProfileConfigHandling:
    ...
    async def test_multiplexer_skips_bad_profile_and_continues(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            "lucifex_cli.profiles.profiles_to_serve",
            lambda multiplex: [
