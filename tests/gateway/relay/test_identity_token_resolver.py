"""Unit tests for the generic-OIDC / Lucifex-portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `lucifex gateway enroll` CLI.
...
Two modes:
  ...
  2. Lucifex portal (resolve_nous_access_token) otherwise — the default.
...
def test_defaults_to_lucifex_portal_when_no_idp_configured(monkeypatch):
    called = {}

    def fake_resolve():
        called["yes"] = True
        return "lucifex-portal-token"

    monkeypatch.setattr(
        "lucifex_cli.auth.resolve_nous_access_token", fake_resolve, raising=False
    )
    assert relay._resolve_relay_identity_token() == "lucifex-portal-token"
