...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("LUCIFEX_MANAGED", "homebrew")

        # Guard: prevent any accidental fall-through from spawning a real
        # `lucifex update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade lucifex-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _lucifex_home to tmp_path and project_root to a dir without .git
        ...
        with patch("gateway.run._lucifex_home", tmp_path):
            ...
            # project_root will be the real lucifex-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_lucifex_binary(self, tmp_path):
        """Returns error when lucifex is not on PATH and lucifex_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "lucifex update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m lucifex_cli.main when lucifex not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._lucifex_home", lucifex_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Lucifex update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m lucifex_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "lucifex_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_lucifex_bin_prefers_which(self, tmp_path):
        """_resolve_lucifex_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_lucifex_bin

        with patch("shutil.which", return_value="/custom/path/lucifex"):
            result = _resolve_lucifex_bin()

        assert result == ["/custom/path/lucifex"]

    @pytest.mark.asyncio
    async def test_resolve_lucifex_bin_fallback(self):
        """_resolve_lucifex_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_lucifex_bin
        ...
        assert result == [sys.executable, "-m", "lucifex_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_lucifex_bin_returns_none_when_both_fail(self):
        """_resolve_lucifex_bin returns None when both strategies fail."""
        from gateway.run import _resolve_lucifex_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._lucifex_home", lucifex_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/lucifex" if x == "lucifex" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = lucifex_home / ".update_pending.json"
        ...
        assert not (lucifex_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((lucifex_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (lucifex_home / ".update_pending.json").exists()
        assert not (lucifex_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_blocks_programmatic_interface(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_MANAGED", "")
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        from lucifex_cli.plugins import PluginManager
        ...
        # Later steps may legitimately return success ("Starting Lucifex
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (lucifex_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = lucifex_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (lucifex_home / ".update_output.txt").write_text("done")
        (lucifex_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (lucifex_home / ".update_pending.json").write_text(json.dumps(pending))
        (lucifex_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (lucifex_home / ".update_pending.json").write_text(json.dumps(pending))
        (lucifex_home / ".update_output.txt").write_text("x" * 5000)
        (lucifex_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (lucifex_home / ".update_pending.json").write_text(json.dumps(pending))
        (lucifex_home / ".update_output.txt").write_text("Traceback: boom")
        (lucifex_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = lucifex_home / ".update_pending.json"
        output_path = lucifex_home / ".update_output.txt"
        exit_code_path = lucifex_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (lucifex_home / ".update_pending.claimed.json").exists()
