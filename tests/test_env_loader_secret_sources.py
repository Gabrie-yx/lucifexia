...
These cover the small public surface that lets `lucifexex model` /lucifexifex setup`
...
from lucifex_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_lucifex_dotenv()`` is called at module-import time from several
    hot modules (cli.py, lucifex_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per LUCIFEX_HOME per process.
    """
    ...
    # cli.py, lucifex_cli/main.py, run_agent.py, trajectory_compressor.py,
