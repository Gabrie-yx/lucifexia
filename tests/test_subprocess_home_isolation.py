...
Lucifex state stays profile-scoped through LUCIFEX_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/lucifex-agent/issues/25114
See: https://github.com/NousResearch/lucifex-agent/issues/36144
See: https://github.com/NousResearch/lucifex-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    ...
    def test_returns_none_when_lucifex_home_unset(self, monkeypatch):
        ...
        from lucifex_constants import get_subprocess_home
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        lucifex_home = real_home / ".lucifex" / "profiles" / "coder"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_REAL_HOME", str(real_home))
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from lucifex_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".lucifex" / "profiles"
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("LUCIFEX_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_lucifex_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_lucifex_home_override(profile)
        try:
            assert get_lucifex_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["LUCIFEX_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_lucifex_home_unset(self, monkeypatch):
        monkeypatch.delenv("LUCIFEX_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["LUCIFEX_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from lucifex_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from lucifex_cli.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(lucifex_home / "home"), original_home)
