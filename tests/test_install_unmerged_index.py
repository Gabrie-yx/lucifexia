...
The ``lucifexex update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m lucifex_cli.main gateway run`` — image name
    ``pythonw``, not ``lucifex.exe`` — so the ``taskkill /IM lucifex.exe`` guard
    ...
    # The lucifex.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM lucifex.exe' in text
