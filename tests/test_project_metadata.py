...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `lucifex update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid lucifex update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `lucifex update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `lucifex update` downgrades the "
