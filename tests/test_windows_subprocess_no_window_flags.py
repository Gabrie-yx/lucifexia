...
def test_copilot_gh_cli_probe_hides_gh_windows(monkeypatch):
    from lucifex_cli import copilot_auth
...
def test_gateway_pid_scan_hides_wmic_and_powershell_windows(monkeypatch):
    from lucifex_cli import gateway
    from lucifex_cli import _subprocess_compat
    ...
    def fake_run(cmd, **kwargs):
        ...
        return _Completed(stdout="CommandLine=lucifexex gateway\nProcessId=123\n")
...
def test_stale_dashboard_windows_scan_hides_wmic(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return _Completed(stdout="CommandLine=lucifexex dashboard\nProcessId=123\n")
