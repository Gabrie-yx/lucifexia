...
Converted from the former ``.github/actions/lucifex-smoke-test`` composite
...
def test_lucifex_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"lucifex --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"lucifex --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"lucifex dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
