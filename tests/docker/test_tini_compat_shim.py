...
def test_tini_compat_shim_exists(built_image: str) -> None:
    ...
    Hostinger's 'Lucifex WebUI' catalog, NAS compose projects that keep
...
def test_entrypoint_is_init_not_tini(built_image: str) -> None:
    ...
    # The entrypoint array should be ["/init", "/opt/lucifex/docker/main-wrapper.sh"]
...
def test_legacy_tini_g_entrypoint_does_not_boot_loop(built_image: str) -> None:
    ...
    restart-looped. The shim must strip ``-g`` / ``--`` and reach lucifex.
    """
    r = subprocess.run(
        [
            "docker", "run", "--rm",
