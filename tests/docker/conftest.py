...
Override the image with ``LUCIFEX_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LUCIFEX_TEST_IMAGE", "lucifex-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LUCIFEX_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LUCIFEX_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged lucifex user
# ---------------------------------------------------------------------------
#
# Background: every Lucifex runtime path inside the container drops to UID
# 10000 (the ``lucifex`` user) via ``s6-setuidgid lucifex``. ``docker exec``
...
# fails (PermissionError swallowed) for lucifex, so a test that ran as root
...
# helpers below run every probe under ``-u lucifex`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: lucifex).
