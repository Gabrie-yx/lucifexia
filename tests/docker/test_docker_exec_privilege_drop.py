...
The shim (docker/lucifex-exec-shim.sh, installed at /opt/lucifex/bin/lucifex)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> lucifex login` would write /opt/data/auth.json as
...
failed: Lucifex is not logged into Lucifex portal" on every message.

These tests verify:

1. ``docker exec <c> lucifex …`` (defaulting to root) gets dropped to the
   lucifex user before the real binary runs.
2. ``docker exec --user lucifex <c> lucifex …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $LUCIFEX_HOME from a ``docker exec`` session land
   as lucifex:lucifex — the actual user-visible invariant.
4. The LUCIFEX_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-lucifex-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown lucifex:lucifex`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to lucifex:lucifex,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$LUCIFEX_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (lucifex_cli.container_boot), which s6 runs
    *strictly after* ``01-lucifex-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_lucifex_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `lucifex` with a Python-style `-c` shim equivalent — there's no
    pure-lucifex "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `lucifex` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `lucifex`
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "lucifex", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by lucifex, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "lucifex:lucifex", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected lucifex:lucifex. "
        "The shim did not drop privileges before invoking lucifex."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user lucifex already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "lucifex", sleep_container,
         "lucifex", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user lucifex failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up lucifex:lucifex — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """LUCIFEX_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "LUCIFEX_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "lucifex", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With LUCIFEX_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``LUCIFEX_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``LUCIFEX_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"LUCIFEX_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "lucifex", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "lucifex:lucifex", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected lucifex:lucifex"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/lucifex/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid lucifex lucifex <args>` which resolves `lucifex`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> lucifex login`` (root) wrote
    ...
    "Lucifex is not logged into Lucifex portal" on every message.
    ...
    `docker exec` path produces is lucifex-readable.

    Specifically: pretend the operator ran `lucifex login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the lucifex UID. We use `lucifex auth list` since that touches the
    ...
    # `lucifex auth list` is read-only but still exercises _load_auth_store
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the lucifex user "
