...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$LUCIFEX_HOME" = "/opt/data" && '
        'test "$LUCIFEX_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_lucifex_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/lucifex/agent/message_sanitization.py")
        try:
