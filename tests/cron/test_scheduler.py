...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``lucifexex tools`` platform config for ``cron``
        ...
        # The cron default is _lucifexex_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``lucifexex tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        # Even if the user has ``lucifexex tools`` configured to enable web+file
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("lucifexex_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("lucifexex_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("lucifexex_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("lucifexex_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("lucifexex_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("lucifexex_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("lucifexex_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("lucifexex_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("lucifexex_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("lucifexex_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("lucifexex_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("lucifexex_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("lucifexex_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_lucifexex_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_lucifexex_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_lucifexex_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_lucifexex_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_lucifexex_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_lucifexex_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_lucifexex_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``lucifexex_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses lucifexex_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("lucifexex_MODEL", "env-model")
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved lucifexex_MODEL.
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """lucifexex_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("lucifexex_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``lucifexex cron create --deliver=<platform>`` silently
