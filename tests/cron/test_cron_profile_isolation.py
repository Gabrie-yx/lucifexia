...
profile's LUCIFEX_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.lucifex/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active LUCIFEX_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever LUCIFEX_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    LUCIFEX_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import lucifex_constants

    monkeypatch.setattr(
        lucifex_constants, "_get_platform_default_lucifex_home", lambda: root
    )
    monkeypatch.setenv("LUCIFEX_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile LUCIFEX_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "LUCIFEX_HOME"
    ...
    # cron/jobs.py computes HERMES_DIR from get_lucifex_home() at import, so a
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_LUCIFEX_HOME", None, raising=False)
    assert scheduler._get_lucifex_home().resolve() == profile_home.resolve()
    assert scheduler._get_lucifex_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (LUCIFEX_HOME == root), the store is the root's cron dir
