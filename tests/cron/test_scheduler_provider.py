...
CronScheduler plan, .lucifexex/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - lucifex_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from lucifex_cli.web_server import _start_desktop_cron_ticker
...
def test_cron_status_reports_alive_but_failing(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("lucifex_cli.gateway.find_gateway_pids", lambda: [4321])
...
class TestGuardJobCredentialExfil:
    ...
    def test_named_custom_offhost_is_blocked(self, monkeypatch):
        import pytest
        import lucifex_cli.runtime_provider as rp
        from cron.scheduler import _guard_job_credential_exfil
