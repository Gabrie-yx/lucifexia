...
These exercise the real store against a temp LUCIFEX_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated LUCIFEX_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_lucifex_home() reads the env live.
    yield tmp_path
