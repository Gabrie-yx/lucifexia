...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED lucifexex
        ...
        due-check compares against ``_lucifexex_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("cron.jobs._lucifexex_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured lucifexex timezone differs from the
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _lucifexex_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _lucifexex_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _lucifexex_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _lucifexex_now(), "next_run_at should remain in the future"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_lucifexex_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_lucifexex_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_stale_repeat_limited_job_consumes_one_run_on_catchup(self, tmp_cron_dir, monkeypatch):
        ...
        from cron.jobs import _lucifexex_now
        job = create_job(prompt="Limited", schedule="every 5m", repeat=3)
        jobs = load_jobs()
        jobs[0]["next_run_at"] = (_lucifexex_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["last_run_at"] = (_lucifexex_now() - timedelta(minutes=11)).isoformat()
        save_jobs(jobs)
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("cron.jobs._lucifexex_now",
    ...
    def test_one_shot_run_claim_expires_after_ttl(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.delenv("lucifexex_CRON_TIMEOUT", raising=False)
        from cron.jobs import _lucifexex_now, _oneshot_run_claim_ttl_seconds
    ...
    def test_run_claim_ttl_derived_from_cron_timeout(self, tmp_cron_dir, monkeypatch):
        """The stale-recovery TTL tracks lucifexex_CRON_TIMEOUT (3x headroom), with
        ...
        monkeypatch.setenv("lucifexex_CRON_TIMEOUT", "1200")
        assert ttl() == 3600.0

        # A tiny timeout is floored so a claim can never expire mid-run.
        monkeypatch.setenv("lucifexex_CRON_TIMEOUT", "30")
        ...
        monkeypatch.setenv("lucifexex_CRON_TIMEOUT", "not-a-number")
        assert ttl() == 1800.0
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while lucifexex/system local time was UTC+10.
...
class TestCronOutputRetention:
    ...
    def test_save_job_output_prunes_old_runs(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._lucifexex_now", lambda: next(seq))
        for _ in range(8):
    ...
    def test_cron_output_keep_reads_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "lucifex_cli.config.load_config", lambda: {"cron": {"output_retention": 7}}
        )
    ...
    def test_cron_output_keep_defaults_on_bad_config(self, monkeypatch):
        import cron.jobs as jobs
        monkeypatch.setattr(
            "lucifex_cli.config.load_config", lambda: {"cron": {"output_retention": "oops"}}
        )
...
class TestLateEnvRepointScopesStore:
    """A LUCIFEX_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_unchanged_home_returns_import_time_constants(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(jobs.lucifexex_DIR))
        store = jobs._current_cron_store()
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(tmp_path / "env-home"))
    ...
    def test_patched_compatibility_constants_beat_env(self, tmp_path, monkeypatch):
        ...
        process-wide escape hatch — they win over a repointed LUCIFEX_HOME."""
        import cron.jobs as jobs
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import LUCIFEX_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "lucifexex_DIR", sim_old_home)
        ...
        monkeypatch.setenv("LUCIFEX_HOME", str(new_home))
