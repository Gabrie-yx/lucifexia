...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        content = "---\nname: test\nmetadata:\n  lucifexex:\n    tags: [a, b]\n---\n\nBody.\n"
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["lucifexex"]["tags"] == ["a", "b"]
...
class TestTrustLevelFor:
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `lucifexex skills browse` must also
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `lucifexex skills browse`."
            )
...
def _make_index_source(skills):
    """Build a lucifexexIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub import lucifexexIndexSource
    src = lucifexexIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestlucifexexIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "lucifexex-index"
...
# ---------------------------------------------------------------------------
# _load_lucifexex_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadlucifexexIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "lucifexex-index.json"
        monkeypatch.setattr(hub, "_lucifexex_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_fetch_does_not_request_brotli(self, monkeypatch, tmp_path):
        ...
        data = hub._load_lucifexex_index()
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (hub.lucifexex_INDEX_TTL + 100)
        import os
