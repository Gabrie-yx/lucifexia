...
# ---------------------------------------------------------------------------
# _load_lucifex_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadlucifexIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "lucifex-index.json"
        monkeypatch.setattr(hub, "_lucifex_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (hub.lucifex_INDEX_TTL + 100)
        import os
