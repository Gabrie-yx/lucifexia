...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("lucifex_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("lucifex_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="lucifex-spill-test-")
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-lucifex-test").exists())
    ...
    def test_default_directory_uses_lucifex_home(self):
        ...
        test_home = tempfile.mkdtemp(prefix="lucifex-home-")
        try:
            ...
            # ~/.lucifex/hook_outputs depending on get_lucifex_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.lucifex/hook_outputs/sess")),
            ]
