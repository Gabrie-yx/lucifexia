"""Regression tests for LUCIFEX_HOME override propagation onto the MCP loop.
...
get_lucifex_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: LucifexTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(LucifexTokenStorage("probe-srv")._tokens_path())
