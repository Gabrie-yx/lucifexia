...
class TestCronApproveMode:
    """When LUCIFEX_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        monkeypatch.setenv("LUCIFEX_CRON_SESSION", "1")
        monkeypatch.setenv("LUCIFEX_YOLO_MODE", "1")
        monkeypatch.delenv("LUCIFEX_INTERACTIVE", raising=False)
        monkeypatch.delenv("LUCIFEX_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting LUCIFEX_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds LUCIFEX_SESSION_PLATFORM via contextvars for
