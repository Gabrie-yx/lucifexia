...
class TestFileDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("lucifex-dedup-")
    ...
    def test_write_allows_large_file_that_quotes_status_text(self, mock_ops):
        ...
        Lucifex' own docs / SKILL.md files may legitimately mention the dedup
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("lucifex-write-dedup-")
