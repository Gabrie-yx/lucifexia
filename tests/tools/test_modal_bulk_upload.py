...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.lucifex/credentials/a.json"),
            (str(src_b), "/root/.lucifex/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.lucifex/credentials/a.json" in names
            assert "root/.lucifex/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.lucifex/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.lucifex/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.lucifex/credentials/f.txt"),
            (str(src), "/root/.lucifex/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.lucifex/credentials" in cmd
        assert "/root/.lucifex/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.lucifex/cache/file_{i}.txt"))
    ...
    def test_timeout_set_to_120(self, monkeypatch, tmp_path):
        ...
        src = tmp_path / "f.txt"
        src.write_text("data")
        files = [(str(src), "/root/.lucifex/f.txt")]
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.lucifex/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.lucifex/large.bin" in names
