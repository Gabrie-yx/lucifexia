...
@pytest.fixture
def fake_lucifexex(tmp_path, monkeypatch):
    """Build a two-profile lucifexex layout and point LUCIFEX_HOME at
    the lucifexex-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-lucifexex"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "lucifexex-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("LUCIFEX_HOME", str(sec_home))

    import lucifex_constants
    monkeypatch.setattr(lucifex_constants, "get_default_lucifex_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_LUCIFEX_HOME_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_lucifexex_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_lucifexex):
        from tools.file_tools import write_file_tool
        target = fake_lucifexex["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_lucifexex):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_lucifexex["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "lucifexex-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_lucifexex):
    ...
    def test_non_lucifexex_path_unaffected(self, faklucifexifex, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_lucifexex):
    ...
    def test_cross_profile_patch_bypass(self, fake_lucifexex):
        from tools.file_tools import patch_tool
        target = fake_lucifexex["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_lucifexex):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_lucifexex, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_lucifexex["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up LUCIFEX_HOME (set in
        ...
        assert "not found in active profile 'lucifexex-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_lucifexex, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_lucifexex["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_lucifexex["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.lucifex/profiles/<name>/."""
        # Don't set LUCIFEX_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_LUCIFEX_HOME_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_lucifexex_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_lucifexex):
        """When active profile is 'lucifexex-security', the prompt warns
        ...
        assert "Active lucifexex profile" in src
        assert "cross_profile=True" in src
        assert "~/.lucifex/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active lucifexex profile: default" in src
        assert "Active lucifexex profile: {active_profile}" in src
