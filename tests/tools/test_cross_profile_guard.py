...
def fake_lucifex(tmp_path, monkeypatch):
    """Build a two-profile Lucifex layout and point LUCIFEX_HOME at
    the lucifex-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_lucifex):
        from tools.file_tools import write_file_tool
        target = fake_lucifex["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_lucifex):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_lucifex["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "lucifex-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_lucifex):
    ...
    def test_non_lucifex_path_unaffected(self, fake_lucifex, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_lucifex):
    ...
    def test_cross_profile_patch_bypass(self, fake_lucifex):
        from tools.file_tools import patch_tool
        target = fake_lucifex["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_lucifex):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'lucifex-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_lucifex, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_lucifex["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_lucifex["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_lucifex_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_lucifex):
        """When active profile is 'lucifex-security', the prompt warns
        ...
        assert "Active Lucifex profile" in src
        assert "cross_profile=True" in src
        assert "~/.lucifex/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Lucifex profile: default" in src
        assert "Active Lucifex profile: {active_profile}" in src
