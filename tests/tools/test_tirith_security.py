...
class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_LUCIFEX_HOME(self):
        """_hermes_bin_dir must use LUCIFEX_HOME, not hardcoded ~/.hermes."""
    ...
    def test_failure_marker_respects_LUCIFEX_HOME(self):
        """_failure_marker_path must use LUCIFEX_HOME, not hardcoded ~/.hermes."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LUCIFEX_HOME": "/custom/hermes"}):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        assert "hermes_test" in LUCIFEX_HOME, "Should point to test temp dir"
