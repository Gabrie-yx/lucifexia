...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across lucifex processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/lucifex-snap-test123.sh"
    env._cwd_file = "/tmp/lucifex-cwd-test123.txt"
    env._cwd_marker = "__LUCIFEX_CWD_test123__"
...
def test_init_env_args_uses_lucifex_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _LUCIFEX_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_lucifex_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_lucifex_dotenv(monkeypatch):
...
def test_init_env_args_uses_lucifex_dotenv_for_empty_shell_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_lucifex_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_lucifex_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Lucifex image, or ``gosu``/``su``
...
def test_run_command_tags_lucifex_agent_label(monkeypatch):
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root Lucifex home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "lucifex-task-id=kanban-42" in labels, (
        f"lucifex-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "lucifex-profile=research-bot" in labels, (
        f"lucifex-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "lucifex-task-id=task_with_weird_chars" in labels, (
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Lucifex process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Lucifex process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("lucifex-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Lucifex startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd Lucifex processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a Lucifex process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=lucifex-profile=<profile>`` to
    ...
    assert "label=lucifex-profile=research-bot" in flat, (
    ...
    assert "status=exited" in flat, (
        ...
        "belong to a sibling Lucifex process and must NEVER be reaped"
    )
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.lucifex/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.lucifex/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.lucifex/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: lucifex-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: lucifex-x", "returncode": 1},
