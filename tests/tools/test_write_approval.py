...
def test_default_gate_is_off(lucifex_home):
...
def test_invalid_subsystem_is_off(lucifex_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(lucifex_home):
...
def test_memory_gate_on_no_interactive_stages(lucifex_home):
...
def test_memory_gate_on_then_apply(lucifex_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(lucifex_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(lucifex_home, monkeypatch):
    ...
    monkeypatch.setattr("lucifex_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(lucifex_home):
...
def test_skill_gate_on_always_stages(lucifex_home):
...
def test_skill_gate_on_then_apply_writes_file(lucifex_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's LUCIFEX_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(lucifex_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(lucifex_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(lucifex_home):
...
def test_handle_approve_all(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(lucifex_home):
...
def test_handle_approval_invalid(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(lucifex_home):
    from lucifex_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(lucifex_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(lucifex_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(lucifex_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(lucifex_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(lucifex_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(lucifex_home):
