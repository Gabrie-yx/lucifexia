...
- The production manager provider
  (:class:`tools.mcp_oauth_manager.LucifexMCPOAuthProvider`) restoring metadata
...
# ---------------------------------------------------------------------------
# Manager-path provider (LucifexMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------
...
def _manager_provider_with_context(storage: LucifexTokenStorage, **context_attrs):
    ...
    provider = _LUCIFEX_PROVIDER_CLS.__new__(_LUCIFEX_PROVIDER_CLS)
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            _LUCIFEX_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            _LUCIFEX_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
