"""Contract tests for the Docker image's immutable /opt/lucifex install tree."""
...
def test_dockerfile_makes_opt_lucifex_readonly_for_lucifex_user() -> None:
    ...
    assert "chown -R root:root /opt/lucifex" not in text
    assert "chmod -R a+rX /opt/lucifex" not in text
    assert "chmod -R a-w /opt/lucifex" not in text
...
def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    ...
    assert "ENV LUCIFEX_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "LUCIFEX_TUI_DIR=/opt/lucifex/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_lucifex() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+lucifex:lucifex\s+/opt/lucifex/\.venv",
        r"chown\s+-R\s+lucifex:lucifex\s+/opt/lucifex/ui-tui",
        r"chown\s+-R\s+lucifex:lucifex\s+/opt/lucifex/gateway",
        r"chown\s+-R\s+lucifex:lucifex\s+/opt/lucifex/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/lucifex must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/lucifex/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $LUCIFEX_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the lucifex
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/lucifex/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/lucifex/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+printf 'docker\\n' > /opt/lucifex/\.install_method",
        text,
    )
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    assert "ENV LUCIFEX_LAZY_INSTALL_TARGET=/opt/lucifex" not in text
    ...
    # succeed as the unprivileged lucifex runtime user.
