...
Lucifex' existing ``cache_image_from_bytes`` so a ``MEDIA:<path>`` tag
...
def _png_bytes():
    ...
    Lucifex' ``cache_image_from_bytes`` has a format-sniff guard that rejects
...
class TestCacheMcpImageBlock:
    def test_returns_media_tag_for_valid_image_block(self, tmp_path, monkeypatch):
        ...
        # The cached file should be in Lucifex' image cache dir
