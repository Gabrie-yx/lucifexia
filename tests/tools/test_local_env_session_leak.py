"""Cross-session lucifexex_SESSION_* leak guard for the local terminal backend.

Regression coverage for the bug where a terminal subprocess could observe a
*different concurrent session's* ``lucifexex_SESSION_KEY`` (and the other
``lucifexex_SESSION_*`` vars).
...
subprocess-env bridge resolves each ``lucifexex_SESSION_*`` from the ContextVar and,
...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, lucifex_subprocess_env
...
def test_set_session_vars_engages_and_overrides_foreign_global(monkeypatch):
    ...
    assert env.get("lucifexex_SESSION_KEY") == "agent:main:discord:group:MY_BUGS_ROOT:111"


def test_engaged_strips_all_session_vars_when_unset(monkeypatch):
    """The strip covers every lucifexex_SESSION_* mirror, not just the key."""
    _engage()
    monkeypatch.setenv("lucifexex_SESSION_KEY", "foreign-key")
    monkeypatch.setenv("lucifexex_SESSION_THREAD_ID", "foreign-thread")
    monkeypatch.setenv("lucifexex_SESSION_CHAT_ID", "foreign-chat")
    monkeypatch.setenv("lucifexex_SESSION_USER_ID", "foreign-user")
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    Pure single-process CLI/one-shot sets lucifexex_SESSION_* directly in os.environ
    ...
    monkeypatch.setenv("lucifexex_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("lucifexex_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("lucifexex_SESSION_KEY") == "cli-session-key"
    assert env.get("lucifexex_SESSION_ID") == "cli-session-id"
...
def test_engaged_explicit_empty_contextvar_clears(monkeypatch):
    ...
    """
    monkeypatch.setenv("lucifexex_SESSION_KEY", "foreign-after-clear")
    ...
    assert env.get("lucifexex_SESSION_KEY", "") == "", (
        f"Foreign key survived an explicit clear: {env.get('lucifexex_SESSION_KEY')!r}"
    )
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    no thread id binds lucifexex_SESSION_THREAD_ID="" and that empty value must win
    over an older non-empty value left in os.environ.
    """
    monkeypatch.setenv("lucifexex_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("lucifexex_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('lucifexex_SESSION_THREAD_ID')!r}"
    )
    assert env.get("lucifexex_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "lucifexex_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "lucifexex_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "lucifexex_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('lucifexex_SESSION_KEY')!r}"
    )
    assert "lucifexex_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_set_contextvar_wins_when_engaged():
    ...
    assert sanitized.get("lucifexex_SESSION_KEY") == "agent:main:discord:group:REAL_BG:222"
...
def test_sanitize_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "lucifexex_SESSION_KEY": "cli-bg-key",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert sanitized.get("lucifexex_SESSION_KEY") == "cli-bg-key"


# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (lucifex_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_lucifex_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """lucifex_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_lucifex_subprocess_env_bound_contextvar_wins(monkeypatch):
    ...
    try:
        env = lucifex_subprocess_env()
        assert env.get("lucifexex_SESSION_KEY") == "agent:main:discord:group:MINE:111"
    finally:
        clear_session_vars(tokens)


def test_lucifex_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    """A pure single-process CLI (never engaged) keeps the inherited fallback."""
    monkeypatch.setenv("lucifexex_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = lucifex_subprocess_env()
    assert env.get("lucifexex_SESSION_KEY") == "cli-fallback-key"
