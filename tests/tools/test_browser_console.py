...
class TestBrowserConsole:
    ...
    def test_restrict_evaluate_reads_browser_config(self):
        ...
        with patch("lucifex_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": "true"}}):
            assert _restrict_browser_evaluate() is True
        with patch("lucifex_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": False}}):
            assert _restrict_browser_evaluate() is False
        # Default (key absent) is off — the denylist is opt-in.
        with patch("lucifex_cli.config.read_raw_config", return_value={}):
            assert _restrict_browser_evaluate() is False
...
class TestBrowserConsoleToolsetWiring:
    ...
    def test_in_lucifexex_core_tools(self):
        from toolsets import _lucifexex_CORE_TOOLS
        assert "browser_console" in _lucifexex_CORE_TOOLS
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("lucifex_constants.get_lucifexex_dir", return_value=shots_dir),
