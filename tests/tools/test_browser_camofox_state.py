...
class TestCamofoxStatePaths:
    def test_paths_are_profile_scoped(self, tmp_path):
        state = _load_module()
        with patch.object(state, "get_lucifex_home", return_value=tmp_path):
...
class TestCamofoxIdentity:
    ...
    def test_identity_differs_by_profile(self, tmp_path):
        state = _load_module()
        with patch.object(state, "get_lucifex_home", return_value=tmp_path / "profile-a"):
            a = state.get_camofox_identity("task-1")
        with patch.object(state, "get_lucifex_home", return_value=tmp_path / "profile-b"):
