...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — LUCIFEX_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping LUCIFEX_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"LUCIFEX_HOME": str(home)}):
