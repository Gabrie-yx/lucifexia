...
class TestConfigureWindowsStdio:
    """``lucifex_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("lucifex_cli.stdio", None)
        # Fresh import now; tests import from lucifex_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import lucifex_cli.stdio as _s
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("lucifex_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "lucifex_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, lucifex_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "lucifex_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call lucifex_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """lucifex_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from lucifex_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from lucifex_cli._subprocess_compat import resolve_node_command
...
# ---------------------------------------------------------------------------
# lucifex_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "lucifex_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the LUCIFEX_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / lucifex_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "lucifex_cli/tools_config.py",
            "lucifex_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_LUCIFEX_HOME(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_lucifex_home" in source
        assert 'cache_dir = get_lucifex_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_lucifex_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "lucifex_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "lucifex_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        lucifex_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "lucifex_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from lucifex_cli._subprocess_compat import" in block
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "LUCIFEX_HOME) so the windowless base pythonw resolves lucifex_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import lucifex_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "lucifex_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from lucifex_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import lucifex_cli.config  # noqa: F401
        ...
        # Mock get_lucifex_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            ...
            "lucifex_cli.config.get_lucifex_home", return_value="C:/hermes"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
