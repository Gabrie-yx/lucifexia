...
``~/.lucifex/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.lucifex/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while lucifex_cli.config.get_env_value is patched must
        ...
        try:
            with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
                 ...
                 patch("lucifex_cli.config.load_env",
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``lucifex_cli.config.load_env`` to simulate
    ``~/.lucifex/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("lucifex_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
