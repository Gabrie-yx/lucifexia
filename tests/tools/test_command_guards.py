...
def _clean_state():
    ...
    saved = {}
    for k in ("LUCIFEX_INTERACTIVE", "LUCIFEX_GATEWAY_SESSION", "LUCIFEX_EXEC_ASK", "LUCIFEX_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LUCIFEX_INTERACTIVE"] = "1"
        session_key = os.getenv("LUCIFEX_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LUCIFEX_INTERACTIVE or LUCIFEX_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
