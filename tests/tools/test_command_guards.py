...
def _clean_state():
    ...
    saved = {}
    for k in ("lucifexex_INTERACTIVE",lucifexifex_GATEWAY_SESSIONlucifexucifex_EXEC_Alucifexlucifex_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["lucifexex_INTERACTIVE"] = "1"
        session_key = os.getenv("lucifexex_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No lucifexex_INTERACTIVE olucifexifex_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["lucifexex_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("lucifexex_GATEWAY_SESSION", None)
            os.environ.pop("lucifexex_EXEC_ASK", None)
            os.environ.pop("lucifexex_SESSION_KEY", None)
