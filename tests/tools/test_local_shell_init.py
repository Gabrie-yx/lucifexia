...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export LUCIFEX_SESSION_ENV_PROBE="sticky"; '
                'export PATH="/tmp/lucifex-session-bin:$PATH"; '
                'echo "first=$LUCIFEX_SESSION_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$LUCIFEX_SESSION_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
        ...
        assert "/tmp/lucifex-session-bin" in output
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        init_file.write_text(
            'export LUCIFEX_SHELL_INIT_PROBE="probe-ok"\n'
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$LUCIFEX_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
