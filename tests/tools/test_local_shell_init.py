...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export lucifexex_SESSION_ENV_PROBE="sticky"; '
                'export PATH="/tmp/lucifexex-session-bin:$PATH"; '
                'echo "first=$lucifexex_SESSION_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$lucifexex_SESSION_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
        ...
        assert "/tmp/lucifexex-session-bin" in output
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        init_file.write_text(
            'export lucifexex_SHELL_INIT_PROBE="probe-ok"\n'
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$lucifexex_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
