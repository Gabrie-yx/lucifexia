...
class TestRmtreeWritableScopeGuard:
    ...
    def test_refuses_lucifex_home_itself(self, tmp_path):
        ...
        lucifex = tmp_path / "home"
        lucifex.mkdir()
        (lucifex / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", lucifex / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(lucifex)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        lucifex = tmp_path / "home"
        lucifex.mkdir()
        skills = lucifex / "skills"
        skills.mkdir()
        not_skills = lucifex / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "lucifex skills reset new-skill" in captured
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """LUCIFEX_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("LUCIFEX_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("LUCIFEX_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty LUCIFEX_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("LUCIFEX_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `lucifex profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.lucifex) rely on so bundled
    skills are never seeded at install time NOR re-injected by `lucifex update`.
    """
...
class TestOptOutToggleAndRemove:
    """`lucifex skills opt-out/opt-in` core: marker toggle + safe removal."""
