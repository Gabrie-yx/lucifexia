...
def _restore_tool_modules():
    original_lucifex_home = os.environ.get("LUCIFEX_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_lucifex_home is None:
            os.environ.pop("LUCIFEX_HOME", None)
        else:
            os.environ["LUCIFEX_HOME"] = original_lucifex_home
        _reset_modules(("tools", "lucifex_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="lucifex-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": lucifex_home / "modal_snapshots.json",
