...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("LUCIFEX_HOME", str(home))
    monkeypatch.setenv("lucifex_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting lucifex_ALLOW_PRIVATE_URLS on a private network.
...
def default_url_guard(monkeypatch):
    ...
    Clears lucifex_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
