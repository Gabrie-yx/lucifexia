...
Older images made /opt/lucifex/ui-tui and /opt/lucifex/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LUCIFEX_TUI_DIR=/opt/lucifex/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lucifex:lucifex /opt/lucifex/ui-tui" not in text
    assert "chown -R lucifex:lucifex /opt/lucifex/node_modules" not in text
