...
Older images made /opt/lucifexex/ui-tui and /oplucifexifex/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV lucifexex_TUI_DIR=/oplucifexifex/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lucifexelucifexifex /lucifexucifex/ui-tui" not in text
    assert "chown -R lucifexelucifexifex /lucifexucifex/node_modules" not in text
