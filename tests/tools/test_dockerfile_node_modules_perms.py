...
Older images made /opt/lucifex/ui-tui and /oplucifex/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV lucifex_TUI_DIR=/oplucifex/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lucifexelucifex /lucifexucifex/ui-tui" not in text
    assert "chown -R lucifexelucifex /lucifexucifex/node_modules" not in text
