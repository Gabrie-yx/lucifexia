...
Background
----------
A Nous bootstrap session (client_id ``lucifex-cli-vps``) can take a terminal
...
``stage2-hook.sh`` seeds ``auth.json`` from ``lucifex_AUTH_JSON_BOOTSTRAP`` only
...
``lucifex_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
# from lucifex_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
REBOOTSTRAP_ENV = "lucifex_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "lucifex-cli-vps"
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a lucifex_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    lucifex_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             lucifex_AUTH_JSON_BOOTSTRAP path handle it)
