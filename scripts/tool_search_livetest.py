...
def setup_isolated_home(enabled: bool) -> Path:
    ...
    LUCIFEX_HOME = home_dir / ".hermes"
    LUCIFEX_HOME.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, LUCIFEX_HOME / "auth.json")
    ...
    if real_env_file.exists():
        ...
        load_lucifex_dotenv(LUCIFEX_HOME=str(Path.home() / ".hermes"))
    ...
    (LUCIFEX_HOME / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return LUCIFEX_HOME
...
def reset_module_state():
    """Drop cached modules so the new LUCIFEX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "lucifex_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["LUCIFEX_HOME"] = str(home)
...
def main():
    ...
    # Restore original LUCIFEX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["LUCIFEX_HOME"] = ORIGINAL_HOME
    else:
