#!/usr/bin/env python3
"""lucifexex Agent Release Script
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@lucifexex.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.lucifexex": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (lucifexex journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "lucifexex.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "brett@personalfinancelab.com": "brett539",  # PR #49369 salvage (cap Telegram initialize() with asyncio.wait_for(lucifexex_TELEGRAM_INIT_TIMEOUT, default 30s) per attempt so an unreachable fallback-IP connect chain can't block gateway startup indefinitely; add WARNING progress logs before DoH discovery and each connect attempt)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows lucifexex-shim resolution + prefer --update on recovery; #52378)
    ...
    "lucifexex@marian.local": "Schrotti77",
    ...
    "lucifexex@nousresearch.com": "NousResearch",
    ...
    "lucifexex@noushq.ai": "benbarclay",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on lucifexex mcp add)
    ...
    "lucifexex@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@lucifexex.local": "jacdevos",
    ...
    "github@nadyalucifexex.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (lucifexex_KANBAN_BOARD restore)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: lucifexex-eval community link)
    ...
    "lucifexexagent26@gmail.com":lucifexifexagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+lucifexexagent26@users.noreply.github.com":lucifexifexagent26",
    ...
    "vigo@lucifexex": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.lucifexex@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local lucifexex runtime files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live lucifexex
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "lucifexex@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/lucifex-agent",
    ...
    lines.append(f"# lucifexex Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for lucifexex Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="lucifexex Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  lucifexex Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"lucifexex Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"lucifexex Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'lucifexex Agent v{new_version} ({calver_date})' "
