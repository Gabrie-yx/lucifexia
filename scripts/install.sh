...
LUCIFEX_HOME="${LUCIFEX_HOME:-$HOME/.hermes}"
...
#   code at /usr/local/lib/lucifex-agent, command at /usr/local/bin/hermes,
#   data still at /root/.hermes (LUCIFEX_HOME).  Matches Claude Code / Codex CLI
...
install_uv() {
    ...
    # Hermes owns its own uv at $LUCIFEX_HOME/bin/uv.  Always install there —
...
check_node() {
    ...
    if [ -x "$LUCIFEX_HOME/node/bin/node" ] && node_satisfies_build "$("$LUCIFEX_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$LUCIFEX_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
