...
# Python-driven tool session (e.g. lucifexex terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/lucifex-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/lucifex-agent.git"
LUCIFEX_HOME="${LUCIFEX_HOME:-$HOME/.lucifexex}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${lucifexex_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$lucifexex_INSTALL_DIR"
...
#   code at /usr/local/lib/lucifex-agent, command at /usr/local/bin/lucifexex,
#   data still at /root/.lucifexex (LUCIFEX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "lucifexex Agent Installer"
            echo ""
            ...
            echo "                   write \$LUCIFEX_HOME/.no-bundled-skills so future"
            echo "                   'lucifexex update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> lucifexex.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.lucifex/lucifex-agent"
            echo "                   default (root, Linux): /usr/local/lib/lucifex-agent"
            echo "  --lucifexex-home PATH  Data directory (default: ~lucifexifex, or \$LUCIFEX_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, lucifexex installs the code under"
            echo "  /usr/local/lib/lucifex-agent and links the command into"
            echo "  /usr/local/bin/lucifexex (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$LUCIFEX_HOME"
            echo "  (default /root/.lucifexex).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$LUCIFEX_HOME/lucifex-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs lucifexex setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `lucifexex update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (lucifexex-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download lucifexex Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Instalucifexfex command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `lucifexex` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $LUCIFEX_HOME/lucifex-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $LUCIFEX_HOME/lucifex-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/lucifex-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $LUCIFEX_HOME/lucifex-agent — then preserve it)
#
# Always no-op when the user set --dir or $lucifexex_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in LUCIFEX_HOME.
    if is_termux; then
        INSTALL_DIR="$LUCIFEX_HOME/lucifex-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$LUCIFEX_HOME/lucifex-agent/.git" ]; then
            INSTALL_DIR="$LUCIFEX_HOME/lucifex-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/lucifex-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/lucifex-agent"
        ...
        # /usr/local/bin/lucifexex wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/lucifexex"
        log_info "  Data:    $LUCIFEX_HOME (unchanged)"
...
# Point a lucifexex-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $LUCIFEX_HOME/node/bin — which is NOT on
...
# lucifexex's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no lucifexex-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$LUCIFEX_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$LUCIFEX_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$LUCIFEX_HOME/node/etc/npmrc"
}

get_lucifexex_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/lucifexex" ]; then
        echo "$link_dir/lucifexex"
    else
        echo "lucifexex"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://lucifex-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # lucifexex owns its own uv at $LUCIFEX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (lucifex_cli/managed_uv.py) looks in the same
    # place, so install.sh and `lucifexex update` stay in sync.
    local _managed_uv="$LUCIFEX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $LUCIFEX_HOME/bin ..."
    mkdir -p "$LUCIFEX_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/lucifexex-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/lucifexex-uv-installer.$$.sh")"
    ...
    # directly into $LUCIFEX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$LUCIFEX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the lucifexex-
...
check_node() {
    ...
    # Repair pre-existing lucifexex-managed installs where `npm install -g` lands
    ...
    if [ -x "$LUCIFEX_HOME/node/bin/node" ] && node_satisfies_build "$("$LUCIFEX_HOME/node/bin/node" --version)"; then
        export PATH="$LUCIFEX_HOME/node/bin:$PATH"
        log_success "Node.js $("$LUCIFEX_HOME/node/bin/node" --version) found (lucifexex-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing lucifexex-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    # the lucifexex command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$LUCIFEX_HOME/node"
    mkdir -p "$LUCIFEX_HOME"
    mv "$extracted_dir" "$LUCIFEX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$LUCIFEX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$LUCIFEX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$LUCIFEX_HOME/node/bin/npx"  "$node_link_dir/npx"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. lucifexex install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "lucifexex Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `lucifexex update` path
                # (#4735).
                ...
                local stash_name
                stash_name="lucifexex-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``lucifexex update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Lucifex behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up lucifexex command..."

    if [ "$USE_VENV" = true ]; then
        lucifexex_BIN="$INSTALL_DIR/venv/bilucifexifex"
    else
        lucifexex_BIN="$(whiclucifexifex 2>/dev/null || echo "")"
        if [ -z "$lucifexex_BIN" ]; then
            log_warn "lucifexex not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$lucifexex_BIN" ]; then
        log_warn "lucifexex entry point not found atlucifexifex_BIN"
    ...
    # Create a user-facing shim for the lucifexex command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $lucifexex_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$lucifexex_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/lucifexex"
    cat > "$command_link_dir/lucifexex" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$lucifexex_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/lucifexex"
    log_success "Installed lucifexex launcher → $command_link_display_dilucifexifex"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "lucifexex command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v lucifexex' \
                >/dev/null 2>&1; then
        ...
        log_info "lucifexex not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# lucifexex Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# lucifexex Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# lucifexex Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so lucifexex works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.lucifexex directory structure (config at top level, code in subdir)
    ...
    # This MUST match DEFAULT_SOUL_MD in lucifex_cli/default_soul.py — the
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `lucifexex update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'lucifexex update'." \
            > "$LUCIFEX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $LUCIFEX_HOME/.no-bundled-skills"
        log_info "  Future 'lucifexex update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
...
strip_snap_browser_override() {
    ...
    local env_file="$LUCIFEX_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# lucifexex Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "lucifexex will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# lucifexex Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (lucifexex --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `lucifexex update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'lucifexex setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m lucifex_cli.main setup < /dev/tty
    else
        python -m lucifex_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for lucifexex to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$LUCIFEX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'lucifexex whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                lucifexex_CMD="$(gelucifexifex_command_path)"
                $lucifexex_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'lucifexex whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'lucifexex gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $lucifexex_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $lucifexex_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: lucifex gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: lucifexex gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $LUCIFEX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $LUCIFEX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $LUCIFEX_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}lucifexex${NC}              Start chatting"
    echo -e "   ${GREEN}lucifexex setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}lucifexex config${NC}       View/edit configuration"
    echo -e "   ${GREEN}lucifexex config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}lucifexex gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}lucifexex update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'lucifexex' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'lucifexex' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'lucifexex' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$LUCIFEX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$LUCIFEX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$LUCIFEX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$LUCIFEX_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$LUCIFEX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up lucifexex for pip install"
    ...
    lucifexex_CMD="$(command -lucifexifex 2>/dev/null || echo "")"
    if [ -n "$lucifexex_CMD" ]; then
        log_info "Running lucifexex setup..."
        "$lucifexex_CMD" setup
    else
        log_warn "lucifexex command not found on PATH"
        log_info "Try: python -m lucifex_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`lucifexex` rename
...
# _purge_electron_build_cache() used by `lucifexex desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the lucifexex-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `lucifexex desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/lucifexex" ]; then
            app="$desktop_dir/release/linux-unpacked/lucifexex"
        elif [ -x "$desktop_dir/release/linux-unpacked/lucifexex" ]; then
            app="$desktop_dir/release/linux-unpacked/lucifexex"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/lucifexex.app" \
            "$desktop_dir/release/mac/lucifexex.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("lucifexex is damaged and can't be opened").
    ...
    # checkout stays clean for the next `lucifexex update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the lucifexex-managed Node
            # provisioned during prerequisites/node-deps (at $LUCIFEX_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $LUCIFEX_HOME.
    # $LUCIFEX_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'lucifexex update' on this host install.
