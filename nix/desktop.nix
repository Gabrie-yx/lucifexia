# nix/desktop.nix — Lucifex Desktop (Electron) app build + wrapper
#
# `lucifexAgent` is the fully-built `.#default` package — it ships the
# `lucifex` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `LUCIFEX_DESKTOP_LUCIFEX` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Lucifex CLI").
...
let
  npm = lucifexNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "lucifex-desktop";
  };
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "lucifex-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/lucifex-desktop $out/bin
    cp -r ${renderer}/* $out/share/lucifex-desktop/
    ...
    substituteInPlace $out/share/lucifex-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/lucifex-desktop'"
    ...
    substituteInPlace $out/share/lucifex-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/lucifex-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # LUCIFEX_DESKTOP_LUCIFEX to the absolute path of the nix-built `lucifex`
    # binary so the desktop's resolver step 4 ("existing Lucifex CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/lucifex-desktop \
      --add-flags "$out/share/lucifex-desktop" \
      --set LUCIFEX_DESKTOP_LUCIFEX "${lib.getExe lucifexAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Lucifex Agent";
    homepage = "https://github.com/NousResearch/lucifex-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "lucifex-desktop";
  };
}
