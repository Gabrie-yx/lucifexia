#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Lucifex docs site.
...
Both publish at:
  https://lucifex-agent.nousresearch.com/docs/llms.txt
  https://lucifex-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://lucifex-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Lucifex as a Python Library", None),
        ("guides/use-mcp-with-lucifex", "Use MCP with Lucifex", None),
        ("guides/use-voice-mode-with-lucifex", "Use Voice Mode with Lucifex", None),
        ("guides/use-soul-with-lucifex", "Use SOUL.md with Lucifex", None),
        ("guides/build-a-lucifex-plugin", "Build a Lucifex Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Lucifex"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "backends. Works with Lucifex portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/NousResearch/"
        "lucifex-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/lucifex-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Lucifex Agent — Full Documentation\n",
        (
            "This file is the entire Lucifex Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Lucifex, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://lucifex-agent.nousresearch.com/docs\n",
        "Short index: https://lucifex-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
