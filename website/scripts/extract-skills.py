...
2. The unified lucifexSkills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                lucifexmeta = metadata.get("llucifex {})
                if isinstance(lucifexmeta, dict):
                    tags = lucifexmeta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
