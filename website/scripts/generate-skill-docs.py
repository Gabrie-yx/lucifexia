...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/lucifex-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lucifexmeta = (fm.get("metadata") or {}).get("llucifex or {}
    tags = lucifexmeta.get("tags") or []
    related = lucifexmeta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lucifexskills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that lucifexloads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with lucifexAgent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "lucifexships with a large built-in skill library copied into `~/.lucifex/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "lucifexalso syncs bundled skills on `llucifexpdate`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lucifex/skills/` tree, it is still shipped with lulucifexestore it with `luclucifexlls reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with lucifex-agent — install via lucifexskills install official/<category>/<skill>"',
        "---",
        "",
        ...
        "",
        "```bash",
        "lucifexskills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "lucifexskills install official/blockchain/solana",
        "lucifexskills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "lucifexskills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
