...
def render_skill_page(
    ...
    lucifex_meta = (fm.get("metadata") or {}).get("lucifex") or {}
    tags = lucifex_meta.get("tags") or []
    related = lucifex_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lucifex skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lucifex Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lucifex ships with a large built-in skill library copied into `~/.lucifex/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lucifex also syncs bundled skills on `lucifex update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lucifex/skills/` tree, it is still shipped with Lucifex; restore it with `lucifex skills reset <name> --restore`.",
        "",
