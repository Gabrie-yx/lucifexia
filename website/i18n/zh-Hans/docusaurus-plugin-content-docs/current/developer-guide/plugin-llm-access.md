...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——Lucifex 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
第三个完整示例（包含图像输入）位于
[`lucifex-example-plugins`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-example)
仓库（参考 plugin 的配套仓库——不随 lucifex-agent 本体打包）。关于异步接口（`acomplete()` / `acomplete_structured()` 与 `asyncio.gather()` 配合使用），请参见同一仓库中的
[`plugin-llm-async-example`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-async-example)。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——Lucifex provider id（如 "openrouter"）
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 Lucifex provider
        # （必须是 Lucifex 已知的 provider——与
        # `lucifex model` 和 config.yaml model.provider 中的名称相同）
...
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 Lucifex 常规的聚合器感知回退流程。
...
现有 `ctx.*` 方法各自扩展一个已有的 Lucifex 子系统：
...
* 参考 plugin（配套仓库）：
  * [`plugin-llm-example`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-example) — 带图像输入的同步结构化提取
  * [`plugin-llm-async-example`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-async-example) — 使用 `asyncio.gather()` 的异步示例
* 辅助客户端（底层引擎）：参见
