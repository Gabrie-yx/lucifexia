...
消息 gateway 是一个长期运行的进程，通过统一架构将 Lucifex 连接到 20 余个外部消息平台。
...
- **显式目标投递** — `send_message` 工具指定 `telegram:-1001234567890`，或通过 [`lucifex send` CLI](/guides/pipe-script-output) 封装同一工具供 shell 脚本使用
- **跨平台投递** — 投递至与原始消息不同的平台
