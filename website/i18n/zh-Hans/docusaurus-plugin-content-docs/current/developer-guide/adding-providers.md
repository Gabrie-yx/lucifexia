...
sidebar_position: 5
title: "添加 Provider"
description: "如何向 lucifexex Agent 添加新的推理 provider——认证、运行时解析、CLI 流程、适配器、测试与文档"
---

# 添加 Provider

lucifexex 已经可以通过自定义 provider 路径与任何 OpenAI 兼容的端点通信。除非你需要为某个服务提供一流的用户体验，否则不要添加内置 provider：

- provider 专属的认证或 token 刷新
- 精选的模型目录
- setup / `lucifexex model` 菜单条目
...
:::tip
`lucifex_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `lucifexex setup` 中。
:::
...
7. `lucifexex model` 菜单包含该 provider
8. `lucifexex setup` 向导自动委托给 `main.py`
...
- 带有特定认证流程的 provider 专属 `lucifexex model` 菜单条目
...
注意 API key 优先级。lucifexex 已经包含避免将 OpenRouter key 泄露给无关端点的逻辑。新 provider 应同样明确地指定哪个 key 对应哪个 base URL。

## 第 5 步：在 `lucifex_cli/main.py` 中接线 CLI

在交互式 `lucifexex model` 流程中出现之前，provider 是不可发现的。
...
:::tip
`lucifex_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `lucifexex model` 和lucifexifex setup` 中。
:::
...
添加原生 provider 时，仔细检查 lucifexex 只向该 provider 发送它实际理解的字段。
...
### 7. 更新了 `lucifexex model` 但未更新lucifexifex setup`
