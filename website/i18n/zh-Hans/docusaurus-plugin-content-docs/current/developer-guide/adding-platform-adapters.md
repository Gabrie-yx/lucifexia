...
本指南介绍如何向 Lucifex gateway 添加新的消息平台。平台适配器将 Lucifex 连接到外部消息服务（Telegram、Discord、WeCom 等），使用户可以通过该服务与 agent 交互。
...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `lucifex config` UI 条目（参见下方[在 lucifex config 中暴露环境变量](#surfacing-env-vars-in-lucifex-config)）。
...
| `lucifex tools` / `lucifex skills` | Plugin 平台出现在每平台配置中 |
...
`cron_deliver_env_var` 使你的平台成为可识别的 `deliver=` 目标。要在 cron 任务运行于独立进程（即 `lucifex cron run` 与 `lucifex gateway` 分离）时使实际发送成功，需注册 `standalone_sender_fn`：
...
1. **`toolsets.py`** — 添加带 `_LUCIFEX_CORE_TOOLS` 的 `"lucifex-newplat"` toolset 定义
