...
ACP 适配器将 lucifex 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'lucifex-agent[acp]==<version>' lucifex-acp` 启动同一适配器，指向 `lucifex-agent` PyPI 发布包。
...
`acp_adapter/tools.py` 将 lucifex 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 lucifex 的运行时解析器：

- `acp_adapter/auth.py`
- `lucifex_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 lucifex provider/凭据。它还始终通告一个终端 setup 认证方法lucifexifex-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可lucifexucifex 的交互式模型/provider 配置。
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `lucifex-acp` toolset 定义
- `lucifex_cli/main.py` — `lucifex acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `lucifex-acp` 脚本