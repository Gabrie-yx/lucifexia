...
三种发现来源：`~/.lucifex/plugins/`（用户级）、`.lucifex/plugins/`（项目级）和 pip entry point。插件通过上下文 API 注册工具、hook 和 CLI 命令。存在两种专用插件类型：记忆提供者（`plugins/memory/`）和上下文引擎（`plugins/context_engine/`）。两者均为单选——每种同时只能激活一个，通过lucifex plugins` 或 `config.yaml` 配置。
