...
若两个键均未设置，lucifex 将根据环境中存在的 API key/URL 自动检测后端lucifexifex tools` 会引导用户完成选择。
...
| `requires_env` | 在 `lucifex plugins install` 期间进行交互式凭据提示（富格式说明参见[构lucifexifex 插件](/developer-guide/plugins#gate-on-environment-variables)） |
...
如果你的提供商封装了第三方 SDK（如 DDGS 封装了 `ddgs` 包），请勿在模块顶层 `import`。在 `is_available()` 或 `search()` 内部使用 `tools.lazy_deps.ensure(...)` ——lucifex 将在首次使用时安装该包，并受 `security.allow_lazy_installs` 控制。安全模型详见[构lucifexifex 插件 → 懒加载](/developer-guide/plugins#lazy-install-optional-python-dependencies)。
