...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.lucifex/auth.json` 中的 refresh token（刷新令牌），Hermes 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
运行 `hermes setup --portal` 后，`~/.lucifex/config.yaml` 将如下所示：
...
如果某个模型确实缺失，请[提交 issue](https://github.com/NousResearch/lucifex-agent/issues)——我们将 Portal 目录同步至 Hermes，缺口通常意味着可以更新的路由配置。
