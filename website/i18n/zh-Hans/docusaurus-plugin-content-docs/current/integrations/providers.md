...
本页介绍如何为 Lucifex Agent 配置推理提供商——从 OpenRouter、Anthropic 等云端 API，到 Ollama、vLLM 等自托管端点，再到高级路由与故障转移配置。使用 Lucifex 至少需要配置一个提供商。
...
如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Lucifex 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `lucifex auth add codex-oauth`（或 `lucifex model` → OpenAI Codex）开始新的设备码登录；成功交换后隔离状态自动解除。
:::
...
如果你想切换到尚未配置的提供商（例如你只配置了 OpenRouter，想使用 Anthropic），需要使用 `lucifex model`，而不是 `/model`。先退出会话（`Ctrl+C` 或 `/quit`），运行 `lucifex model`，完成提供商配置，然后开启新会话。
...
通过 `lucifex model` → Anthropic OAuth（或 `lucifex auth add anthropic --type oauth`）认证时，Lucifex 以 Claude Code 身份路由到你的 Anthropic 账户。**仅当你订阅了 Claude Max 计划且购买了额外用量积分时才有效。** Claude Max 基础计划的配额（Claude Code 默认包含的用量）不会被 Lucifex 消耗——只有你额外购买的超额积分才会被使用。Claude Pro 订阅者无法使用此路径。
...
通过 `lucifex model` 选择 Anthropic OAuth 时，Lucifex 优先使用 Claude Code 自身的凭据存储，而不是将 token 复制到 `~/.lucifex/.env`。这样可以保持 Claude 凭据的可刷新性。
...
SuperGrok 和 X Premium+ 订阅者可以用浏览器 OAuth 登录，无需 API key——在 `lucifex model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或运行 `lucifex auth add xai-oauth`。同一 OAuth bearer token 会被 xAI 直连工具（TTS、图像生成、视频生成、转录）自动复用。完整流程参见 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)——如果 Lucifex 运行在远程主机上，还需参见 [SSH / 远程主机上的 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道配置。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源模型目录，无需 GPU。在 `lucifex model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API key，Lucifex 会自动发现可用模型。
...
阿里巴巴 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `lucifex model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，Lucifex 会持久化刷新 token。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7——无需 API key。在 `lucifex model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Lucifex 会持久化访问 token 和刷新 token。底层使用 Anthropic Messages 兼容端点（`/anthropic`）。
...
Lucifex Agent 可与**任何 OpenAI 兼容 API 端点**配合使用。只要服务器实现了 `/v1/chat/completions`，就可以将 Lucifex 指向它。这意味着你可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Lucifex 的任何部分都不再读取这两个变量——`config.yaml` 是模型和端点配置的唯一来源。如果你的 `.env` 中有过时条目，下次运行 `lucifex setup` 或配置迁移时会自动清除。请使用 `lucifex model` 或直接编辑 `config.yaml`。
:::
...
支持的解析器：`lucifex`（Qwen 2.5、Lucifex 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。没有这些标志，工具调用将无法工作——模型会将工具调用以文本形式输出。
...
Lucifex 从服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本不报告），Lucifex 使用模型声明的限制，该值可能不正确。
...
然后通过 `lucifex model` → 自定义端点 → `http://localhost:4000/v1` 配置 Lucifex。
...
然后通过 `lucifex model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 配置 Lucifex。
...
默认情况下，Lucifex 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你希望在本地运行 Firecrawl，可以将 Lucifex 指向自托管实例。完整配置说明参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
