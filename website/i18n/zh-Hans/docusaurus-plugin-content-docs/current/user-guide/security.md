...
Lucifex Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
在执行任何命令之前，Lucifex 会将其与一份精心维护的危险模式列表进行比对。若匹配，用户必须明确批准。
...
1. **CLI 标志**：使用 `lucifex --yolo` 或 `lucifex chat --yolo` 启动会话
2. **斜杠命令**：在会话中输入 `/yolo` 以切换开/关
3. **环境变量**：设置 `LUCIFEX_YOLO_MODE=1`
...
YOLO 模式在 CLI 和 gateway 会话中均可使用。在内部，它会设置 `LUCIFEX_YOLO_MODE` 环境变量，该变量在每次命令执行前都会被检查。

当 YOLO 激活时，Lucifex 会显示两个持久的视觉提醒，以确保用户不会忘记审批提示已被绕过：
...
某些命令极具破坏性——不可逆的文件系统清除、fork 炸弹、直接写入块设备——无论以下任何情况，Lucifex 都**拒绝**执行：
...
| `pkill`/`killall` lucifex/gateway | 防止自我终止 |
...
运行 gateway 时，`LUCIFEX_EXEC_ASK=1` 环境变量会自动设置。
...
运行消息 gateway 时，Lucifex 通过分层授权系统控制谁可以与机器人交互。
...
为实现更灵活的授权，Lucifex 提供了基于验证码的配对系统。无需预先提供用户 ID，未知用户会收到一次性配对码，由机器人所有者通过 CLI 批准。

**工作原理：**

1. 未知用户向机器人发送 DM
2. 机器人回复一个 8 位配对码
3. 机器人所有者在 CLI 上运行 `lucifex pairing approve <platform> <code>`
4. 该用户在该平台上获得永久批准
...
使用 `docker` 终端后端时，Lucifex 对每个容器应用严格的安全加固。
...
加载后，Lucifex 会检查这些文件是否存在于活跃 profile 的 `LUCIFEX_HOME` 中，并将其注册为挂载：
...
| **terminal**（本地） | 阻止明确的 Lucifex 基础设施变量（提供商密钥、gateway token、工具 API 密钥） | ✅ 透传变量绕过黑名单 |
...
- Lucifex 基础设施密钥（提供商 API 密钥、gateway token）不应添加到 `env_passthrough`——它们有专用机制
...
Lucifex 集成了 [tirith](https://github.com/sheeki03/tirith) 用于在执行前进行内容级命令扫描。Tirith 能检测单纯模式匹配所遗漏的威胁：
...
10. **保持更新** — 定期运行 `lucifex update` 以获取安全补丁
...
Lucifex 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `lucifex_cli/security_advisories.py`。

运行方式：

- **CLI 启动横幅。** 若有任何公告匹配，会打印一行警告，并指向 `lucifex doctor` 获取完整修复方案。
- **`lucifex doctor`。** 显示所有活跃公告的版本详情和 2-4 步修复说明。
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。Lucifex 在首次使用时**懒加载**安装这些包，而非在 `lucifex-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
...
3. 若安装失败或用户已禁用懒加载安装，调用会抛出 `FeatureUnavailable`，附带实际的 pip stderr 和指向 `lucifex tools` 的提示。
...
禁用后，需要可选依赖的后端会提示用户手动运行安装（`pip install …`）或通过 `lucifex tools` 选择其他后端。