...
2. Hermes 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.lucifex/.env` 中。
3. 每次 `hermes`（或 gateway，或 cron 任务）启动时，在加载 `~/.lucifex/.env` 之后，Hermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
