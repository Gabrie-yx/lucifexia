...
sidebar_position: 13
title: "Webhooks"
description: "接收来自 GitHub、GitLab 等服务的事件以触发 lucifex agent 运行"
---

# Webhooks

接收来自外部服务（GitHub、GitLab、JIRA、Stripe 等）的事件，并自动触发 lucifex agent 运行。Webhook 适配器运行一个 HTTP 服务器，接受 POST 请求、验证 HMAC 签名、将 payload（载荷）转换为 agent prompt（提示词），并将响应路由回来源或其他已配置的平台。
...
1. 通过 `lucifex gateway setup` 或环境变量启用
2. 在 `config.yaml` 中定义路由，**或**使用 `lucifex webhook subscribe` 动态创建
...
- 空 stdout、精确的 `[SILENT]`、`{"__lucifex_ignore__": true}`、超时、脚本缺失或非零退出码，均返回 HTTP 200 及 `{"status":"ignored","reason":"script"}`。
...
在仓库中打开一个 pull request。webhook 触发后，lucifex 处理事件并在 PR 上发布审查评论。
...
除了 `config.yaml` 中的静态路由，还可以使用 `lucifex webhook` CLI 命令动态创建 webhook 订阅。当 agent 本身需要设置事件驱动触发器时，这尤为有用。
...
agent 可通过 terminal 工具在 `webhook-subscriptions` skill 的引导下创建订阅。向 agent 请求"为 GitHub issues 设置 webhook"，它将运行相应的 `lucifex webhook subscribe` 命令。
