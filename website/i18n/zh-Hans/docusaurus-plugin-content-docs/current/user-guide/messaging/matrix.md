...
sidebar_position: 9
title: "Matrix"
description: "将 Lucifex Agent 设置为 Matrix 机器人"
---

# Matrix 设置

Lucifex Agent 与 Matrix 集成，Matrix 是一种开放的联邦消息协议。Matrix 允许你运行自己的 homeserver，也可以使用 matrix.org 等公共 homeserver——无论哪种方式，你都保持对通信的控制权。机器人通过 `mautrix` Python SDK 连接，通过 Lucifex Agent 管道（包括工具调用、记忆和推理）处理消息，并实时响应。它支持文本、文件附件、图片、音频、视频，以及可选的端对端加密（E2EE）。

Lucifex 兼容任何 Matrix homeserver——Synapse、Conduit、Dendrite 或 matrix.org。

在开始设置之前，先了解大多数人最想知道的：Lucifex 连接后的行为方式。
...
| **线程** | Lucifex 支持 Matrix 线程（MSC3440）。在线程中回复时，Lucifex 会将线程上下文与主房间时间线隔离。机器人已参与的线程无需提及即可响应。 |
...
2. 为机器人选择一个用户名（例如 `lucifex-bot`）。

### 方式 C：使用你自己的账户

你也可以以自己的用户身份运行 Lucifex。这意味着机器人以你的名义发帖——适合个人助手场景。

## 第二步：获取访问令牌

Lucifex 需要访问令牌（access token）来向 homeserver 进行身份验证。有两种方式：
...
你可以不提供访问令牌，而是提供机器人的用户 ID 和密码。Lucifex 会在启动时自动登录。这种方式更简单，但密码会存储在你的 `.env` 文件中。
...
Lucifex Agent 使用你的 Matrix 用户 ID 来控制谁可以与机器人交互。Matrix 用户 ID 的格式为 `@username:server`。
...
Lucifex 支持 Matrix 端对端加密，你可以在加密房间中与机器人聊天。
...
每次启动时，如果设置了 `MATRIX_RECOVERY_KEY`，Lucifex 会从 homeserver 的安全密钥存储中导入交叉签名密钥并对当前设备进行签名。此操作是幂等的，可以永久启用。
...
Lucifex 在启动时会检测到此情况并拒绝启用 E2EE，日志显示：`device XXXX has stale one-time keys on the server signed with a previous identity key`。
...
如果你之前使用 `MATRIX_ENCRYPTION=true` 运行 Lucifex，并正在升级到使用新的基于 SQLite 的加密存储的版本，机器人的加密身份已发生变化。你的 Matrix 客户端（Element）可能缓存了旧的设备密钥，并拒绝与机器人共享加密会话。
...
:::tip
**新安装不受影响。** 此迁移仅在你之前使用旧版 Lucifex 配置了可用的 E2EE 并正在升级时才需要。
...
| 设置 | 说明 |
|---------|-------------|
| `GATEWAY_PROXY_URL` | 远程 Lucifex API 服务器的 URL（例如 `http://192.168.1.100:8642`） |
...
:::tip
会话连续性通过 `X-Lucifex-Session-Id` 请求头维护。主机的 API 服务器按此 ID 跟踪会话，因此对话在消息之间持续存在，就像使用本地 agent 一样。
:::
...
**解决方法**：同步循环在出错时每 5 秒自动重试。检查 Lucifex 日志中与同步相关的警告。如果机器人持续落后，请确保你的 homeserver 有足够的资源。

### 机器人离线

**原因**：Lucifex gateway 未运行，或连接失败。

**解决方法**：检查 `lucifex gateway` 是否正在运行。查看终端输出中的错误消息。常见问题：homeserver URL 错误、访问令牌过期、homeserver 不可达。
...
- **媒体支持**：Lucifex 可以发送和接收图片、音频、视频和文件附件。媒体通过 Matrix 内容仓库 API 上传到你的 homeserver。
