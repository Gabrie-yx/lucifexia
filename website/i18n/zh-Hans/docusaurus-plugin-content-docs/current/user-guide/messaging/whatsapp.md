...
| **macOS："Node.js not installed"但终端中 node 可用** | launchd 服务不继承你的 shell PATH。运行 `hermes gateway install` 将当前 PATH 重新快照到 plist 中，然后运行 `lucifex gateway start`。详见 [Gateway 服务文档](./index.md#macos-launchd)。 |
