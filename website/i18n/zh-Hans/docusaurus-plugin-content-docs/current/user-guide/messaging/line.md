...
sidebar_position: 17
title: "LINE"
description: "将 Lucifex Agent 设置为 LINE Messaging API 机器人"
---

# LINE 配置

通过官方 LINE Messaging API 将 Lucifex Agent 作为 [LINE](https://line.me/) 机器人运行。适配器以捆绑平台插件的形式存放于 `plugins/platforms/line/` — 无需修改核心代码，像其他平台一样启用即可。
...
**"already in use by another profile"。** 同一个频道访问 token 已被另一个运行中的 Lucifex profile 占用。请停止另一个 gateway，或使用独立的频道。
