...
lucifex Agent 以机器人形式与 Discord 集成，让你可以通过私信或服务器频道与 AI 助手对话。机器人接收你的消息，通lucifex Agent 管道（包括工具调用、记忆和推理）进行处理，并实时响应。它支持文本、语音消息、文件附件和斜杠命令。
...
| **服务器频道** | 默认情况下，lucifex 仅在被 `@提及` 时响应。如果你在频道中发帖但未提及lucifex 会忽略该消息。 |
...
因此，如果 Alice 和 Bob 都在 `#research` 中与 lucifex 对话，即使他们使用的是同一个可见的 Discord 频lucifex 默认也会将其视为独立的对话。
...
- [与 lucifex 使用语音模式](/guides/use-voice-mode-witlucifex)
