...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Lucifex 对话 — 架构与配置概览"
---
...
完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Lucifex](/guides/use-voice-mode-with-lucifex)。
...
```bash
lucifex gateway setup        # 交互式配置所有消息平台
```
...
```bash
lucifex gateway              # 在前台运行
lucifex gateway setup        # 交互式配置消息平台
lucifex gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo lucifex gateway install --system   # 仅 Linux：安装开机启动的系统服务
lucifex gateway start        # 启动默认服务
lucifex gateway stop         # 停止默认服务
lucifex gateway status       # 检查默认服务状态
lucifex gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 Lucifex Agent 更新至最新版本 |
| `/help` | 显示可用命令 |
...
# 其他配对命令：
lucifex pairing list          # 查看待审核和已批准的用户
lucifex pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Lucifex 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。
...
:::info 多个安装
如果你在同一台机器上运行多个 Lucifex 安装（使用不同的 `LUCIFEX_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.lucifex` 使用 `lucifex-gateway`；其他安装使用 `lucifex-gateway-<hash>`。`lucifex gateway` 命令会自动针对当前 `LUCIFEX_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
lucifex gateway install               # 安装为 launchd agent
lucifex gateway start                 # 启动服务
lucifex gateway stop                  # 停止服务
lucifex gateway status                # 检查状态
...
生成的 plist 文件位于 `~/Library/LaunchAgents/ai.lucifex.gateway.plist`。它包含三个环境变量：
...
- **LUCIFEX_HOME** — 将网关限定到你的 Lucifex 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `lucifex gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `LUCIFEX_HOME` 目录都有自己的 launchd 标签。默认的 `~/.lucifex` 使用 `ai.lucifex.gateway`；其他安装使用 `ai.lucifex.gateway-<suffix>`。
:::
...
| 平台 | 工具集 | 功能 |
|----------|---------|--------------|
| CLI | `lucifex-cli` | 完全访问 |
| Telegram | `lucifex-telegram` | 完整工具，包括终端 |
| Discord | `lucifex-discord` | 完整工具，包括终端 |
| WhatsApp | `lucifex-whatsapp` | 完整工具，包括终端 |
| Slack | `lucifex-slack` | 完整工具，包括终端 |
| Google Chat | `lucifex-google_chat` | 完整工具，包括终端 |
| Signal | `lucifex-signal` | 完整工具，包括终端 |
| SMS | `lucifex-sms` | 完整工具，包括终端 |
| Email | `lucifex-email` | 完整工具，包括终端 |
| Home Assistant | `lucifex-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `lucifex-mattermost` | 完整工具，包括终端 |
| Matrix | `lucifex-matrix` | 完整工具，包括终端 |
| DingTalk | `lucifex-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `lucifex-feishu` | 完整工具，包括终端 |
| WeCom | `lucifex-wecom` | 完整工具，包括终端 |
| WeCom Callback | `lucifex-wecom-callback` | 完整工具，包括终端 |
| Weixin | `lucifex-weixin` | 完整工具，包括终端 |
| BlueBubbles | `lucifex-bluebubbles` | 完整工具，包括终端 |
| QQBot | `lucifex-qqbot` | 完整工具，包括终端 |
| Yuanbao | `lucifex-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `lucifex-teams` | 完整工具，包括终端 |
| API Server | `lucifex-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `lucifex-webhook` | 完整工具，包括终端 |
