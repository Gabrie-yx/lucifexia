...
- 用户希望 **Lucifex 本身驱动浏览器** → 使用 Lucifex 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
