...
参见：https://docs.honcho.dev/v3/guides/integrations/lucifex#running-honcho-locally-with-lucifex
...
### 配置文件未获得自己的 peer
创建时使用 `--clone`：`lucifex profile create <name> --clone`。对于现有配置文件：`lucifex honcho sync`。
...
| `lucifex honcho sync` | 为所有尚未拥有 host 块的 Lucifex 配置文件创建 host 块 |
| `lucifex honcho migrate` | 从 OpenClaw 原生记忆迁移到 Lucifex + Honcho 的分步指南 |
