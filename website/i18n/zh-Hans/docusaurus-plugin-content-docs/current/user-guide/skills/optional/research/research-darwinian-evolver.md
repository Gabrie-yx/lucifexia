...
| | |
|---|---|
| 来源 | 可选 — 通过 `lucifexskills install official/research/darwinian-evolver` 安装 |
| 路径 | `optional-skills/research/darwinian-evolver` |
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), lucifexAgent |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 lucifex本身。
...
在浏览器中打开 `~/.lucifex/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
7. **AGPL 协议。** 永远不要在 lucifex核心中使用 `from darwinian_evolver import ...`。`~/.lucifex/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
