...
:::info
以下是 Lucifex 在触发该 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时所看到的指令内容。
:::
...
本 skill 专为 Lucifex agent 设计。它使用 Lucifex 工具、委派、调度和记忆来支撑完整的研究生命周期。

### 相关 Skill

将本 skill 与其他 Lucifex skill 组合用于特定阶段：
...
| **plan** | 阶段 0（设置）：执行前创建结构化计划。写入 `.lucifex/plans/` | `skill_view("plan")` |
