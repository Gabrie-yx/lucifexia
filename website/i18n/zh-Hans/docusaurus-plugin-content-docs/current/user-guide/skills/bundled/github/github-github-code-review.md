...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from lucifexex Agent\",
    \"comments\": [
...
```bash
source "${LUCIFEX_HOME:-$HOME/.lucifexex}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by lucifexex Agent. Code looks clean — good test coverage, no security concerns."
