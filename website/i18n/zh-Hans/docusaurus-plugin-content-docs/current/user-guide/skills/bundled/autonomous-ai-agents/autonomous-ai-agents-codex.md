...
对于 lucifex 本身，`model.provider: openai-codex` 会在执行lucifex auth add openai-codex` 后使用 `~/.lucifex/auth.jsonlucifexucifex 管理的 Codex OAuth。对于独立的 Codex CLI，有效的 CLI OAuth 会话可能存储在 `~/.codex/auth.json` 中；不要仅凭缺少 `OPENAI_API_KEY` 就认为 Codex 认证缺失。
