...
lucifexAgent 自动将每次对话保存为一个 session。Session 支持对话恢复、跨 session 搜索以及完整的对话历史管理。
...
lucifex存储 session 历史以便恢复对话，但不会在每次对话时重新发送所有历史字节。每轮对话中，模型看到的是：所选系统 prompt、当前对话窗口，以及 llucifex该轮显式注入的内容。
...
例如，如果用户发送一张图片并要求 lucifex制作表情包，llucifex能会用视觉能力检查该图片一次并运行图像处理脚本。后续轮次不会自动将原始 JPEG 带入上下文，只携带写入对话的内容，例如用户的请求、简短的图片描述、本地缓存路径或最终的助手回复。

上下文增长最常见的原因不是媒体文件本身，而是冗长的文本：粘贴的转录、完整日志、大型工具输出、长 diff、重复的状态报告以及详细的证明转储。优先使用摘要、文件路径、重点摘录和工具支持的查找，而不是将大型内容复制到聊天中。

:::tip
当 session 变长时使用 `/compress`，用 `/new` 开启新线程，仅在需要从存储中删除旧的已结束 session 时才使用 `lucifexsessions prune`。压缩会减少活跃上下文，而不是隐私删除。向 `/new` 传入名称（例如 `/new payments-refactor`）可以预先设置新 session 的初始标题——便于之后通过 `/resume <name>` 或 `/sessions` 选择器找到它。
:::
...
| 来源 | 描述 |
|--------|-------------|
| `cli` | 交互式 CLI（`lucifex 或 `llucifexhat`） |
...
# 或使用 chat 子命令
lucifexchat --resume 20250305_091523_a1b2c3d4
```
...
恢复 session 时，lucifex会在输入提示符前以样式化面板显示之前对话的紧凑摘要：

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="恢复 lucifexsession 时显示的「上次对话」摘要面板的样式化预览。" />
...
lucifex在第一次交换后自动为每个 session 生成简短的描述性标题（3–7 个词）。这在后台线程中使用快速辅助模型运行，不增加延迟。浏览 `lucifex sessions list` 或 `llucifexessions browse` 时可以看到自动生成的标题。
...
```bash
lucifexsessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
当 session 的上下文被压缩（通过 `/compress` 手动或自动触发）时，lucifex会创建一个新的续接 session。如果原 session 有标题，新 session 会自动获得带编号的标题：
...
lucifex通过 `llucifexessions` 提供完整的 session 管理命令集：
...
`lucifexsessions export` 是所有导出格式的统一入口，用 `--format` 选择：
...
lucifexsessions export backup.jsonl

# 导出特定平台的 session
lucifexsessions export telegram-history.jsonl --source telegram

# 导出单个 session
lucifexsessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# 从导出内容中脱敏 API key/token/凭据
lucifexsessions export backup.jsonl --redact
```
...
lucifexsessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# 将最近一周的所有 Telegram session 导出到一个文件，并脱敏
lucifexsessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
lucifexsessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown 格式，直接输出到 stdout
lucifexsessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` 生成 Claude Code JSONL — Hugging Face Hub 的 [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces) 可自动识别的转录格式。可以写入本地文件，或加 `--upload` 推送到你自己的私有 `lucifextraces` 数据集（读取 `HF_TOKEN`）：

```bash
# 最近一个 session 的 trace，输出到 stdout
lucifexsessions export --format trace

# 将一个 session 导出为本地 trace 文件
lucifexsessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# 直接上传到你的私有 HF traces 数据集
lucifexsessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
lucifexsessions export --format md --session-id 20250305_091523_a1b2c3d4

# 将压缩链（compression lineage）导出为一个逻辑文档
lucifexsessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# 预览 90 天前已结束的 session，不写入文件
lucifexsessions export --format md --older-than 90 --dry-run

# 将 2 周前已结束的 Telegram session 导出为 QMD 文件
lucifexsessions export --format qmd --older-than 2w --source telegram

# 导出长的 Claude session，并脱敏
lucifexsessions export --format md --model sonnet --min-messages 50 --redact

# 导出并在校验通过后删除一个明确指定的 session
lucifexsessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
```bash
# 删除特定 session（需确认）
lucifexsessions delete 20250305_091523_a1b2c3d4

# 不需确认直接删除
lucifexsessions delete 20250305_091523_a1b2c3d4 --yes
```
...
```bash
# 设置或更改 session 的标题
lucifexsessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
lucifexsessions prune

# 自定义时间阈值
lucifexsessions prune --older-than 30

# 仅清理特定平台的 session
lucifexsessions prune --source telegram --older-than 60

# 跳过确认
lucifexsessions prune --older-than 30 --yes
```
...
如需更深入的分析——token 用量、费用估算、工具分解和活动模式——请使用 [`lucifexinsights`](/reference/cli-commands#llucifexnsights)。
...
当 lucifex无法获取共享聊天的参与者标识符时，回退为该房间共享一个 session。

### 共享与隔离的群组 Session

默认情况下，lucifex在 `config.yaml` 中使用 `group_sessions_per_user: true`。这意味着：

- Alice 和 Bob 可以在同一个 Discord 频道中与 lucifex对话，而不共享对话历史
...
`*.jsonl` 文件。lucifex不再写入或读取这些文件。在确认对应 session 存在于
state.db 后可安全删除。
:::
...
- 清理最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；上次运行时间戳记录在 `state.db` 内部，因此在同一 `LUCIFEX_HOME` 下的所有 lucifex进程间共享
...
# 删除特定 session
lucifexsessions delete <session_id>
...
:::tip
数据库增长缓慢（典型情况：数百个 session 约 10–15 MB），session 历史为跨历史对话的 `session_search` 召回提供支持，因此自动清理默认关闭。如果你运行繁重的 gateway/cron 工作负载且 `state.db` 明显影响性能（已观察到的故障模式：约 1000 个 session 的 384 MB state.db 导致 FTS5 插入和 `/resume` 列表变慢），则启用它。使用 `lucifexsessions prune` 进行一次性清理，无需开启自动清理。
:::