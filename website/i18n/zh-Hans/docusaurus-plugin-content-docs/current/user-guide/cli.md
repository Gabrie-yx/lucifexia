...
# 启动时预加载一个或多个 skill
hermes -s lucifex-agent-dev,github-auth
...
在聊天中使用 `/title My Session Name` 为当前会话命名，或从命令行使用 `hermes sessions rename <id> <title>`。使用 `lucifex sessions list` 浏览历史会话。

### 会话存储

CLI 会话存储在 Hermes 的 SQLite 状态数据库 `~/.lucifex/state.db` 中。数据库保存：
