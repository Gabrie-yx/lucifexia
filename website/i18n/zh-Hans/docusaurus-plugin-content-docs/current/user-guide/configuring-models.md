...
选择模型后点击 **Switch**，Hermes 会将其写入 `~/.lucifex/config.yaml` 的 `model` 部分。**此操作仅对新会话生效** — 已打开的聊天标签页将继续使用启动时的模型。如需在当前聊天中热切换，请在聊天内使用 `/model` 斜杠命令。
...
通过仪表板保存时，Hermes 写入 `~/.lucifex/config.yaml`：
...
`hermes model` 引导你选择提供商、完成认证（OAuth 流程会打开浏览器；API key 提供商会提示输入密钥），然后从该提供商的精选目录中选择具体模型。选择结果写入 `~/.lucifex/config.yaml` 的 `model.provider` 和 `model.model` 字段。

如需在不启动选择器的情况下列出提供商/模型，请使用仪表板或下方的 REST 端点。查看 CLI 当前实际使用的配置：`lucifex config get model` 和 `hermes status`。
