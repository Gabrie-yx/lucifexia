...
Lucifex Web Dashboard（`lucifex dashboard`）在设计上支持换肤和扩展，无需 fork 代码库。对外暴露三个层次：
...
参考插件（[`lucifex-example-plugins`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
| 3 | `./.lucifex/plugins/<name>/dashboard/` | `project`——仅在设置 `LUCIFEX_ENABLE_PROJECT_PLUGINS` 时生效 |
...
1. Dashboard 加载。`main.tsx` 在 `window.__LUCIFEX_PLUGIN_SDK__` 上暴露 SDK，在 `window.__LUCIFEX_PLUGINS__` 上暴露注册表。
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `lucifex-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
