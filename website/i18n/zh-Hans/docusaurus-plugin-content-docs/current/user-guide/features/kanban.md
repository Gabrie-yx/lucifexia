...
- **Workspace（工作区）** —— worker 操作的目录。三种类型：
  - `scratch`（默认）—— 在 `~/.lucifex/kanban/workspaces/<id>/` 下（非默认看板为 `~/.lucifex/kanban/boards/<slug>/workspaces/<id>/`）创建的临时目录。**任务完成时删除** —— scratch 是临时性的，worker（或 `lucifex kanban complete <id>`）将任务标记为完成的那一刻，目录即被清除。如果想保留 worker 的输出，请使用 `worktree:` 或 `dir:<path>`。在某次安装中首次创建 scratch 工作区时，调度器会记录警告并在任务上发出 `tip_scratch_workspace` 事件（可通过 `lucifex kanban show <id>` 查看）。
