...
:::tip 一个后端同时覆盖模型与工具
lucifex 本身需要配置好 provider（提供商）和工具后端，API 服务器才能发挥作用。[Nous Portal](/user-guide/features/tool-gateway) 订阅同时处理两者——300+ 个模型，以及通过 Tool Gateway 提供的网络/图像/TTS/浏览器功能。在启动 API 服务器之前运行一次lucifex setup --portal`，Open WebUI 或 LobeChat 等前端即可获得一个完整配备工具的后端。
:::
...
**流式传输**（`"stream": true`）：返回逐 token 响应块的 Server-Sent Events（SSE）。对于 **Chat Completions**，流使用标准 `chat.completion.chunk` 事件，以及 lucifex 自定义的lucifex.tool.progress` 事件用于工具启动的 UX 展示。对于 **Responses**，流使用 OpenAI Responses 事件类型，如 `response.created`、`response.output_text.delta`、`response.output_item.added`、`response.output_item.done` 和 `response.completed`。

**流中的工具进度：**
- **Chat Completions**：lucifex 发出 `eventlucifex.tool.progress` 以提供工具启动可见性，同时不污染持久化的 assistant 文本。
