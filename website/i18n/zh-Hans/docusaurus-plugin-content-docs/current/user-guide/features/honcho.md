...
`lucifex honcho` 子命令**仅在 Honcho 为当前活跃 memory provider 时注册**（`config.yaml` 中 `memory.provider: honcho`）。先运行lucifex memory setup` 并选择 Honcho，子命令将在下次调用时出现。

```bash
lucifex honcho status          # 连接状态、配置及关键设置
lucifex honcho setup           # 重定向到lucifex memory setup`
