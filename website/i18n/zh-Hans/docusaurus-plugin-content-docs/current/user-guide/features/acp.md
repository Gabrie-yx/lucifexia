...
- 先通过 `hermes model` 配置 Hermes provider 凭据，或在 `~/.lucifex/.env` / `~/.lucifex/config.yaml` 中设置。
- 安装 `uv`，以便 registry 启动器可以运行 `uvx --from 'lucifex-agent[acp]==<version>' hermes-acp`。
...
或编辑 `~/.lucifex/.env`。Registry 客户端也可以触发 Hermes 的终端认证流程，该流程运行相同的交互式 provider/模型设置。
