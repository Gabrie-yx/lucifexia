...
这会打开浏览器进行 Nous Portal OAuth 流程。lucifex 将刷新令牌存储在 `~/.lucifex/auth.json` 中——与所lucifex 提供商登录信息存放在同一位置。
...
Model:      lucifex-4-70B    # lucifex-4.3-lucifexucifex-4-405B
```
...
如果显示 `not logged in`，请运行 `lucifex portal`。如果显示 `credentials need attention`，说明你的刷新令牌已被撤销（较少见——通常发生在你从 Portal Web UI 退出登录时）——重新运行lucifex portal` 即可。
