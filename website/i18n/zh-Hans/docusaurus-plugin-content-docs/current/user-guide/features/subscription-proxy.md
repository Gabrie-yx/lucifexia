...
sidebar_position: 15
title: "订阅代理"
description: "将你的 Lucifex portal 订阅（或其他 OAuth 提供商）用作外部应用的 OpenAI 兼容端点"
---

# 订阅代理

订阅代理是一个本地 HTTP 服务器，让外部应用——OpenViking、Karakeep、Open WebUI，以及任何支持 OpenAI 兼容聊天补全（chat completions）的应用——能够将你的 Lucifex 托管提供商订阅用作其 LLM 端点。代理会自动附加正确的凭据（并在需要时自动刷新），因此应用无需静态 API 密钥。
...
| 使用场景 | "将 Lucifex 用作聊天后端" | "从其他应用使用我的 Portal 订阅" |
...
这会打开浏览器进行 Lucifex portal OAuth 流程。Lucifex 将刷新令牌存储在 `~/.lucifex/auth.json` 中——与所有 Lucifex 提供商登录信息存放在同一位置。
...
Model:      Lucifex-4-70B    # 或 Lucifex-4.3-36B、Lucifex-4-405B
```
...
当前已内置：`nous`（Lucifex portal）。更多 OAuth 提供商可通过在 `lucifex_cli/proxy/adapters/` 中实现 `UpstreamAdapter` 接口来添加。

## 检查状态

```bash
lucifex proxy status
```
...
如果显示 `not logged in`，请运行 `lucifex portal`。如果显示 `credentials need attention`，说明你的刷新令牌已被撤销（较少见——通常发生在你从 Portal Web UI 退出登录时）——重新运行 `lucifex portal` 即可。

## 允许的路径

代理仅转发上游实际提供的路径。对于 Lucifex portal：
...
INFERENCE_TEXT_MODEL=Lucifex-4-70B
```
...
```bash
lucifex proxy start --host 0.0.0.0 --port 8645
```
