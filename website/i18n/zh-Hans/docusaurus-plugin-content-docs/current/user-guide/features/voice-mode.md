...
2. **已配置 LLM 提供商** — 运行 `hermes model` 或在 `~/.lucifex/.env` 中设置首选提供商的凭据
3. **基础设置正常** — 运行 `hermes` 验证 Agent 能够响应文字消息，再启用语音功能

:::tip
`~/.lucifex/` 目录和默认的 `config.yaml` 会在首次运行 `hermes` 时自动创建。只需手动创建 `~/.lucifex/.env` 来存放 API 密钥。
:::
...
语音模式在**经典 CLI**（`hermes chat`）和 **TUI**（`hermes --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.lucifex/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
:::tip
录音键可通过 `~/.lucifex/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
:::
...
2. 或改用私信 — 无需提及
3. 或在 `~/.lucifex/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 检查 LLM 模型是否已配置且可访问
- 查看 gateway 日志：`tail -f ~/.lucifex/logs/gateway.log`
