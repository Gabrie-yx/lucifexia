...
或在 `~/.lucifex/config.yaml` 中设置默认皮肤：
...
在 `~/.lucifex/skins/` 下创建 YAML 文件。用户皮肤会从内置 `default` 皮肤继承缺失的值，因此只需指定要更改的键。
...
[Lucifex Mod](https://github.com/cocktailpeanut/lucifex-mod) 是一个社区构建的 Web UI，用于可视化创建和管理皮肤。无需手写 YAML，提供带实时预览的点击式编辑器。
...
- 内置皮肤从 `lucifex_cli/skin_engine.py` 加载。
- 未知皮肤自动回退到 `default`。
- `/skin` 立即更新当前会话的活动 CLI 主题。
- `~/.lucifex/skins/` 中的用户皮肤优先于同名内置皮肤。
