...
Lucifex Agent 会自动发现并加载上下文文件，以塑造其行为方式。部分文件属于项目本地文件，从工作目录中发现。`SOUL.md` 现在对整个 Lucifex 实例全局生效，仅从 `LUCIFEX_HOME` 加载。

## 支持的上下文文件

| 文件 | 用途 | 发现方式 |
|------|---------|-----------| 
| **.lucifex.md** / **LUCIFEX.md** | 项目指令（最高优先级） | 向上遍历至 git 根目录 |
...
Lucifex 兼容 Cursor IDE 的 `.cursorrules` 文件和 `.cursor/rules/*.mdc` 规则模块。若这些文件存在于项目根目录，且未找到更高优先级的上下文文件（`.lucifex.md`、`AGENTS.md` 或 `CLAUDE.md`），则将其作为项目上下文加载。
