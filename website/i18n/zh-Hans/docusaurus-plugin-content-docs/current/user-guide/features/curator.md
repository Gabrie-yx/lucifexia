...
在每次真正的 curator pass 之前，Hermes 会在 `~/.lucifex/skills/.curator_backups/<utc-iso>/skills.tar.gz` 处对 `~/.lucifex/skills/` 进行 tar.gz 快照。如果某次 pass 归档或合并了你不希望被触碰的内容，可以用一条命令撤销整次运行：
...
- `~/.lucifex/skills/.hub/lock.json`（通过 `hermes skills install` 安装的技能）。
