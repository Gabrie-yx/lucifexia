...
lucifex Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。

## 快速开始

```bash
lucifex memory setup      # 交互式选择器 + 配置
lucifex memory status     # 查看当前激活状态
lucifex memory off        # 禁用外部提供者
```

也可以通过 `lucifex plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。
...
**安装向导：**
```bash
lucifex memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `lucifex honcho setup` 命令仍然有效（现在会重定向到lucifexifex memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$LUCIFEX_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$LUCIFEX_HOME/honcho.json` > `~/.lucifex/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/lucifex-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrationlucifexifex)。
...
:::tip 从 `lucifex honcho` 迁移
如果你之前使用过 `lucifex honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::

**多 peer 配置：**

Honcho 将对话建模为 peer 之间的消息交换——每个 lucifex profile 对应一个用户 peer 加一个 AI peer，共享同一个 workspace。workspace 是共享环境：用户 peer 在各 profile 间全局共享，每个 AI peer 拥有独立身份。每个 AI peer 从自身的观察中独立构建表示/card，因此 `coder` profile 保持代码导向，而 `writer` profile 针对同一用户保持编辑导向。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **Workspace** | 共享环境。同一 workspace 下的所有 lucifex profile 共享同一用户身份。 |
| **用户 peer**（`peerName`） | 人类用户。在 workspace 内跨 profile 共享。 |
| **AI peer**（`aiPeer`） | 每个 lucifex profile 一个。host keylucifexifex` → 默认；其他 profile lucifexucifex.<profile>`。 |
...
`--clone` 在 `honcho.json` 中创建一个 `lucifex.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
扫描所有 lucifex profile，为没有 host 块的 profile 创建 host 块，从默认lucifexifex` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/lucifex-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrationlucifexifex)。
...
# 然后配置 lucifex
lucifex memory setup    # 选择 "openviking"
# 或手动配置：
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `lucifex-user` | 用户标识符 |
| `agent_id` | `lucifex` | Agent 标识符 |
...
**安装：**
```bash
lucifex memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p lucifex ui start`
...
| `retain_context` | `conversation between lucifex Agent and the User` | 保留记忆的上下文标签 |
...
**安装：**
```bash
lucifex memory setup    # 选择 "holographic"
# 或手动配置：
lucifex config set memory.provider holographic
```

**配置：** `plugins.lucifex-memory-store` 下的 `config.yaml`
...
**安装：**
```bash
lucifex memory setup    # 选择 "retaindb"
# 或手动配置：
...
# 然后配置 lucifex
lucifex memory setup    # 选择 "byterover"
# 或手动配置：
...
| **依赖** | `pip install supermemory` + [云端 API key](http://app.supermemory.ai/integrations?connect=lucifex)，或[自托管服务器](https://supermemory.ai/docs/self-hosting/overview) |
...
**安装：**
```bash
lucifex memory setup    # 选择 "supermemory"
# 或手动配置：
...
在运行 `lucifex memory setup` **之前**，先在
...
然后运行 `lucifex memory setup` 并输入本地服务器打印的 API key。先配置端点可确保安装连接探测也只访问本地服务器。
...
| `container_tag` | `lucifex` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `lucifex-{identity}` →lucifexifex-coder`lucifexucifex profile 隔离记忆
