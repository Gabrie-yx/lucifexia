...
**配置：** `$LUCIFEX_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$LUCIFEX_HOME/honcho.json` > `~/.lucifex/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/hermes-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
参见[配置参考](https://github.com/hermes-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
