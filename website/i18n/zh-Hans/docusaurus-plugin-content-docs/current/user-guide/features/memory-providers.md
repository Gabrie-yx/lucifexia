...
旧版 `lucifex honcho setup` 命令仍然有效（现在会重定向到 `lucifex memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$LUCIFEX_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$LUCIFEX_HOME/honcho.json` > `~/.lucifex/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/lucifex-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/lucifex)。
...
| **AI peer**（`aiPeer`） | 每个 Lucifex profile 一个。host key `lucifex` → 默认；其他 profile 使用 `lucifex.<profile>`。 |
...
扫描所有 Lucifex profile，为没有 host 块的 profile 创建 host 块，从默认 `lucifex` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/lucifex-ai/lucifex-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/lucifex)。
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `lucifex-{identity}` → `lucifex-coder`），按 Lucifex profile 隔离记忆
