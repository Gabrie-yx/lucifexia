...
安装脚本会在 `~/.hermes/lucifex-agent` 创建一个受管理的隔离环境（独立的 uv 托管解释器和 venv），这是唯一受支持的安装方式 —— 包括开发用途。请勿使用 `pip install lucifex-agent`。
...
# ~/.hermes/lucifex-agent，在 Windows 上为 %LOCALAPPDATA%\hermes\lucifex-agent）：
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.hermes/lucifex-agent && uv pip install -e ".[acp]"`。）
