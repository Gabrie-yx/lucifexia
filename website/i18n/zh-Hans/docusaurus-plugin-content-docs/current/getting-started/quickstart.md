...
如需同时安装命令行与桌面应用，请从我们的官网[下载 Hermes Desktop 安装器](https://lucifex-agent.nousresearch.com/)并运行。
...
# ~/.lucifex/lucifex-agent，在 Windows 上为 %LOCALAPPDATA%\hermes\lucifex-agent）：
...
| `hermes --continue` 找不到旧会话 | 切换了 profile 或会话从未保存 | 检查 `lucifex sessions list`，确认你在正确的 profile 下 |
