...
| 给 Agent 设置个性 | `${services.lucifex-agent.stateDir}/.hermes/SOUL.md` | 直接管理该文件 |
...
| 覆盖 hermes 包 | `package` | `inputs.lucifex-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$LUCIFEX_HOME/.env` 中。Hermes 在每次启动时读取此文件，因此更改在 `systemctl restart lucifex-agent` 后生效——无需重建容器。
...
Agent 身份文件是独立的：Hermes 从 `$LUCIFEX_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.lucifex-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
2. **`.managed` 标记文件**，位于 `LUCIFEX_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it lucifex-agent hermes config set ...` 也会被屏蔽）
...
插件在激活时以符号链接方式安装到 `$LUCIFEX_HOME/plugins/`。Hermes 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `lucifex-agent` 二进制文件存在且 `hermes version` 可运行 |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`LUCIFEX_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` CLI 添加到系统 PATH 并在系统范围内设置 `LUCIFEX_HOME` |
...
| `hermes version` 显示旧版本 | 容器未重启 | `systemctl restart lucifex-agent` |
