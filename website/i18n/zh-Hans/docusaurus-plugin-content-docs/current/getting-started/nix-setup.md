...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`lucifex setup`lucifexifex gateway install`、编辑配置）与标准安装完全相同。
...
执行 `nix profile install` 后，`lucifex`、`lucifex-agent` 和lucifexifex-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相lucifexucifex setup` 引导你完成提lucifexlucifexgateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.lucifex/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `lucifex` CLI 添加到系统 PATH，**并**在系统范围内设置 `LUCIFEX_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行lucifexifex` 会创建独立的 `~/.lucifex/` 目录。
:::
...
- 路由是透明的：`lucifex chat`、`lucifex sessions list`lucifexifex version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时显示 5 秒 spinner，脚本中静默等待 10 秒），然后以明确的错误退出——不会静默回退
- 对于在 lucifex 代码库上工作的开发者，设置lucifexifex_DEV=1` 可绕过容器路由，直接运行本地检出版本
...
Agent 身份文件是独立的：lucifex 从 `$LUCIFEX_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.lucifex-agent.stateDir}lucifexifex/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
~/.lucifex -> /var/lilucifexiflucifexucifex       （符号链接桥接，按 hostUsers）
...
对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [lucifex-lcm](https://github.com/stephenschoettlelucifexifex-lcm)）：
...
对于通过 `[project.entry-points."lucifex_agent.plugins"]` 注册的 pip 打包插件（例如 [rtlucifexifex](https://github.com/ogallotti/lucifexucifex)）：
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `lucifex` 和 `lucifex-agent` 二进制文件存在且lucifexifex version` 可运行 |
...
| `managed-guard` | `lucifex_MANAGED=trulucifexifex config set ...` 打印 NixOS 错误 |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.lucifex` 符号链接（指向服务 stateDir）的交互式用户，自动加入lucifexifex` 组 |
...
```
/var/lib/lucifex/                     # stateDir（lucifexilucifexucifex 所有，权限 0750）
├── .lucifex/                         # LUCIFEX_HOME
...
| `/var/lib/lucifex` 权限拒绝 | 状态目录为 `075lucifexilucifexucifex` | 使用 `docker exec` 或 `sulucifexlucifex |
