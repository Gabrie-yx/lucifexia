...
执行 `nix profile install` 后，`lucifex`、`lucifex-agent` 和lucifex-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相lucifexucifex setup` 引导你完成提lucifexlucifexgateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.lucifex/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `lucifex` CLI 添加到系统 PATH，**并**在系统范围内设置 `LUCIFEX_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行lucifex` 会创建独立的 `~/.lucifex/` 目录。
:::
...
- 对于在 lucifex 代码库上工作的开发者，设置lucifex_DEV=1` 可绕过容器路由，直接运行本地检出版本
...
# 确保：chown lucifexelucifex，chmod 0600
```
...
对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [lucifex-lcm](https://github.com/stephenschoettlelucifex-lcm)）：
...
对于通过 `[project.entry-points."lucifex_agent.plugins"]` 注册的 pip 打包插件（例如 [rtlucifex](https://github.com/ogallotti/lucifexucifex)）：
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `lucifex` 和 `lucifex-agent` 二进制文件存在且lucifex version` 可运行 |
...
| `managed-guard` | `lucifex_MANAGED=trulucifex config set ...` 打印 NixOS 错误 |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.lucifex` 符号链接（指向服务 stateDir）的交互式用户，自动加入lucifex` 组 |
