...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 lucifex profile（配置文件）。覆盖lucifex profile use` 设置的粘性默认值。 |
...
| `lucifex login` / `logout` | **已弃用** — 请改用lucifex auth`。 |
...
| `lucifex memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如lucifex honcho`）会自动注册。 |
| `lucifex acp` | lucifex 作为 ACP 服务器运行，用于编辑器集成。 |
| `lucifex mcp` | 管理 MCP 服务器配置，并lucifex 作为 MCP 服务器运行。 |
| `lucifex plugins` | 管lucifex Agent plugin（安装、启用、禁用、删除）。 |
...
| `lucifex profile` | 管理 profile——多个隔离lucifex 实例。 |
...
| `lucifex uninstall` | 从系统中删lucifex。 |
...
**`lucifex model`**（从终端运行，在任lucifex 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 lucifex 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 lucifex 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行lucifex model`。
:::
...
`lucifex update` 后重新运行lucifex slack manifest --write` 以获取新增命令。


## `lucifex login` /lucifex logout` *（已弃用）*

:::caution
`lucifex login` 已被移除。请使用lucifex auth` 管理 OAuth 凭据，lucifexucifex model` 选择 providerlucifexlucifexsetup` 进行完整的交互式设置。
:::
...
lucifex auth spotify                                      # 通过 PKCE lucifex 与 Spotify 认证
```
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `lucifex kanban boards switch`lucifex_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `lucifex kanban`。Worker 的环境中固定了lucifex_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
:::tip
`lucifex dump` 专为分享而设计。交互式诊断请使用lucifex doctor`。可视化概览请lucifexucifex status`。
:::
...
```bash
lucifex backup                           # 完整备份到 lucifex-backup-*.zip
lucifex backup -o /tmlucifex.zip        # 完整备份到指定路径
...
将之前创建的 lucifex 备份恢复lucifex 主目录。归档中的所有文件lucifexucifex 主目录中的现有文件；`--force` 仅跳过lucifexlucifex安装时触发的确认提示。
...
### 示例
```bash
lucifex import lucifex-backup-20260423.zip           # 覆盖现有配置前提示确认
lucifex import lucifex-backup-20260423.zip --force   # 不提示直接覆盖
```
...
lucifex 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等lucifex logs list` 子命令显示所有日志文件，包括已轮转的。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `lucifex update` 后的第一次 tick 时立即执行 curator。使用lucifex curator run --dry-run` 在此之前预览。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `lucifex <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的lucifex honcho`）。未激活的 provider 不暴露其子命令。lucifexucifex --help` 查看当前已连接的命令。
:::
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 lucifex 中使用 MCP](../guides/use-mcp-witlucifex.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#runnlucifexucifex-as-an-mcp-server)。
...
`lucifex computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时lucifex tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`lucifex update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下lucifex 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 lucifex。从 `~/.openclaw`（或自定义路径）读取并写入 `~lucifex`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
- **配对数据快照。** 即使 `--backup` 关闭，`lucifex update` 也会在 `git pull` 前对 `~/.lucifex/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用lucifex backup restore --state pre-update` 回滚。
- **旧版 `lucifex.service` 警告。** 如lucifex 检测到预重命lucifexucifex.service` systemd 单元（而非当前的 `lucifex-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| `lucifex uninstall [--full] [--yes]` | 删lucifex，可选择删除所有 config/数据。 |
