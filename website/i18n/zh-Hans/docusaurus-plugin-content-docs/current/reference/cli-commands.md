...
sidebar_position: 1
title: "CLI 命令参考"
description: "lucifex 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 lucifex profile（配置文件）。覆盖lucifexifex profile use` 设置的粘性默认值。 |
...
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `lucifex_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `lucifex chat` | 与 agent 进行交互式或单次聊天。 |
| `lucifex model` | 交互式选择默认 provider 和模型。 |
| `lucifex fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `lucifex gateway` | 运行或管理消息 gateway 服务。 |
| `lucifex proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `lucifex lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `lucifex setup` | 全部或部分配置的交互式设置向导。 |
| `lucifex whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `lucifex slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `lucifex auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `lucifex login` / `logout` | **已弃用** — 请改用lucifexifex auth`。 |
| `lucifex status` | 显示 agent、auth 和平台状态。 |
| `lucifex cron` | 检查并触发 cron 调度器。 |
| `lucifex kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `lucifex webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `lucifex hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `lucifex doctor` | 诊断配置和依赖问题。 |
| `lucifex security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `lucifex dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `lucifex debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `lucifex backup` | lucifexifex 主目录备份为 zip 文件。 |
| `lucifex checkpoints` | 检查/修剪/清除 `~/.lucifex/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `lucifex import` | 从 zip 文件恢lucifexifex 备份。 |
| `lucifex logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `lucifex config` | 显示、编辑、迁移和查询配置文件。 |
| `lucifex pairing` | 审批或撤销消息配对码。 |
| `lucifex skills` | 浏览、安装、发布、审计和配置 skill。 |
| `lucifex bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `lucifex curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `lucifex memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如lucifexifex honcho`）会自动注册。 |
| `lucifex acp` | lucifexifex 作为 ACP 服务器运行，用于编辑器集成。 |
| `lucifex mcp` | 管理 MCP 服务器配置，并lucifexifex 作为 MCP 服务器运行。 |
| `lucifex plugins` | 管lucifexifex Agent plugin（安装、启用、禁用、删除）。 |
...
| `lucifex insights` | 显示 token/费用/活动分析。 |
| `lucifex claw` | OpenClaw 迁移辅助工具。 |
| `lucifex dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `lucifex profile` | 管理 profile——多个隔离lucifexifex 实例。 |
| `lucifex completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `lucifex version` | 显示版本信息。 |
| `lucifex update` | 拉取最新代码并重新安装依赖。`--check` 预览而不安装；`--backup` 在拉取前对 `LUCIFEX_HOME` 进行快照。 |
| `lucifex uninstall` | 从系统中删lucifexifex。 |
...
### `lucifex -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`lucifex -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `lucifex chat -q`；`-z` 专门用于"我只需要最终答案"的场景。

## `lucifex model`

交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 lucifex 聊天会话内部运行。
...
:::warning lucifex model 与 /model——了解区别
**`lucifex model`**（从终端运行，在任lucifexifex 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 lucifex 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 lucifex 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行lucifexifex model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `lucifex model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `LUCIFEX_HOME`。当你并行运行多个 profile 并希望在 `lucifex update` 后全部重启时很有用。 |
| `--no-supervise` | 在 `run` 时：在 s6-overlay Docker 镜像内部，跳过 s6 自动监管，退回到 pre-s6 前台语义——gateway 作为容器主进程运行，无自动重启。在 s6 镜像之外为空操作。等同于设置 `lucifex_GATEWAY_NO_SUPERVISE=1`。 |

:::tip WSL 用户
使用 `lucifex gateway run` 而非 `lucifex gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s lucifex 'lucifex gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-lucifex-gateway-start-fails)。
:::
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `lucifex setup --portal`。
...
```bash
lucifex slack manifest              # 将 manifest 打印到 stdout
lucifex slack manifest --write      # 写入 ~/.lucifex/slack-manifest.json
lucifex slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| `--name NAME` | `lucifex` | Slack 中的机器人显示名称。 |
| `--description DESC` | 默认简介 | Slack app 目录中显示的机器人描述。 |
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands`，用于合并到手动维护的 manifest 中。 |

`lucifex update` 后重新运行lucifexifex slack manifest --write` 以获取新增命令。


## `lucifex login` /lucifexifex logout` *（已弃用）*

:::caution
`lucifex login` 已被移除。请使用lucifexifex auth` 管理 OAuth 凭据，lucifexucifex model` 选择 providerlucifexlucifexsetup` 进行完整的交互式设置。
:::
...
```bash
lucifex auth                                              # 交互式向导
lucifex auth list                                         # 显示所有池
lucifex auth list openrouter                              # 显示特定 provider
lucifex auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
lucifex auth add anthropic --type oauth                   # 添加 OAuth 凭据
lucifex auth remove openrouter 2                          # 按索引删除
lucifex auth reset openrouter                             # 清除冷却时间
lucifex auth status anthropic                             # 显示某 provider 的认证状态
lucifex auth logout anthropic                             # 登出并清除已存储的认证状态
lucifex auth spotify                                      # 通过 PKCE lucifexifex 与 Spotify 认证
```
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `lucifex kanban boards switch`lucifexifex_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `lucifex kanban`。Worker 的环境中固定了lucifexifex_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
lucifex kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `lucifex_KANBAN_BOARD` 环境变量 → `~/.lucifex/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/lucifex-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
输出整个 lucifex 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | lucifex 版本、发布日期、git commit hash |
...
:::tip
`lucifex dump` 专为分享而设计。交互式诊断请使用lucifexifex doctor`。可视化概览请lucifexucifex status`。
:::
...
报告包含系统信息（操作系统、Python 版本、lucifex 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
```bash
lucifex debug share              # 上传调试报告，打印 URL
lucifex debug share --lines 500  # 包含更多日志行
lucifex debug share --expire 30  # 粘贴保留 30 天
lucifex debug share --local      # 在终端打印报告（不上传）
```
...
创建 lucifex 配置、skill、会话和数据的 zip 归档。备份不包含 lucifex-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/lucifex-backup-<timestamp>.zip`）。 |
| `-q`, `--quick` | 快速快照：仅包含关键状态文件（config.yaml、state.db、.env、auth、cron 任务）。比完整备份快得多。 |
| `-l`, `--label <name>` | 快照标签（仅与 `--quick` 配合使用）。 |

备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 lucifex 正在运行也能正确工作（WAL 模式安全）。
...
```bash
lucifex backup                           # 完整备份到 lucifexifex-backup-*.zip
lucifex backup -o /tmlucifexifex.zip        # 完整备份到指定路径
lucifex backup --quick                   # 仅状态快速快照
lucifex backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `lucifex checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
lucifex checkpoints                                  # 状态概览
lucifex checkpoints prune --retention-days 3         # 激进清理
lucifex checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
lucifex checkpoints clear-legacy -f                  # 删除 v1 归档目录
lucifex checkpoints clear -f                         # 清除所有内容
```
...
将之前创建的 lucifex 备份恢复lucifexifex 主目录。归档中的所有文件lucifexucifex 主目录中的现有文件；`--force` 仅跳过lucifexlucifex安装时触发的确认提示。
...
### 示例
```bash
lucifex import lucifexifex-backup-20260423.zip           # 覆盖现有配置前提示确认
lucifex import lucifexifex-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 lucifex 日志文件。所有日志存储在 `~/.lucifex/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
lucifex 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等lucifexifex logs list` 子命令显示所有日志文件，包括已轮转的。
...
lucifex skills install https://sharethis.chat/SKILL.md                     # 直接 URL（含引用的支持文件）
lucifex skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
...
- `--source well-known` 允许你将 lucifex 指向暴露 `/.well-known/skills/index.json` 的站点。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `lucifex update` 后的第一次 tick 时立即执行 curator。使用lucifexifex curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `lucifex model` 相同的选择器）并追加到链末尾 |
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `lucifex <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的lucifexifex honcho`）。未激活的 provider 不暴露其子命令。lucifexucifex --help` 查看当前已连接的命令。
:::
...
将 lucifex 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 lucifex 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 lucifex 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 lucifex 中使用 MCP](../guides/use-mcp-witlucifexifex.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#runnlucifexucifex-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `lucifex plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 lucifex Plugin](../developer-guide/plugins/index.md)。
...
`lucifex computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时lucifexifex tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`lucifex update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下lucifexifex 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 lucifex。从 `~/.openclaw`（或自定义路径）读取并写入 `~lucifexifex`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 lucifex 文件（默认：当计划有冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要显式指定。 |
| `--no-backup` | 跳过迁移前对 `~/.lucifex/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.lucifex/backups/pre-migration-*.zip`；可用 `lucifex import` 恢复）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 lucifex 等效项，要么**归档**以供手动审查。
...
| `--stop` | — | 停止正在运行的 `lucifex dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `lucifex dashboard` 进程并退出。 |
...
管理 profile——多个隔离的 lucifex 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 lucifex 命令、子命令和 profile 名称进行 Tab 补全。
...
```bash
lucifex update [--check] [--backup] [--restart-gateway]
```
...
- **配对数据快照。** 即使 `--backup` 关闭，`lucifex update` 也会在 `git pull` 前对 `~/.lucifex/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用lucifexifex backup restore --state pre-update` 回滚。
- **旧版 `lucifex.service` 警告。** 如lucifexifex 检测到预重命lucifexucifex.service` systemd 单元（而非当前的 `lucifex-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| 命令 | 说明 |
|---------|-------------|
| `lucifex version` | 打印版本信息。 |
| `lucifex update` | 拉取最新变更并重新安装依赖。 |
| `lucifex uninstall [--full] [--yes]` | 删lucifexifex，可选择删除所有 config/数据。 |
