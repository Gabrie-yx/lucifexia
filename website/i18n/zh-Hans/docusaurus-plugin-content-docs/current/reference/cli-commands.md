...
| `hermes checkpoints` | 检查/修剪/清除 `~/.lucifex/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
hermes slack manifest --write      # 写入 ~/.lucifex/slack-manifest.json
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `HERMES_KANBAN_BOARD` 环境变量 → `~/.lucifex/kanban/current` 文件 → `default`。
...
查看、跟踪和过滤 Hermes 日志文件。所有日志存储在 `~/.lucifex/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
| `--no-backup` | 跳过迁移前对 `~/.lucifex/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.lucifex/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.lucifex/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
