...
| 工具 | 描述 | 所需环境 |
|------|------|----------|
| `skill_manage` | 管理 skill（创建、更新、删除）。Skill 是你的程序性记忆——针对重复任务类型的可复用方法。新 skill 保存到 `~/.lucifex/skills/`；现有 skill 可在其所在位置修改。操作：create（完整 SKILL.m…） | — |
...
可选工具集（默认 `lucifex-cli` 集中不加载）。通过 `--toolsets video_gen` 添加，或在 `lucifex tools` → Video Generation 中启用（同时引导你选择后端）。
