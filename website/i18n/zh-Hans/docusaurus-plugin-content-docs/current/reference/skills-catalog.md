...
lucifex 在执行lucifex update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.lucifex/skills/` 目录树中缺失，lucifexucifex 一同发布lucifexlucifexskills reset <name> --restore` 恢复。
