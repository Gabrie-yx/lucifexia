...
**原生不支持。** lucifex Agent 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运lucifex。标准安装命令在 WSL2 中可完美运行：
...
:::tip 本地模型超时问题
lucifex 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置lucifex_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::
...
**原因：** lucifex 在启动时通过运行一次 `bash -l` 构建每个会话的环境快照。bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`** — 因此在 `~/.bashrc` 中安装自身的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义 `PATH` 导出）对快照不可见。lucifex 在 systemd 下运行或在未预加载交互式 shell 配置的最小 shell 中运行时，此问题最为常见。
...
- [在 lucifex 中使用 MCP](/guides/use-mcp-witlucifex)
...
:::warning
如果 MCP 服务器在请求中途崩溃，lucifex 会报告超时。请检查服务器自身的日志（而非lucifex 日志）以诊断根本原因。
:::
...
`lucifex update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次lucifex update` — 它覆盖机器上的每个 profile。
...
### `lucifex backup` 与lucifex profile export` 的对比
...
:::tip
`lucifex backup` 即使lucifex 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::
...
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、lucifex 版本lucifex --version`）以及完整的错误信息