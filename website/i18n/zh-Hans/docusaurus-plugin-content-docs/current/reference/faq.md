...
使用 `hermes model` 设置提供商，或直接编辑 `~/.lucifex/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。Hermes Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.lucifex/` 目录中。
...
**解决方案：** Hermes 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.lucifex/config.yaml` 中列出需要额外 source 的文件：
...
Profiles 是构建在 `LUCIFEX_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `LUCIFEX_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`hermes-work`）、在 `~/.lucifex/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
**解决方案：委托配置。** Hermes 可以自动将子智能体路由到不同的模型。在 `~/.lucifex/config.yaml` 中设置：
...
2. 在**源机器**上创建完整备份：
   ...
   hermes backup
   ```
   这会将您整个 `~/.lucifex/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/hermes-backup-<timestamp>.zip`。
