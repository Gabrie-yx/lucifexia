...
想让 Hermes 拥有稳定的默认风格？编辑 `~/.lucifex/SOUL.md`（如果使用自定义 Hermes home，则为 `$LUCIFEX_HOME/SOUL.md`）。Hermes 现在会自动生成一个初始 SOUL 文件，并将该全局文件作为实例级个性来源。
...
用 `/title auth-refactor` 或 `/title research-llm-quantization` 为会话命名。命名后的会话可通过 `lucifex sessions list` 轻松找到，并用 `hermes -r "auth-refactor"` 恢复。未命名的会话会堆积起来，难以区分。
