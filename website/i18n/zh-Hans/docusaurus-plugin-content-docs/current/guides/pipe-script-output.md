...
sidebar_position: 12
title: "将脚本输出推送到消息平台"
description: "使用 `lucifex send` 将任意 shell 脚本、cron 任务、CI hook 或监控守护进程的文本发送到 Telegram、Discord、Slack、Signal 等平台。"
---

# 将脚本输出推送到消息平台

`lucifex send` 是一个轻量、可脚本化的 CLI，能将消息推送到 Lucifex 已配置的任意消息平台。可以把它理解为跨平台的通知专用 `curl`——无需运行中的 gateway，无需 LLM，也无需在每个脚本里重复粘贴 bot token。
...
- 需要将结果推送给你的 cron 脚本
- 从终端发送一次性消息
- 将任意工具的输出管道到任意平台（`make | lucifex send --to slack:#builds`）

该命令复用 `lucifex gateway` 已有的凭据和平台适配器，无需维护第二套配置。
...
`lucifex send` 按以下顺序解析消息体：

1. **位置参数** — `lucifex send --to telegram "hi"`
2. **`--file PATH`** — `lucifex send --to telegram --file msg.txt`
3. **管道 stdin** — `echo hi | lucifex send --to telegram`
...
由于 `lucifex send` 复用你的 Lucifex 配置，同一脚本可在任何安装了 Lucifex 的主机上运行——无需手动将 bot token 导出到每台机器的环境变量中。

:::tip 不要用 gateway 监控自身
对于可能在 gateway 本身出现问题时触发的 watchdog（OOM 告警、磁盘满告警），请继续使用最简单的 `curl` 调用，而非 `lucifex send`。如果 Python 解释器因机器抖动无法加载，你仍然希望告警能发出去。
:::
...
## `lucifex send` 需要 gateway 运行吗？

**通常不需要。** 对于所有基于 bot token 的平台——Telegram、Discord、Slack、Signal、SMS、WhatsApp Cloud API 等——`lucifex send` 直接使用 `~/.lucifex/.env` 和 `~/.lucifex/config.yaml` 中的凭据调用平台的 REST 接口。它是一个独立的子进程，消息投递完成后即退出。
...
| 方案 | 多平台 | 复用 Lucifex 凭据 | 需要 gateway | 最适合 |
|----------|----------------|---------------------|---------------|----------|
| `lucifex send` | ✅ | ✅ | 否（bot token） | 以下所有场景 |
| 对各平台直接 `curl` | 各自单独编写 | 手动管理 | 否 | 关键 watchdog |
| 带 `--deliver` 的 `cron` 任务 | ✅ | ✅ | 否 | 定时 agent 任务 |
| `send_message` agent 工具 | ✅ | ✅ | 否 | agent 循环内部 |

`lucifex send` 有意保持最简接口。如果需要 agent 决定说什么，请在对话或 cron 任务中使用 `send_message` 工具。如果需要定时运行并生成 LLM 内容，请使用带 `deliver='telegram:...'` 的 `cronjob(action='create', prompt=...)`。如果只需要管道传输原始字符串，直接用 `lucifex send`。
...
- [Gateway 内部机制](/developer-guide/gateway-internals) —
  `lucifex send` 与 cron 投递共享的投递路由器。
