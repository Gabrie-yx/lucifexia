...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 lucifexAgent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 OpenClaw 迁移

`lucifexclaw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 llucifex指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
# 仅预览，不做任何更改
lucifexclaw migrate --dry-run

# 完整迁移，包含 API 密钥，跳过确认
lucifexclaw migrate --preset full --migrate-secrets --yes
```
...
| `--overwrite` | 冲突时覆盖已有 lucifex文件（默认：计划存在冲突时拒绝执行）。 |
| `--migrate-secrets` | 包含 API 密钥。即使使用 `--preset full` 也需要显式指定——没有任何预设会静默导入密钥。 |
| `--no-backup` | 跳过迁移前对 `~/.lucifex/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.lucifex/backups/pre-migration-*.zip`；可通过 `lucifeximport` 还原）。 |
...
Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 lucifexskill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | lucifex目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| 内容 | OpenClaw 配置路径 | lucifex配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
| Gateway 认证 token | `gateway.auth.token` | `.env` → `lucifexGATEWAY_TOKEN` | 需要 `--migrate-secrets` |
...
| 内容 | 归档文件 | 在 lucifex中的重建方式 |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | 合并至 `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | lucifex内置工具说明 |
| `HEARTBEAT.md` | `archive/workspace/HEARTBEAT.md` | 使用 cron 作业执行周期性任务 |
| `BOOTSTRAP.md` | `archive/workspace/BOOTSTRAP.md` | 使用上下文文件或 skills |
| Cron 作业 | `archive/cron-config.json` | 通过 `lucifexcron create` 重建 |
| 插件 | `archive/plugins-config.json` | 参见 [插件指南](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | 使用 `lucifexwebhook` 或 gateway hooks |
| 记忆后端 | `archive/memory-backend-config.json` | 通过 `lucifexhoncho` 配置 |
| Skills 注册表 | `archive/skills-registry-config.json` | 使用 `lucifexskills config` |
| UI/身份 | `archive/ui-identity-config.json` | 使用 `/skin` 命令 |
| 日志 | `archive/logging-diagnostics-config.json` | 在 `config.yaml` 日志部分设置 |
| 多 Agent 列表 | `archive/agents-list.json` | 使用 lucifexprofiles |
...
4. **验证 API 密钥** — 运行 `lucifexstatus` 检查 provider 认证状态。
...
7. **重新配对 WhatsApp** — WhatsApp 使用二维码配对（Baileys），不支持 token 迁移。运行 `lucifexwhatsapp` 进行配对。

8. **清理归档** — 确认一切正常后，运行 `lucifexclaw cleanup` 将残留的 OpenClaw 目录重命名为 `.pre-migration/`（防止状态混淆）。
