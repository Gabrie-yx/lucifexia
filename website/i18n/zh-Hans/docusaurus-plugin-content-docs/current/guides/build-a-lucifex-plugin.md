...
title: "构建 Lucifex 插件"
description: "逐步指南：构建包含工具、钩子、数据文件和技能的完整 Lucifex 插件"
---

# 构建 Lucifex 插件

本指南从零开始构建一个完整的 Lucifex 插件。完成后，你将拥有一个包含多个工具、生命周期钩子（hook）、随附数据文件和捆绑技能的可用插件——涵盖插件系统支持的所有功能。

:::info 不确定需要哪份指南？
Lucifex 有多种不同的可插拔接口——有些使用 Python `register_*` API，另一些是配置驱动或放入指定目录即可生效。请先查阅下表：
...
| **STT 后端**（自定义 whisper / ASR CLI） | [语音消息转录](/user-guide/features/tts#voice-message-transcription-stt)——将 `LUCIFEX_LOCAL_STT_COMMAND` 设置为 shell 模板 |
...
| **额外技能来源**（自定义 GitHub 仓库、私有技能索引） | [技能](/user-guide/features/skills)——`lucifex skills tap add <repo>` · [发布 tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
这告诉 Lucifex："我是一个名为 calculator 的插件，我提供工具和钩子。" `provides_tools` 和 `provides_hooks` 字段是插件注册内容的列表。
...
4. **接受 `**kwargs`：** Lucifex 未来可能传入额外上下文。
...
- `ctx.register_cli_command()` 注册 CLI 子命令（例如 `lucifex my-plugin <subcommand>`）
...
- 如果此函数崩溃，插件将被禁用，但 Lucifex 继续正常运行
...
- **未在配置中启用**——插件需要手动启用。运行 `lucifex plugins enable <name>`（名称来自 `plugins list` 输出，嵌套布局下可能是 `<category>/<plugin>`）。
...
用户运行 `lucifex plugins install` 时，会**交互式提示**输入任何缺失的 `requires_env` 变量。值会自动保存到 `.env`。
...
如果你的插件封装了一个并非所有用户都会安装的 SDK（供应商 SDK、重型 ML 库、平台特定包），不要在模块顶部 `import` 它。在工具处理器内部使用 `tools.lazy_deps.ensure(...)` 辅助函数——Lucifex 会在首次使用时安装该包，并受用户 `security.allow_lazy_installs` 配置的控制。
...
| 规则 | 原因 |
|---|---|
| 你的功能键必须出现在内置的 `LAZY_DEPS` 允许列表中 | 防止恶意配置诱使 Lucifex 安装任意包——只有 Lucifex 自身随附的规格才符合条件 |
...
这是唯一一个返回值有意义的钩子。当 `pre_llm_call` 回调返回包含 `"context"` 键的字典（或纯字符串）时，Lucifex 会将该文本注入**当前轮次的用户消息**中。这是记忆插件、RAG 集成、护栏以及任何需要向模型提供额外上下文的插件所使用的机制。
...
- **系统提示词是 Lucifex 的领地**——它包含模型特定的指导、工具执行规则、个性指令和缓存的技能内容。插件在用户输入旁边贡献上下文，而非修改代理的核心指令。
...
|---|---|---|
| 调用方式 | 会话中的 `/name` | 终端中的 `lucifex name` |
...
Lucifex 在通用接口之外还有五种专用插件类型。每种都以目录形式存放在 `plugins/<category>/<name>/`（内置）或 `~/.lucifex/plugins/<category>/<name>/`（用户）下。各类别的约定不同——选择你需要的类型，然后阅读其完整指南。
...
def register(ctx):
    ctx.register_platform(
        ...
        # 在 `lucifex gateway status` 中显示，无需 SDK 实例化。
...
Lucifex 也接受完全不是 Python 插件的扩展。这些在[可插拔接口表](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each)中有所展示；以下各节简要介绍每种编写方式。

### MCP 服务器——注册外部工具

Model Context Protocol（MCP）服务器无需任何 Python 插件即可将自己的工具注册到 Lucifex。在 `~/.lucifex/config.yaml` 中声明：
...
Lucifex 在启动时连接到每个服务器，列出其工具，并与内置工具一起注册。LLM 看到它们的方式与其他工具完全相同。**完整指南：** [MCP](/user-guide/features/mcp)。
...
对于 STT，将 `LUCIFEX_LOCAL_STT_COMMAND` 指向一个 shell 模板。支持的占位符：`{input_path}`、`{output_path}`、`{format}`、`{voice}`、`{model}`、`{speed}`（TTS）；`{input_path}`、`{output_dir}`、`{language}`、`{model}`（STT）。任何与路径交互的 CLI 都自动成为插件。
