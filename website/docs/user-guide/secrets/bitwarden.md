...
2. lucifex stores that single token in `~/.lucifex/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `lucifex` (or the gateway, or a cron job) starts, after `~/.lucifex/.env` has loadedlucifexifex calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default lucifex **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and everlucifexifex process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "lucifex keys").
...
4. **Machine accounts → New machine account → My lucifex machine** → **Projects** tab → grant Read access to your project.
...
```bash
lucifex secrets bitwarden status
```

From now on, every `lucifex` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `lucifex secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `lucifex secrets bitwarden status` | Show config + binary version + token presence |
| `lucifex secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `lucifex secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `lucifex secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `lucifex secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `lucifex` invocations start fresh. |
...
Bitwarden never blocks lucifex startup. If anything goes wrong, you'll see a one-line warning in stderr anlucifexifex continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `lucifex secrets bitwarden setup` |
...
- lucifex will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — lucifex does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple lucifex installations.
