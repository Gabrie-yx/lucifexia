...
3. Every time `lucifex` (or the gateway, or a cron job) starts, after `~/.lucifex/.env` has loaded, Lucifex calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Lucifex **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Lucifex process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
Bitwarden never blocks Lucifex startup. If anything goes wrong, you'll see a one-line warning in stderr and Lucifex continues with whatever credentials `.env` already had:
