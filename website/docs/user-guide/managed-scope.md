...
Managed scope is read from a system-level directory, default `/etc/lucifex`:
...
The location can be relocated with the `LUCIFEX_MANAGED_DIR` environment variable
...
files. It is **never persisted** to any `.env` by Lucifex.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export LUCIFEX_MANAGED_DIR=/opt/org/lucifex-policy
```

:::warning
A user who can set `LUCIFEX_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `lucifex doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/lucifex/config.yaml` (managed) | `/etc/lucifex/.env` (managed) |
...
```bash
lucifex config        # shows a header naming the managed source + the pinned keys
lucifex doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Lucifex refuses and names the source:

```bash
$ lucifex config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/lucifex/config.yaml) and cannot be changed.
```
...
```bash
sudo mkdir -p /etc/lucifex

# Pin some config values for every user on this machine
sudo tee /etc/lucifex/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/lucifex/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/lucifex
sudo chmod 0644 /etc/lucifex/config.yaml /etc/lucifex/.env
```
...
`lucifex doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Lucifex as `root`), managed scope is advisory.
