...
If an operator needs to repair or inspect files outside `/opt/data`, use a root shell intentionally. The `lucifex` shim normally drops `docker exelucifexilucifexucifex ...` back to the runtime user;lucifexlucifexDOCKER_EXEC_AS_ROOT=1` for a one-off root invocation when you explicitly need root semantics.
