...
sidebar_position: 7
title: "Docker"
description: "Running Lucifex Agent in Docker and using Docker as a terminal backend"
---

# Lucifex Agent — Docker

There are two distinct ways Docker intersects with Lucifex Agent:

1. **Running Lucifex IN Docker** — the agent itself runs inside a container (this page's primary focus)
2. **Docker as a terminal backend** — the agent runs on your host but executes every command inside a single, persistent Docker sandbox container that survives across tool calls, `/new`, and subagents for the life of the Lucifex process (see [Configuration → Docker Backend](./configuration.md#docker-backend))

This page covers option 1. The container stores all user data (config, API keys, sessions, skills, memories) in a single directory mounted from the host at `/opt/data`. The image itself is stateless and can be upgraded by pulling a new version without losing any configuration.

## Quick start

If this is your first time running Lucifex Agent, create a data directory on the host and start the container interactively to run the setup wizard:
...
like `-v ~/.lucifex:/opt/data`, `-e KEY=value`, and pasted API keys / tokens.
...
:::tip
Inside the container, run `lucifex setup --portal` once — the refresh token persists in the mounted `~/.lucifex` volume. See [Lucifex portal](/integrations/lucifex-portal).
:::
...
Inside the official Docker image, `gateway run` is **automatically supervised by s6-overlay**: if the gateway process crashes it's restarted within a couple of seconds without losing the container, and the dashboard (when `LUCIFEX_DASHBOARD=1` is set) is supervised alongside it. The `gateway run` CMD process itself is a `sleep infinity` heartbeat that keeps the container alive while s6 manages the actual gateway process — so `docker stop` still shuts everything down cleanly, but `docker logs` shows the supervised gateway's output.

You'll see a one-line breadcrumb in `docker logs` confirming the upgrade. To opt out — and get the historical "gateway is the container's main process, container exit = gateway exit" semantics — pass `--no-supervise` or set `LUCIFEX_GATEWAY_NO_SUPERVISE=1`. The opt-out is useful for CI smoke tests that want the container to exit with the gateway's status code; for production deployments the supervised default is strictly better.
...
The built-in web dashboard runs as a supervised s6-rc service alongside the gateway in the same container. Set `LUCIFEX_DASHBOARD=1` to bring it up:
...
|---------------------|-------------|---------|
| `LUCIFEX_DASHBOARD` | Set to `1` (or `true` / `yes`) to enable the supervised dashboard service | *(unset — service is registered but stays down)* |
| `LUCIFEX_DASHBOARD_HOST` | Bind address for the dashboard HTTP server | `0.0.0.0` |
| `LUCIFEX_DASHBOARD_PORT` | Port for the dashboard HTTP server | `9119` |
| `LUCIFEX_DASHBOARD_INSECURE` | **Deprecated / no-op.** Formerly bypassed the auth gate; as of the June 2026 hardening it no longer disables authentication. A non-loopback bind always requires an auth provider | *(ignored — configure a provider instead)* |

The dashboard inside the container defaults to binding `0.0.0.0` — without it, the published `-p 9119:9119` port would not be reachable from the host. To restrict the bind to container loopback (for sidecar / reverse-proxy setups), set `LUCIFEX_DASHBOARD_HOST=127.0.0.1`.
...
- **Username/password** — the simplest for a self-hosted / on-prem / homelab container on a trusted network or behind a VPN: set `LUCIFEX_DASHBOARD_BASIC_AUTH_USERNAME` + `LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD` (and `LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET` for restart-stable sessions). Not suitable for direct public-internet exposure.
- **OAuth (Lucifex portal)** — for hosted/public deploys: the `dashboard_auth/nous` provider activates whenever `LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID` is set.
...
```sh
/opt/lucifex/.venv/bin/lucifex
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Lucifex state. It maps to your host's `~/.lucifex/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Lucifex configuration |
| `SOUL.md` | Agent personality/identity |
| `sessions/` | Conversation history |
| `memories/` | Persistent memory store |
| `skills/` | Installed skills |
| `home/` | Per-profile HOME for Lucifex tool subprocesses (`git`, `ssh`, `gh`, `npm`, and skill CLIs) |
...
In hosted and published Docker images, `/opt/lucifex` is the installed application tree. It is root-owned and read-only to the runtime `lucifex` user, so agent turns, gateway sessions, dashboard actions, and normal `docker exec lucifex lucifex ...` commands cannot edit the core source, bundled `.venv`, `node_modules`, or TUI bundle in place.

All mutable Lucifex state belongs under `/opt/data`: config, `.env`, profiles, skills, memories, sessions, logs, dashboard uploads, plugins, and other user-managed files. The image also disables runtime `.pyc` writes and Lucifex lazy dependency installs into `/opt/lucifex`; optional platform dependencies needed by the published image should be baked into the image or installed through a new image build.
...
If an operator needs to repair or inspect files outside `/opt/data`, use a root shell intentionally. The `lucifex` shim normally drops `docker exec lucifex lucifex ...` back to the runtime user; set `LUCIFEX_DOCKER_EXEC_AS_ROOT=1` for a one-off root invocation when you explicitly need root semantics.

Skill CLIs that store credentials under `~` must be initialized against the subprocess HOME, not just the data-volume root. For example, the [xurl skill](./skills/bundled/social-media/social-media-xurl.md) stores OAuth state in `~/.xurl`; in the official Docker layout, Lucifex tool calls read that as `/opt/data/home/.xurl`, so run manual xurl auth with `HOME=/opt/data/home` and verify with `HOME=/opt/data/home xurl auth status`.

:::warning
Never run two Lucifex **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access.
:::

## Multi-profile support

Lucifex supports [multiple profiles](../reference/profile-commands.md) — separate `~/.lucifex/` subdirectories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **Inside the official Docker image, the s6 supervision tree treats each profile as a first-class supervised service**, so the recommended deployment is **one container hosting all profiles**.

Each profile created with `lucifex profile create <name>` gets:
...
docker exec lucifex lucifex profile create coder

# Start / stop / restart — dispatches s6-svc; the gateway lifecycle survives docker restart.
docker exec lucifex lucifex -p coder gateway start
docker exec lucifex lucifex -p coder gateway stop
docker exec lucifex lucifex -p coder gateway restart

# Status — reports `Manager: s6 (container supervisor)` inside the container.
docker exec lucifex lucifex -p coder gateway status

# Remove a profile — tears down the s6 slot too.
docker exec lucifex lucifex profile delete coder
```
...
**Lucifex Desktop (and the web dashboard).** The Desktop app's **Remote Gateway** connection talks to a `lucifex dashboard` backend (default **port 9119**, enabled by `LUCIFEX_DASHBOARD=1`) — *not* the OpenAI API server. One dashboard backend serves **every** co-located profile: the app's profile switcher sends the target profile with each request and the backend opens that profile's `LUCIFEX_HOME` on disk. So you do **not** need a second port — or a second connection — per profile for Desktop; one `:9119` connection covers them all through the switcher.
...
docker exec lucifex lucifex profile create work
...
docker exec lucifex lucifex -p work gateway restart
```
...
| Profile creation | `docker exec ... lucifex profile create <name>` (seconds) | New `docker run` invocation + port allocation + bind-mount config |
...
| Backup | One `~/.lucifex` directory | N directories to coordinate |
...
services:
  lucifex-work:
    ...
    container_name: lucifex-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
    volumes:
      - ~/.lucifex-work:/opt/data

  lucifex-personal:
    image: nousresearch/lucifex-agent:latest
    container_name: lucifex-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.lucifex-personal:/opt/data
```

The warning from [Persistent volumes](#persistent-volumes) still applies: never point two containers at the same `~/.lucifex` directory simultaneously. The s6 supervisor inside each container manages its own profile set; cross-container sharing of a data volume corrupts session files and memory stores.
...
| **Dashboard** (when `LUCIFEX_DASHBOARD=1`) | `docker logs <container>` (no prefix) | `docker logs -f lucifex` — interleaved with gateway lines |
...
| **Generic Lucifex logs** (`agent.log`, `errors.log`) | `${LUCIFEX_HOME}/logs/` (profile-aware) | `docker exec lucifex lucifex logs --follow [--level WARNING] [--session <id>]` |
...
services:
  lucifex:
    image: nousresearch/lucifex-agent:latest
    container_name: lucifex
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"   # gateway API
      - "9119:9119"   # dashboard (only reached when LUCIFEX_DASHBOARD=1)
    volumes:
      - ~/.lucifex:/opt/data
    environment:
      - LUCIFEX_DASHBOARD=1
...
```sh
export LUCIFEX_UID="$(id -u)"
export LUCIFEX_GID="$(id -g)"
...
```sh
docker exec lucifex /opt/lucifex/.venv/bin/python -c "import sounddevice as sd; print(sd.query_devices())"
```

## Resource limits

The Lucifex container needs moderate resources. Recommended minimums:
...
- Python 3.13 with dependencies synced from the lockfile via `uv sync --frozen --no-install-project` for the baked extras (`all`, `messaging`, Anthropic/Bedrock/Azure identity, Hindsight, Matrix), followed by a no-dependency editable install of Lucifex itself.
...
The image treats `/opt/lucifex` as an immutable install tree at runtime. Optional Python extras, Node workspaces, and TUI assets that must be available inside Docker need to be baked during the image build; runtime lazy installs are disabled so supervised gateways and `docker exec lucifex …` commands do not try to write dependency artifacts back into the read-only source tree.

The container's `ENTRYPOINT` is s6-overlay's `/init`. On boot it:
1. Runs `/etc/cont-init.d/01-lucifex-setup` (= `docker/stage2-hook.sh`) as root: optional UID/GID remap, fixes volume ownership, seeds `.env` / `config.yaml` / `SOUL.md` on first boot, runs non-interactive config-schema migrations unless `LUCIFEX_SKIP_CONFIG_MIGRATION=1`, syncs bundled skills.
...
4. Exec's the container's CMD as the main program (`/opt/lucifex/docker/main-wrapper.sh`), which routes the arguments the user passed to `docker run`:
   - no args → `lucifex` (the default)
   - first arg is an executable on PATH (e.g. `sleep`, `bash`) → exec it directly
   - anything else → `lucifex <args>` (subcommand passthrough)
...
Do not override the image entrypoint unless you keep `/init` (or, equivalently, the legacy `docker/entrypoint.sh` shim that forwards to the stage2 hook) in the command chain. s6-overlay's `/init` runs as root so it can chown the volume on first boot, then drops to the `lucifex` user via `s6-setuidgid` for every supervised service AND for the main program. Starting `lucifex gateway run` as root inside the official image is refused by default because it can leave root-owned files in `/opt/data` and break later dashboard or gateway starts. Set `LUCIFEX_ALLOW_ROOT_GATEWAY=1` only when you intentionally accept that risk.
:::

### `docker exec` automatically drops to the `lucifex` user

`docker exec lucifex <cmd>` defaults to running as root inside the container, but the image ships a thin shim at `/opt/lucifex/bin/lucifex` (earliest on PATH) that detects root callers and transparently re-execs through `s6-setuidgid lucifex`. So `docker exec lucifex login`, `docker exec lucifex profile create …`, `docker exec lucifex setup`, etc. all write files owned by UID 10000 — i.e. readable by the supervised gateway — with no extra `--user` flag needed. Non-root callers (the supervised processes themselves, `docker exec --user lucifex`, kanban subagents inside the container) hit a short-circuit that exec's the venv binary directly, so there's no overhead on the hot paths.
...
```sh
docker exec -e LUCIFEX_DOCKER_EXEC_AS_ROOT=1 lucifex <cmd>
```
...
`lucifex status` inside the container reports `Manager: s6 (container supervisor)`. Use `/command/s6-svstat /run/service/gateway-<name>` for the raw supervisor view (note `/command/` is on PATH for supervision-tree processes only; pass the absolute path when calling from `docker exec`).
...
When a migration is needed, Lucifex writes timestamped backups next to
...
For any tool published to npm or PyPI, instruct Lucifex to run it via `npx` (npm) or `uvx` (Python) and to remember that command in its persistent memory. If the tool needs a config file or credentials, instruct it to drop those under `/opt/data` (e.g. `/opt/data/<tool>/config.yaml`).
...
```sh
docker build -t my-lucifex:latest .
docker run -d \
  --name lucifex \
  --restart unless-stopped \
  -v ~/.lucifex:/opt/data \
  -p 8642:8642 \
  my-lucifex:latest gateway run
```
...
For tools that bring their own service (a database, a web server, a queue, a headless browser farm) or that are too heavy to live inside the Lucifex container, run them as a separate container on a shared Docker network. Lucifex reaches the sidecar by container name, the same way it reaches a local inference server (see [Connecting to local inference servers](#connecting-to-local-inference-servers-vllm-ollama-etc)).
...
From inside the Lucifex container, the sidecar is reachable at `http://my-tool:<port>` (or whatever protocol it serves). This pattern keeps each service's lifecycle, resource limits, and upgrade cadence independent, and avoids bloating the Lucifex image with dependencies that are only needed by one tool.

### Broadly useful tools — open an issue or pull request

If a tool is likely to be useful to most Lucifex Agent users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [lucifex-agent repository](https://github.com/NousResearch/lucifex-agent) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.

## Connecting to local inference servers (vLLM, Ollama, etc.)

When running Lucifex in Docker and your inference server (vLLM, Ollama, text-generation-inference, etc.) is also running on the host or in another container, networking requires extra attention.
...
:::tip Key points
- Use the **container name** (`vllm`) as the hostname — not `localhost` or `127.0.0.1`, which refer to the Lucifex container itself.
...
From inside the Lucifex container, confirm the inference server is reachable:

```sh
docker exec lucifex curl -s http://vllm:8000/v1/models
```

You should see a JSON response listing your served model. If this fails, check:

1. Both containers are on the same Docker network (`docker network inspect lucifex-net`)
...
Check logs: `docker logs lucifex`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container's stage2 hook drops privileges to the non-root `lucifex` user (UID 10000) via `s6-setuidgid` inside each supervised service. If your host `~/.lucifex/` is owned by a different UID, set `LUCIFEX_UID`/`LUCIFEX_GID` — or their `PUID`/`PGID` aliases, for parity with LinuxServer.io and NAS images — to match your host user, or ensure the data directory is writable:
...
docker run -d \
  ...
  -v /volume1/docker/lucifex:/opt/data \
  nousresearch/lucifex-agent gateway run
```

`docker exec lucifex <cmd>` automatically drops to UID 10000 too — see [`docker exec` automatically drops to the `lucifex` user](#docker-exec-automatically-drops-to-the-lucifex-user) for details and the per-invocation opt-out.
...
```sh
docker restart lucifex
```

### Checking container health

```sh
docker logs --tail 50 lucifex          # Recent logs
docker run -it --rm nousresearch/lucifex-agent:latest version     # Verify version
docker stats lucifex                    # Resource usage
```
