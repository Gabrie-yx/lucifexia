...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/productivity/canvas` |
...
4. Name the token (e.g., "lucifexex Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${LUCIFEX_HOME:-~/.lucifexex}/.env`:
...
```bash
CANVAS="python $LUCIFEX_HOME/skills/productivity/canvas/scripts/canvas_api.py"
