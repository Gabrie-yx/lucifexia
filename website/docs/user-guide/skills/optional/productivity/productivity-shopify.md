...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/productivity/shopify` |
...
4. Save to `${LUCIFEX_HOME:-~/.lucifex}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Lucifex","productType":"Apparel","tags":["test"]}}'
```
