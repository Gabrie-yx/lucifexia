...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/productivity/shopify` |
...
4. Save to `${LUCIFEX_HOME:-~/.lucifexex}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"lucifexex","productType":"Apparel","tags":["test"]}}'
```
