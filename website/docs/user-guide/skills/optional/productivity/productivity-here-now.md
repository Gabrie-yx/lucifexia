...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LUCIFEX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LUCIFEX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LUCIFEX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client lucifex
```
...
```bash
PUBLISH="${LUCIFEX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client lucifex
```
...
```bash
DRIVE="${LUCIFEX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `lucifex`)    |
