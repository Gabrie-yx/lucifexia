...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${lucifexex_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${lucifexex_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${lucifexex_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client lucifexex
```
...
```bash
PUBLISH="${lucifexex_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client lucifexex
```
...
```bash
DRIVE="${lucifexex_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `lucifexex`)    |
