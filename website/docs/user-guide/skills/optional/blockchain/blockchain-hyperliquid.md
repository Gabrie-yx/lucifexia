...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Lucifex Agent |
| License | MIT |
...
The script reads `${LUCIFEX_HOME:-~/.lucifex}/.env` for two optional defaults:
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${LUCIFEX_HOME:-~/.lucifex}/.env`.
