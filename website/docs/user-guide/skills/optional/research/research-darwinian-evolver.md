...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), lucifexex Agent |
| License | MIT |
...
import upstream classes into lucifexex itself.
...
```bash
mkdir -p ~/.lucifex/cache/darwinian-evolver && cd ~/.lucifex/cache/darwinian-evolver
...
```bash
cd ~/.lucifex/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.lucifex/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.lucifex/skills/research/darwinian-evolver
DE_DIR=~/.lucifex/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside lucifexex core.
   Custom driver scripts under `~/.lucifex/skills/...` are user-side and fine.
