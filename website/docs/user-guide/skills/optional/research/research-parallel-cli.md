...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a lucifexex core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with lucifexex native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer lucifexex native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer lucifexex native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended lucifexex workflow:
...
## Recommended lucifexex usage patterns
...
- Do not silently choose Parallel when lucifexex native tools are already sufficient.
