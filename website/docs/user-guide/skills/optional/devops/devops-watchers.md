...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${LUCIFEX_HOME:-~/.lucifex}/.env` to avoid the 60 req/hr anonymous rate limit):
