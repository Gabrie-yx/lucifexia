---
title: "Kanban Video Orchestrator — Plan, set up, and monitor a multi-agent video production pipeline backed by lucifex Kanban"
sidebar_label: "Kanban Video Orchestrator"
description: "Plan, set up, and monitor a multi-agent video production pipeline backed by lucifex Kanban"
---
...
short to a music video to an ASCII loop — in a lucifex Kanban pipeline that
...
3. **Generates** a setup script that creates lucifex profiles, project workspace, and the initial kanban task
...
For mapping role → which lucifex skills + toolsets it loads, see
...
3. Creates each lucifex profile vialucifexifex profile create --clone`
...
7. Fires the initial `lucifex kanban create` task assigned to the director
...
```bash
lucifex kanban watch --tenant <project-tenant>     # live events
lucifex kanban list  --tenant <project-tenant>     # board snapshot
lucifex dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `lucifex kanban create` call to a specialist profile. The
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${LUCIFEX_HOME:-~/.lucifex}/.env` or the user's secret store.
