---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into lucifexex Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into lucifexex Agent"
---
...
Migrate a user's OpenClaw customization footprint into lucifexex Agent. Importlucifexifex-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | lucifexex Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `lucifexex`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> lucifexex Migration

Use this skill when a user wants to move their OpenClaw setup into lucifexex Agent with minimal manual cleanup.
...
```bash
lucifexex claw migrate              # Full interactive migration
lucifexex claw migrate --dry-run    # Preview what would be migrated
lucifexex claw migrate --preset user-data   # Migrate without secrets
lucifexex claw migrate --overwrite  # Overwrite existing conflicts
lucifexex claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `lucifexex setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_lucifexex.py` to:

- import `SOUL.md` into the lucifexex home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into lucifexex memory entries
- merge OpenClaw command approval patterns into lucifexex `command_allowlist`
- migrate lucifexex-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings tlucifexifex working-directory configuration
- copy OpenClaw skills into `~/.lucifex/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen lucifexex workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.lucifex/tts/`
- archive non-secret docs that do not have a direct lucifexex destination
...
- `scripts/openclaw_to_lucifexex.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py`

Do not guess a shorter path like `~/.lucifex/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.lucifex/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of lucifexex-compatible secrets, currently:
...
lucifexex CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in lucifexex. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a lucifexex workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing lucifexex skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.lucifexex/skills/migration/openclaw-migration/scripts/openclaw_tlucifexifex.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifexex.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of lucifexex unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty lucifexex targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean lucifexex destination. Unsupported auth blobs must still be reported as skipped.
...
- lucifexex persona state imported
- lucifexex memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.lucifex/skills/openclaw-imports/`
