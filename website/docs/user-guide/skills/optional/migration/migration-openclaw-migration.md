---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Lucifex Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Lucifex Agent"
---
...
Migrate a user's OpenClaw customization footprint into Lucifex Agent. Imports Lucifex-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Lucifex Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Lucifex`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Lucifex Migration

Use this skill when a user wants to move their OpenClaw setup into Lucifex Agent with minimal manual cleanup.
...
```bash
lucifex claw migrate              # Full interactive migration
...
lucifex claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `lucifex setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_lucifex.py` to:
...
- migrate Lucifex-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Lucifex working-directory configuration
...
- archive non-secret docs that do not have a direct Lucifex destination
...
- `scripts/openclaw_to_lucifex.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py`
...
With `--migrate-secrets`, it will also import a small allowlisted set of Lucifex-compatible secrets, currently:
...
Lucifex CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Lucifex. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Lucifex workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Lucifex skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.lucifex/skills/migration/openclaw-migration/scripts/openclaw_to_lucifex.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Lucifex unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Lucifex targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Lucifex destination. Unsupported auth blobs must still be reported as skipped.
...
- Lucifex persona state imported
- Lucifex memory files populated with converted OpenClaw knowledge
