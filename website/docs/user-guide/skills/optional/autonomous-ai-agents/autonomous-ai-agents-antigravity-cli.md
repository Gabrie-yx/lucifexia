...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/autonomous-ai-agents/antigravity-cli` |
| Path | `optional-skills/autonomous-ai-agents/antigravity-cli` |
| Version | `0.1.0` |
| Author | Tony Simons (asimons81), Lucifex Agent |
| License | MIT |
...
commands through the Lucifex `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Lucifex itself.
