...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by lucifexex Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during lucifexex terminal calls (desktop app flow only)
...
### Service Account (recommended for lucifexex)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${LUCIFEX_HOME:-~/.lucifexex}/.env` (the skill will prompt for this on first load).
...
## lucifexex Execution Pattern (desktop app flow)

lucifexex terminal commands are non-interactive by default and can lose auth context between calls.
