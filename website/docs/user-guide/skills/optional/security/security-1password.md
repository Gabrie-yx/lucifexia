...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Lucifex Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Lucifex terminal calls (desktop app flow only)
...
### Service Account (recommended for Lucifex)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${LUCIFEX_HOME:-~/.lucifex}/.env` (the skill will prompt for this on first load).
...
## Lucifex Execution Pattern (desktop app flow)

Lucifex terminal commands are non-interactive by default and can lose auth context between calls.
