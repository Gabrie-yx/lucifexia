...
5. **Lucifex models don't need jailbreaking** — nousresearch/lucifex-3-* and lucifex-4-* are already uncensored. Use them directly for the fastest path.
...
8. **System prompt vs ephemeral system prompt** — The `agent.system_prompt` in config.yaml is appended AFTER Lucifex's own system prompt. It doesn't replace the default prompt; it augments it. This means the jailbreak instructions coexist with Lucifex's normal personality.
...
12. **execute_code sandbox has no env vars** — When Lucifex runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Lucifex `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("LUCIFEX_HOME", os.path.expanduser("~/.lucifex")), ".env"))`
