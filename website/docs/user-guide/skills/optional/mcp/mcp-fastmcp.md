...
| | |
|---|---|
| Source | Optional — install with `lucifexex skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into lucifexex or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to lucifexex. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is lucifexex integration, either:
...
### lucifexex cannot see the deployed server

The server-building part may be correct while the lucifexex config is not. Load the `native-mcp` skill and configure the server in `~/.lucifex/config.yaml`, then restarlucifexifex.
