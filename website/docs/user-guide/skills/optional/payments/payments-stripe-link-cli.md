...
| | |
|---|---|
| Source | Optional — install with `lucifex skills install official/payments/stripe-link-cli` |
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Lucifex attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Lucifex" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Lucifex' native MCP:

```
lucifex mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `lucifex mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
