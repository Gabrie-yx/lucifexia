...
description: "Use when a Lucifex Kanban worker wants to run Codex CLI as an isolated implementation lane while Lucifex keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Lucifex Kanban worker wants to run Codex CLI as an isolated implementation lane while Lucifex keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
1. Lucifex owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Lucifex board CLI as a substitute for the worker.
2. Lucifex owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Lucifex owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Lucifex with the repo's canonical wrapper.
...
Lucifex owns the Kanban lifecycle; do not call Lucifex kanban tools or messaging.
...
- [ ] Lucifex ran the canonical tests itself, using `scripts/run_tests.sh` for Lucifex Agent or the repo's documented wrapper for other repos.
