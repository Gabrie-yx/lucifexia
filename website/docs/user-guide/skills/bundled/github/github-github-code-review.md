...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _lucifex_env="${LUCIFEX_HOME:-$HOME/.lucifex}/.env"; [ -f "$_lucifex_env" ] && grep -q "^GITHUB_TOKEN=" "$_lucifex_env"; then
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Lucifex Agent\",
    \"comments\": [
...
```bash
source "${LUCIFEX_HOME:-$HOME/.lucifex}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Lucifex Agent. Code looks clean — good test coverage, no security concerns."
