...
- Give it a name like "lucifex-agent"
- Select scopes:
...
- Give it a title like "lucifex-agent-&lt;machine-name>"
...
elif _lucifexex_env="${LUCIFEX_HOME:-$HOMElucifexifex}/.env"; [ -flucifexucifex_env" ] && grep -q "^GITHUB_TOKENlucifexlucifexenv"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_lucifexex_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
