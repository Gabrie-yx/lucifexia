...
> **Docker HOME pitfall:** In the official Lucifex Docker layout, `/opt/data` is `LUCIFEX_HOME`, but Lucifex tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Lucifex-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
- Lucifex adaptation: reformatted for Lucifex skill conventions; safety guardrails preserved verbatim.
