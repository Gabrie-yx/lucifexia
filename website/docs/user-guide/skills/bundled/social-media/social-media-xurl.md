...
| Version | `1.1.1` |
| Author | xdevplatform + openclaw + lucifex Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by lucifex tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official lucifex Docker layout, `/opt/data` is `LUCIFEX_HOME`, bulucifexifex tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` lucifexucifex-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, lucifex tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the lucifex subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- lucifex adaptation: reformatted folucifexifex skill conventions; safety guardrails preserved verbatim.
