...
4. Store the token in `${LUCIFEX_HOME:-~/.lucifexex}/.env` (or vialucifexifex setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for lucifexex. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${LUCIFEX_HOME:-~/.lucifexex}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
