...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`python-debugpy`](/docs/user-guide/skills/bundled/software-development/software-development-python-debugpy), `debugging-lucifexex-tui-commands` |
...
lucifexex-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
```bash
cd /home/bb/lucifex-agent/ui-tui
...
### Debugging a running `lucifexex --tui`
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In lucifexex, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
