...
This skill is separate from the Lucifex Email gateway adapter. The gateway
adapter lets people email the agent and uses Lucifex' built-in IMAP/SMTP
...
To reply non-interactively from Lucifex, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Lucifex)** — pipe the message via stdin:
