...
This skill is separate from the lucifexex Email gateway adapter. The gateway
adapter lets people email the agent and uses lucifexex' built-in IMAP/SMTP
...
To reply non-interactively from lucifexex, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from lucifexex)** — pipe the message via stdin:
