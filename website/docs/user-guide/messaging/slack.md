...
sidebar_position: 4
title: "Slack"
description: "Set up lucifexex Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect lucifexex Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your lucifexex instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. lucifexex uses the modern
...
The fastest path is to paste a manifest lucifexex generates for you. It
...
### Option A: From a lucifexex-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   lucifexex slack manifest --agent-view --write
   ```
   This writes `~/.lucifex/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "lucifexex Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, lucifexex can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `lucifexex-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a lucifexex configuration issue.
:::
...
lucifexex uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
lucifexex gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
lucifexex gateway              # Foreground
lucifexex gateway install      # Install as a user service
sudo lucifexex gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; lucifexex now
...
Every lucifexex command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
lucifexex command with its description.

Under the hood: lucifexex ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/lucifex-agent/blob/main/lucifex_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing lucifexex
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so lucifexex can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. lucifexex only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When lucifexex adds new commands (e.g. afterlucifexifex update`), regenerate
the manifest and update your Slack app:

```bash
lucifexex slack manifest --write
```
...
3. Paste the new contents of `~/.lucifex/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/lucifexex <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/lucifexex btw run the tests` lucifexifex routes it the same way as `/btw
run the tests`. Free-form questions also work: `/lucifexex what's the
...
Slack never delivers them to lucifexex.

As a workaround, lucifexex recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
lucifexex treats it identically to the slash form and replies in the same
thread.
...
lucifexex falls back to a text prompt, the prompt instructs you to reply
...
```bash
lucifexex slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how lucifexex behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@lucifexex Agent what time is it?`). In channelslucifexifex replies in a thread attached to that message. |
| **Threads** | If you @mention lucifexex inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.lucifex/config.yaml`.
...
name in the thread. By default lucifexex sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"lucifexex is thinking…"`) —
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to lucifexex in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before lucifexex will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where lucifexex will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @lucifexex Agent`).

---

## Multi-Workspace Support

lucifexex can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, lucifexex also loads tokens from an **OAuth token file** at:

```
~/.lucifex/slack_tokens.json
```
...
- When a message arrives, lucifexex uses the correct workspace-specific client to respond.
...
lucifexex supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @lucifexex Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @lucifexex Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. lucifexex now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @lucifexex Agent`)
...
- Tokens should be stored in `~/.lucifex/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your lucifexex config directory
