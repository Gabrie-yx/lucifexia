...
Connect Lucifex to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Lucifex and the BlueBubbles server
...
By default, Lucifex responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Lucifex uses conservative defaults for `Lucifex` and `@Lucifex agent` variants.
...
When someone messages your iMessage, Lucifex automatically sends them a pairing code. Approve it with:
```bash
lucifex pairing approve bluebubbles <CODE>
```
Use `lucifex pairing list` to see pending codes and approved users.
...
Lucifex will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Lucifex
Lucifex → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Lucifex sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Lucifex wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |
...
### Chat Addressing
You can address chats by email or phone number — Lucifex resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `lucifex logs gateway` for webhook errors (or `lucifex logs -f` to follow in real-time)
