...
Connect lucifexex to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
lucifexex supports two WeCom integration modes:
...
> Run `lucifexex gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. lucifexex decrypts the message, queues it for the agent
...
(Use `lucifex gateway start` only after `lucifexex gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in lucifexex and the token the
...
in `~/.lucifex/.env` values around `=` will also break signature checks. After
fixing, restart `lucifex gateway run`.
...
Check `lucifex gateway run` logs for the bound host/port. If the adapter bound to
