...
Connect Lucifex to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lucifex supports two WeCom integration modes:
...
> Run `lucifex gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Lucifex decrypts the message, queues it for the agent
...
(Use `lucifex gateway start` only after `lucifex gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Lucifex and the token the
