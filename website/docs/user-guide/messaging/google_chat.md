...
`~/.lucifex/config.yaml` — e.g. a kitten assistant named Ada:
...
The token lands at `~/.lucifex/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.lucifex/google_chat_user_token.json` (if present from
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.lucifex/google_chat_user_tokens/<sanitized_email>.json` (filesystem
