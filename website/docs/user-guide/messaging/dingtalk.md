...
sidebar_position: 10
title: "DingTalk"
description: "Set up Lucifex Agent as a DingTalk chatbot"
---

# DingTalk Setup

Lucifex Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Lucifex behaves once it's in your DingTalk workspace.
...
| **Group chats** | Lucifex responds when you `@mention` it. Without a mention, Lucifex ignores the message. |
...
Lucifex Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
Lucifex can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
Lucifex automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Check that `lucifex gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
