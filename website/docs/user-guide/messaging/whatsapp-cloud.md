...
description: "Set up Lucifex Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Lucifex can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Lucifex gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Lucifex limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Lucifex degrades gracefully if absent.
...
2. **Add** → name (e.g. `lucifex-bot`) → role: **Admin**.
...
## Exposing Lucifex to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Lucifex gateway has to be reachable from Meta's servers.  Three common ways:
...
## Allowlist (Lucifex-side)

In addition to Meta's recipient whitelist, Lucifex has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.lucifex/.env`:
...
The `lucifex whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.
...
When the agent invokes any of these flows, Lucifex uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Lucifex acknowledges inbound messages immediately:
...
Lucifex TTS produces MP3. Two paths:
...
Lucifex warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Lucifex. If you need it, please [open an issue](https://github.com/NousResearch/lucifex-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Lucifex's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Lucifex doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Gateway not running** — check `lucifex gateway` is up.
- **App Secret not set** — without it, Lucifex refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Lucifex.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `lucifex tools list` and verify the platform is using `lucifex-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
```bash
lucifex config set stt.provider openai
lucifex config set stt.use_gateway true
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Lucifex will accept as authentic.
...
Most users running Lucifex for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
