...
| **Threads** | Lucifex supports Matrix threads (MSC3440). If you reply in a thread, Lucifex keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
...
| **Commands** | Lucifex accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Lucifex normalizes known `!command` aliases to `/command`. |
...
Lucifex sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Lucifex now includes the current Matrix room name, room ID, topic, message ID,
...
2. Pick a username for your bot (e.g., `lucifex-bot`).

### Option C: Use Your Own Account

You can also run Lucifex as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Lucifex needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Lucifex the bot's user ID and password. Lucifex will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Lucifex Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Lucifex Agent
...
# MATRIX_USER_ID=@lucifex:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. Lucifex always
...
Lucifex supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with lucifex extras
...
In Matrix conversations, Lucifex exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Lucifex knows who requested the action.

### Media Limits

Lucifex uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Lucifex rejects arbitrary
...
Lucifex includes an opt-in Synapse harness for local validation:

```bash
docker compose -f tests/e2e/matrix_synapse_gateway/docker-compose.yml up -d
LUCIFEX_MATRIX_SYNAPSE_INTEGRATION=1 \
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Lucifex imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Lucifex bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
Lucifex detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lucifex:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lucifex:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lucifex:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@lucifex:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40lucifex%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart Lucifex:
   ```bash
...
Lucifex supports the same gateway commands in Matrix that it supports on other
...
Lucifex only normalizes `!command` when the command is known to the gateway, a
...
If you previously used Lucifex with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@lucifex:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Lucifex Agent"
     }'
   ```
...
a working E2EE setup with a previous version of Lucifex and are upgrading.
...
services:
  lucifex-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "LUCIFEX_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.lucifex/platforms/matrix/store
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Lucifex API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Lucifex-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Lucifex uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Lucifex's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Lucifex logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Lucifex gateway isn't running, or it failed to connect.

**Fix**: Check that `lucifex gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.
...
- **Media support**: Lucifex can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
