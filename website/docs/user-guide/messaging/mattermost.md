...
| **Public/private channels** | Lucifex responds when you `@mention` it. Without a mention, Lucifex ignores the message. |
...
**Fix**: Add your User ID to `MATTERMOST_ALLOWED_USERS` in `~/.lucifex/.env` and restart the gateway. Remember: the User ID is a 26-character alphanumeric string, not your `@username`.
