...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Lucifex Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Lucifex Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Lucifex opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Lucifex responds to every message. |
| Group chats | Lucifex responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Lucifex will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `lucifex gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Lucifex runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Lucifex behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Lucifex starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Lucifex responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Lucifex responds to @mentions from any user in any group. |
| `allowlist` | Lucifex only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Lucifex ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Lucifex read all group traffic without requiring an @mention:
...
Lucifex auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Lucifex ignores messages sent by other bots. Enable bot-to-bot messaging when you want Lucifex to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Lucifex. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `lucifex update --gateway` needs confirmation, the adapter records the selected answer in Lucifex's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Lucifex reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
You can invite the Lucifex Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Lucifex can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends Lucifex the meeting invitation event.
- Lucifex extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, Lucifex drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so Lucifex can reply to the inviter.
...
|---------|---------|---------|
| Quiet period | `LUCIFEX_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `LUCIFEX_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `LUCIFEX_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `LUCIFEX_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `LUCIFEX_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `LUCIFEX_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `LUCIFEX_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `LUCIFEX_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `LUCIFEX_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `LUCIFEX_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `Another local Lucifex gateway is already using this Feishu app_id` | Only one Lucifex instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Lucifex can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `lucifex-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
