...
Run multiple independent lucifexex agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate lucifexex home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing ulucifexifex state.
...
```bash
lucifexex profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own lucifexex profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `lucifexex setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
lucifexex profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `lucifexex profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
lucifexex profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every lucifexex subcommand — it's justlucifexifex -p <name>` under the hood.
...
```bash
lucifexex -p coder chat
lucifexex --profile=coder doctor
lucifexex chat -p coder -q "hello"    # works in any position
```

### Sticky default (`lucifexex profile use`)

```bash
lucifexex profile use coder
lucifexex chat                   # now targets coder
lucifexex tools                  # configures coder's tools
lucifexex profile use default    # switch back
```

Sets a default so plain `lucifexex` commands target that profile. Like `kubectl config use-context`.
...
- **`lucifexex profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives lucifexex its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory lucifexex was launched from", not "the profile directory".
...
```bash
coder gateway install         # creates lucifex-gateway-coder systemd/launchd service
assistant gateway install     # creates lucifex-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `lucifexex profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`.lucifexifex -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
default for **future CLI/gateway runs** (same as `lucifexex profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`lucifexex update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
lucifexex profile list           # show all profiles with status
lucifexex profile show coder     # detailed info for one profile
lucifexex profile rename coder dev-bot   # rename (updates alias + service)
lucifexex profile export coder   # export to coder.tar.gz
lucifexex profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
lucifexex profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `lucifexex profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.lucifexex`). To remove everything, uselucifexifex uninstall`.
:::
...
```bash
# Bash
eval "$(lucifexex completion bash)"

# Zsh
eval "$(lucifexex completion zsh)"
```
...
Profiles use the `LUCIFEX_HOME` environment variable. When you run `coder chat`, the wrapper script sets `LUCIFEX_HOME=~/.lucifex/profiles/coder` before launching lucifexex. Since 119+ files in the codebase resolve paths via `get_lucifex_home()`lucifexifex state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
isolated by `LUCIFEX_HOME`, not by changing `HOME`. Container backends still use
`{LUCIFEX_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `LUCIFEX_HOME` is the profile boundary. It controls lucifexex config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other lucifexex
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, lucifexex keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode lucifexex launches tool
subprocesses with `HOME={LUCIFEX_HOME}/home`; you then need to initialize or link
...
lucifexex also exposeslucifexifex_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.lucifexex` itself. No migration needed — existing installs work identically.
