...
Run multiple independent Lucifex agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Lucifex home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Lucifex state.
...
```bash
lucifex profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Lucifex profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `lucifex setup --portal` inside the new profile to wire up models + tools at once. See [Lucifex portal](/integrations/lucifex-portal).
:::
...
```bash
lucifex profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `lucifex profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
lucifex profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every lucifex subcommand — it's just `lucifex -p <name>` under the hood.
...
```bash
lucifex -p coder chat
lucifex --profile=coder doctor
lucifex chat -p coder -q "hello"    # works in any position
```

### Sticky default (`lucifex profile use`)

```bash
lucifex profile use coder
lucifex chat                   # now targets coder
lucifex tools                  # configures coder's tools
lucifex profile use default    # switch back
```

Sets a default so plain `lucifex` commands target that profile. Like `kubectl config use-context`.
...
- **`lucifex profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Lucifex its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Lucifex was launched from", not "the profile directory".
...
default for **future CLI/gateway runs** (same as `lucifex profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`lucifex update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
lucifex profile list           # show all profiles with status
lucifex profile show coder     # detailed info for one profile
lucifex profile rename coder dev-bot   # rename (updates alias + service)
lucifex profile export coder   # export to coder.tar.gz
lucifex profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
lucifex profile delete coder
```
...
:::note
You cannot delete the default profile (`~/.lucifex`). To remove everything, use `lucifex uninstall`.
:::
...
```bash
# Bash
eval "$(lucifex completion bash)"

# Zsh
eval "$(lucifex completion zsh)"
```
...
- `LUCIFEX_HOME` is the profile boundary. It controls Lucifex config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Lucifex
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Lucifex keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Lucifex launches tool
...
Lucifex also exposes `LUCIFEX_REAL_HOME` to subprocesses so scripts can still find
