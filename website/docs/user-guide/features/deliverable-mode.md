...
When lucifexex Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.lucifex/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.lucifex/config.yaml` (switchable per session
via `/personality`).
...
If you use lucifexex' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.lucifex/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. lucifexex Agent's deliverable
