...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `lucifex tools`.
...
Lucifex Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `lucifex tools` and persists in `config.yaml`.
...
If you have a paid [Lucifex portal](https://portal.nousresearch.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `lucifex setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the image-gen backend via `lucifex tools`.
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Lucifex materializes them to the local cache so delivery still works after expiry
