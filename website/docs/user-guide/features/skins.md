...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Lucifex CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Lucifex CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Lucifex Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Lucifex Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Lucifex Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Lucifex Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Lucifex ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default LUCIFEX_AGENT banner) | `""` |
...
[Lucifex Mod](https://github.com/cocktailpeanut/lucifex-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Lucifex Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/lucifex-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Lucifex skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
Lucifex Mod respects the `LUCIFEX_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.
