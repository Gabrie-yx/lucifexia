...
If the file contains `{transcript}` or `{{ transcript }}`, Lucifex replaces that placeholder with the live TTS text. Otherwise, Lucifex appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have Lucifex run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
Each provider has a documented per-request input-character cap. Lucifex truncates text before calling the provider so requests never fail with a length error:
...
On the first TTS call for a voice that isn't cached locally, Lucifex runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.lucifex/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](#example-doubao--volcengine-asr) for Lucifex integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. Lucifex substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file Lucifex wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if Lucifex picks a path.      |
| `voice_compatible` | `false` | When `true`, Lucifex converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. Lucifex quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input — treat it the same way you would a shell script on your PATH.
...
| A voice-listing API used by `lucifex setup` | **Plugin** (override `list_voices()`) |
...
Enable it (`lucifex plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Lucifex can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `LUCIFEX_LOCAL_STT_COMMAND`.
:::
...
**Custom local CLI fallback** — Set `LUCIFEX_LOCAL_STT_COMMAND` if you want Lucifex to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export LUCIFEX_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
Lucifex writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

If your configured provider isn't available, Lucifex automatically falls back:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `LUCIFEX_LOCAL_STT_COMMAND` before cloud providers
...
This complements the legacy `LUCIFEX_LOCAL_STT_COMMAND` escape hatch — that env var still works untouched via the built-in `local_command` path. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.

#### STT placeholders

Your command template can reference these placeholders. Lucifex substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty → Lucifex reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → Lucifex uses that.
...
For `format: json` / `srt` / `vtt`, Lucifex returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as Lucifex with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `LUCIFEX_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `LUCIFEX_LOCAL_STT_COMMAND` env var (preserved for back-compat)  |
...
Enable it (`lucifex plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
