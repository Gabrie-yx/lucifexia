...
```bash
uvx --from 'lucifex-agent[acp]==<version>' hermes-acp
```
...
- Configure Hermes provider credentials first with `hermes model`, or set them in `~/.lucifex/.env` / `~/.lucifex/config.yaml`.
- Install `uv` so the registry launcher can run `uvx --from 'lucifex-agent[acp]==<version>' hermes-acp`.
...
or by editing `~/.lucifex/.env`. Registry clients can also trigger Hermes' terminal auth flow, which runs the same interactive provider/model setup.
