...
lucifexex can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<LUCIFEX_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), lucifexex renders the sprite at full fidelity when your
...
lucifexex pets list
lucifexex pets list cat

# Install a pet and make it active in one step
lucifexex pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
lucifexex pets show

# Check your setup
lucifexex pets doctor
```

## `lucifexex pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `lucifexex pets list [query] [--limit N]` |
| List installed pets | `lucifexex pets list --installed` |
| Install a pet | `lucifexex pets install <slug> [--select] [--force]` |
| Set the active pet | `lucifexex pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `lucifexex pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `lucifexex pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `lucifexex pets off` |
| Remove an installed pet | `lucifexex pets remove <slug>` |
| Diagnose setup | `lucifexex pets doctor` |

`lucifexex pets show` flags:
...
Beyond installing pre-made pets from the gallery, lucifexex can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `lucifexex pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `lucifexex tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `lucifexex_PET_IMAGE_PROVIDER` env var (e.g.lucifexifex_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while lucifexex is
...
Run `lucifexex pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; lucifexex uses its own
  profile-scoped `<LUCIFEX_HOME>/pets/` instead — install through `lucifexex pets`.
