...
Lucifex can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Lucifex renders the sprite at full fidelity when your
...
lucifex pets list
lucifex pets list cat

# Install a pet and make it active in one step
lucifex pets install boba --select
...
## `lucifex pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `lucifex pets list [query] [--limit N]` |
| List installed pets | `lucifex pets list --installed` |
| Install a pet | `lucifex pets install <slug> [--select] [--force]` |
...
| Preview/animate | `lucifex pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `lucifex pets off` |
| Remove an installed pet | `lucifex pets remove <slug>` |
| Diagnose setup | `lucifex pets doctor` |

`lucifex pets show` flags:
...
Beyond installing pre-made pets from the gallery, Lucifex can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `lucifex pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Lucifex state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Lucifex portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nous run a quality-first model chain by default.

- Resolution order prefers Lucifex portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `lucifex tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `LUCIFEX_PET_IMAGE_PROVIDER` env var (e.g. `LUCIFEX_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Lucifex is
...
Run `lucifex pets doctor` — it reports:
