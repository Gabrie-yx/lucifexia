...
**Tool Search** is lucifexex' opt-in progressive-disclosure layer for that
...
:::info Built-in lucifexex tools never defer
The tools that make up lucifexex' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_lucifexex_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, lucifexex **unwraps the bridge** and
...
- **No JS sandbox.** lucifexex uses the simpler "structured tools" mode
