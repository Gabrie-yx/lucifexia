...
**Tool Search** is Lucifex' opt-in progressive-disclosure layer for that
...
:::info Built-in Lucifex tools never defer
The tools that make up Lucifex' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_LUCIFEX_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Lucifex **unwraps the bridge** and
...
- **No JS sandbox.** Lucifex uses the simpler "structured tools" mode
