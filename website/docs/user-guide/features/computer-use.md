...
lucifexex Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `lucifexex computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `lucifexex tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or lucifexex app).lucifexifex computer-use doctor` will tell you which permission is missing. |
...
```
lucifexex -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.lucifex/config.yaml`.

## `lucifexex computer-use doctor` — your first triage stop

`lucifexex computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ lucifexex computer-use doctor
...
agent is acting here." Each lucifexex run declares its own cua-driver
**session id** (something like `lucifexex-3a7b9c14d2e8`); the cursor's
...
lucifexex intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the lucifexex-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the lucifexex skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **lucifexex
...
lucifexex applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.lucifex/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. lucifexex applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  lucifexex agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the lucifexex agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. lucifexex warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive lucifexex from inside the RDP / console
...
```
lucifexex_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
lucifexex_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **lucifexex disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) lucifexex sets
...
When it's on, `lucifexex computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point lucifexex at a binary you built from source instead
of the published release. lucifexex resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`lucifexex_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for lucifexex testing (see notes).
...
### Option B — point lucifexex straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`lucifexex_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
lucifexex_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
lucifexex_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm lucifexex is using your build

- `lucifexex computer-use status` prints the resolved binary path and
  version.
- `lucifexex computer-use doctor` confirms the binary is reachable and
...
- **lucifexex spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no lucifexex
  change needed either way.
- **Local builds skip the version check.** lucifexex warns when the
...
**First action when anything's off: run `lucifexex computer-use doctor`.**
...
Run `lucifexex computer-use install` to fetch the cua-driver binary, or
run `lucifexex tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `lucifexex computer-use doctor` will
...
- **lucifexex-side skill** — `skills/computer-use/SKILL.md` — teaches the
  lucifexex `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects lucifexex (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (lucifexex among them)
