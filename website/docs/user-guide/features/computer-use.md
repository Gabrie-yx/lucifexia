...
lucifex Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `lucifex computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `lucifex tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or lucifex app).lucifexifex computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each lucifex run declares its own cua-driver
**session id** (something like `lucifex-3a7b9c14d2e8`); the cursor's
...
lucifex intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the lucifex-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the lucifex skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **lucifex
...
lucifex applies multi-layer guardrails:
...
Screenshots are expensive. lucifex applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  lucifex agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the lucifex agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. lucifex warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive lucifex from inside the RDP / console
...
```
lucifex_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
lucifex_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **lucifex disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) lucifex sets
...
When it's on, `lucifex computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point lucifex at a binary you built from source instead
of the published release. lucifex resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`lucifex_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for lucifex testing (see notes).
...
### Option B — point lucifex straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`lucifex_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
lucifex_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
lucifex_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm lucifex is using your build

- `lucifex computer-use status` prints the resolved binary path and
  version.
- `lucifex computer-use doctor` confirms the binary is reachable and
...
- **lucifex spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no lucifex
  change needed either way.
- **Local builds skip the version check.** lucifex warns when the
...
**First action when anything's off: run `lucifex computer-use doctor`.**
...
Run `lucifex computer-use install` to fetch the cua-driver binary, or
run `lucifex tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `lucifex computer-use doctor` will
...
- **lucifex-side skill** — `skills/computer-use/SKILL.md` — teaches the
  lucifex `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects lucifex (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (lucifex among them)
