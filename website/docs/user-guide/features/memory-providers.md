...
Lucifex Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
lucifex memory setup      # interactive picker + configuration
lucifex memory status     # check what's active
lucifex memory off        # disable external provider
```

You can also select the active memory provider via `lucifex plugins` → Provider Plugins → Memory Provider.
...
When a memory provider is active, Lucifex automatically:
...
The legacy `lucifex honcho setup` command still works (it now redirects to `lucifex memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
:::tip Migrating from `lucifex honcho`
If you previously used `lucifex honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lucifex profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.
...
| **AI peer** (`aiPeer`) | One per Lucifex profile. Host key `lucifex` → default; `lucifex.<profile>` for others. |
...
`--clone` creates a `lucifex.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Lucifex profile, creates host blocks for any profile without one, inherits settings from the default `lucifex` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `lucifex memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/lucifex-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lucifex).
...
lucifex memory setup    # select "openviking"
# Or manually:
lucifex config set memory.provider openviking
...
`OPENVIKING_AGENT` is Lucifex' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
lucifex memory setup    # select "mem0" → "Platform"
# Or manually:
lucifex config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.lucifex/.env
```

**Setup (OSS):**
```bash
lucifex memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
lucifex memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
lucifex memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
lucifex memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
lucifex memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```
...
| `user_id` | `lucifex-user` | User identifier |
| `agent_id` | `lucifex` | Agent identifier |
...
**Switching modes:** Re-run `lucifex memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
lucifex memory setup    # select "hindsight"
# Or manually:
lucifex config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.lucifex/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lucifex ui start`
...
| `bank_id` | `lucifex` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Lucifex Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
lucifex memory setup    # select "holographic"
# Or manually:
lucifex config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.lucifex-memory-store`
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=lucifex), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
Before running `lucifex memory setup`, set `base_url` in
...
Then run `lucifex memory setup` and enter the API key printed by the local
...
| `container_tag` | `lucifex` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `lucifex-{identity}` → `lucifex-coder`) to isolate memories per Lucifex profile
...
**Setup:**
```bash
pip install lucifex-memori
lucifex-memori install
lucifex config set memory.provider memori
lucifex memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `lucifex-memori` | Tool-aware memory + structured recall |
