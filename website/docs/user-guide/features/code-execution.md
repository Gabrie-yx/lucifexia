...
The `execute_code` tool lets the agent write Python scripts that call Lucifex tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Lucifex over a Unix domain socket RPC.
...
(for example `LUCIFEX_BASE_URL`, `LUCIFEX_KANBAN_DB`, or a `LUCIFEX_*_WEBHOOK`
