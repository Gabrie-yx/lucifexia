...
lucifexex runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. lucifexex
...
1. lucifexex captures a baseline of current diagnostics for the file.
...
…). lucifexex auto-detects the binary on PATH or in
`<LUCIFEX_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point lucifexex at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<LUCIFEX_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`lucifexex lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — lucifexex installs both packages together when you
run `lucifexex lsp install typescript` or auto-install fires on first
use.

## CLI

```
lucifexex lsp status          # service state + per-server install status
lucifexex lsp list            # registry, optionally --installed-only
lucifexex lsp install <id>    # eagerly install one server
lucifexex lsp install-all     # try every server with a known recipe
lucifexex lsp restart         # tear down running clients
lucifexex lsp which <id>      # print resolved binary path
```

`lucifexex lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <LUCIFEX_HOME>/lsp/bin
...
When `install_strategy: auto`, lucifexex installs binaries into
`<LUCIFEX_HOME>/lsp/bin/`. NPM packages land in
`<LUCIFEX_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully lucifexex-owned and is
...
Servers are kept alive for the life of the lucifexex process. There's
...
**`lucifexex lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LUCIFEX_HOME>/lsp/bin/`. Run
`lucifexex lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lucifexex lsp status`**
...
When `lucifexex lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `lucifexex lsp restart` to clear the set;
