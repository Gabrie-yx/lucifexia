...
Lucifex runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lucifex
...
1. Lucifex captures a baseline of current diagnostics for the file.
...
…). Lucifex auto-detects the binary on PATH or in
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Lucifex at the extracted bundle — the directory that contains
...
`lucifex lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Lucifex installs both packages together when you
run `lucifex lsp install typescript` or auto-install fires on first
use.

## CLI

```
lucifex lsp status          # service state + per-server install status
lucifex lsp list            # registry, optionally --installed-only
lucifex lsp install <id>    # eagerly install one server
lucifex lsp install-all     # try every server with a known recipe
lucifex lsp restart         # tear down running clients
lucifex lsp which <id>      # print resolved binary path
```

`lucifex lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Lucifex installs binaries into
...
shared location — the staging dir is fully Lucifex-owned and is
...
Servers are kept alive for the life of the Lucifex process. There's
...
**`lucifex lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LUCIFEX_HOME>/lsp/bin/`. Run
`lucifex lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lucifex lsp status`**
...
When `lucifex lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `lucifex lsp restart` to clear the set;
