...
Type `@` followed by a reference to inject content directly into your message. lucifexex expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- lucifexex env: `$LUCIFEX_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LUCIFEX_HOME/skills/.hub/`
