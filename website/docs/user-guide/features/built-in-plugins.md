...
description: "Plugins shipped with Lucifex Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Lucifex ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.lucifex/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Lucifex Plugin](/guides/build-a-lucifex-plugin) to write your own.
...
3. **Project** — `./.lucifex/plugins/<name>/` (requires `LUCIFEX_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `lucifex memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `lucifex plugins list` and the interactive `lucifex plugins` UI), but none load until you explicitly enable them:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Lucifex. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
lucifex plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `lucifex plugins enable <name>`.
...
| `lucifex-achievements` | dashboard tab | Steam-style collectible badges generated from your real Lucifex session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `lucifex memory` and `lucifex plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LUCIFEX_HOME` or `/tmp/lucifex-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Enabling:** `lucifex plugins enable disk-cleanup` (or check the box in `lucifex plugins`).
...
**Enabling:** `lucifex plugins enable security-guidance` (or check the box in `lucifex plugins`).
...
Traces Lucifex turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Lucifex' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `lucifex logs`.
...
```bash
lucifex tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Lucifex and the next turn ships a trace.
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Lucifex turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Lucifex session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `lucifex chat` session lives under one Langfuse session.

**Verify:**

```bash
lucifex plugins list                 # observability/langfuse should show "enabled"
lucifex chat -q "hello"              # check the Langfuse UI for a "Lucifex turn" trace
```
...
| `LUCIFEX_LANGFUSE_RELEASE` | — | Release/version tag |
| `LUCIFEX_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
...
**Disabling:** `lucifex plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
lucifex plugins enable google_meet
...
- Unlock state is persisted to `$LUCIFEX_HOME/plugins/lucifex-achievements/state.json`
...
| Secret | Hidden until Lucifex detects the first related signal in your history |

**API** — routes mount under `/api/plugins/lucifex-achievements/`:
...
**State files** — live under `$LUCIFEX_HOME/plugins/lucifex-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Lucifex updates. |
...
**Enabling:** Nothing to enable — `lucifex-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `lucifex dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/lucifex-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.lucifex/plugins/lucifex-achievements/` that ships no dashboard. The plugin's state files under `$LUCIFEX_HOME/plugins/lucifex-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Lucifex plugin — see [Build a Lucifex Plugin](/guides/build-a-lucifex-plugin). The only differences are:
