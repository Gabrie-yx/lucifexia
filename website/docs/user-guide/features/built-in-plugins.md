...
description: "Plugins shipped with lucifexex Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

lucifexex ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.lucifex/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a lucifexex Plugin](/developer-guide/plugins) to write your own.
...
3. **Project** — `./.lucifexex/plugins/<name>/` (requireslucifexifex_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `lucifexex_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `lucifexex memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `lucifexex plugins list` and the interactivelucifexifex plugins` UI), but none load until you explicitly enable them:

```bash
lucifexex plugins enable disk-cleanup
```

Or via `~/.lucifex/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer lucifexex. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
lucifexex plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `lucifexex plugins enable <name>`.
...
| `lucifexex-achievements` | dashboard tab | Steam-style collectible badges generated from your realucifexifex session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `lucifexex memory` andlucifexifex plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LUCIFEX_HOME` or `/tmp/lucifexex-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under LUCIFEX_HOME | always | Never |
...
**State** — everything lives at `$LUCIFEX_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `LUCIFEX_HOME` or `/tmp/lucifexex-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `lucifexex plugins enable disk-cleanup` (or check the box inlucifexifex plugins`).

**Disabling again:** `lucifexex plugins disable disk-cleanup`.
...
**Enabling:** `lucifexex plugins enable security-guidance` (or check the box inlucifexifex plugins`).

**Disabling again:** `lucifexex plugins disable security-guidance`.
...
```bash
lucifexex tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart lucifexex and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
lucifexex plugins enable observability/langfuse
```

Then put the credentials in `~/.lucifex/.env`:

```bash
lucifexex_LANGFUSE_PUBLIC_KEY=pk-lf-...
lucifexex_LANGFUSE_SECRET_KEY=sk-lf-...
lucifexex_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "lucifexex turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `lucifexex_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the lucifexex session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a singlelucifexifex chat` session lives under one Langfuse session.

**Verify:**

```bash
lucifexex plugins list                 # observability/langfuse should show "enabled"
lucifexex chat -q "hello"              # check the Langfuse UI for alucifexifex turn" trace
```
...
| `lucifexex_LANGFUSE_RELEASE` | — | Release/version tag |
| `lucifexex_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `lucifexex_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `lucifexex_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

lucifexex-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted lucifexifex-prefixed wins when both are set.
...
**Disabling:** `lucifexex plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.lucifex/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
lucifexex plugins enable google_meet
...
**Disabling:** `lucifexex plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.lucifex/cache/google_meet/` until you remove them.

### lucifexex-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real lucifexex session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep witlucifexifex feature changes.

**How it works:**

- Scans your entire `~/.lucifex/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$LUCIFEX_HOME/plugins/lucifexex-achievements/state.json`
...
| Secret | Hidden until lucifexex detects the first related signal in your history |

**API** — routes mount under `/api/plugins/lucifexex-achievements/`:
...
**State files** — live under `$LUCIFEX_HOME/plugins/lucifexex-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across lucifexex updates. |
