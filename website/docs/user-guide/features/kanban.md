...
description: "Durable SQLite-backed task board for coordinating multiple Lucifex profiles"
---
...
Lucifex Kanban is a durable task board, shared across all your Lucifex profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.lucifex/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `lucifex kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `lucifex kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/lucifex-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `LUCIFEX_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
- Workers spawned for a task see **only** their board's tasks — the
  dispatcher sets `LUCIFEX_KANBAN_BOARD` in the child env and every
...
lucifex kanban boards list

# Create a new board.
lucifex kanban boards create atm10-server \
    --name "ATM10 Server" \
...
lucifex kanban --board atm10-server list
lucifex kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
lucifex kanban boards switch atm10-server
lucifex kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
lucifex kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `LUCIFEX_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.lucifex/kanban/current` — the slug persisted by `lucifex kanban
   boards switch`.
4. `default`.
...
`lucifex dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<lucifex-home>/kanban/attachments/<task_id>/` for the default board, or
  `<lucifex-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `LUCIFEX_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
lucifex kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
lucifex kanban watch

# 5. See the board (you)
lucifex kanban list
lucifex kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `lucifex kanban show t_abcd`.
...
Override the config flag at runtime via `LUCIFEX_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `lucifex gateway
...
until one comes up — `lucifex kanban create` warns about this at creation
time.

Running `lucifex kanban daemon` as a separate process is **deprecated**;
...
lucifex kanban create "nightly ops review" \
    --assignee ops \
...
```bash
lucifex kanban complete t_abc t_def t_hij --result "batch wrap"
lucifex kanban archive  t_abc t_def t_hij
lucifex kanban unblock  t_abc t_def
lucifex kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `lucifex kanban`.** When the dispatcher spawns a worker it sets `LUCIFEX_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `lucifex kanban` CLI.
...
kanban_show()                                     # no args — uses LUCIFEX_KANBAN_TASK
...
**Zero schema footprint on normal sessions.** A regular `lucifex chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `LUCIFEX_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
2. `cd $LUCIFEX_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, Lucifex injects up to two
...
guard is active only for dispatcher-spawned workers (`LUCIFEX_KANBAN_TASK` is
set) and can be disabled with `LUCIFEX_KANBAN_STOP_NUDGE=0`.
...
```bash
lucifex kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

lucifex kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `lucifex skills list` to see what's available); there's no runtime install.
...
```bash
lucifex kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `lucifex kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `lucifex profile describe --auto`). |
...
The plugin uses the standard Lucifex dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
lucifex kanban init                                     # create kanban.db + print daemon hint
lucifex kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
lucifex kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
lucifex kanban show <id> [--json]
lucifex kanban assign <id> <profile>                    # or 'none' to unassign
lucifex kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
lucifex kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
lucifex kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
lucifex kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
lucifex kanban diagnostics [--json]                     # board health snapshot (alias: diag)
lucifex kanban link <parent_id> <child_id>
lucifex kanban unlink <parent_id> <child_id>
lucifex kanban claim <id> [--ttl SECONDS]
lucifex kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
lucifex kanban complete <id>... [--result "..."]
lucifex kanban block <id> "<reason>" [--ids <id>...]
lucifex kanban unblock <id>...
lucifex kanban archive <id>...

lucifex kanban tail <id>                                # follow a single task's event stream
lucifex kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
lucifex kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
lucifex kanban runs <id> [--json]                       # attempt history (one row per run)
lucifex kanban assignees [--json]                       # profiles on disk + per-assignee task counts
lucifex kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
lucifex kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `lucifex gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
lucifex kanban stats [--json]                           # per-status + per-assignee counts
lucifex kanban log <id> [--tail BYTES]                  # worker log from ~/.lucifex/kanban/logs/
lucifex kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
lucifex kanban notify-list [<id>] [--json]
lucifex kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
lucifex kanban context <id>                             # what a worker sees
lucifex kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
lucifex kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
lucifex kanban create "nightly backup audit" \
...
Every `lucifex kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `lucifex chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `lucifex_cli.kanban.run_slash()` entry point that reuses the `lucifex kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `lucifex kanban`. You don't have to leave the chat to drive the board.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`lucifex kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `lucifex kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/lucifex-kanban-v1-spec.pdf`.
...
```bash
lucifex kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$LUCIFEX_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
lucifex kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
lucifex kanban notify-list
lucifex kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
lucifex kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
lucifex kanban runs t_abcd
...
**Bulk close caveat.** `lucifex kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`lucifex kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `lucifex kanban heartbeat $TASK` to signal liveness during long operations. |
...
The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/lucifex-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
