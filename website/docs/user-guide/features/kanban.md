...
Hermes Kanban is a durable task board, shared across all your Hermes profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.lucifex/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
3. `~/.lucifex/kanban/current` — the slug persisted by `hermes kanban
   boards switch`.
4. `default`.
...
Tasks in `~/.lucifex/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `hermes -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
hermes kanban log <id> [--tail BYTES]                  # worker log from ~/.lucifex/kanban/logs/
