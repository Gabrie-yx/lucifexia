...
description: "Durable SQLite-backed task board for coordinating multiple lucifexex profiles"
---
...
lucifexex Kanban is a durable task board, shared across all youlucifexifex profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.lucifex/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.lucifex/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `lucifexex kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `lucifexex kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/lucifexex-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `lucifexex_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
lucifexex kanban boards list

# Create a new board.
lucifexex kanban boards create atm10-server \
    --name "ATM10 Server" \
...
lucifexex kanban --board atm10-server list
lucifexex kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
lucifexex kanban boards switch atm10-server
lucifexex kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
lucifexex kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `lucifexex_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.lucifex/kanban/current` — the slug persisted by `lucifexex kanban
   boards switch`.
4. `default`.
...
`lucifexex dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<lucifexex-home>/kanban/attachments/<task_id>/` for the default board, or
  `<lucifexex-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `lucifexex_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
lucifexex kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
lucifexex kanban watch

# 5. See the board (you)
lucifexex kanban list
lucifexex kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `lucifexex kanban show t_abcd`.
...
Override the config flag at runtime via `lucifexex_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `lucifexex gateway
...
until one comes up — `lucifexex kanban create` warns about this at creation
time.

Running `lucifexex kanban daemon` as a separate process is **deprecated**;
...
lucifexex kanban create "nightly ops review" \
    --assignee ops \
...
```bash
lucifexex kanban complete t_abc t_def t_hij --result "batch wrap"
lucifexex kanban archive  t_abc t_def t_hij
lucifexex kanban unblock  t_abc t_def
lucifexex kanban block    t_abc "need input" --ids t_def t_hij
```
...
kanban_show()                                     # no args — uses lucifexex_KANBAN_TASK
...
**Zero schema footprint on normal sessions.** A regular `lucifexex chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools becauselucifexifex_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
2. `cd $lucifexex_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, lucifexex injects up to two
...
guard is active only for dispatcher-spawned workers (`lucifexex_KANBAN_TASK` is
set) and can be disabled with `lucifexex_KANBAN_STOP_NUDGE=0`.
...
```bash
lucifexex kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

lucifexex kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `lucifexex skills list` to see what's available); there's no runtime install.
...
```bash
lucifexex kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `lucifexex kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `lucifexex kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `lucifexex profile create --description "..."`,lucifexifex profile describe <name> --text "..."lucifexucifex profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.lucifex/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `lucifexex profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.lucifex/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard lucifexex dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
lucifexex kanban init                                     # create kanban.db + print daemon hint
lucifexex kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
lucifexex kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
lucifexex kanban show <id> [--json]
lucifexex kanban assign <id> <profile>                    # or 'none' to unassign
lucifexex kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
lucifexex kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
lucifexex kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
lucifexex kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
lucifexex kanban diagnostics [--json]                     # board health snapshot (alias: diag)
lucifexex kanban link <parent_id> <child_id>
lucifexex kanban unlink <parent_id> <child_id>
lucifexex kanban claim <id> [--ttl SECONDS]
lucifexex kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
lucifexex kanban complete <id>... [--result "..."]
lucifexex kanban block <id> "<reason>" [--ids <id>...]
lucifexex kanban unblock <id>...
lucifexex kanban archive <id>...

lucifexex kanban tail <id>                                # follow a single task's event stream
lucifexex kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
lucifexex kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
lucifexex kanban runs <id> [--json]                       # attempt history (one row per run)
lucifexex kanban assignees [--json]                       # profiles on disk + per-assignee task counts
lucifexex kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
lucifexex kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `lucifex gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
lucifexex kanban stats [--json]                           # per-status + per-assignee counts
lucifexex kanban log <id> [--tail BYTES]                  # worker log from ~/.lucifex/kanban/logs/
lucifexex kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
lucifexex kanban notify-list [<id>] [--json]
lucifexex kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
lucifexex kanban context <id>                             # what a worker sees
lucifexex kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
lucifexex kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
lucifexex kanban create "nightly backup audit" \
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.lucifex/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`lucifexex kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `lucifexex kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/lucifexex-kanban-v1-spec.pdf`.
...
```bash
lucifexex kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$lucifexex_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
lucifexex kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
lucifexex kanban notify-list
lucifexex kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
lucifexex kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
lucifexex kanban runs t_abcd
...
**Bulk close caveat.** `lucifexex kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`lucifexex kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `lucifexex kanban heartbeat $TASK` to signal liveness during long operations. |
...
`lucifexex kanban tail <id>` shows these for a single task.lucifexifex kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.lucifex/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/lucifexex-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
