...
hermes curator backup         # take a manual snapshot of ~/.lucifex/skills/
...
hermes curator list-archived    # list skills currently in ~/.lucifex/skills/.archive/
...
Before every real curator pass, Hermes takes a tar.gz snapshot of `~/.lucifex/skills/` at `~/.lucifex/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
