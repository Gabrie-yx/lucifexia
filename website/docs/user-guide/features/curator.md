...
The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Lucifex checks whether:
...
**Easiest — `lucifex model`:**

```bash
lucifex model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`lucifex model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
lucifex curator status         # last run, counts, pinned list, LRU top 5
lucifex curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
lucifex curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
lucifex curator run --background  # fire-and-forget: start the run in a background thread
lucifex curator run --dry-run  # preview only — report without any mutations
lucifex curator backup         # take a manual snapshot of ~/.lucifex/skills/
lucifex curator rollback       # restore from the newest snapshot
lucifex curator rollback --list     # list available snapshots
lucifex curator rollback --id <ts>  # restore a specific snapshot
lucifex curator rollback -y         # skip the confirmation prompt
lucifex curator pause          # stop runs until resumed
lucifex curator resume
lucifex curator pin <skill>    # never auto-transition this skill
lucifex curator unpin <skill>
lucifex curator restore <skill>  # move an archived skill back to active
lucifex curator list-archived    # list skills currently in ~/.lucifex/skills/.archive/
lucifex curator archive <skill>  # manually archive a single skill now
lucifex curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Lucifex takes a tar.gz snapshot of `~/.lucifex/skills/` at `~/.lucifex/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
lucifex curator rollback        # restore newest snapshot (with confirmation)
lucifex curator rollback -y     # skip the prompt
lucifex curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Lucifex takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `lucifex curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `lucifex curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`lucifex curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
If you manually created a `SKILL.md` or pointed Lucifex at an external skill
...
**To see which skills the curator actually manages**, run `lucifex curator status`.
...
- Archives are recoverable via `lucifex curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `lucifex curator pin <name>`. See the next
section.
...
```bash
lucifex curator restore <skill-name>
```
...
- **For just one run:** `lucifex curator pause` — the pause persists across sessions; use `resume` to re-enable.
