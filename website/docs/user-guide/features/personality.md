...
description: "Customize lucifexex Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

lucifexex Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `LUCIFEX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who lucifexex is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

lucifexex now seeds a default `SOUL.md` automatically in:

```text
~/.lucifex/SOUL.md
```

More precisely, it uses the current instance's `LUCIFEX_HOME`, so if you run lucifexex with a custom home directory, it will use:

```text
$LUCIFEX_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- lucifexex creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- lucifexex loads `SOUL.md` only from `LUCIFEX_HOME`
- lucifexex does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, lucifexex falls back to a built-in default identity
...
If lucifexex loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `LUCIFEX_HOME`, the personality belongs to thlucifexifex instance itself.

That also makes it easier to teach users:
- "Edit `~/.lucifex/SOUL.md` to change lucifexex' default personality."
...
- how lucifexex should handle uncertainty, disagreement, or ambiguity
...
## What lucifexex injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, lucifexex falls back to a built-in default identity ("You arlucifexifex Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
lucifexex ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain lucifexex, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives lucifexex its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.lucifex/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.lucifex/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-lucifexex)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how lucifexex speaks
- `display.skin` and `/skin` affect how lucifexex looks in the terminal
