...
description: "Customize Lucifex Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Lucifex Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Lucifex is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Lucifex now seeds a default `SOUL.md` automatically in:

```text
~/.lucifex/SOUL.md
```

More precisely, it uses the current instance's `LUCIFEX_HOME`, so if you run Lucifex with a custom home directory, it will use:
...
- Lucifex creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Lucifex loads `SOUL.md` only from `LUCIFEX_HOME`
- Lucifex does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Lucifex falls back to a built-in default identity
...
If Lucifex loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `LUCIFEX_HOME`, the personality belongs to the Lucifex instance itself.
...
- how Lucifex should handle uncertainty, disagreement, or ambiguity
...
## What Lucifex injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Lucifex falls back to a built-in default identity ("You are Lucifex Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Lucifex ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Lucifex, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Lucifex its persistent default personality unless the overlay meaningfully changes it.
...
- [SOUL.md Guide](/guides/use-soul-with-lucifex)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Lucifex speaks
- `display.skin` and `/skin` affect how Lucifex looks in the terminal
