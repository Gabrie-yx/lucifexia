...
lucifexex Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — lucifexex can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`lucifexex fallback` reuses the provider picker fromlucifexifex model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — lucifexex still honors it for back-compat, butlucifexifex fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Nous Portal | `nous` | `lucifexex setup --portal` (fresh) orlucifexifex auth add nous` (OAuth) |
| OpenAI Codex | `openai-codex` | `lucifexex model` (ChatGPT OAuth) |
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `lucifexex model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `lucifexex model` (Qwen Portal OAuth; optional:lucifexifex_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `lucifexex model` (MiniMax portal OAuth) |
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `lucifexex fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
lucifexex uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `lucifexex kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), lucifexex first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style errorlucifexifex now honors user-configured fallback policy before using the built-in discovery chain:
...
Every task above follows the same **provider / model / base_url** pattern. Each task can also declare its own `fallback_chain`; if omitted, `provider: auto` uses the top-level `fallback_providers` chain before lucifexex' built-in auxiliary discovery chain.
...
| `"codex"` | Force Codex OAuth | `lucifexex model` → Codex |
...
4. **Warn + re-raise** — if every layer fails, lucifexex logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
lucifexex recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and lucifexex doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
If no provider is available for compression, lucifexex drops middle conversation turns without generating a summary rather than failing the session.
