...
Lucifex has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.lucifex/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.lucifex/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.lucifex/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".lucifex" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.lucifex/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.lucifex/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".lucifex" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.lucifex/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.lucifex/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/lucifex-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.lucifex/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Lucifex does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.lucifex/BOOT.md` with natural-language startup instructions.
...
Create `~/.lucifex/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `lucifex cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.lucifex/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.lucifex/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.lucifex/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.lucifex/hooks/boot-md/handler.py`**

```python
"""Run ~/.lucifex/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".lucifex" / "BOOT.md"
...
```bash
lucifex logs --follow --level INFO | grep boot-md
```
...
Delete `~/.lucifex/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Lucifex shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.lucifex/hooks/`
...
See the **[Build a Plugin guide](/guides/build-a-lucifex-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Lucifex will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`lucifex_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.lucifex/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.lucifex/agent-hooks/` (by convention) | `~/.lucifex/plugins/<name>/` | `~/.lucifex/hooks/<name>/` |
...
Each time the event fires, Lucifex spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Lucifex-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.lucifex/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.lucifex/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.lucifex/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.lucifex/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.lucifex/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.lucifex/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Lucifex event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.lucifex/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.lucifex/agent-hooks/log-orchestration.sh
log=~/.lucifex/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Lucifex sees it, then persists the decision to `~/.lucifex/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `lucifex --accept-hooks chat`)
2. `LUCIFEX_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `lucifex hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.lucifex/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/lucifex/.lucifex/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `lucifex hooks list`.

### The `lucifex hooks` CLI

| Command | What it does |
|---------|--------------|
| `lucifex hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `lucifex hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `lucifex hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `lucifex hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.lucifex/agent-hooks/` so the path is easy to audit.
- Re-run `lucifex hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
