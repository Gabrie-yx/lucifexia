...
chat completions — use your lucifexex-managed provider subscription as their
...
| Use case | "Use lucifexex as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. lucifexex stores
the refresh token in `~/.lucifex/auth.json` — the same place all lucifexex
...
Model:      lucifexex-4-70B    # olucifexifex-4.3-3lucifexucifex-4-405B
```
...
If you see `not logged in`, run `lucifexex portal`. If you see
...
`lucifex_cli/proxy/adapters/<provider>.py` and registering it in
