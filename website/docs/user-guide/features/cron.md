...
Schedule tasks to run automatically with natural language or cron expressions. lucifex exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
```bash
lucifex cron create "every 2h" "Check server status"
lucifex cron create "every 1h" "Summarize new feed items" --skill blogwatcher
lucifex cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask lucifex normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

lucifex will use the unified `cronjob` tool internally.
...
lucifex cron create "every 1d at 09:00" \
...
```bash
lucifex cron edit <job_id> --schedule "every 4h"
lucifex cron edit <job_id> --prompt "Use the revised task"
lucifex cron edit <job_id> --skill blogwatcher --skill maps
lucifex cron edit <job_id> --add-skill maps
lucifex cron edit <job_id> --remove-skill blogwatcher
lucifex cron edit <job_id> --clear-skills
```
...
```bash
lucifex cron list
lucifex cron pause <job_id_or_name>
lucifex cron resume <job_id_or_name>
lucifex cron run <job_id_or_name>
lucifex cron remove <job_id_or_name>
lucifex cron edit <job_id_or_name> [...flags]
lucifex cron status
lucifex cron tick
```
...
```bash
lucifex gateway install     # Install as a user service
sudo lucifex gateway install --system   # Linux: boot-time system service for servers
lucifex gateway             # Or run in foreground
...
lucifex records each claimed cron attempt in the profile-local
...
`completed`, `failed`, or `unknown`. After restart, lucifex marks an abandoned
...
Inspect recent attempts with `lucifex cron runs [job-id] --limit 20` (alias:
...
Or set the `lucifex_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
The `cronjob` tool's schema exposes `no_agent` to lucifex directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

lucifex will write the check script to `~/.lucifex/scripts/` via `write_file`, then call:
...
- When Job 2 fires, lucifex reads Job 1's most recent output from `~/.lucifex/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `lucifex tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `lucifex tools` cron-platform config wins; otherwislucifexifex falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether lucifex should even invoke the agent. Emit a final stdout line of the form:
...
STATE="$HOME/.lucifex/scripts/.feed-changed.last"
...
:::tip
lucifex's own `~/.lucifex/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
