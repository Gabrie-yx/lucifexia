...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, lucifexex automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, lucifexex auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
lucifexex auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
lucifexex auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
lucifexex auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  lucifexex_pkce          oauth lucifexifex_pkce ←
...
Run `lucifexex auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `lucifexex auth` | Interactive pool management wizard |
| `lucifexex auth list` | Show all pools and credentials |
| `lucifexex auth list <provider>` | Show a specific provider's pool |
| `lucifexex auth add <provider>` | Add a credential (prompts for type and key) |
| `lucifexex auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `lucifexex auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `lucifexex auth remove <provider> <index>` | Remove credential by 1-based index |
| `lucifexex auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `lucifexex auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `lucifexex model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via lucifexex model:
lucifexex auth list
# Shows:
...
lucifexex auth add Together.ai --api-key sk-together-second-key
```
...
lucifexex automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| lucifexex PKCE OAuth | `~/.lucifex/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `lucifexex auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `lucifexex auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. lucifexex can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries anlucifexifex-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to lucifexex' credential store, so its token remains persistable.
