...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Lucifex' own `lucifex auth login codex` writes to `~/.lucifex/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- **Lucifex auth and codex auth are separate sessions.** You need both `codex login` AND `lucifex auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Lucifex' `_import_codex_cli_tokens` — Lucifex won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
