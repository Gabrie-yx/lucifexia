...
This is **opt-in only**. Default Lucifex behavior is unchanged unless you flip the flag. Lucifex never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `lucifex setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Lucifex portal](/integrations/lucifex-portal).
:::
...
- **Lucifex' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Lucifex for tools it doesn't have built in.
...
These ship with `codex app-server` itself — no Lucifex involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Lucifex' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Lucifex also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Lucifex queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Lucifex tool callback (MCP server, registered in `~/.codex/config.toml`)

Lucifex registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Lucifex' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Lucifex' skill library.
- **`text_to_speech`** — TTS through Lucifex' configured provider.

When the model wants one of these, codex spawns the `lucifex_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Lucifex' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Lucifex tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Lucifex' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Lucifex' todo store (codex's `update_plan` is the in-runtime equivalent)
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which Lucifex does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `lucifex chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `LUCIFEX_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `LUCIFEX_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
|  | Lucifex default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Lucifex' own `lucifex auth add openai-codex` writes to `~/.lucifex/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Lucifex auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ...
   ```
   Lucifex will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.

## Enabling

In a Lucifex session:
...
- **Registers Lucifex' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Lucifex runtime)
```

## Self-improvement loop (memory + skill nudges)

Lucifex' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Lucifex runtime.
...
One detail: the review fork itself needs to call Lucifex' agent-loop tools (`memory`, `skill_manage`), which require Lucifex' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Lucifex owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Lucifex' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Lucifex shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Lucifex' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Lucifex' managed block:

```toml
default_permissions = ":read-only"
```

(Lucifex will preserve your override on re-migration as long as it lives outside the `# managed by lucifex-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Lucifex' auxiliary client uses the main provider/model when no per-task override is set.
...
Lucifex wraps everything it manages between two marker comments:

```toml
# managed by lucifex-agent — `lucifex codex-runtime migrate` regenerates this section
...
- Add your own MCP servers Lucifex doesn't know about
...
By default, Lucifex points the codex subprocess at `~/.codex/` regardless of which Lucifex profile is active. This means `lucifex -p work` and `lucifex -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.
...
```bash
# Inside the work profile, you might wrap lucifex:
CODEX_HOME=~/.lucifex/profiles/work/codex lucifex chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `lucifex -p work` will operate on isolated Codex state.
...
Lucifex does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Lucifex' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Lucifex (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Lucifex-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Lucifex writes a `[plugins."<name>@openai-curated"]` block enabling it in your Lucifex session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Lucifex' codex runtime, Lucifex activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Lucifex doesn't touch credentials.

## Lucifex tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Lucifex registers itself as an MCP server in `~/.codex/config.toml`:
...
When the model calls `web_search` (or another exposed Lucifex tool), codex spawns the `lucifex_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Lucifex runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Lucifex Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Lucifex UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `lucifex-tools` callback)
...
- Lucifex web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Lucifex auth and codex auth are separate sessions.** You need both `codex login` AND `lucifex auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Lucifex' `_import_codex_cli_tokens` — Lucifex won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/lucifex-agent/issues) with the output of `lucifex logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Lucifex shell (CLI / TUI / gateway) ───┐
