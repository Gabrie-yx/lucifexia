...
description: "Build themes and plugins for the Lucifex web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---
...
The CSS is injected as a single scoped `<style data-lucifex-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Lucifex Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Lucifex Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Lucifex Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Lucifex plugins in `~/.lucifex/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__LUCIFEX_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
(function () {
  ...
  const SDK = window.__LUCIFEX_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__LUCIFEX_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__LUCIFEX_PLUGIN_SDK__`. Plugins should never import React directly.
...
// Lucifex API client + raw fetcher
...
#### Calling built-in Lucifex endpoints
...
```javascript
window.__LUCIFEX_PLUGINS__.registerSlot("my-plugin", "sidebar", MySidebar);
window.__LUCIFEX_PLUGINS__.registerSlot("my-plugin", "header-left", MyCrest);
```
...
| `header-left` | Before the Lucifex brand in the top bar. |
...
window.__LUCIFEX_PLUGINS__.registerSlot("my-plugin", "sessions:top", PinnedSessionsBanner);
```
...
(function () {
  ...
  window.__LUCIFEX_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__LUCIFEX_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
A reference plugin (`example-dashboard` in [`lucifex-example-plugins`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
| 3 | `./.lucifex/plugins/<name>/dashboard/` | `project` — only when `LUCIFEX_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `lucifex dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__LUCIFEX_PLUGIN_SDK__` and the registry on `window.__LUCIFEX_PLUGINS__`.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `lucifex-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/lucifex-example-plugins.git

# Theme
cp lucifex-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.lucifex/dashboard-themes/

# Plugin
cp -r lucifex-example-plugins/strike-freedom-cockpit ~/.lucifex/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Lucifex Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__LUCIFEX_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__LUCIFEX_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__LUCIFEX_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__LUCIFEX_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__LUCIFEX_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
2. Restart `lucifex dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
