...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Lucifex Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---
...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Lucifex](/guides/use-voice-mode-with-lucifex).

## Prerequisites

Before using voice features, make sure you have:

1. **Lucifex Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `lucifex model` or set your preferred provider credentials in `~/.lucifex/.env`
3. **A working base setup** — run `lucifex` to verify the agent responds to text before enabling voice

:::tip
The `~/.lucifex/` directory and default `config.yaml` are created automatically the first time you run `lucifex`. You only need to create `~/.lucifex/.env` manually for API keys.
:::

:::tip Lucifex portal covers both
A paid [Lucifex portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `lucifex setup --portal` wires both up at once.
:::
...
```bash
lucifex                # Start the interactive CLI
```
...
1. Start the CLI with `lucifex` and enable voice mode with `/voice on`
...
```bash
lucifex gateway        # Start the gateway (connects to configured platforms)
lucifex gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@lucifexbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.
...
```bash
lucifex gateway        # Start with existing configuration
```
...
If you are running Lucifex inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
