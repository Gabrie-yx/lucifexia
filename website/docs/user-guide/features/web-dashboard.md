...
```bash
# Custom port
lucifex dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
lucifex dashboard --host 0.0.0.0

# Start without opening browser
lucifex dashboard --no-open
```
...
processes (manage them via `lucifex -p <name> gateway …`), each profile's
...
When you run `lucifex dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `lucifex dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `lucifex --tui`; the TUI bundle is built on first launch)
...
LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
Create and manage [profiles](../profiles.md) — isolated Lucifex instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `lucifex profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `lucifex skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `lucifex mcp` reads from.
...
Connect Lucifex to any messaging platform from the browser — full parity with
`lucifex setup gateway`. The page lists every supported channel (Telegram,
...
|-------|-----------|----------|
| `lucifex dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `lucifex dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
When you run `lucifex dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Lucifex doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `lucifex dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
- **CLI — `lucifex dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `lucifex setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.lucifex/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://lucifex.example.com/auth/callback` and set `LUCIFEX_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `LUCIFEX_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `LUCIFEX_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
cat >> ~/.lucifex/.env <<EOF
LUCIFEX_DASHBOARD_BASIC_AUTH_USERNAME=admin
LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/lucifex/   # required
      client_id: lucifex-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `LUCIFEX_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `LUCIFEX_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `LUCIFEX_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-lucifex.json` — it defines a `lucifex` realm, a **public PKCE client** (`lucifex-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "lucifex-dashboard",
      "name": "Lucifex Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-lucifex.json:/opt/keycloak/data/import/realm-lucifex.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/lucifex/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/lucifex`). The admin console is at
...
```bash
export LUCIFEX_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/lucifex"
export LUCIFEX_DASHBOARD_OIDC_CLIENT_ID="lucifex-dashboard"
export LUCIFEX_DASHBOARD_PUBLIC_URL="http://localhost:9119"
lucifex dashboard --host 0.0.0.0 --port 9119 --no-open
```

`LUCIFEX_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → lucifex-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `LUCIFEX_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/lucifex"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `LUCIFEX_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `LUCIFEX_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `lucifex_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `lucifex_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `lucifex_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `lucifex_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
```bash
# Quick env-var path.
LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  lucifex dashboard --host 0.0.0.0
...
lucifex dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/lucifex` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `LUCIFEX_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `LUCIFEX_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
...
When you run `lucifex update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `lucifex dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Lucifex Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Lucifex Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
