...
The web dashboard is a browser-based UI for managing your lucifexex Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `lucifexex setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
lucifexex dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
lucifexex dashboard --host 0.0.0.0

# Start without opening browser
lucifexex dashboard --no-open
```
...
processes (manage them via `lucifexex -p <name> gateway …`), each profile's
...
```bash
cd ~/.lucifex/lucifex-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.lucifex/lucifex-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `lucifexex dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `lucifexex dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `lucifexex --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.lucifex/lucifex-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `lucifex config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated lucifexex instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `lucifexex profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.lucifex/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `lucifexex skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `lucifexex mcp` reads from.
...
`lucifexex mcp catalog` /lucifexifex mcp install` use.
...
Connect lucifexex to any messaging platform from the browser — full parity with
`lucifexex setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.lucifex/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
This re-reads `~/.lucifex/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/lucifexex/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — lucifexex Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `lucifexex dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `lucifexex dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `lucifexex dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `lucifexex dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yetlucifexifex doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you lucifexucifex dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
|---------|-----------|--------|----------------|
| `lucifexex_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` |lucifexifex dashboard register` |

Per the lucifexex Agent convention (`~/.lucifex/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
From a logged-in lucifexex install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `lucifexex dashboard register` uses your existing Nous login to provision an OAuth client and writeslucifexifex_DASHBOARD_OAUTH_CLIENT_ID` into `~/.lucifex/.env` for you:

```bash
lucifexex setup            # if you're not already logged into Nous Portal
lucifexex dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes lucifexex_DASHBOARD_OAUTH_CLIENT_ID to ~/.lucifex/.env
```
...
```bash
lucifexex dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://lucifexex.example.com/auth/callback` and setlucifexifex_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `lucifexex_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `lucifexex_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `lucifexex_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `lucifexex_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `lucifexex_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.lucifex/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.lucifex/.env <<EOF
lucifexex_DASHBOARD_BASIC_AUTH_USERNAME=admin
lucifexex_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/lucifexex/   # required
      client_id: lucifexex-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `lucifexex_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `lucifexex_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `lucifexex_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-lucifexex.json` — it defines alucifexifex` realm, a **public PKCE client*lucifexucifex-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "lucifexex-dashboard",
      "name": "lucifexex Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-lucifexex.json:/opt/keycloak/data/import/realucifexfex.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/lucifexex/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/lucifexex`). The admin console is at
...
```bash
export lucifexex_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realmlucifexifex"
export lucifexex_DASHBOARD_OIDC_CLIENT_IDlucifexifex-dashboard"
export lucifexex_DASHBOARD_PUBLIC_URL="http://localhost:9119"
lucifexex dashboard --host 0.0.0.0 --port 9119 --no-open
```

`lucifexex_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → lucifexex-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `lucifexex_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/lucifexex"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `lucifexex_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `lucifexex_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `lucifexex_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `lucifexex_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `lucifexex_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `lucifexex_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
```python
# ~/.lucifex/plugins/dashboard-auth-myidp/__init__.py
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `lucifexex_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
lucifexex_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  lucifexex dashboard --host 0.0.0.0
...
lucifexex Desktop can drive lucifexifex backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see tlucifexucifex Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`lucifexex dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.lucifex/.env (secrets file, 0600).
cat >> ~/.lucifex/.env <<'EOF'
lucifexex_DASHBOARD_BASIC_AUTH_USERNAME=admin
lucifexex_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
lucifexex dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `lucifexex_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.lucifex/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.lucifexex/.env`, so the credentials are in the environment at boot.
...
### In lucifexex Desktop

**Settings → Gateway → Remote gateway:**

- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/lucifexex` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `lucifexex_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `lucifexex_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `lucifexex_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `lucifexex_DASHBOARD_BASIC_AUTH_USERNAME` /lucifexifex_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `lucifexex_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
When you run `lucifexex update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build andlucifexifex dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **lucifexex Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **lucifexex Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
