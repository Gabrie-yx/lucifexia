...
The approval system supports three modes, configured via `approvals.mode` in `~/.lucifex/config.yaml`:
...
Configure the timeout in `~/.lucifex/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.lucifex/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.lucifex/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.lucifex/config.yaml`:
...
:::tip
Use `lucifex config edit` to review or remove patterns from your permanent allowlist.
:::
...
| Hermes credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under LUCIFEX_HOME (active profile and global root) |
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/lucifex-agent) (`HERMES_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.lucifex/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.lucifex/cron/jobs.json`, profile skills, or other Hermes state outside that prefix
...
Do not ask the agent to `patch` `~/.lucifex/cron/jobs.json` directly. Use the `cronjob` tool, [`hermes cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Hermes control files when write safety blocks direct edits.
...
Set allowed user IDs as comma-separated values in `~/.lucifex/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.lucifex/.env to allow open access,
...
```bash
docker exec -u hermes lucifex-agent hermes pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/NousResearch/lucifex-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.lucifex/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.lucifex/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.lucifex/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `LUCIFEX_HOME`. Skills declare these in frontmatter:
...
When loaded, Hermes checks if these files exist in the active profile's `LUCIFEX_HOME` and registers them for mounting:
...
4. **Store secrets securely** — keep API keys in `~/.lucifex/.env` with proper file permissions
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.lucifex/.env`:
...
Hermes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `lucifex_cli/security_advisories.py`.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hermes installs these **lazily** on first use rather than eagerly under `lucifex-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
