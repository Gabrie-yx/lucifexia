...
1. **CLI flag**: Start a session with `lucifex --yolo` or `lucifex chat --yolo`
...
Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [LUCIFEX_WRITE_SAFE_ROOT](../reference/environment-variables.md#lucifex_write_safe_root).

### Cron and other Lucifex state

Do not ask the agent to `patch` `~/.lucifex/cron/jobs.json` directly. Use the `cronjob` tool, [`lucifex cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Lucifex control files when write safety blocks direct edits.
...
```bash
docker exec -u lucifex lucifex-agent lucifex pairing approve telegram ABC12DEF
```
