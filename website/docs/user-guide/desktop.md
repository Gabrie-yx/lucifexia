...
sidebar_position: 3
title: "Desktop App"
description: "The native Lucifex desktop app — a polished experience for chatting with Lucifex, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Lucifex desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Lucifex Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `lucifex` in a terminal, everything you set up there is already here, and anything you do here shows up there.
...
- **CLI** (`lucifex`) and **[TUI](./tui.md)** (`lucifex --tui`) — terminal interfaces.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `lucifex desktop --cwd <path>` (or the `LUCIFEX_DESKTOP_CWD` environment variable).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Lucifex agent, your config, and your chats stay. (Same as `lucifex uninstall --gui`.)
...
You can do the same from the terminal — `lucifex uninstall --gui` for the GUI alone, or `lucifex uninstall` / `lucifex uninstall --full` for the agent too.

:::note
Running `lucifex uninstall --gui` from a **source checkout** (a `lucifex desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `lucifex desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::
...
| `--lucifex-root PATH` | Override the Lucifex source root the app uses (sets `LUCIFEX_DESKTOP_LUCIFEX_ROOT`)          |
...
The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Lucifex Agent runtime into `LUCIFEX_HOME` (`~/.lucifex`, or `%LOCALAPPDATA%\lucifex` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `LUCIFEX_DESKTOP_LUCIFEX_ROOT`, then a completed managed install, then a probed `lucifex` on `PATH` (unless `--ignore-existing` / `LUCIFEX_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `LUCIFEX_DESKTOP_LUCIFEX` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `lucifex serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `lucifex --tui`. The desktop app is **self-contained**: it runs its own `lucifex serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Lucifex Desktop to a remote backend](./features/web-dashboard.md#connecting-lucifex-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Lucifex Desktop](../reference/environment-variables.md#web-dashboard--lucifex-desktop).
...
rm "$HOME/.lucifex/lucifex-agent/.lucifex-bootstrap-complete"
...
```bash
LUCIFEX_DESKTOP_LUCIFEX_ROOT=/path/to/clone npm run dev
