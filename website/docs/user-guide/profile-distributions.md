...
A **profile distribution** packages a complete lucifexex agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a lucifexex agent meant sending someone:
...
```bash
lucifexex profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `lucifexex profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running lucifexex in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`lucifexex profile export` / `import`](../reference/profile-commands.mlucifexifex-profile-export) — that's what those are for.
...
:::caution
**lucifexex does not control git.** The file exclusions described on this page are applied by the **installer** when someone runslucifexifex profile install` lucifexucifex profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
lucifexex profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.lucifex/profiles/research-bot/SOUL.md
...
Create `~/.lucifex/profiles/research-bot/distribution.yaml`:
...
Create `~/.lucifex/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.lucifex/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `lucifexex profile update research-bot` will pull the latest.
...
```bash
lucifexex profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.lucifex/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
lucifexex profile install github.com/you/research-bot

# Full HTTPS
lucifexex profile install https://github.com/you/research-bot.git

# SSH
lucifexex profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
lucifexex profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
lucifexex profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
lucifexex profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
lucifexex profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
lucifexex profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this lucifexex distribution.
...
```bash
cp ~/.lucifex/profiles/research-bot/.env.EXAMPLE ~/.lucifex/profiles/research-bot/.env
...
```bash
lucifexex profile info research-bot
```
...
`lucifexex profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
lucifexex profile update research-bot
```
...
```bash
lucifexex profile delete research-bot
```
...
Path:    ~/.lucifex/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `lucifexex profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.lucifex/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
lucifexex profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `lucifexex profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.lucifex/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/lucifexex-polymarket-trader.git
git push -u origin main --tags

# Anyone
lucifexex profile install github.com/yolucifexifex-polymarket-trader --alias
```
...
You built lucifexex-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
lucifexex profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
lucifexex profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `lucifexex profile info <name>` and hold off on updates until you're ready.
:::
...
lucifexex profile info research-bot | grep Version
...
```yaml
# ~/.lucifex/profiles/research-bot/local/my-overrides.yaml
...
lucifexex profile delete research-bot --yes
lucifexex profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
lucifexex profile update research-bot --force-config --yes
```
...
lucifexex profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.lucifex/profiles/forked-research-bot/
...
lucifexex profile install ~/.lucifex/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
lucifexex profile delete research-bot-test --yes
lucifexex profile install ~/.lucifex/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `lucifexex -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `lucifexex profile` — not a parallel command tree.
...
- Reserved profile names (`lucifexex`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`lucifexex profile export` / `import`](../reference/profile-commands.mlucifexifex-profile-export) — local backup / restore (not distribution)
- [Using SOUL with lucifexex](../guides/use-soul-witlucifexifex.md) — authoring personalities
