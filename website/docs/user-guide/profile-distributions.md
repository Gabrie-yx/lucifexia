...
A **profile distribution** packages a complete Lucifex agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Lucifex agent meant sending someone:
...
```bash
lucifex profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `lucifex profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Lucifex in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`lucifex profile export` / `import`](../reference/profile-commands.md#lucifex-profile-export) — that's what those are for.
...
:::caution
**Lucifex does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `lucifex profile install` or `lucifex profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
lucifex profile create research-bot
...
Recipients who run `lucifex profile update research-bot` will pull the latest.
...
```bash
lucifex profile install github.com/you/research-bot --alias
```
...
```bash
# GitHub shorthand
lucifex profile install github.com/you/research-bot

# Full HTTPS
lucifex profile install https://github.com/you/research-bot.git

# SSH
lucifex profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
lucifex profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
lucifex profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
lucifex profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
lucifex profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
lucifex profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Lucifex distribution.
...
```bash
lucifex profile info research-bot
```
...
`lucifex profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
lucifex profile update research-bot
```
...
```bash
lucifex profile delete research-bot
```
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `lucifex profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
lucifex profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `lucifex profile update pr-reviewer` and everyone's on the new version within minutes.
...
git remote add origin https://github.com/you/lucifex-polymarket-trader.git
git push -u origin main --tags

# Anyone
lucifex profile install github.com/you/lucifex-polymarket-trader --alias
```
...
You built Lucifex-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
lucifex profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
lucifex profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `lucifex profile info <name>` and hold off on updates until you're ready.
:::
...
lucifex profile info research-bot | grep Version
...
lucifex profile delete research-bot --yes
lucifex profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
lucifex profile update research-bot --force-config --yes
```
...
lucifex profile install github.com/yourname/forked-research-bot --alias
...
lucifex profile install ~/.lucifex/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
lucifex profile delete research-bot-test --yes
lucifex profile install ~/.lucifex/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `lucifex -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `lucifex profile` — not a parallel command tree.
...
- Reserved profile names (`lucifex`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`lucifex profile export` / `import`](../reference/profile-commands.md#lucifex-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Lucifex](../guides/use-soul-with-lucifex.md) — authoring personalities
