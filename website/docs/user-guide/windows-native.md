...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\lucifex-agent` | Override code location |
...
On first launch (and on demand when a missing tool is detected), Hermes runs a small Python bootstrapper — `lucifex_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:
...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\lucifex-agent` and creates a virtualenv inside it.
...
9. **Adds `%LOCALAPPDATA%\hermes\lucifex-agent\venv\Scripts` to User PATH and sets `LUCIFEX_HOME=%LOCALAPPDATA%\hermes`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal.
...
| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\hermes\lucifex-agent\` | Git checkout + venv. `venv\Scripts\lucifex.exe` is the command added to User PATH. Safe to `Remove-Item -Recurse` and reinstall. |
...
On native Windows the installer sets `LUCIFEX_HOME=%LOCALAPPDATA%\hermes`, so your data and the disposable install live under the **same** `%LOCALAPPDATA%\hermes` root: the install/runtime is the `lucifex-agent\`, `git\`, `node\`, and `bin\` subdirectories, while your data files sit directly in `%LOCALAPPDATA%\hermes`. Reinstalling only replaces the `lucifex-agent\` checkout, so your data survives — but because the two share a root, **don't** `Remove-Item -Recurse %LOCALAPPDATA%\hermes` if you want to keep your data; delete the `lucifex-agent\` subdirectory instead. Your data directory is identical in shape to a Linux `~/.hermes`, so you can mirror it between machines.

**Override `LUCIFEX_HOME`:** set the environment variable to point at a different data dir (e.g. `%USERPROFILE%\.hermes` to match a Linux/WSL layout). Works the same as on Linux.
...
The installer adds `%LOCALAPPDATA%\hermes\lucifex-agent\venv\Scripts` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command hermes        # should print C:\Users\<you>\AppData\Local\hermes\lucifex-agent\venv\Scripts\lucifex.exe
hermes --version
```

### Environment variables

Hermes honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\hermes\.env` (your `LUCIFEX_HOME`) is the normal path — same as Linux:
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\hermes\lucifex-agent\`, and trims the User PATH. It leaves the rest of `%LOCALAPPDATA%\hermes\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.
