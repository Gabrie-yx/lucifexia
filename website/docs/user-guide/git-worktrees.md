...
- CLI: the directory where you run `lucifex` or `lucifex chat`
...
- Works on its own branch (`feature/lucifex-a` vs `feature/lucifex-b`).
...
- **Name branches after the experiment**
  - e.g. `feature/lucifex-checkpoints-docs`, `feature/lucifex-refactor-tests`.
- **Commit frequently**
...
- Check out an isolated branch (e.g. `lucifex/lucifex-<hash>`).
