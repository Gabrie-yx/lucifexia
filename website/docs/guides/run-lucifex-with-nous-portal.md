...
- Lucifex Agent installed ([Quickstart](/getting-started/quickstart))
...
OAuth needs a browser, but the loopback callback runs on the machine where Lucifex is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
lucifex setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
lucifex auth add nous --type oauth
# Then re-run `lucifex setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Lucifex Agent release notes" and summarize the top 3 hits.
```

You should see Lucifex call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`lucifex setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
lucifex config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Lucifex-4 for agent work

Lucifex-4-70B and Lucifex-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Lucifex Agent itself, stick to the frontier agentic models above.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Lucifex will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
lucifex cron create "every day at 9am" \
...
If you use [Lucifex profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.
...
### `lucifex portal info` shows "not logged in" after `lucifex setup --portal`
...
```bash
lucifex config set model.provider nous
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Lucifex doesn't replay it endlessly. Just log in again:

```bash
lucifex auth add nous
```
...
`lucifex portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Lucifex profiles where you're using the wrong one (check `lucifex profile list`)

### Want to revoke and start clean

```bash
lucifex auth logout nous       # wipes the local refresh token
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Lucifex configurations
