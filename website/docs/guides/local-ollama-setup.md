---
sidebar_position: 9
title: "Run lucifexex Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running lucifexex Agent entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run lucifexex Locally with Ollama — Zero API Cost
...
```bash
# ~/.lucifex/.env
lucifexex_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
lucifexex is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the fulucifexfex experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside lucifexex with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Run the lucifexex setup wizard:
...
lucifexex will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. lucifexex requires at least 64,000 tokens for agentic work with tools:
...
Then update your lucifexex config to use `gemma4-64k` as the model name.
...
Once lucifexex works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.
...
- **lucifexex has auto-repair** — it detects malformed tool calls and attempts to fix them automatically.
- **Set up a fallback** — if the local model fails 3 times, lucifexex falls back to a cloud provider.
...
- **Terminal commands** — lucifexex wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below lucifexex' 64K minimum unless you configure them
