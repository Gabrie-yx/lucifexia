---
sidebar_position: 8
title: "Use Voice Mode with Lucifex"
description: "A practical guide to setting up and using Lucifex voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Lucifex
...
:::tip
[Lucifex portal](/integrations/lucifex-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Lucifex sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Lucifex.
...
## Step 1: make sure normal Lucifex works first
...
Lucifex supports both local and cloud speech stacks.
...
### If you use `lucifex setup`

If you choose NeuTTS in the setup wizard, Lucifex checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Lucifex transcribes and responds
...
- asking Lucifex to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Lucifex loop.

## Tuning CLI behavior

### Silence threshold

If Lucifex starts/stops too aggressively, tune:
...
Lucifex stays a normal chat bot, but can speak replies.
...
- you want Lucifex to function like a portable research or ops assistant
...
Lucifex joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Lucifex detects speech boundaries
- transcripts are posted in the associated text channel
- Lucifex responds in text and audio
...
1. get text Lucifex working
2. install `lucifex-agent[voice]`
