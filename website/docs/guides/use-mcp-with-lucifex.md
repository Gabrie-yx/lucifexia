---
sidebar_position: 6
title: "Use MCP with Lucifex"
description: "A practical guide to connecting MCP servers to Lucifex Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lucifex

This guide shows how to actually use MCP with Lucifex Agent in day-to-day workflows.
...
- you want to connect Lucifex to internal APIs, databases, or company systems without modifying Lucifex core
...
- Lucifex remains the agent
- MCP servers contribute tools
- Lucifex discovers those tools at startup or reload time
...
If you installed Lucifex with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lucifex banner/status should show MCP integration when configured
- ask Lucifex what tools it has available
...
## WSL2: bridge Lucifex in WSL to Windows Chrome

This is the practical setup when:

- Lucifex runs inside WSL2
...
In this setup, Lucifex does **not** connect to Chrome directly. Instead:

- Lucifex runs in WSL
- Lucifex starts a local stdio MCP server
...
```text
Lucifex (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Lucifex stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lucifex core browser transport
...
```bash
lucifex mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lucifex mcp test chrome-devtools-win
```

Then start a fresh Lucifex session or run:
...
Once loaded, Lucifex can use the MCP-prefixed browser tools directly. For example:
...
If Lucifex runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lucifex from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lucifex from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lucifex:
...
2. Lucifex-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Lucifex will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lucifex to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Lucifex to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Lucifex remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
lucifex mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
lucifex mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `lucifex mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Lucifex to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Lucifex to merge, publish, deploy, or spawn runtimes.
...
Now Lucifex can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lucifex core.
...
Because Lucifex now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
