...
Copy-paste blueprints for common automation patterns. Each blueprint uses lucifexex's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`lucifexex webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `lucifexex webhook subscribe`) |
...
```bash
lucifexex cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/lucifex-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/lucifex-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
lucifexex webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
lucifexex cron create "0 9 * * 1" \
  "Scan the NousResearch/lucifex-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/lucifex-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (lucifex_cli/commands.py, lucifex_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (lucifex_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
lucifexex webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
lucifexex webhook subscribe alert-triage \
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "lucifexex-Monitor/1.0"})
...
```bash
lucifexex cron create "every 30m" \
...
```bash
lucifexex cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to lucifexex Agent development." \
...
```bash
lucifexex webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
lucifexex webhook subscribe stripe-payments \
...
```bash
lucifexex cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the lucifex-agent codebase.
