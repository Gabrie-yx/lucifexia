...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.lucifex/.env` at the end.
...
Put the three values you collected into `~/.lucifex/.env`:
...
Lucifex ships a Graph auth smoke-test. From your Lucifex install:
