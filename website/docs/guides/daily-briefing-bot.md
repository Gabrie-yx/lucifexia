...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `lucifex setup --portal`. See [Lucifex portal](/integrations/lucifex-portal).
:::
...
2. **Lucifex spins up** a fresh agent session with your prompt
...
Lucifex will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Lucifex Agent
```
...
Before creating cron jobs, ensure Lucifex has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Lucifex what you want:
...
Lucifex will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Lucifex to delegate each topic to a sub-agent:
...
Lucifex will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
