...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `lucifexex setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **lucifexex spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo lucifexex gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.lucifex/cron/output/` and you can read them anytime.
:::
...
lucifexex will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by lucifexex Agent
```
...
Before creating cron jobs, ensure lucifexex has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell lucifexex what you want:
...
lucifexex will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell lucifexex to delegate each topic to a sub-agent:
...
lucifexex will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
