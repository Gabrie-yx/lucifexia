...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lucifexex send`."
---

# Pipe Script Output to Messaging Platforms

`lucifexex send` is a small, scriptable CLI that pushes a message to any
messaging platform lucifexex is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lucifexex send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lucifexex
...
lucifexex send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lucifexex send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lucifexex send --list

# Filter by platform
lucifexex send --list telegram
```
...
Any platform lucifexex ships adapters for works as a target:
...
`lucifexex send` resolves the message body in this order:

1. **Positional argument** — `lucifexex send --to telegram "hi"`
2. **`--file PATH`** — `lucifexex send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lucifexex send --to telegram`

When stdin is a TTY (no pipe), lucifexex does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lucifexex send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lucifexex send` reuses youlucifexifex config, the same script works on
any host where lucifexex is installed — no need to export bot tokens into
...
`lucifexex send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lucifexex send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | lucifexex send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.lucifexex/bilucifexifex send \
...
```bash
./train.py --epochs 200 && \
  lucifexex send --to telegram "training done" || \
  lucifexex send --to telegram "training failed (exit $?)"
```
...
lucifexex send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(lucifexex send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `lucifexex send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lucifexex send` calls
...
"no channels discovered yet", start the gateway once (`lucifexex gateway
...
| Approach | Multi-platform | Reuses lucifexex creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lucifexex send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lucifexex send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lucifexex send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lucifexex send` shares with cron delivery.
