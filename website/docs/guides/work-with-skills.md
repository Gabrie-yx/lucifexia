...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Lucifex new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Lucifex how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Lucifex installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Lucifex to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Lucifex but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
lucifex skills install official/research/arxiv
...
```bash
# Check it's there
lucifex skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a Lucifex Plugin → Bundle skills](/guides/build-a-lucifex-plugin#bundle-skills).
...
When a skill with config is first loaded, Lucifex prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
lucifex skills config gif-search

# View all skill config
lucifex config show | grep '^skills\.config'
```
...
```bash
lucifex chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.lucifex/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Lucifex may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Lucifex will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Lucifex to update the skill with what you learned. Skills that aren't maintained become liabilities.
