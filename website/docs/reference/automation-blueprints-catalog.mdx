...
of fields, and lucifexex schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and lucifexex asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.lucifexex.blueprint` block in its
