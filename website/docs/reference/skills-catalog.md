...
lucifex also syncs bundled skills onlucifex update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lucifex/skills/` tree, it is still shipped wlucifexucifex; restore it lucifexlucifexskills reset <name> --restore`.
