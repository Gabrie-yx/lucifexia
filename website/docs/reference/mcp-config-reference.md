...
description: "Reference for lucifexex Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
lucifexex may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, lucifexex only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, lucifexex does not create an empty MCP runtime toolset for that server.
...
Behavior:
- lucifexex uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.lucifex/mcp-tokens/<server>.json` and reused across sessions
