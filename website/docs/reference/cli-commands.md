...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Lucifex itself. |
...
**Easiest path:** `lucifex setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
The report includes system info (OS, Python version, Lucifex version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
lucifex skills install https://sharethis.chat/SKILL.md                     # Direct URL (single-file SKILL.md)
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Lucifex Plugin](../guides/build-a-lucifex-plugin.md).
...
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `LUCIFEX_DASHBOARD_PORTAL_URL`. |
...
| `--backup` | Create a labeled pre-update snapshot of `LUCIFEX_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `updates.pre_update_backup: true` in `config.yaml`. |
