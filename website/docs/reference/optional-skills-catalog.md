...
description: "Official optional skills shipped with lucifex-agent — install via lucifexex skills install official/<category>/<skill>"
---
...
```bash
lucifexex skills install official/<category>/<skill>
```

For example:

```bash
lucifexex skills install official/blockchain/solana
lucifexex skills install official/mlops/flash-attention
```
...
```bash
lucifexex skills uninstall <skill-name>
```
...
| [**honcho**](/docs/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho) | Configure and use Honcho memory with lucifexex -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshoo... |
...
| [**kanban-video-orchestrator**](/docs/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by lucifexex Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| [**lucifexex-s6-container-supervision**](/docs/user-guide/skills/optional/devops/devoplucifexifex-s6-container-supervision) | Modify, debug, or extend the s6-overlay supervision tree inside lucifexucifex Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. |
...
| Skill | Description |
|-------|-------------|
| [**openclaw-migration**](/docs/user-guide/skills/optional/migration/migration-openclaw-migration) | Migrate a user's OpenClaw customization footprint into lucifexex Agent. Importlucifexifex-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be mig... |
...
| [**telephony**](/docs/user-guide/skills/optional/productivity/productivity-telephony) | Give lucifexex phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
