...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Lucifex Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Lucifex setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Lucifex Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Lucifex Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Lucifex for a team, bot, or always-on workflow
...
| I want multi-provider fallback | `lucifex model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Lucifex cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Lucifex Agent
...
The single most important setup step. Use `lucifex model` to walk through the choice interactively:

```bash
lucifex model
```

:::tip Easiest path: Lucifex portal
...
:::info Setup modes
On a fresh install, `lucifex setup` offers three modes:

- **Quick Setup (Lucifex portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `lucifex update`. Re-enable anything later with `lucifex tools`, seed skills with `lucifex skills opt-in --sync`, or tune settings with `lucifex setup agent`.
:::
...
|----------|-----------|---------------|
| **Lucifex portal** | Subscription-based, zero-config | OAuth login via `lucifex model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `lucifex model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `lucifex model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `lucifex_cli/models.py` may change between releases) | `lucifex model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `lucifex model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `lucifex model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `lucifex model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `lucifex model` (requires `copilot` CLI + `copilot login`) |
...
Lucifex Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `lucifex model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Lucifex separates secrets from normal config:
...
```bash
lucifex config set model anthropic/claude-opus-4.6
lucifex config set terminal.backend docker
lucifex config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
lucifex            # classic CLI
lucifex --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Lucifex ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `lucifex` vs `lucifex --tui`.
:::
...
```bash
lucifex --continue    # Resume the most recent session
...
```bash
lucifex gateway setup    # Interactive platform configuration
```
...
- `lucifex tools` — tune tool access per platform
- `lucifex skills` — browse and install reusable workflows
...
```bash
lucifex config set terminal.backend docker    # Docker isolation
lucifex config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Lucifex install directory (the curl installer placed it at
# ~/.lucifex/lucifex-agent on Linux/macOS or %LOCALAPPDATA%\lucifex\lucifex-agent on Windows):
...
Skills are on-demand instruction documents that teach Lucifex how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Lucifex ships with a catalog of bundled skills already installed in `~/.lucifex/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
lucifex skills browse                      # list everything available
lucifex skills search kubernetes           # find skills by keyword
lucifex skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `lucifex skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Lucifex ask what you need
```
...
|---|---|---|
| Lucifex opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `lucifex model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `lucifex gateway setup` and check `lucifex gateway status` |
...
| `lucifex tools` | Configure which tools are enabled per platform |
| `lucifex setup` | Full setup wizard (configures everything at once) |
| `lucifex doctor` | Diagnose issues |
| `lucifex update` | Update to latest version |
| `lucifex gateway` | Start the messaging gateway |
| `lucifex --continue` | Resume last session |
