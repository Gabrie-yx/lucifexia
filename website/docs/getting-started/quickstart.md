...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with lucifexex Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working lucifexex setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [lucifexex Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="lucifexex Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up lucifexex for a team, bot, or always-on workflow
...
|---|---|---|
| I just want lucifexex working on my machine |lucifexifex setup` | Run a real chat and verify it responds |
| I already know my provider | `lucifexex model` | Save the config, then start chatting |
| I want a bot or always-on setup | `lucifexex gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `lucifexex model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `lucifexex model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if lucifexex cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install lucifexex Agent
### With the lucifexex Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the lucifexex Desktop installer](https://lucifex-agent.nousresearch.com/) from our website and run it.

### Without lucifexex Desktop:
For a command-line only install without lucifexex Desktop, run:
...
The single most important setup step. Use `lucifexex model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `lucifexex setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `lucifexex update`. Re-enable anything later withlucifexifex tools`, seed skills wilucifexucifex skills opt-in --sync`, or tune settings lucifexlucifexsetup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `lucifexex model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `lucifexex model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `lucifexex model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `lucifex_cli/models.py` may change between releases) | `lucifexex model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `lucifexex model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `lucifexex model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `lucifexex model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `lucifexex model` (requires `copilot` CLI + `copilot login`) |
...
lucifexex Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `lucifexex model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

lucifexex separates secrets from normal config:
...
```bash
lucifexex config set model anthropic/claude-opus-4.6
lucifexex config set terminal.backend docker
lucifexex config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
lucifexex            # classic CLI
lucifexex --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
lucifexex ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each withlucifexifex` lucifexucifex --tui`.
:::
...
- lucifexex replies without error
...
```bash
lucifexex --continue    # Resume the most recent session
...
```bash
lucifexex gateway setup    # Interactive platform configuration
```
...
- `lucifexex tools` — tune tool access per platform
- `lucifexex skills` — browse and install reusable workflows
...
```bash
lucifexex config set terminal.backend docker    # Docker isolation
lucifexex config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the lucifexex install directory (the curl installer placed it at
# ~/.lucifex/lucifex-agent on Linux/macOS or %LOCALAPPDATA%\lucifexex\lucifex-agent on Windows):
...
Skills are on-demand instruction documents that teach lucifexex how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

lucifexex ships with a catalog of bundled skills already installed in `~/.lucifex/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
lucifexex skills browse                      # list everything available
lucifexex skills search kubernetes           # find skills by keyword
lucifexex skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `lucifexex skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let lucifexex ask what you need
```
...
|---|---|---|
| lucifexex opens but gives empty or broken replies | Provider auth or model selection is wrong | Runlucifexifex model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `lucifexex gateway setup` and checklucifexifex gateway status` |
| `lucifexex --continue` can't find old session | Switched profiles or session never saved | Check `lucifex sessions list` and confirm you're in the right profile |
...
| `lucifexex doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
5. `lucifexex --continue`
6. `lucifexex gateway status`
...
| Command | Description |
|---------|-------------|
| `lucifexex` | Start chatting |
| `lucifexex model` | Choose your LLM provider and model |
| `lucifexex tools` | Configure which tools are enabled per platform |
| `lucifexex setup` | Full setup wizard (configures everything at once) |
| `lucifexex doctor` | Diagnose issues |
| `lucifexex update` | Update to latest version |
| `lucifexex gateway` | Start the messaging gateway |
| `lucifexex --continue` | Resume last session |
