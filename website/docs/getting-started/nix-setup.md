...
description: "Install and deploy Lucifex Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Lucifex Agent ships a Nix flake & a NixOS module.
...
After `nix profile install`, `lucifex`, `lucifex-agent`, and `lucifex-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `lucifex setup` walks you through provider selection, `lucifex gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.lucifex/`.
...
That's it. `nixos-rebuild switch` creates the `lucifex` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o lucifex /dev/stdin /var/lib/lucifex/env
```

```nix
services.lucifex-agent.environmentFiles = [ "/var/lib/lucifex/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `lucifex` CLI on your system PATH **and** sets `LUCIFEX_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `lucifex` in your shell creates a separate `~/.lucifex/` directory.
:::
...
- For developers working on the lucifex codebase, set `LUCIFEX_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `lucifex` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo lucifex chat` manually.
:::
...
lucifex version
lucifex config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Lucifex defaults to OpenRouter.
:::
...
```nix
services.lucifex-agent.configFile = /etc/lucifex/config.yaml;
```
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."lucifex-env".path ]` |
| Give the agent a personality | `${services.lucifex-agent.stateDir}/.lucifex/SOUL.md` | manage the file directly |
...
| Override the lucifex package | `package` | `inputs.lucifex-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/lucifex"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$LUCIFEX_HOME/.env` at activation time (`nixos-rebuild switch`). Lucifex reads this file on every startup, so changes take effect with a `systemctl restart lucifex-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/lucifex.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."lucifex-env" = { format = "yaml"; };
  };

  services.lucifex-agent.environmentFiles = [
    config.sops.secrets."lucifex-env".path
  ];
}
```
...
```yaml
# secrets/lucifex.yaml (encrypted with sops)
lucifex-env: |
...
```nix
{
  age.secrets.lucifex-env.file = ./secrets/lucifex-env.age;
...
{
  services.lucifex-agent = {
    authFile = config.sops.secrets."lucifex/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Lucifex looks for specific filenames by convention:
...
Set `auth = "oauth"` for servers using OAuth 2.1. Lucifex implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Lucifex prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u lucifex` (native):
...
# Native mode
sudo -u lucifex LUCIFEX_HOME=/var/lib/lucifex/.lucifex \
  lucifex mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
scp ~/.lucifex/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/lucifex/.lucifex/mcp-tokens/
# Ensure: chown lucifex:lucifex, chmod 0600
```
...
When lucifex runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `lucifex setup` | Config is declarative — edit `settings` in your Nix config |
| `lucifex config edit` | Config is generated from `settings` |
| `lucifex config set <key> <value>` | Config is generated from `settings` |
| `lucifex gateway install` | The systemd service is managed by NixOS |
| `lucifex gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`LUCIFEX_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `LUCIFEX_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it lucifex-agent lucifex config set ...` is also blocked)
...
When container mode is enabled, lucifex runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-lucifex-agent-0.1.0  ──►  /nix/store/... (ro)
~/.lucifex -> /var/lib/lucifex/.lucifex       (symlink bridge, per hostUsers)
/var/lib/lucifex/                    ──►  /data/          (rw)
  ├── current-package -> /nix/store/...    (symlink, updated each rebuild)
  ├── .gc-root -> /nix/store/...           (prevents nix-collect-garbage)
  ├── .container-identity                  (sha256 hash, triggers recreation)
  ├── .lucifex/                             (LUCIFEX_HOME)
...
| Event | Container recreated? | `/data` (state) | `/home/lucifex` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the lucifex package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/lucifex` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/lucifex-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current lucifex package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `lucifex plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [lucifex-lcm](https://github.com/stephenschoettler/lucifex-lcm)):
...
Plugins are symlinked into `$LUCIFEX_HOME/plugins/` at activation time. Lucifex discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."lucifex_agent.plugins"]` (e.g., [rtk-lucifex](https://github.com/ogallotti/rtk-lucifex)):
...
The package's `site-packages` is added to PYTHONPATH in the lucifex wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core lucifex dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # LUCIFEX_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `lucifex` and `lucifex-agent` binaries exist and `lucifex version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `lucifex --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `LUCIFEX_MANAGED=true lucifex config set ...` prints the NixOS error |
...
| `user` | `str` | `"lucifex"` | System user |
| `group` | `str` | `"lucifex"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/lucifex"` | State directory (`LUCIFEX_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `lucifex` CLI to system PATH and set `LUCIFEX_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `lucifex gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the lucifex user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.lucifex` symlink to the service stateDir and are auto-added to the `lucifex` group |
...
```
/var/lib/lucifex/                     # stateDir (owned by lucifex:lucifex, 0750)
├── .lucifex/                         # LUCIFEX_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Lucifex binary + all Nix deps |
| `/data` | `/var/lib/lucifex` | `rw` | All state, config, workspace |
| `/home/lucifex` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
sudo rm /var/lib/lucifex/.container-identity
...
```bash
# Native mode
sudo -u lucifex cat /var/lib/lucifex/.lucifex/.env

# Container mode
docker exec lucifex-agent cat /data/.lucifex/.env
```
...
| `lucifex version` shows old version | Container not restarted | `systemctl restart lucifex-agent` |
| Permission denied on `/var/lib/lucifex` | State dir is `0750 lucifex:lucifex` | Use `docker exec` or `sudo -u lucifex` |
| `nix-collect-garbage` removed lucifex | GC root missing | Restart the service (preStart recreates the GC root) |
...
| `unable to find user lucifex` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
