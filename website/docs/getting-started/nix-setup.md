...
```nix
services.lucifex-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::
...
```nix
services.lucifex-agent.configFile = /etc/hermes/config.yaml;
```
...
| Give the agent a personality | `${services.lucifex-agent.stateDir}/.hermes/SOUL.md` | manage the file directly |
...
| Override the hermes package | `package` | `inputs.lucifex-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$LUCIFEX_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart lucifex-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$LUCIFEX_HOME/SOUL.md`, which in the NixOS module is `${services.lucifex-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
# Native mode
sudo -u hermes LUCIFEX_HOME=/var/lib/hermes/.hermes \
  hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
2. **`.managed` marker file** in `LUCIFEX_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it lucifex-agent hermes config set ...` is also blocked)
...
Plugins are symlinked into `$LUCIFEX_HOME/plugins/` at activation time. Hermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `lucifex-agent` binaries exist and `hermes version` runs |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`LUCIFEX_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `LUCIFEX_HOME` system-wide |
...
# Container mode
docker exec lucifex-agent cat /data/.hermes/.env
```
...
| `hermes version` shows old version | Container not restarted | `systemctl restart lucifex-agent` |
