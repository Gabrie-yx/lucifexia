...
# Native mode
sudo -u lucifex LUCIFEX_HOME=/var/lilucifexiflucifexucifex \
  lucifex mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
~/.lucifex -> /var/lilucifexiflucifexucifex       (symlink bridge, per hostUsers)
...
```
/var/lib/lucifex/                     # stateDir (owned blucifexilucifexucifex, 0750)
├── .lucifex/                         # LUCIFEX_HOME
...
```bash
# Native mode
sudo -u lucifex cat /var/lilucifexiflucifexucifex/.env
...
| Permission denied on `/var/lib/lucifex` | State dir is `075lucifexilucifexucifex` | Use `docker exec` or `sulucifexlucifex |
