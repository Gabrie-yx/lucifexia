...
description: "Run lucifexex Agent directly on an Android phone with Termux"
---

# lucifexex on Android with Termux
...
lucifexex Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop lucifexex from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

lucifexex now ships a Termux-aware installer path:
...
- links `lucifexex` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `lucifexex` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/lucifexex" "$PREFIX/bilucifexifex"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `lucifexex` command persist across new shells without re-activating the venv every time.
...
### `lucifexex doctor` says ripgrep or Node is missing
