...
This page covers setting up inference providers for lucifexex Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to uslucifexifex.

## Inference Providers

You need at least one way to connect to an LLM. Use `lucifexex model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `lucifexex model` (OAuth, subscription-based) |
| **OpenAI Codex** | `lucifexex model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `lucifexex model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `lucifexex model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `lucifexex model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.lucifex/.env` |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.lucifex/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.lucifex/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.lucifex/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.lucifex/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.lucifex/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.lucifex/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.lucifex/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.lucifex/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.lucifex/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.lucifex/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `lucifexex model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.lucifex/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.lucifex/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.lucifex/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.lucifex/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.lucifex/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.lucifex/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.lucifex/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.lucifex/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.lucifex/.env` (provider: `gemini`) |
| **Google Vertex AI** | `lucifexex model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.lucifex/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `lucifexex model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `lucifexex model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.lucifex/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `lucifexex model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `lucifexex model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `lucifexex model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.lucifex/.env` (provider: `stepfun`) |
| **LM Studio** | `lucifexex model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `lucifexex model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run lucifexex Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [Nous Chat](https://chat.nousresearch.com) — billed against your Nous subscription instead of separate per-provider accounts.

```bash
lucifexex setup --portal     # fresh install — OAuth + provider + gateway in one command
lucifexex model              # existing install — pick "Nous Portal" from the list
lucifexex portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run lucifexex Agent with Nous Portal guide](/guides/rulucifexifex-with-nous-portal).

**Client identification.** Every Portal request from lucifexex Agent carries a `clienlucifexifex-client-v<version>` tag (e.g. `clilucifexucifex-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinlucifexlucifextraffic from other clients. No config required; the tag updates automatically whlucifex `llucifexpdate`.

**JWT auth (automatic).** lucifexex prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). lucifexex stores the resulting credentials in its own auth store under `~/.lucifex/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), lucifexex marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Runlucifexifex auth add openai-codex` (lucifexucifex model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), lucifexex routes these tasks to your **main chat model** — the same model you picked inlucifexifex model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `lucifexex setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it fromlucifexifex model` or per-tool frlucifexucifex tools`. Inspect routing at any time lucifexlucifexportal info`.
:::

### Two Commands for Model Management

lucifexex has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`lucifexex model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a lucifexex chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `lucifexex model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), runlucifexifex model`, complete the provider setup, then start a new session.
...
lucifexex chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `lucifexex model`
# lucifexex will use Claude Code's credential store directly when available
lucifexex model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
lucifexex chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
lucifexex chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `lucifexex model`lucifexifex prefers Claude Code's own credential store over copying the token into `~/.lucifex/.env`. That keeps refreshable Claude credentials refreshable.
...
lucifexex supports GitHub Copilot as a first-class provider with two modes:
...
```bash
lucifexex chat --provider copilot --model gpt-5.4
```
...
If no token is found, `lucifexex model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `lucifexex model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `lucifexex model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in lucifexex
lucifexex sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, lucifexex now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). lucifexex therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
lucifexex chat --provider copilot-acp --model copilot-acp
...
| `lucifexex_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `lucifexex_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
lucifexex chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.lucifex/.env

# NovitaAI Model API
lucifexex chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.lucifex/.env

# z.ai / ZhipuAI GLM
lucifexex chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.lucifex/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
lucifexex chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.lucifex/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
lucifexex chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.lucifex/.env

# MiniMax (global endpoint)
lucifexex chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.lucifex/.env

# MiniMax (China endpoint)
lucifexex chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.lucifex/.env

# Qwen Cloud / DashScope (Qwen models)
lucifexex chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.lucifex/.env

# Xiaomi MiMo
lucifexex chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.lucifex/.env

# Tencent TokenHub (Hy3 Preview)
lucifexex chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.lucifex/.env

# Arcee AI (Trinity models)
lucifexex chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.lucifex/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
lucifexex chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.lucifex/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `lucifexex model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, lucifexex automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.lucifex/.env` and pick xAI in `lucifexex model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `lucifexex model`, or runlucifexifex auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — andlucifexucifex runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), lucifexex automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `lucifexex tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `lucifexex doctor` andlucifexifex chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Ulucifexucifex migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
lucifexex migrate xai          # preview replacements
lucifexex migrate xai --apply  # rewrite ~/.lucifex/config.yaml in place
```
...
# Short alias
lucifexex chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `lucifexex model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), anlucifexifex auto-discovers the available models.
...
lucifexex chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 lucifexex chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
lucifexex model   # → "Google Vertex AI" → project → region → model
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'lucifex-agent[vertex]'` (or let lucifexex lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `lucifexex model`, sign in through the browser, anlucifexifex persists the refresh token.

```bash
lucifexex model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.lucifex/auth.json

lucifexex chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `lucifexex_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), lucifexex exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
lucifexex chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `lucifexex model`, sign in through the browser, anlucifexifex persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
lucifexex chat   # uses api.minimax.io/anthropic endpoint
```
...
lucifexex chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.lucifex/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 lucifexex chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
lucifexex automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
lucifexex chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.lucifex/.env
```
...
```bash
# StepFun
lucifexex chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.lucifex/.env
```
...
```bash
# Use any available model
lucifexex chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.lucifex/.env

# Short alias
lucifexex chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
lucifexex Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can poinlucifexifex at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `lucifexex model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the nextlucifexifex setup` or config migration.
:::
...
:::warning lucifexex model vs /model
**`lucifexex model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active lucifexex chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `lucifexex model`, set up the new provider, then start a new session.
:::
...
When switching providers, lucifexex persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
lucifexex Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like lucifexex.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in lucifexex) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `lucifexex` (Qwen 2.5lucifexifex 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** lucifexex preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for lucifexex. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is belolucifexifex' minimum per active session.

Then configure lucifexex to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but lucifexex won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), lucifexex 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
By default, lucifexex explicitly asks LM Studio to load the selected model with 64K context length before the first request.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip lucifexex' explicit preload step:

```bash
lucifexex config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
lucifexex config set model.lmstudio_load_mode explicit
```

**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, lucifexex) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since lucifexex Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your lucifex config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your lucifexex config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with lucifexex.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running lucifexex inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser lucifexex` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. lucifexex's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what lucifexex thinks the context is
...
lucifexex auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all)lucifexifex uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure lucifexex with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in lucifexex.
...
Then configure lucifexex withlucifexifex model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure lucifexex withlucifexifex model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `lucifexex model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). lucifexex uses this to decide when to compress history and to validate API requests.
...
lucifexex uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`lucifexex model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — lucifexex uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `lucifexex model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and lucifexex will merge it into each chat-completions request for that endpoint:
...
The `lucifexex model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so lucifexex routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `lucifexex model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.lucifex/config.yaml` and the matching API key goes in `~/.lucifex/.env`.
...
Together's `/v1/models` endpoint works, so `lucifexex model` can auto-discover available models.
...
:::tip Troubleshooting
- `lucifexex doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `lucifexex model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `lucifexex model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point lucifexex at your instance (no API key needed):
   ```bash
   lucifexex config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.lucifex/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.lucifex/config.yaml`:
...
Configure a chain of backup providers lucifexex tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `lucifexex fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
