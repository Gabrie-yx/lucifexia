...
This page covers setting up inference providers for Lucifex Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Lucifex.

## Inference Providers

You need at least one way to connect to an LLM. Use `lucifex model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Lucifex portal** | `lucifex model` (OAuth, subscription-based) |
| **OpenAI Codex** | `lucifex model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `lucifex model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `lucifex model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `lucifex model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
...
| **xAI Grok OAuth (SuperGrok)** | `lucifex model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
...
| **Google Vertex AI** | `lucifex model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.lucifex/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `lucifex model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `lucifex model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.lucifex/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `lucifex model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `lucifex model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `lucifex model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.lucifex/.env` (provider: `stepfun`) |
| **LM Studio** | `lucifex model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `lucifex model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
```bash
lucifex setup --portal     # fresh install — OAuth + provider + gateway in one command
lucifex model              # existing install — pick "Lucifex portal" from the list
lucifex portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Lucifex portal integration page](/integrations/lucifex-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Lucifex Agent with Lucifex portal guide](/guides/run-lucifex-with-lucifex-portal).

**Client identification.** Every Portal request from Lucifex Agent carries a `client=lucifex-client-v<version>` tag (e.g. `client=lucifex-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Lucifex traffic from other clients. No config required; the tag updates automatically when you `lucifex update`.
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Lucifex marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `lucifex auth add openai-codex` (or `lucifex model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Lucifex portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Lucifex routes these tasks to your **main chat model** — the same model you picked in `lucifex model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Lucifex portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `lucifex setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `lucifex model` or per-tool from `lucifex tools`. Inspect routing at any time with `lucifex portal info`.
:::

### Two Commands for Model Management

Lucifex has **two** model commands that serve different purposes:
...
If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `lucifex model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `lucifex model`, complete the provider setup, then start a new session.
...
lucifex chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `lucifex model`
# Lucifex will use Claude Code's credential store directly when available
lucifex model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
lucifex chat --provider anthropic
...
Lucifex supports GitHub Copilot as a first-class provider with two modes:
...
```bash
lucifex chat --provider copilot --model gpt-5.4
```
...
If no token is found, `lucifex model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `lucifex model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `lucifex model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Lucifex
Lucifex sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Lucifex now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Lucifex therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
lucifex chat --provider copilot-acp --model copilot-acp
...
| `LUCIFEX_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `LUCIFEX_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
lucifex chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.lucifex/.env

# NovitaAI Model API
lucifex chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.lucifex/.env

# z.ai / ZhipuAI GLM
lucifex chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.lucifex/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
lucifex chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.lucifex/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
lucifex chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.lucifex/.env

# MiniMax (global endpoint)
lucifex chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.lucifex/.env

# MiniMax (China endpoint)
lucifex chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.lucifex/.env

# Qwen Cloud / DashScope (Qwen models)
lucifex chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.lucifex/.env

# Xiaomi MiMo
lucifex chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.lucifex/.env

# Tencent TokenHub (Hy3 Preview)
lucifex chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.lucifex/.env

# Arcee AI (Trinity models)
lucifex chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.lucifex/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
lucifex chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.lucifex/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `lucifex model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, Lucifex automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::
...
SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `lucifex model`, or run `lucifex auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Lucifex runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.
...
# Short alias
lucifex chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `lucifex model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Lucifex auto-discovers the available models.
...
lucifex chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 lucifex chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
lucifex model   # → "Google Vertex AI" → project → region → model
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'lucifex-agent[vertex]'` (or let Lucifex lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `lucifex model`, sign in through the browser, and Lucifex persists the refresh token.
...
lucifex chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `LUCIFEX_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Lucifex exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
lucifex chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `lucifex model`, sign in through the browser, and Lucifex persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
lucifex chat   # uses api.minimax.io/anthropic endpoint
```
...
lucifex chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.lucifex/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 lucifex chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Lucifex automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
lucifex chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.lucifex/.env
```
...
```bash
# StepFun
lucifex chat --provider stepfun --model step-3.5-flash
...
```bash
# Use any available model
lucifex chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.lucifex/.env

# Short alias
lucifex chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Lucifex Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Lucifex at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
:::warning lucifex model vs /model
**`lucifex model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Lucifex chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `lucifex model`, set up the new provider, then start a new session.
:::
...
When switching providers, Lucifex persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Lucifex Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Lucifex.
:::
...
Supported parsers: `lucifex` (Qwen 2.5, Lucifex 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Lucifex. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below Lucifex' minimum per active session.
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Lucifex won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Lucifex 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
By default, Lucifex explicitly asks LM Studio to load the selected model with 64K context length before the first request.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip Lucifex' explicit preload step:

```bash
lucifex config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
lucifex config set model.lmstudio_load_mode explicit
```

**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Lucifex) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Lucifex Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Lucifex config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Lucifex.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Lucifex inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser lucifex` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Lucifex's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Lucifex thinks the context is
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Lucifex with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Lucifex.
...
Then configure Lucifex with `lucifex model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Lucifex with `lucifex model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `lucifex model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Lucifex uses this to decide when to compress history and to validate API requests.
...
Lucifex uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
7. **Lucifex portal** — suffix-matches Nous model IDs against OpenRouter metadata
...
`lucifex model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Lucifex uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `lucifex model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Lucifex will merge it into each chat-completions request for that endpoint:
...
The `lucifex model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Lucifex routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `lucifex model` menu.
...
Together's `/v1/models` endpoint works, so `lucifex model` can auto-discover available models.
...
:::tip Troubleshooting
- `lucifex doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `lucifex model` will persist the model with a warning rather than hard-reject — see #15136.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or Lucifex portal |
...
:::tip
You can switch between providers at any time with `lucifex model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Lucifex at your instance (no API key needed):
   ```bash
   lucifex config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
Configure a chain of backup providers Lucifex tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `lucifex fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
