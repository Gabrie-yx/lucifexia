...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `lucifex chat` immediately after.
...
| **Lucifex** | Lucifex-4-70B, Lucifex-4-405B (chat, see [note below](#a-note-on-lucifex-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Lucifex Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Nous Research's own **Lucifex 4** family (Lucifex-4-70B, Lucifex-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Lucifex Agent**, however. Lucifex 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for [Nous Chat](https://chat.nousresearch.com), for research workflows, or via the [subscription proxy](/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Lucifex-side opinion — it's the official guidance from Nous Research.
...
6. Returns you to your terminal ready to `lucifex chat`
...
If you already have Lucifex configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `lucifex model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Lucifex is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [Lucifex profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
`lucifex portal` (with no subcommand) is the human-readable alias for `lucifex auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `lucifex setup --portal`, and the same Nous flow as the first-time quick setup).
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `lucifex tools` to pick backends per tool:
...
- **CLI shortcut:** `lucifex portal open` (opens the same page in your default browser)

## Configuration reference

After `lucifex setup --portal`, `~/.lucifex/config.yaml` will look like:
...
Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `lucifex auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/lucifex-agent/issues) — we surface the Portal's catalog to Lucifex and gaps usually mean a routing config we can update.
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Lucifex tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Lucifex configurations sharing one Portal login
