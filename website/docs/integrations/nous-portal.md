...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.lucifex/auth.json` is the only credential on disk, and Hermes mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
After `hermes setup --portal`, `~/.lucifex/config.yaml` will look like:
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/lucifex-agent/issues) — we surface the Portal's catalog to Hermes and gaps usually mean a routing config we can update.
