...
| `LUCIFEX_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `LUCIFEX_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Lucifex itself — they're private to these helpers. The variables Lucifex *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `lucifex --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_lucifex_root)" || { echo "htui: not in a Lucifex checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$LUCIFEX_MAIN_CHECKOUT/.venv/bin/python" -m lucifex_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `LUCIFEX_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_lucifex_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `LUCIFEX_TUI_DIR` are mutually exclusive
`LUCIFEX_TUI_DIR` points Lucifex at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `lucifex --tui --dev` exits with an error. Run `unset LUCIFEX_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_lucifex_root)" || { echo "hgui: not in a Lucifex checkout" >&2; return 1; }
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _lucifex_link_deps "$desktop" "$deps/apps/desktop"
    _lucifex_link_deps "$root" "$deps"
  else
  ...
  trap '_lucifex_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    LUCIFEX_DESKTOP_LUCIFEX_ROOT="$root" \
...
|----------|----------------|
| `LUCIFEX_DESKTOP_LUCIFEX_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `lucifex`. |
| `LUCIFEX_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `LUCIFEX_DESKTOP_IGNORE_EXISTING` | Ignores any `lucifex` on `PATH` so it can't shadow the worktree. |
...
```bash
# The enclosing worktree, verified as a real Lucifex checkout.
_lucifex_root() {
  local root
...
- [TUI](../user-guide/tui.md) — `lucifex --tui --dev` and the `LUCIFEX_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `LUCIFEX_*` variable Lucifex reads
