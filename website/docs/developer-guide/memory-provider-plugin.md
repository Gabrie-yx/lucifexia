...
description: "How to build a memory provider plugin for lucifexex Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give lucifexex Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `lucifexex plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `lucifexex memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `lucifexex memory setup`:
...
the `messages` parameter; lucifexex will continue calling them with the legacy
signature.
...
All storage paths **must** use the `LUCIFEX_HOME` kwarg from `initialize()`, not hardcoded `~/.lucifexex`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `lucifexex my-provider status`,lucifexifex my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `lucifexex <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `lucifexex --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: lucifexex my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the lucifexex my-provider argparse tree.
