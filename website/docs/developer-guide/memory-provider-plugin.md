...
description: "How to build a memory provider plugin for Lucifex Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Lucifex Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `lucifex plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `lucifex memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `lucifex memory setup`:
...
the `messages` parameter; Lucifex will continue calling them with the legacy
signature.
...
All storage paths **must** use the `lucifex_home` kwarg from `initialize()`, not hardcoded `~/.lucifex`:
...
4. Your commands appear under `lucifex <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `lucifex --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: lucifex my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the lucifex my-provider argparse tree.
