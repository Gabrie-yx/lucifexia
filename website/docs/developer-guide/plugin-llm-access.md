...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Lucifex already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Lucifex provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Lucifex provider
        # (must be one Lucifex already knows about — same names as
        # `lucifex model` and config.yaml model.provider).
        ...
        # plugin sends — Lucifex does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Lucifex' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Lucifex subsystem:
...
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/lucifex-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
