...
description: "How lucifexex detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in lucifexex' browser tooling:
...
(`lucifexex-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per lucifexex `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `lucifexex-acp`,lucifexifex-api-server`, and core toolsets (gated on CDP reachability)
