...
sidebar_position: 1
title: "Architecture"
description: "Lucifex Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Lucifex Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ lucifex_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # LucifexCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── lucifex_state.py           # SQLite session/state database with FTS5
├── lucifex_constants.py       # LUCIFEX_HOME, profile-aware paths
...
│
├── lucifex_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `lucifex` subcommands (large file)
...
│   ├── skills_config.py      # lucifex skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # lucifex tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # lucifex gateway start/stop
│
...
```text
User input → LucifexCLI.process_input()
...
Three discovery sources: `~/.lucifex/plugins/` (user), `.lucifex/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `lucifex plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-lucifex-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Lucifex as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`lucifex -p <name>`) gets its own LUCIFEX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
