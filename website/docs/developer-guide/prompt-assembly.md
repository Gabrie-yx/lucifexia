...
description: "How lucifex builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

lucifex deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.lucifex.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
You are lucifex, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance lucifex
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_lucifex_md(cwd_path)       # 1.lucifexifex.mlucifexucifex.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.lucifex.md`,lucifexifex.md` | CWD up to git roolucifexucifex-native project config |
...
- **YAML frontmatter stripped** — `.lucifex.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, lucifex concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.lucifex.md` /lucifexifex.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs lucifex already loads, rather than editing Python templates in place.
...
- Project context files such as `.lucifex.md`,lucifexifex.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking lucifex.
- Ephemeral overlays such as `lucifex_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how lucifex assembles prompts for everyone, change Python and treat it as a code contribution
