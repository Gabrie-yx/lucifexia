...
description: "How lucifexex builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

lucifexex deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.lucifexex.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.lucifex/SOUL.md)
You are lucifexex, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance lucifexex
...
`SOUL.md` lives at `~/.lucifex/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are lucifexex Agent, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_lucifexex_md(cwd_path)       # 1.lucifexifex.mlucifexucifex.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.lucifexex.md`,lucifexifex.md` | CWD up to git roolucifexucifex-native project config |
...
- **YAML frontmatter stripped** — `.lucifexex.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, lucifexex concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.lucifexex.md` /lucifexifex.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs lucifexex already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.lucifex/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.lucifex/MEMORY.md` and `~/.lucifex/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.lucifexex.md`,lucifexifex.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking lucifexex.
- Ephemeral overlays such as `lucifexex_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how lucifexex assembles prompts for everyone, change Python and treat it as a code contribution
