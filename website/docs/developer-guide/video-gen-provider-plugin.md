...
description: "How to build a video-generation backend plugin for Lucifex Agent"
---
...
Lucifex scans for video-gen backends in three places:
...
Each plugin's `register(ctx)` function calls `ctx.register_video_gen_provider(...)`. The active provider is picked by `video_gen.provider` in `config.yaml`; `lucifex tools` → Video Generation walks users through selection. Unlike `image_generate`, there is no in-tree legacy backend — every provider is a plugin.
...
class MyVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "My Backend",
            "badge": "paid",
            "tag": "Short description shown in `lucifex tools`",
            "env_vars": [
                {
                    "key": "MY_API_KEY",
...
The provider's `capabilities()` advertises which of these are honored. The agent sees the active backend's capabilities in the tool description, dynamically rebuilt when the user changes backend via `lucifex tools`.
...
The user picks `veo3.1` once in `lucifex tools`. The agent never thinks about endpoints — it just passes (or doesn't pass) `image_url`.
