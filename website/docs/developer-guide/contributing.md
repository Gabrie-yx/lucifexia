...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Lucifex Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Lucifex Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Lucifex core? Start with [Build a Lucifex Plugin](../developer-guide/plugins/index.md)
...
You can also run a fully isolated Lucifex instance (throwaway LUCIFEX_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m lucifex_cli.main
scripts/dev-sandbox.sh --persistent python -m lucifex_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
- **Profile-safe paths**: Never hardcode `~/.lucifex` — use `get_lucifex_home()` from `lucifex_constants` for code paths and `display_lucifex_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/lucifex-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/lucifex-agent/blob/main/LICENSE).
