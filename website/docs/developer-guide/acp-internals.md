...
So ACP advertises and uses the currently configured Lucifex provider/credentials. It also always advertises a terminal setup auth method (`lucifex-setup`, args `--setup`) so first-run registry clients can open Lucifex' interactive model/provider configuration before starting a normal ACP session.
