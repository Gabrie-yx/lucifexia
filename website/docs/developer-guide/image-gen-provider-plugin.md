...
description: "How to build an image-generation backend plugin for lucifexex Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** lucifexex supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build lucifexifex Plugin](/developer-guide/plugins).
:::

## How discovery works

lucifexex scans for image-gen backends in three places:
...
3. **Pip** — packages declaring a `lucifexex_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `lucifexex tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `lucifexex tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.lucifex/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `lucifexex plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `lucifexex tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `lucifexex tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `lucifexex tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `lucifexex plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `lucifexex tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `lucifexex tools` model picker |
...
Drop a user plugin at `~/.lucifex/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `lucifexex plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export LUCIFEX_HOME=/tmp/lucifexex-imggen-test
...
lucifexex plugins enable my-backend
...
# Exercise it
lucifexex -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `lucifexex tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a lucifexex Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
