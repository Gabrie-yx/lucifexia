...
title: "Desktop Plugin SDK (@lucifex/plugin-sdk)"
description: "Extend the native Lucifex Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Lucifex Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `LucifexPlugin`.
It imports one module — `@lucifex/plugin-sdk` — and gets everything: the app's
...
CLI/gateway plugins are documented at [Build a Lucifex Plugin](/developer-guide/plugins).
...
Both take the same `LucifexPlugin` contract, appear in **Settings → Plugins**, and
...
import { host, haptic, useValue } from '@lucifex/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Lucifex' }),
      jsx('div', {
...
The only importable specifiers are `@lucifex/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `LucifexPlugin`:

```ts
interface LucifexPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `lucifex.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@lucifex/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@lucifex/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@lucifex/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@lucifex/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@lucifex/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@lucifex/plugin-sdk'
...
```javascript
import { queryClient } from '@lucifex/plugin-sdk'
...
`dashboard/` subfolder of a regular Lucifex plugin and declare it in a
`manifest.json`:
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.lucifex/`)
...
(`lucifex.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `LucifexPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@lucifex/plugin-sdk` alias.
2. It's still lint-fenced to `@lucifex/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@lucifex/plugin-sdk`, `react`,
...
| Plugin contract | `LucifexPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `lucifex-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`lucifex-desktop-plugins`** skill — it carries the same contract as this page in
...
toast naming the failure, and tail `lucifex logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@lucifex/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
