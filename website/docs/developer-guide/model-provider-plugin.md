...
description: "How to build a model provider (inference backend) plugin for lucifexex Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that lucifexex can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$LUCIFEX_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with lucifexex
...
The only required file is `__init__.py`. `plugin.yaml` is used by `lucifexex plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
| `lucifexex model` picker | `lucifex_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `lucifexex doctor` | `lucifex_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `lucifexex setup` | `lucifex_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| `display_name` | str | Human label shown in `lucifexex model` picker |
...
Four values are recognized. lucifexex picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but lucifexex' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
```bash
export LUCIFEX_HOME=/tmp/lucifexex-plugin-test
...
lucifexex -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `lucifexex plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$LUCIFEX_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any lucifexex plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."lucifexex_agent.plugins"]
acme-inference = "acme_lucifexex_plugin:register"
```

…where `acme_lucifexex_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a lucifexex Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a lucifexex Plugin](/developer-guide/plugins) — general plugin authoring
