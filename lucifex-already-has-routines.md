# Lucifex Agent Has Had "Routines" Since March
...
Lucifex equivalent — works today:
```bash
...
Lucifex equivalent — works today:
```bash
lucifex webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Lucifex implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Lucifex Agent |
|---|---|---|
...
## Things Lucifex Does That Routines Can't
...
Lucifex has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Lucifex Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
lucifex gateway setup    # enable webhooks
lucifex webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [lucifex-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://lucifex-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [lucifex-agent.nousresearch.com](https://lucifex-agent.nousresearch.com)

GitHub: [github.com/NousResearch/lucifex-agent](https://github.com/NousResearch/lucifex-agent)

---

*Lucifex Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
