...
# 5. Symlinks the 'lucifex' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Lucifex Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lucifex-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink lucifex into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lucifex command..."

LUCIFEX_BIN="$SCRIPT_DIR/venv/bin/lucifex"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LUCIFEX_BIN" "$COMMAND_LINK_DIR/lucifex"
echo -e "${GREEN}✓${NC} Symlinked lucifex → $COMMAND_LINK_DISPLAY_DIR/lucifex"
...
# ============================================================================
# Seed bundled skills into ~/.lucifex/skills/
# ============================================================================

LUCIFEX_SKILLS_DIR="${LUCIFEX_HOME:-$HOME/.lucifex}/skills"
mkdir -p "$LUCIFEX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.lucifex/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LUCIFEX_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  lucifex status        # Check configuration"
if is_termux; then
    echo "  lucifex gateway       # Run gateway in foreground"
else
    echo "  lucifex gateway install # Install gateway service (messaging + cron)"
fi
echo "  lucifex cron list     # View scheduled jobs"
echo "  lucifex doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lucifex_cli.main setup
fi
