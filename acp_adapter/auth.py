"""ACP auth helpers — detect and advertise Lucifex authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "lucifex-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Lucifex runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Lucifex can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lucifex.
    ...
    not have Lucifex provider credentials configured yet, so Lucifex always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Lucifex using the currently configured "
