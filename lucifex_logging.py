"""Centralized logging setup for Lucifex Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.lucifex/logs/`` (profile-aware via ``get_lucifex_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``lucifex_cli.web_server``,
    ``lucifex_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Lucifex (TUI, gateway, ``hy_memory`` server, MCP
...
from lucifex_constants import get_config_path, get_lucifex_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_lucifex_session_injector", False):
    ...
    _session_record_factory._lucifex_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``lucifex logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``lucifex logs --component gateway``.
    "gateway": ("gateway", "lucifex_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("lucifex_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "lucifex_cli.web_server",
        "lucifex_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Lucifex logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    lucifex_home
        Override for the Lucifex home directory.  Falls back to
        ``get_lucifex_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_lucifex_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._lucifex_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the lucifex group. However, both ``_open()`` (initial creation)
...
# the module header): when several Lucifex processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._lucifex_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="lucifex-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_lucifex_queue", False):
