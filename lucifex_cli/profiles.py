...
_PROFILE_DIRS = [
    ...
    # See lucifex_constants.get_subprocess_home().
    "home",
]
...
#
# Rationale per item:
#   lucifex-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_lucifex_home().resolve()
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "lucifex-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# (#58394). Add new artifacts here when introduced in ``lucifex_constants``.
...
_LUCIFEX_SUBCOMMANDS = frozenset({
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_lucifex_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LUCIFEX_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_lucifex_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _LUCIFEX_SUBCOMMANDS:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            lucifex_exe = shutil.which("lucifex") or "lucifex"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(lucifex_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        from lucifex_cli.config import check_config_version, migrate_config
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``LUCIFEX_HOME``.
    """
    try:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_lucifex_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, lucifex_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own LUCIFEX_HOME.
    ...
    The returned ``lucifex_home`` is the path to pass to
    ``set_lucifex_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_lucifex_home())]
...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from lucifex_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LUCIFEX_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_lucifex_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    lucifex_markers = ("lucifex_cli.main", "lucifex-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            is_lucifex = (
                any(marker in joined for marker in lucifex_markers)
                or exe_name == "lucifex"
            ...
            # ...or by LUCIFEX_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("LUCIFEX_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_lucifex_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``LUCIFEX_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from lucifex_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LUCIFEX_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LUCIFEX_HOME")
    try:
        os.environ["LUCIFEX_HOME"] = str(profile_dir)
        from lucifex_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LUCIFEX_HOME"] = old_home
        elif "LUCIFEX_HOME" in os.environ:
            del os.environ["LUCIFEX_HOME"]
...
def get_active_profile_name() -> str:
    ...
    resolved = lucifex_home.resolve()

    default_resolved = _get_default_lucifex_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="lucifex_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"lucifex_{old_name}"
    legacy_old_host = f"lucifex.{old_name}"
    new_host = f"lucifex_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_lucifex_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("lucifex_"):
