...
from lucifex_cli.dashboard_auth import list_session_providers
from lucifex_cli.dashboard_auth.audit import AuditEvent, audit_log
from lucifex_cli.dashboard_auth.base import (
...
from lucifex_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /lucifex``, the
    ``login_url`` is prefixed (``/lucifex/login?next=...``) so the
    ...
    """
    from lucifex_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from lucifex_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["lucifex.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``lucifex_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Lucifex portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from lucifex_cli.dashboard_auth.cookies import clear_session_cookies
        from lucifex_cli.dashboard_auth.prefix import prefix_from_request
