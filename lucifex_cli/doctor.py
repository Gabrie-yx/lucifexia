...
from lucifex_cli.config import get_project_root, get_lucifex_home, get_env_path
from lucifex_cli.env_loader import load_lucifex_dotenv
from lucifex_constants import display_lucifex_home
from lucifex_constants import agent_browser_runnable
...
from lucifex_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from lucifex_cli.auth import get_minimax_oauth_auth_status
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ("LUCIFEX_TOOL_PROGRESS", "display.tool_progress in config.yaml"),
    ("LUCIFEX_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from lucifex_cli import __version__ as init_version
    except Exception:
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from lucifex_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and lucifex_cli/__init__.py versions
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from lucifex_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from lucifex_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from lucifex_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from lucifex_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        ...
        try:
            import yaml
            from lucifex_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("Lucifex portal auth", "(logged in)")
        else:
            check_warn("Lucifex portal auth", "(not logged in)")
    ...
    if state_db_path.exists():
        try:
            ...
            from lucifex_state import _db_opens_cleanly, repair_state_db_schema
    ...
    try:
        from lucifex_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable LUCIFEX_HOME mirror
    ...
    _section("Skills Hub")
    hub_dir = LUCIFEX_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = LUCIFEX_HOME / "config.yaml"
    ...
    try:
        from lucifex_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
