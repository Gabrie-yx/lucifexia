...
# load_lucifex_dotenv() calls (user env + project env, gateway hot-reload,
...
# LUCIFEX_HOME paths we've already pulled external secrets for during this
# process.  ``load_lucifex_dotenv()`` is called at module-import time from
# several hot modules (cli.py, lucifex_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_lucifex_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which LUCIFEX_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from lucifex_cli.config import _sanitize_env_lines
    except ImportError:
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of LUCIFEX_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Lucifex reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_lucifex_dotenv()`` runs at import
    time from several hot modules (cli.py, lucifex_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``lucifex secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `lucifex model`
