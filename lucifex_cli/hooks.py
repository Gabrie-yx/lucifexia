"""lucifex hooks — inspect and manage shell-script hooks.

Usage::

    lucifex hooks list
    lucifex hooks test <event> [--for-tool X] [--payload-file F]
    lucifex hooks revoke <command>
    lucifex hooks doctor

Consent records live under ``~/.lucifex/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.lucifex/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``lucifex hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lucifex hooks {list|test|revoke|doctor}")
        print("Run 'lucifex hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.lucifex/config.yaml.")
        print("See `lucifex hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lucifex hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lucifex hooks test` and `lucifex hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lucifex wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lucifex hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lucifex hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lucifex hooks doctor`.")
