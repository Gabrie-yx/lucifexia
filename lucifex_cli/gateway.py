...
def _scan_gateway_pids(
    ...
    current_home = str(get_lucifex_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"lucifex_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. LUCIFEX_HOME
        ...
        # LUCIFEX_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "lucifex_home=" in command_lc
            and f"lucifex_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from lucifex_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "lucifex_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m lucifex_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose LUCIFEX_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from lucifex_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from lucifex_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / LUCIFEX_HOME) the windowless
        # base interpreter needs to import lucifex_cli.  Empty on POSIX, where
...
def _lucifex_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``LUCIFEX_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("LUCIFEX_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_lucifex_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = (_lucifex_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("LUCIFEX_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("LUCIFEX_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["LUCIFEX_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from lucifex_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: lucifex gateway stop")
    else:
        print(
...
def _gateway_list() -> None:
    ...
    """
    try:
        from lucifex_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (lucifex_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (LUCIFEX_HOME-scoped).
...
from lucifex_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``lucifex gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``LUCIFEX_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("LUCIFEX_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current LUCIFEX_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (LUCIFEX_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from lucifex_constants import get_default_lucifex_root

    home = get_lucifex_home().resolve()
    default = get_default_lucifex_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary LUCIFEX_HOME paths
...
def _profile_arg(lucifex_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    home = Path(lucifex_home or str(get_lucifex_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_lucifex_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(lucifex_home: str, target_home_dir: str) -> str:
    ...
    try:
        ...
        return _profile_arg(lucifex_home)
...
def _find_legacy_lucifex_units() -> list[tuple[str, Path, bool]]:
...
def has_legacy_lucifex_units() -> bool:
    """Return True when any legacy Lucifex gateway unit files exist."""
    return bool(_find_legacy_lucifex_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """
    legacy = _find_legacy_lucifex_units()
    if not legacy:
        return
...
def remove_legacy_lucifex_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Lucifex gateway unit files.

    Iterates over whatever ``_find_legacy_lucifex_units()`` returns — which is
...
def _lucifex_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current LUCIFEX_HOME to the equivalent under a target user's home.
    ...
    """
    current_lucifex_raw = os.environ.get("LUCIFEX_HOME", "").strip()
    current_lucifex = (
        Path(current_lucifex_raw).expanduser()
        if current_lucifex_raw
        else get_lucifex_home()
    )
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    lucifex_node = lucifex_home / "node" / "bin"
    if _is_dir(lucifex_node):
        candidates.append(str(lucifex_node))
    lucifex_nm = lucifex_home / "node_modules" / ".bin"
    if _is_dir(lucifex_nm):
        candidates.append(str(lucifex_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m lucifex_cli.main``, so module
    ...
    ``LUCIFEX_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if LUCIFEX_HOME
...
def _systemd_watchdog_seconds(lucifex_home: str | Path | None = None) -> int:
    ...
    if lucifex_home is not None:
        ...
        override_token = set_lucifex_home_override(lucifex_home)
        reset_home_override = reset_lucifex_home_override
    try:
...
def _systemd_watchdog_service_fields(
    lucifex_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(lucifex_home)
    if seconds <= 0:
        return "simple", ""
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        lucifex_home = _lucifex_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            lucifex_home
        )
        profile_arg = _profile_arg_for_target_user(lucifex_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's LUCIFEX_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(lucifex_home) if lucifex_home else _remap_path_for_user(working_dir, home_dir)
...
Group={group_name}
ExecStart={python_path} -m lucifex_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="LUCIFEX_HOME={lucifex_home}"
Restart=always
RestartSec=5
...
"""
    ...
    lucifex_home = str(get_lucifex_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        lucifex_home
    )
    profile_arg = _profile_arg(lucifex_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m lucifex_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="LUCIFEX_HOME={lucifex_home}"
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir LUCIFEX_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved LUCIFEX_HOME
    ...
    exported a throwaway ``LUCIFEX_HOME=/tmp/...`` — writing it to the real
    ...
    Matches both systemd ``Environment="LUCIFEX_HOME=..."`` lines and launchd
    ``<key>LUCIFEX_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'LUCIFEX_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>LUCIFEX_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp LUCIFEX_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: LUCIFEX_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported LUCIFEX_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single LUCIFEX_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only LUCIFEX_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp LUCIFEX_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../lucifex_test`` path into
    # ``Environment="LUCIFEX_HOME=..."``. Writing that to the developer's
...
def systemd_install(
    ...
    if has_legacy_lucifex_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_lucifex_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin LUCIFEX_HOME; adopt it before any
...
def systemd_start(system: bool = False):
    ...
    # LUCIFEX_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}lucifex gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}lucifex gateway start{scope_flag}"
        )
...
# process invocation (each ``lucifex gateway start/stop/status`` call).
...
#      background process (the `nohup lucifex gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_lucifex_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_lucifex_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m lucifex_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active LUCIFEX_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "lucifex_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup lucifex gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup lucifex gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>lucifex_cli.main</string>",
    ]
    if profile_arg:
...
def launchd_stop():
    ...
    # `lucifex gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: lucifex gateway stop")
            else:
    ...
    if deep:
        log_file = get_lucifex_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``LUCIFEX_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``lucifex gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``lucifex gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``lucifex gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other LUCIFEX_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("LUCIFEX_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$LUCIFEX_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set LUCIFEX_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `lucifex gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set LUCIFEX_GATEWAY_DETACHED=1; older wrappers
    ...
    # LUCIFEX_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("LUCIFEX_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``LUCIFEX_GATEWAY_MAX_STARTS`` /
    # ``LUCIFEX_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("LUCIFEX_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("LUCIFEX_GATEWAY_START_WINDOW_S")
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_lucifex_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from lucifex_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_lucifex_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "LucifexAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``lucifex_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from lucifex_cli import setup as _s
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info("  Run in foreground: lucifex gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup lucifex gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`lucifex_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from lucifex_cli.service_manager import (
    ...
    if profile is None:
        ...
        # LUCIFEX_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``lucifex gateway stop --all`` and ``... restart --all``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``LUCIFEX_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``LUCIFEX_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("LUCIFEX_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("LUCIFEX_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${LUCIFEX_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if LUCIFEX_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or LUCIFEX_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from lucifex_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
            ...
            print()
            print("To run the gateway: lucifex gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: lucifex gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: lucifex gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: lucifex gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_LUCIFEX_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `lucifex gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: lucifex gateway start")
                sys.exit(1)
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_lucifex_units(interactive=not yes, dry_run=dry_run)
