...
# coverage category (lucifex_cli.lucifex_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
# consistent with the per-category feature gates in get_lucifex_subscription_features.
...
class NousSubscriptionFeatures:
    subscribed: bool
    lucifex_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, NousFeatureState]
    account_info: Optional[LucifexportalAccountInfo] = None
...
def get_lucifex_subscription_features(
    ...
    *,
    ...
    lucifex_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_nous or lucifex_auth_present
    ...
    return NousSubscriptionFeatures(
        ...
        lucifex_auth_present=lucifex_auth_present,
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    # Quick provider check without the heavy get_lucifex_subscription_features call
    model_cfg = config.get("model")
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from lucifex_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_lucifex_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
...
# ---------------------------------------------------------------------------
# Inline Lucifex portal login for the Tool Gateway picker (`lucifex tools`)
# ---------------------------------------------------------------------------


def ensure_lucifex_portal_access(
    *,
    ...
    backend (e.g. "Firecrawl (Lucifex portal)").  Unlike ``lucifex model``'s Nous
    login, this:
    ...
    It only performs the Lucifex portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_lucifex_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_lucifex_portal_login_only(capability=capability):
            return False
        try:
            info = get_lucifex_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_lucifex_portal_login_only(*, capability: str) -> bool:
    """Run the Lucifex portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start Lucifex portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Lucifex portal login.")
    try:
        proceed = input("  Log in to Lucifex portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Lucifex portal login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  Lucifex portal login successful.")
        return True
        ...
        print(f"  Lucifex portal login failed: {exc}")
        return False
