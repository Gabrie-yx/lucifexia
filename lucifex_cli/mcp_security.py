...
2. The persistence shape from the June 2026 ``lucifex-0day`` campaign: a shell
   ...
   whose payload appended an attacker SSH key to ``authorized_keys``; Lucifex
...
3. A hardcoded indicator-of-compromise (IOC) blocklist for that campaign — the
   attacker's ``lucifex-0day`` SSH public key and source IPs. Any entry whose
...
# A shell payload that touches any of these is the June 2026 lucifex-0day shape
...
# ── Indicators of compromise: June 2026 lucifex-0day campaign ──────────────────
...
# multiple compromised public instances (r/lucifexagent, 854.media).
_IOC_SUBSTRINGS = (
    # Attacker SSH public key (the "lucifex-0day" persistence key).
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a known lucifex-0day IOC anywhere in command/args/env (hardcoded blocklist);
    ...
    * a shell interpreter whose inline script writes to an OS persistence
      surface (June 2026 lucifex-0day SSH/PAM/sudoers/cron shape).
    """
    ...
    for ioc in _IOC_SUBSTRINGS:
        if ioc in flat:
            issues.append(
                f"MCP server '{name}' contains a known lucifex-0day "
    ...
    if _PERSISTENCE_PATTERN.search(script):
        issues.append(
            ...
            f"shell rc) — this is the lucifex-0day backdoor shape, not a real "
            f"MCP server"
        )
