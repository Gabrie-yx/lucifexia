...
``$LUCIFEX_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$LUCIFEX_HOME``,
    not under ``profiles/``). The dispatcher in ``lucifex_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``lucifex gateway start`` (no ``-p``) targets. Without it,
    bare ``lucifex gateway start`` inside the container would land on
    ...
    ``$LUCIFEX_HOME/gateway_state.json`` (sibling to the profile root,
...
def main() -> int:
    ...
    # bind-mounted LUCIFEX_HOME, both race to flock() the same s6-log lock
