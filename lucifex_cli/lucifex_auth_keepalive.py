"""Background keepalive for long-lived Lucifex portal sessions."""
...
LUCIFEX_AUTH_KEEPALIVE_INTERVAL_SECONDS = 6 * 60 * 60
LUCIFEX_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS = 60
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("LUCIFEX_NOUS_TIMEOUT_SECONDS", "15"))
...
def refresh_lucifex_auth_keepalive_once(
    *,
...
def _keepalive_loop(
    ...
    *,
    ...
    while not stop_event.is_set():
        refresh_lucifex_auth_keepalive_once(
            min_key_ttl_seconds=min_key_ttl_seconds,
...
def start_lucifex_auth_keepalive(
    *,
    interval_seconds: int = LUCIFEX_AUTH_KEEPALIVE_INTERVAL_SECONDS,
    initial_delay_seconds: int = LUCIFEX_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = NOUS_INVOKE_JWT_MIN_TTL_SECONDS,
    timeout_seconds: Optional[float] = None,
...
def stop_lucifex_auth_keepalive(timeout: float = 5.0) -> None:
