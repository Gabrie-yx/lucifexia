...
``cli.py``'s ``LucifexCLI`` class. ``LucifexCLI`` inherits
...
Import discipline mirrors ``lucifex_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_lucifex_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors LUCIFEX_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `lucifex portal` and enable terminal billing, then re-run /subscription.")
            return
        confirm_choices = [
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `lucifex portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `lucifex portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `lucifex portal` and enable terminal billing, then retry.")
            return
        confirm_choices = [
            ("yes", "Enable terminal billing", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
