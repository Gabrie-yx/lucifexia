...
from lucifex_constants import get_default_lucifex_root, get_lucifex_home, display_lucifex_home
...
# ``lucifex-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/lucifex-agent/``
...
# LUCIFEX_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "lucifex-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under LUCIFEX_HOME) —
...
#     stuck "starting"/"cooking", disconnecting it from the Lucifex portal
...
# Reserved archive subtree for provider state that lives OUTSIDE LUCIFEX_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside LUCIFEX_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``lucifex-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/lucifex-agent/`` — must be preserved.
        if part == "lucifex-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside LUCIFEX_HOME.
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_lucifex_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(lucifex_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "lucifex-agent" and not is_root)
        ]
    ...
    # outside LUCIFEX_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_lucifex_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_lucifex_home()}")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under LUCIFEX_HOME.
            ...
            # Lucifex portal. Matched by basename so both the root profile and
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_lucifex_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: lucifex profile list  (after installing lucifex)")
...
_QUICK_STATE_FILES = (
    ...
    # are at $LUCIFEX_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside LUCIFEX_HOME and the entry is silently
...
# Relative path of the cron job database inside LUCIFEX_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_lucifex_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    but writes to ``<LUCIFEX_HOME>/backups/pre-update-<timestamp>.zip`` and
