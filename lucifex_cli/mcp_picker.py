"""MCP picker — interactive `lucifex mcp picker` (also the default `lucifex mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``lucifex mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `lucifex plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from lucifex_cli.cli_output import prompt_yes_no
from lucifex_cli.curses_ui import curses_single_select
from lucifex_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Lucifex session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from lucifex_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `lucifex mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: lucifex mcp install <name>    Picker: lucifex mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Lucifex is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Lucifex — run `lucifex update` "
...
def show_catalog() -> None:
    """`lucifex mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`lucifex mcp picker` (and default `lucifex mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`lucifex mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `lucifex mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
