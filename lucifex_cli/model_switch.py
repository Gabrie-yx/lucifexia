...
- ``lucifex_cli.providers``        -- canonical provider identity + overlays
- ``lucifex_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from lucifex_cli.model_switch import
...
def switch_model(
    ...
    # =================================================================
        ...
        try:
            from lucifex_cli.config import load_config
            from lucifex_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from lucifex_cli.providers import resolve_user_provider as _ruser
    ...
    # lucifex_cli.runtime_provider.resolve_runtime_provider; without the strip,
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from lucifex_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from lucifex_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against lucifex_id / mdev_id (section 1), pid / lucifex_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from lucifex_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for lucifex_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if mdev_id in seen_mdev_ids:
            continue
        if lucifex_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if not is_runtime_provider_routable(lucifex_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(lucifex_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        lucifex_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        if isinstance(user_providers, dict):
            configured = user_providers.get(lucifex_id)
    ...
    for pid, overlay in LUCIFEX_OVERLAYS.items():
        ...
        if lucifex_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or lucifex_slug.lower() in _excluded:
            continue
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(lucifex_slug):
                    has_creds = True
                elif for_picker:
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from lucifex_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`lucifex_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from lucifex_cli.models import fetch_openrouter_models
