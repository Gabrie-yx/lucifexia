...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("lucifex_cli.main")
    builder = getattr(module, builder_name)
...
class HermesConsoleEngine:
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Lucifex component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Lucifex logs.", _logs)
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            "version": (
                "lucifex_cli.subcommands.version",
            ...
            "prompt-size": (
                "lucifex_cli.subcommands.prompt_size",
            ...
            "insights": (
                "lucifex_cli.subcommands.insights",
            ...
            "security": (
                "lucifex_cli.subcommands.security",
            ...
            "backup": (
                "lucifex_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "lucifex_cli.subcommands.import_cmd",
            ...
            "config": (
                "lucifex_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "plugins": (
                "lucifex_cli.subcommands.plugins",
            ...
            "skills": (
                "lucifex_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "lucifex_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "lucifex_cli.subcommands.pairing",
            ...
            "webhook": (
                "lucifex_cli.subcommands.webhook",
            ...
            "profile": (
                "lucifex_cli.subcommands.profile",
        ...
        self.register(
            ...
            _adder_handler("send", (), "lucifex_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("lucifex_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "lucifex_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("lucifex_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("lucifex_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from lucifex_cli.logs import list_logs, tail_log
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from lucifex_cli.config import set_config_value
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from lucifex_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from lucifex_cli.cron import cron_list
