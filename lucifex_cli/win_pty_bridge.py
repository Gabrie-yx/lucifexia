"""Windows ConPTY bridge for the `lucifex dashboard` chat tab.

Drop-in counterpart to ``lucifex_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``lucifex_cli.web_server`` consumes: ``spawn``, ``read``,
