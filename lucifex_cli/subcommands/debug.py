"""``lucifex debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Lucifex Agent. Use 'lucifex debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lucifex debug share              Upload debug report (asks for confirmation)
    lucifex debug share --yes        Skip confirmation (for scripts/CI)
    lucifex debug share --lines 500  Include more log lines
    lucifex debug share --expire 30  Keep paste for 30 days
    lucifex debug share --local      Print report locally (no upload)
    lucifex debug share --no-redact  Disable upload-time secret redaction
    lucifex debug share --nous       Upload to Nous-internal storage (private)
    lucifex debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'lucifex debug share'",
    )
