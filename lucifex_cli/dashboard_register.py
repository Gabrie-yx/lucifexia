...
This command:
  ...
  3. Writes ``LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``LUCIFEX_DASHBOARD_PORTAL_URL`` into ``~/.lucifex/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``LUCIFEX_DASHBOARD_PORTAL_URL`` env (used for testing against a
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``lucifex dashboard register`` idempotent. The portal
...
def _print_post_register_hint(
    *,
    ...
    from lucifex_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    LUCIFEX_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    LUCIFEX_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `lucifex dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    lucifex dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    # stamped in by the orchestrator (NAS sets LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `lucifex dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    # LUCIFEX_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      LUCIFEX_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("LUCIFEX_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("LUCIFEX_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://lucifex.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking LUCIFEX_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as LUCIFEX_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("LUCIFEX_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("LUCIFEX_DASHBOARD_PUBLIC_URL", public_url)
