...
from lucifex_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from lucifex_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``LUCIFEX_NONINTERACTIVE=1`` (see ``lucifex_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("LUCIFEX_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``LUCIFEX_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from lucifex_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, lucifex_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from lucifex_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {lucifex_home}/cron/, sessions/, logs/"
    )
    print()
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into LUCIFEX_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("LUCIFEX_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # LUCIFEX_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from lucifex_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_lucifex_home(Path(get_lucifex_home()))
    except Exception:
        pass
...
def _profile_name_from_lucifex_home(lucifex_home) -> str | None:
    """Return the active profile name when LUCIFEX_HOME is a profile dir."""
    if lucifex_home.parent.name == "profiles":
        return lucifex_home.name
    return None
...
def _setup_qqbot():
    ...
    from lucifex_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
...
def setup_gateway(config: dict):
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_lucifex_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("   lucifex gateway run          # Run as container main process")
                print_info("")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``lucifex_cli.auth`` — lists every supported
    ...
    """
    try:
        from lucifex_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from lucifex_cli.config
    ...
    if section_key == "model":
        ...
        from lucifex_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_lucifex migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_lucifex", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(lucifex_home: Path) -> bool:
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, lucifex_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(lucifex_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, lucifex_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, lucifex_home, is_existing)
            return
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from lucifex_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_blank_slate_setup(config: dict, lucifex_home, is_existing: bool):
    ...
    _blank_slate_walkthrough(config, lucifex_home)


def _blank_slate_walkthrough(config: dict, lucifex_home):
...
def _run_quick_setup(config: dict, lucifex_home):
