...
from lucifex_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this LUCIFEX_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "HERMES_KANBAN_BOARD env var). Use `lucifex kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `lucifex gateway start`.",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # LUCIFEX_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh LUCIFEX_HOME that has never called ``kanban init``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `lucifex kanban complete <id>` from the terminal tool and
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from lucifex_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from lucifex_cli import kanban_decompose as decomp
