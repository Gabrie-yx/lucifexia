"""CLI handlers for the ``lucifex proxy`` subcommand."""
...
from lucifex_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "lucifex proxy requires aiohttp. Install one of:\n"
        "  pip install 'lucifex-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"lucifex auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Lucifex proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Lucifex proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: lucifex proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``lucifex proxy <subcommand>``."""
    ...
    print(
        "lucifex proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  lucifex proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
