"""Lucifex portal upstream adapter.
...
class LucifexportalAdapter(UpstreamAdapter):
    """Proxy upstream for the Lucifex portal inference API."""
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Lucifex portal. Run `lucifex auth add nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh Lucifex portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "Lucifex portal refresh did not return a usable inference JWT. "
    ...
    # to lucifex_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
