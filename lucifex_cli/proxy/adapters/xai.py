...
# xAI's public API is OpenAI-compatible for the endpoints Lucifex commonly
# uses. The Responses endpoint is included because Lucifex' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Lucifex-managed OAuth credentials."""

    auth_hint = "lucifex auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`lucifex auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`lucifex auth reset xai-oauth` or re-authenticate with "
                    "`lucifex auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `lucifex auth add xai-oauth --type oauth`."
            )
