"""Normalized Lucifex portal account entitlement helpers."""
...
_account_info_cache: tuple[str, float, "LucifexportalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class LucifexportalSubscriptionInfo:
...
@dataclass(frozen=True)
class LucifexportalAccountInfo:
    logged_in: bool
    ...
    subscription: Optional[LucifexportalSubscriptionInfo] = None
...
def lucifex_portal_billing_url(account_info: Optional[LucifexportalAccountInfo] = None) -> str:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_LUCIFEX_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def lucifex_portal_topup_url(account_info: Optional[LucifexportalAccountInfo] = None) -> str:
    ...
    """
    base_billing = lucifex_portal_billing_url(account_info)  # {base}/billing
...
def format_lucifex_portal_entitlement_message(
    account_info: Optional[LucifexportalAccountInfo],
    *,
    ...
    """
    billing_url = lucifex_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Lucifex portal "
    ...
    if account_info is None:
        return (
            f"Lucifex could not verify your Lucifex portal entitlement, so {capability} "
    ...
    if reason == "account_missing":
        return (
            f"Lucifex could not find a Lucifex portal account or organisation for this "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `lucifex model` to refresh Lucifex."
        return message

    return (
        f"Your Lucifex portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Lucifex portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Lucifex portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Lucifex portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Lucifex portal account has no usable paid credits{credit_detail}, so "
...
def _fresh_account_info(
    *,
    ...
    try:
        from lucifex_cli.auth import get_provider_auth_state, resolve_nous_access_token
        ...
        payload = _fetch_lucifex_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[LucifexportalAccountInfo]:
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_lucifex_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _fetch_lucifex_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from lucifex_cli.auth import _decode_jwt_claims
    except Exception:
        return None
...
def _subscription_from_payload(value: Any) -> Optional[LucifexportalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return LucifexportalSubscriptionInfo(
