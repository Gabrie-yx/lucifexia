"""`lucifex checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.lucifex/checkpoints/``.  Actions:

    lucifex checkpoints               # same as `status`
    lucifex checkpoints status        # total size, project count, breakdown
    lucifex checkpoints list          # per-project checkpoint counts + workdir
    lucifex checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lucifex checkpoints clear [-f]    # nuke the entire base (asks first)
    lucifex checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lucifex checkpoints
    lucifex checkpoints prune --retention-days 3 --max-size-mb 200
    lucifex checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lucifex checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lucifex checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lucifex checkpoints` → status
