"""Lucifex portal terminal-billing HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``lucifex_cli/lucifex_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Lucifex already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``LUCIFEX_PORTAL_BASE_URL`` → ``LUCIFEX_PORTAL_BASE_URL`` → the
...
# lucifex_cli.auth.LUCIFEX_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``LUCIFEX_PORTAL_BASE_URL`` → ``LUCIFEX_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("LUCIFEX_PORTAL_BASE_URL") or os.getenv("LUCIFEX_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Lucifex portal — run `lucifex portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Lucifex portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Lucifex portal: timed out", error="network_error"
        ) from exc
