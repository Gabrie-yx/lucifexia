...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains lucifex_cli reference)
                content = wrapper.read_text()
                if 'lucifex_cli' in content or 'lucifex-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lucifex gateway run`` processes
    ...
    try:
        from lucifex_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from lucifex_cli.gateway import get_launchd_plist_path
...
def _uninstall_profile(profile) -> None:
    ...
    # 3. Wipe the profile's LUCIFEX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    """
    from lucifex_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def run_uninstall(args):
    ...
    # their alias wrappers, and wiping their LUCIFEX_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, lucifex_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Lucifex PATH entries from shell configs / Windows User PATH")
    print("  • Lucifex wrapper scripts and Lucifex-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Lucifex config/data: {lucifex_home}")
        ...
        print(f"  • Keep Lucifex config/data: {lucifex_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://lucifex-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://lucifex-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m lucifex_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import lucifex_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``lucifex_constants`` + ``lucifex_cli.colors``
    (and lazily ``lucifex_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m lucifex_cli.uninstall")
    parser.add_argument(
        "--mode",
