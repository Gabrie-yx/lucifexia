...
the tool registry or launching Lucifex (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``lucifex-cli`` to the non-existent ``lucifex``. ``resolve_toolset('lucifex')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``lucifex-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``lucifex`` instead of ``lucifex-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `lucifex tools` to reconfigure."
        )
    return warnings
