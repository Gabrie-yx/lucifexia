...
from lucifex_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_lucifex_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from lucifex_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return LucifexTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # lucifex_cli/main.py for why the dest is renamed.
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_lucifex_home()}/config.yaml ({tool_count}/{total} tools enabled)")
