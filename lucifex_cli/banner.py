...
def _fetch_pypi_latest(package: str = "lucifex-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    if embedded_rev:
        ...
        # $LUCIFEX_HOME/lucifex-agent/ may be a stale copy from --clone-all;
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``lucifex_cli/build_info.py``) and return it as a frozen
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else LUCIFEX_CADUCEUS
    except Exception:
        _bskin = None
        _hero = LUCIFEX_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from lucifex_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from lucifex_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
