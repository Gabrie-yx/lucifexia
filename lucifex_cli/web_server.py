...
from lucifex_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _spawn_lucifex_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "LUCIFEX_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _recent_upstream_commits(n: int = 20) -> List[Dict[str, Any]]:
    """Always return empty list to prevent commit log/hash leakage in any UI."""
    return []
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `lucifex model`
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        _LUCIFEX_OAUTH_FILE = None  # type: ignore
    ...
    if lucifex_creds and lucifex_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lucifex_pkce",
            "source_label": f"Lucifex PKCE ({_LUCIFEX_OAUTH_FILE})",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _LUCIFEX_OAUTH_FILE
                if _LUCIFEX_OAUTH_FILE.exists():
                    _LUCIFEX_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _LUCIFEX_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _LUCIFEX_OAUTH_FILE.with_name(
        f"{_LUCIFEX_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _LUCIFEX_OAUTH_FILE)
        try:
            _LUCIFEX_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `lucifex sessions prune`)."""
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process LUCIFEX_HOME at import time. The dashboard is a single
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from lucifex_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``lucifex mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        ...
        # resolve LUCIFEX_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = LucifexTokenStorage(name)
...
# `lucifex memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `lucifex memory setup` to configure a new one.",
            )
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``lucifex tools`` picker uses, so the GUI and CLI stay in
...
def _resolve_chat_argv(
    ...
    """
    from lucifex_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_lucifex_root_is_opt_data() else "local"
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_lucifex_home() / "dashboard-themes"
...
def start_server(
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"LUCIFEX_DASHBOARD_READY port={actual_port}", flush=True)
