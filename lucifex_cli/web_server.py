"""
Lucifex Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m lucifex_cli.main web          # Start on http://127.0.0.1:9119
    python -m lucifex_cli.main web --port 8080
"""
...
from lucifex_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from lucifex_cli import __version__, __release_date__
...
try:
    ...
    # running `lucifex dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["LUCIFEX_WEB_DIST"]) if "LUCIFEX_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``lucifex gateway run`` — but the
    desktop app spawns a ``lucifex dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same LUCIFEX_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import lucifex_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from lucifex_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire lucifex_cli.gateway import into a background thread so the event
    # loop is not blocked and LUCIFEX_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (LUCIFEX_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `lucifex
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("LUCIFEX_DESKTOP") == "1":
...
app = FastAPI(title="Lucifex Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from lucifex_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# LUCIFEX_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("LUCIFEX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Lucifex-Session-Token"
...
#
# This list is defined in ``lucifex_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from lucifex_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Lucifex-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``lucifex-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from lucifex_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from lucifex_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from lucifex_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Lucifex (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during lucifex update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``lucifex model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Lucifex provider slug — fine.
    ...
    1. Vendor-name → Lucifex-provider mapping: when the provider string is
       not a known Lucifex provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from lucifex_cli.config import get_compatible_custom_providers
    from lucifex_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from lucifex_cli.model_normalize import normalize_model_for_provider
    from lucifex_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from lucifex_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Lucifex process is writing to it.
    """
    from lucifex_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "LUCIFEX_DASHBOARD_FILES_ROOT"
...
# LUCIFEX_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to LUCIFEX_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Lucifex credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no LUCIFEX_HOME resolution, so it blocks these trees wherever
...
def _default_lucifex_root_is_opt_data() -> bool:
    raw = os.environ.get("LUCIFEX_HOME", "").strip()
    if not raw:
        return False
    try:
        from lucifex_constants import get_default_lucifex_root

        root = get_default_lucifex_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``lucifex update``.
    ...
    a container — e.g. the lucifex-webui image sharing the Lucifex source tree),
    the dashboard's ``lucifex update`` button is the correct update path and
    ...
    """
    if _default_lucifex_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Lucifex root is actually /opt/data
    # (the container/hosted layout) or when LUCIFEX_DASHBOARD_FILES_ROOT is set.
    if _default_lucifex_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Lucifex inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``lucifex:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.lucifex-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``lucifex_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from lucifex_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from lucifex_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_lucifex_home() at call time
    ...
    try:
        ...
        # Windows install the first import of lucifex_cli.gateway blocks the
        ...
        # and which providers are registered so ``lucifex status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from lucifex_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from lucifex_cli.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_lucifex": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Lucifex Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the lucifex home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_lucifex_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_lucifex_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_lucifex_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_lucifex_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_lucifex_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from lucifex_cli.debug import build_debug_share
...
_ACTION_LOG_DIR: Path = get_lucifex_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "lucifex-update": "lucifex-update.log",
...
def _spawn_lucifex_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lucifex <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``lucifex_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "lucifex_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _LUCIFEX_GATEWAY=1. Inheriting it makes a spawned `lucifex gateway restart`
    ...
    action_env = {**os.environ, "LUCIFEX_NONINTERACTIVE": "1"}
    action_env.pop("_LUCIFEX_GATEWAY", None)
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["lucifex", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``lucifex gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``lucifex gateway restart`` children race each other on the
    ...
    return _spawn_lucifex_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``lucifex gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``LUCIFEX_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/lucifex/update")
async def update_lucifex():
    """Kick off ``lucifex update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Lucifex updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("lucifex-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "lucifex-update",
    ...
    try:
        proc = _spawn_lucifex_action(["update"], "lucifex-update")
    except Exception as exc:
        _log.exception("Failed to spawn lucifex update")
...
@app.get("/api/lucifex/update/check")
async def check_lucifex_update(force: bool = False):
    """Report whether a Lucifex update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/lucifex/update`` actually runs ``lucifex update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Lucifex version string
    ...
    try:
        from lucifex_cli.banner import check_for_updates

        if force:
            try:
                (get_lucifex_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Lucifex surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Lucifex supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_lucifex_home() / provider.name / "config.json"
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        lucifex_home / f"{name}.json",
        lucifex_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_lucifex_home()))
            return
...
# in lucifex_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from lucifex_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `lucifex model` does so GUI onboarding lands on a
    ...
    try:
        from lucifex_cli.inventory import build_models_payload, load_picker_context
        from lucifex_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from lucifex_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from lucifex_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from lucifex_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (lucifex_cli/main.py
        ...
        # endpoint, mirroring the ``lucifex model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from lucifex_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``lucifex model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from lucifex_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from lucifex_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `lucifex model` provider — would be
        ...
        # Vertex — a `lucifex model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `lucifex model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from lucifex_cli.credential_lifecycle import save_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Lucifex from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Lucifex to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Lucifex from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Lucifex to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Lucifex in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Lucifex through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Lucifex via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Lucifex through an IMAP/SMTP mailbox.",
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Lucifex to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Lucifex inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Lucifex to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Lucifex through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Lucifex to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Lucifex to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Lucifex as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in lucifex gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in lucifex gateway setup",
...
def _whatsapp_session_path() -> Path:
    ...
    return get_lucifex_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.lucifex-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"LucifexDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Lucifex Agent").strip() or "Lucifex Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Lucifex appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's LUCIFEX_HOME.
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized LUCIFEX_HOME)
...
# the canonical ``lucifex auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.lucifex/.anthropic_oauth.json`` — Lucifex-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_lucifex_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_lucifex_oauth_file,
        )
    except ImportError:
        read_lucifex_oauth_credentials = None  # type: ignore
        _get_lucifex_oauth_file = None  # type: ignore

    lucifex_creds = None
    if read_lucifex_oauth_credentials:
        try:
            lucifex_creds = read_lucifex_oauth_credentials()
        except Exception:
            lucifex_creds = None
    if lucifex_creds and lucifex_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lucifex_pkce",
            "source_label": f"Lucifex PKCE ({_get_lucifex_oauth_file() if _get_lucifex_oauth_file else None})",
            "token_preview": _truncate_token(lucifex_creds.get("accessToken")),
            "expires_at": lucifex_creds.get("expiresAt"),
            "has_refresh_token": bool(lucifex_creds.get("refreshToken")),
        }
    ...
    try:
        from lucifex_cli.env_loader import format_secret_source_suffix
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Lucifex even
    when they also have a separate Lucifex-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Lucifex never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lucifex auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "lucifex auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lucifex auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "lucifex auth add xai-oauth",
        "docs_url": "https://lucifex-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "lucifex auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Lucifex, so the disconnect
    ...
    user sees exactly what executes, and Lucifex then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Lucifex — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``lucifex model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `lucifex model` universe (zero-edit for new plugins).
    try:
        from lucifex_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"lucifex auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("lucifex_pkce", "claude_code", ...)
    ...
    sync with the `lucifex model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Lucifex-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_lucifex_oauth_file
                oauth_file = _get_lucifex_oauth_file()
            ...
            try:
                from lucifex_cli.auth import clear_provider_auth
...
# Guarded so lucifex web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Lucifex file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``lucifex auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_lucifex_oauth_file
    oauth_file = _get_lucifex_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "lucifex-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from lucifex_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from lucifex_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``lucifex auth add minimax-oauth``.
    """
    from lucifex_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``lucifex auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``lucifex model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Lucifex "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from lucifex_cli.auth import _save_codex_tokens
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Lucifex backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `lucifex sessions stats`).
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `lucifex sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_lucifex_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from lucifex_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_lucifex_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from lucifex_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (LUCIFEX_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.lucifex``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    LUCIFEX_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, LUCIFEX_HOME)."""
    from lucifex_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["lucifex_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from lucifex_constants import (
        reset_lucifex_home_override,
        set_lucifex_home_override,
    )

    token = set_lucifex_home_override(str(home))
    try:
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Lucifex home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (lucifex_cli.mcp_config), so
# servers managed here show up under `lucifex mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from lucifex_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from lucifex_cli.mcp_security import validate_mcp_server_entry
...
async def list_mcp_servers(profile: Optional[str] = None):
    from lucifex_cli.mcp_config import _get_mcp_servers
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from lucifex_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from lucifex_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the LUCIFEX_HOME
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from lucifex_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.lucifex_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from lucifex_constants import reset_lucifex_home_override, set_lucifex_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import LucifexTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_lucifex_home_override(flow.lucifex_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.lucifex_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = LucifexTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        lucifex_home=flow.lucifex_home,
                    )
            ...
            reset_lucifex_home_override(home_token)
    except Exception as exc:
        msg = str(exc)
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_lucifex_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_lucifex_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.lucifex_home == flow_home
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Lucifex and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Lucifex for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Lucifex.</p>")
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds LUCIFEX_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (lucifex_cli.webhook); the webhook
...
async def list_webhooks():
    import lucifex_cli.webhook as wh
...
# spawn the real `lucifex gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_lucifex_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_lucifex_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from lucifex_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `lucifex auth remove`) can seed again.
        # Mirrors the `lucifex auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``lucifex auth remove`` uses: each source cleans up its
    ...
    from lucifex_cli.auth import suppress_credential_source
...
async def get_memory_status():
    ...
    mem_dir = get_lucifex_home() / "memories"
    files = {}
...
async def run_doctor():
    try:
        proc = _spawn_lucifex_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_lucifex_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_lucifex_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"lucifex-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_lucifex_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `lucifex import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_lucifex_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from lucifex_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from lucifex_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <lucifex_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_lucifex_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_lucifex_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``lucifex`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_lucifex_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from lucifex_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`lucifex -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the LUCIFEX_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_lucifex_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lucifex setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local LUCIFEX_HOME override so the write lands in the target
    ...
    """
    from lucifex_constants import set_lucifex_home_override, reset_lucifex_home_override

    token = set_lucifex_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local LUCIFEX_HOME override (same mechanism as
    ...
    """
    from lucifex_constants import set_lucifex_home_override, reset_lucifex_home_override
    from lucifex_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the LUCIFEX_HOME override. Returns the
    number of skills newly disabled.
    """
    from lucifex_constants import set_lucifex_home_override, reset_lucifex_home_override
    from lucifex_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's LUCIFEX_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``lucifex profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from LUCIFEX_HOME).
    """
    from lucifex_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``lucifex profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the LUCIFEX_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``lucifex profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from lucifex_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_lucifex_home()`` at call
       time — the context-local override from ``set_lucifex_home_override``
    ...
    retargeted to the *current* ``get_lucifex_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a LUCIFEX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_lucifex_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_lucifex_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from lucifex_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from lucifex_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``lucifex tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `lucifex tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``lucifex tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``lucifex tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``lucifex tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``lucifex -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same LUCIFEX_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Lucifex,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``lucifex computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``lucifex --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from lucifex_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from lucifex_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from lucifex_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from lucifex_cli.dashboard_auth.audit import AuditEvent, audit_log
        from lucifex_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``lucifex --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``LUCIFEX_TUI_RESUME`` env var —
    matching what ``lucifex_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``LUCIFEX_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``LUCIFEX_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``LUCIFEX_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``LUCIFEX_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_lucifex_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``lucifex -p <name>`` performs. The in-process
    ``LUCIFEX_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from lucifex_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    env = os.environ.copy()
    try:
        from lucifex_cli.config import apply_terminal_config_to_env
    ...
    env.setdefault("LUCIFEX_TUI_DISABLE_MOUSE", "1")
    env.setdefault("LUCIFEX_TUI_INLINE", "1")
    ...
    env["LUCIFEX_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["LUCIFEX_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["LUCIFEX_TUI_RESUME"] = resume

    if sidecar_url:
        env["LUCIFEX_TUI_SIDECAR_URL"] = sidecar_url

    if active_session_file:
        env["LUCIFEX_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    # inherits the profile LUCIFEX_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["LUCIFEX_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``LUCIFEX_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("LUCIFEX_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from lucifex_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="lucifex-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Lucifex Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Lucifex CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="lucifex-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from lucifex_cli.console_engine import LucifexConsoleEngine

        engine = LucifexConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Lucifex Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Lucifex inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("LUCIFEX_TUI_RESUME") or raw_resume
...
# LUCIFEX_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`lucifex_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from lucifex_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Lucifex Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="lucifex-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/lucifex/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /lucifex`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__LUCIFEX_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `lucifex serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("LUCIFEX_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (lucifex serve): web UI disabled — use "
            "`lucifex dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/lucifex``)
        ...
        ``__LUCIFEX_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__LUCIFEX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__LUCIFEX_BASE_PATH__="{prefix}";'
                f"window.__LUCIFEX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__LUCIFEX_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__LUCIFEX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__LUCIFEX_BASE_PATH__="{prefix}";'
                f"window.__LUCIFEX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``LUCIFEX_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Lucifex Teal canvas, then
    ...
    # under ``/lucifex`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Lucifex backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Lucifex Teal",         "description": "Classic dark teal — the canonical Lucifex look"},
    {"name": "default-large", "label": "Lucifex Teal (Large)", "description": "Lucifex Teal with bigger fonts and roomier spacing"},
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_lucifex_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``LUCIFEX_HOME``.
    """
    themes_dir = get_process_lucifex_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.lucifex/plugins/  (only if LUCIFEX_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from lucifex_cli.plugins import get_bundled_plugins_dir
    ...
    # LUCIFEX_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_lucifex_home() / "plugins", "user"),
    ...
    # ``lucifex_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("LUCIFEX_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".lucifex" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from lucifex_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_lucifex_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lucifex auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from lucifex_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from lucifex_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lucifex_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lucifex_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.lucifex/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"lucifex_dashboard_plugin_{plugin['name']}"
...
from lucifex_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``LUCIFEX_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("LUCIFEX_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``LUCIFEX_SERVE_HEADLESS``), so
    ...
    # the lucifex-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        from lucifex_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Lucifex Agent exposes a
            ...
            try:
                ...
                from lucifex_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`lucifex plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # LUCIFEX_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "LUCIFEX_BACKEND_READY" if headless else "LUCIFEX_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Lucifex backend listening on {host}:{actual_port}")
            else:
                print(f"  Lucifex Web UI → http://{host}:{actual_port}")
