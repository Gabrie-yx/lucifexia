...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from lucifex_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      ...
      4. No-op on Windows (the .exe name is already ``lucifex.exe``).
    """
...
# that run *before* lucifex_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("LUCIFEX_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``LUCIFEX_TUI=1``, then a real-TTY gate (a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("LUCIFEX_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``LUCIFEX_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("LUCIFEX_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _try_termux_ultrafast_version() -> bool:
    ...
    if os.environ.get("LUCIFEX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from lucifex_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from lucifex_cli.subcommands.cron import build_cron_parser
from lucifex_cli.subcommands.gateway import build_gateway_parser
from lucifex_cli.subcommands.profile import build_profile_parser
from lucifex_cli.subcommands.model import build_model_parser
from lucifex_cli.subcommands.setup import build_setup_parser
from lucifex_cli.subcommands.postinstall import build_postinstall_parser
from lucifex_cli.subcommands.whatsapp import build_whatsapp_parser
from lucifex_cli.subcommands.slack import build_slack_parser
from lucifex_cli.subcommands.login import build_login_parser
from lucifex_cli.subcommands.logout import build_logout_parser
from lucifex_cli.subcommands.auth import build_auth_parser
from lucifex_cli.subcommands.status import build_status_parser
from lucifex_cli.subcommands.webhook import build_webhook_parser
from lucifex_cli.subcommands.hooks import build_hooks_parser
from lucifex_cli.subcommands.doctor import build_doctor_parser
from lucifex_cli.subcommands.security import build_security_parser
from lucifex_cli.subcommands.dump import build_dump_parser
from lucifex_cli.subcommands.debug import build_debug_parser
from lucifex_cli.subcommands.backup import build_backup_parser
from lucifex_cli.subcommands.import_cmd import build_import_cmd_parser
from lucifex_cli.subcommands.config import build_config_parser
from lucifex_cli.subcommands.console import build_console_parser
from lucifex_cli.subcommands.version import build_version_parser
from lucifex_cli.subcommands.update import build_update_parser
from lucifex_cli.subcommands.uninstall import build_uninstall_parser
from lucifex_cli.subcommands.dashboard import build_dashboard_parser
from lucifex_cli.subcommands.gui import build_gui_parser
from lucifex_cli.subcommands.logs import build_logs_parser
from lucifex_cli.subcommands.prompt_size import build_prompt_size_parser
from lucifex_cli.subcommands.memory import build_memory_parser
from lucifex_cli.subcommands.acp import build_acp_parser
from lucifex_cli.subcommands.tools import build_tools_parser
from lucifex_cli.subcommands.insights import build_insights_parser
from lucifex_cli.subcommands.skills import build_skills_parser
from lucifex_cli.subcommands.pairing import build_pairing_parser
from lucifex_cli.subcommands.plugins import build_plugins_parser
from lucifex_cli.subcommands.mcp import build_mcp_parser
from lucifex_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors lucifex_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # run-script as LUCIFEX_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``lucifex gateway run`` to mean
    # "the root LUCIFEX_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("LUCIFEX_S6_SUPERVISED_CHILD"):
        try:
            from lucifex_constants import get_default_lucifex_root

            active_path = get_default_lucifex_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set LUCIFEX_HOME
    if profile_name is not None:
        try:
            from lucifex_cli.profiles import resolve_profile_env

            lucifex_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            lucifex_home = _resolve_sudo_user_profile_env(profile_name)
            if not lucifex_home:
...
load_lucifex_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → LUCIFEX_REDACT_SECRETS env
# var BEFORE lucifex_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_lucifex_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "LUCIFEX_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["LUCIFEX_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from lucifex_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from lucifex_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if lucifex_constants not importable yet
...
# Provider model-selection wizard flows extracted to lucifex_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (lucifex_cli.main._model_flow_*) keep resolving unchanged.
from lucifex_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_lucifex_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("LUCIFEX_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("LUCIFEX_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_lucifex_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Lucifex portal OAuth credentials
    auth_file = get_lucifex_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, lucifex_bin
    ...
    lucifex_bin = container_info["lucifex_bin"]
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``LUCIFEX_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("LUCIFEX_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``LUCIFEX_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("LUCIFEX_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "LUCIFEX_HOME": lucifex_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(lucifex_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(lucifex_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if lucifex_cli_dir is None:
        lucifex_cli_dir = Path(__file__).parent
    bundled = lucifex_cli_dir / "tui_dist" / "entry.js"
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("LUCIFEX_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (LUCIFEX_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("LUCIFEX_NODE")
    ...
    ext_dir = os.environ.get("LUCIFEX_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with LUCIFEX_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset LUCIFEX_TUI_DIR (e.g. `unset LUCIFEX_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # ships lucifex_cli/tui_dist/entry.js in the wheel but never ships ui-tui/
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from lucifex_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("LUCIFEX_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["LUCIFEX_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("LUCIFEX_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["LUCIFEX_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("LUCIFEX_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["LUCIFEX_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["LUCIFEX_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["LUCIFEX_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["LUCIFEX_MODEL"] = model
        env["LUCIFEX_INFERENCE_MODEL"] = model
    if provider:
        env["LUCIFEX_TUI_PROVIDER"] = provider
        env["LUCIFEX_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["LUCIFEX_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["LUCIFEX_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["LUCIFEX_TUI_SKILLS"] = value
    if query:
        env["LUCIFEX_TUI_QUERY"] = query
    if image:
        env["LUCIFEX_TUI_IMAGE"] = image
    if checkpoints:
        env["LUCIFEX_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["LUCIFEX_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["LUCIFEX_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["LUCIFEX_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["LUCIFEX_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["LUCIFEX_ACCEPT_HOOKS"] = "1"
    ...
    # LUCIFEX_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set LUCIFEX_TUI_RESUME themselves.
    env.pop("LUCIFEX_TUI_RESUME", None)
    if resume_session_id:
        env["LUCIFEX_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``LUCIFEX_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("LUCIFEX_KANBAN_BOARD"):
        return
    try:
        from lucifex_cli.kanban_db import get_current_board

        os.environ["LUCIFEX_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def _resolve_use_tui(args) -> bool:
    ...
    try:
        ...
        return False
    if os.environ.get("LUCIFEX_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from lucifex_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from lucifex_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from lucifex_cli.banner import prefetch_update_check
    ...
    if getattr(args, "ignore_user_config", False):
        os.environ["LUCIFEX_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["LUCIFEX_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["LUCIFEX_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from lucifex_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from lucifex_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    session_dir = get_lucifex_home() / "whatsapp" / "session"
...
def cmd_whatsapp_cloud(args):
    ...
    from lucifex_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from lucifex_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from lucifex_cli.config import stamp_install_method
    from lucifex_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from lucifex_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    from lucifex_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("LUCIFEX_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # PROVIDER_GROUPS in lucifex_cli/models.py). A multi-member group shows one
    ...
    # matching against lucifex_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _clear_stale_openai_base_url():
    ...
    """
    from lucifex_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`lucifex_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from lucifex_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Lucifex portal) if the main model is unavailable.  Override a")
...
def _aux_flow_provider_model(
    ...
    from lucifex_cli.auth import _prompt_model_selection
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from lucifex_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from lucifex_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from lucifex_cli.runtime_provider import _detect_api_mode_for_url
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from lucifex_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from lucifex_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def cmd_logout(args):
    ...
    from lucifex_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from lucifex_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from lucifex_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from lucifex_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from lucifex_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from lucifex_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from lucifex_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from lucifex_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from lucifex_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from lucifex_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from lucifex_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from lucifex_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from lucifex_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        ...
        from lucifex_cli.backup import run_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from lucifex_cli.banner import format_banner_version_label
    ...
    try:
        ...
        from lucifex_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from lucifex_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from lucifex_cli.uninstall import run_gui_uninstall
    ...
    from lucifex_cli.uninstall import run_uninstall
...
_UPDATE_CRITICAL_FILES = (
    "lucifex_cli/main.py",
    "lucifex_cli/config.py",
    "lucifex_cli/__init__.py",
    "lucifex_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "lucifex_constants.py",
)
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``lucifex_cli/web_dist/`` (per vite.config.ts
    outDir: "../lucifex_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "lucifex_cli" / "web_dist"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m lucifex_cli.main web` invocation.
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $LUCIFEX_HOME."""
    from lucifex_constants import get_lucifex_home
    return get_lucifex_home() / "desktop-build-stamp.json"
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the LUCIFEX_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["LUCIFEX_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["LUCIFEX_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "lucifex_root", None):
        env["LUCIFEX_DESKTOP_LUCIFEX_ROOT"] = str(Path(args.lucifex_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["LUCIFEX_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["LUCIFEX_DESKTOP_CWD"] = os.getcwd()
    ...
    # `LUCIFEX_DESKTOP_DISABLE_GPU=... lucifex desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "LUCIFEX_DESKTOP_DISABLE_GPU" not in os.environ:
        env["LUCIFEX_DESKTOP_DISABLE_GPU"] = config_disable_gpu
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    environment variable ``LUCIFEX_DESKTOP_CHILD_PID`` on the spawned
...
def _kill_stale_dashboard_processes(
    ...
    # backend child, it sets LUCIFEX_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("LUCIFEX_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _update_via_zip(args):
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for lucifex-agent itself
        ...
        # GitHub ZIPs extract to lucifex-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"lucifex-agent-{branch}")
    ...
    from lucifex_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from lucifex_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "lucifex-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Lucifex behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Lucifex couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Lucifex couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Lucifex behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Lucifex couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Lucifex couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_lucifex_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_lucifex_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    default_home = get_default_lucifex_root()
...
def _recover_from_interrupted_install() -> None:
    ...
    # Windows self-lock guard: if lucifex.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _lucifex_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        try:
            from lucifex_cli.managed_uv import ensure_uv
...
def _lucifex_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_lucifex_instances(
    ...
    for shim in _lucifex_exe_shims(scripts_dir):
        try:
...
def _quarantine_running_lucifex_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``lucifex.exe``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_lucifex_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``lucifex.exe`` first.
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_lucifex_exe(scripts_dir)
    try:
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``lucifex.exe`` / ``lucifex-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_lucifex_exe`` for the rationale.
    """
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m lucifex_cli.main <command>"
        )
    else:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from lucifex_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$LUCIFEX_HOME/bin/uv``, but on Termux the official
    ...
    """
    from lucifex_cli.managed_uv import resolve_uv
...
def _npm_lockfile_changed(lucifex_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = lucifex_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(lucifex_root: Path) -> None:
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from lucifex_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from lucifex_constants import is_wsl
    ...
    shared_lucifex_root = get_default_lucifex_root()
    if not _npm_lockfile_changed(shared_lucifex_root):
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_lucifex_root)
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # lucifex_cli.config.get_lucifex_home to simulate setup failure.
        from lucifex_cli.config import get_lucifex_home as _get_lucifex_home

        logs_dir = _get_lucifex_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "pip":
        from lucifex_cli.config import recommended_update_command
        from lucifex_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    try:
        from lucifex_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        ...
        home = get_lucifex_home()
        try:
            display_path = f"{display_lucifex_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The lucifex.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m lucifex_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m lucifex_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "lucifex_cli.main" in cmdline_low:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    lucifex.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from lucifex_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m lucifex_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: lucifex gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``lucifex gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from lucifex_cli import gateway_windows
        from lucifex_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Lucifex Agent")
        return
...
def _cmd_update_pip(args):
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "lucifex-agent"]
        ...
        cmd = [pipx, "upgrade", "lucifex-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "lucifex-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "lucifex-agent"]
    ...
    print("✓ Update complete! Restart lucifex to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Lucifex Agent...")
    print()

    # On Windows, abort early if another lucifex.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_lucifex_instances(scripts_dir)
            if concurrent:
    ...
    # the lucifex.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        # the old bytecode (e.g. get_lucifex_home added to lucifex_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "lucifex_cli.main", "desktop", "--build-only"]
        ...
        # modules in this process.  Reload lucifex_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_lucifex_home() added since the last release.
        try:
            import importlib
            import lucifex_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit LUCIFEX_HOME so
        # it is not affected by sync_skills()'s module-level LUCIFEX_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's LUCIFEX_HOME env var points at the default or a named profile.
        try:
            from lucifex_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from lucifex_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from lucifex_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            ...
            if (
                ...
                from lucifex_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_lucifex_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... lucifex-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... lucifex-gateway*)
            ...
            # Discover all lucifex-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. lucifex-gateway.service or lucifex-gateway-coder.service
        ...
        try:
            from lucifex_cli.gateway import (
                has_legacy_lucifex_units,
                _find_legacy_lucifex_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_lucifex_units():
                print()
                print("⚠ Legacy Lucifex gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_lucifex_units():
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from lucifex_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from lucifex_constants import get_lucifex_home as _hh
        ...
        from lucifex_cli import profile_describer as _pd
        ...
        from lucifex_cli.profiles import profile_exists, validate_alias_name
        ...
        from lucifex_cli.profiles import rename_profile
        ...
        from lucifex_cli.profiles import export_profile
        ...
        from lucifex_cli.profiles import import_profile
        ...
        import tempfile
        from lucifex_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="lucifex_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from lucifex_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from lucifex_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("lucifex_requires"):
            print(f"Requires:     Lucifex {data['lucifex_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from lucifex_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.lucifex_requires:
        print(f"  Requires: Lucifex {mf.lucifex_requires}")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from lucifex_cli.web_server import should_require_auth
    ...
    print("    [2] OAuth via Lucifex portal (run `lucifex dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "  It provisions a Lucifex portal OAuth client and writes "
            "LUCIFEX_DASHBOARD_OAUTH_CLIENT_ID into ~/.lucifex/.env for you.\n"
    ...
    try:
        from lucifex_cli.config import load_config, save_config
        from lucifex_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → LUCIFEX_HOME points into
    ...
    if (
        ...
        and os.environ.get("LUCIFEX_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "lucifex_cli.main",
            "-p", "default",
        ...
        try:
            from lucifex_constants import get_default_lucifex_root
            env["LUCIFEX_HOME"] = str(get_default_lucifex_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop LUCIFEX_HOME) rather than block the reroute.
            env.pop("LUCIFEX_HOME", None)
    ...
    if _headless_backend:
        ...
        os.environ["LUCIFEX_SERVE_HEADLESS"] = "1"
    elif "LUCIFEX_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["LUCIFEX_WEB_DIST"])
            if "LUCIFEX_WEB_DIST" in os.environ
            else PROJECT_ROOT / "lucifex_cli" / "web_dist"
        )
        ...
        # LUCIFEX_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["LUCIFEX_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ LUCIFEX_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset LUCIFEX_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads LUCIFEX_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["LUCIFEX_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from LUCIFEX_WEB_DIST: {_dist_root}")
    ...
    try:
        from lucifex_cli.mcp_startup import start_background_mcp_discovery
    ...
    from lucifex_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from lucifex_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from lucifex_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from lucifex_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from lucifex_cli.prompt_size import cmd_prompt_size as _impl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from lucifex_cli.models import CANONICAL_PROVIDERS as _cp
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("LUCIFEX_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that LUCIFEX_YOLO_MODE is set before ANY
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["LUCIFEX_SAFE_MODE"] = "1"
    os.environ["LUCIFEX_IGNORE_USER_CONFIG"] = "1"
    os.environ["LUCIFEX_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from lucifex_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["LUCIFEX_DEFER_AGENT_STARTUP"] = "1"
            os.environ["LUCIFEX_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["LUCIFEX_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_lucifex_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_lucifex_home()}/memories/\n")
    else:
        from lucifex_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from lucifex_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from lucifex_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from lucifex_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from lucifex_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from lucifex_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from lucifex_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from lucifex_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from lucifex_cli.claw import claw_command
...
def main():
    ...
    try:
        from lucifex_cli.stdio import configure_windows_stdio
    ...
    # there's nothing to clean. See ``_quarantine_running_lucifex_exe``.
    try:
    ...
    # =========================================================================
    # model command  (parser built in lucifex_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from lucifex_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from lucifex_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://lucifex-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    from lucifex_cli import secrets_cli as _secrets_cli
    from lucifex_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from lucifex_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in lucifex_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in lucifex_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in lucifex_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in lucifex_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in lucifex_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from lucifex_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in lucifex_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in lucifex_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in lucifex_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in lucifex_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in lucifex_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in lucifex_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — Lucifex portal status + Tool Gateway routing
    # =========================================================================
    from lucifex_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from lucifex_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from lucifex_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in lucifex_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in lucifex_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in lucifex_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in lucifex_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in lucifex_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in lucifex_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from lucifex_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in lucifex_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in lucifex_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in lucifex_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in lucifex_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in lucifex_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from lucifex_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in lucifex_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from lucifex_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from lucifex_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from lucifex_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from lucifex_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in lucifex_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in lucifex_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from lucifex_cli.tools_config import _cua_driver_cmd
            # Honor LUCIFEX_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from lucifex_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in lucifex_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from lucifex_state import workspace_key as _ws_key
            ...
            from lucifex_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from lucifex_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from lucifex_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_lucifex_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from lucifex_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_lucifex_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_lucifex_home() / "sessions"
    ...
    # =========================================================================
    # insights command  (parser built in lucifex_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in lucifex_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in lucifex_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in lucifex_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in lucifex_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in lucifex_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in lucifex_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in lucifex_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in lucifex_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in lucifex_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in lucifex_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from lucifex_cli.config import get_container_exec_info
    ...
    # --yolo: set LUCIFEX_YOLO_MODE *before* plugin discovery.  The call to
