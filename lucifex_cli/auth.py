...
SPOTIFY_DOCS_URL = "https://lucifex-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://lucifex-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _auth_file_path() -> Path:
    path = get_lucifex_home() / "auth.json"
    # Seat belt: if pytest is running and LUCIFEX_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch LUCIFEX_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set LUCIFEX_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom LUCIFEX_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_lucifex_root()
    except Exception:
        return None
    profile_home = get_lucifex_home()
    try:
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print("       App name:     anything (e.g. lucifex-agent)")
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.lucifex/.env")
    print()
    return raw
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.lucifex/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.lucifex/auth.json)."""
...
def _nous_shared_auth_dir() -> Path:
    ...
    from lucifex_constants import get_default_lucifex_root
    return get_default_lucifex_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from lucifex_constants import get_default_lucifex_root
        real_home_shared = (
            get_default_lucifex_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No LUCIFEX_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.lucifex/auth.json) called POST /api/oauth/token with Hermes's "
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from lucifex_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from lucifex_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from lucifex_cli.config import save_config, load_config
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.lucifex/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
