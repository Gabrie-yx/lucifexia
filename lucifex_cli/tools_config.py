...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``lucifex tools post-setup <key>`` child. On Windows, every console child
...
from lucifex_cli.cli_output import (  # noqa: E402 — late import block
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from lucifex_cli.plugins import discover_plugins, get_plugin_toolsets
...
from lucifex_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `lucifex computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            from lucifex_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
