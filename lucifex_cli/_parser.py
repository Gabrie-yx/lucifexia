...
# argparse runs (it sets ``LUCIFEX_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``lucifex_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    lucifex                        Start interactive chat
    lucifex chat -q "Hello"        Single query mode
    lucifex --tui                  Launch the modern TUI (or set display.interface: tui)
    lucifex --cli                  Force the classic REPL (overrides display.interface: tui)
    lucifex -c                     Resume the most recent session
    lucifex -c "my project"        Resume a session by name (latest in lineage)
    lucifex --resume <session_id>  Resume a specific session by ID
    lucifex setup                  Run setup wizard
    lucifex logout                 Clear stored authentication
    lucifex auth add <provider>    Add a pooled credential
    lucifex auth list              List pooled credentials
    lucifex auth remove <p> <t>    Remove pooled credential by index, id, or label
    lucifex auth reset <provider>  Clear exhaustion status for a provider
    lucifex model                  Select default model
    lucifex fallback [list]        Show fallback provider chain
    lucifex fallback add           Add a fallback provider (same picker as `lucifex model`)
    lucifex fallback remove        Remove a fallback provider from the chain
    lucifex config                 View configuration
    ...
    lucifex sessions browse        Interactive session picker
    lucifex sessions rename ID T   Rename/title a session
    lucifex logs                   View agent.log (last 50 lines)
    lucifex logs -f                Follow agent.log in real time
    lucifex logs errors            View errors.log
    lucifex logs --since 1h        Lines from the last hour
    lucifex debug share             Upload debug report for support
    lucifex console                Open the safe Lucifex command console
    lucifex update                 Update to latest version
    lucifex dashboard              Start web UI dashboard (port 9119)
    lucifex dashboard --stop       Stop running dashboard processes
    lucifex dashboard --status     List running dashboard processes

For more help on a command:
    lucifex <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="lucifex",
        description="Lucifex Agent - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via LUCIFEX_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
