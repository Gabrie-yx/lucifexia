...
the existing module-level functions in lucifex_cli.gateway and
lucifex_cli.gateway_windows directly. This protocol is a thin facade
...
``lucifex gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``lucifex gateway start/stop/restart``).
    """
    ...
    # `lucifex gateway start/stop/restart` fell through to host code that spawns
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in lucifex_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from lucifex_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from lucifex_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from lucifex_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from lucifex_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in lucifex_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from lucifex_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from lucifex_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from lucifex_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from lucifex_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``lucifex_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's LUCIFEX_HOME blindly;
    derive the shared profile root from the current LUCIFEX_HOME and map the
    ...
    if lucifex_home.parent.name == "profiles":
        root = lucifex_home.parent.parent
    else:
        root = lucifex_home
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _LUCIFEX_UID, _LUCIFEX_GID)
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _LUCIFEX_UID, _LUCIFEX_GID)
        except PermissionError:
            pass
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _LUCIFEX_UID, _LUCIFEX_GID)
            except PermissionError:
                pass
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``lucifex_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$LUCIFEX_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``LUCIFEX_HOME``,
        ...
        allocator in ``lucifex_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export LUCIFEX_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's LUCIFEX_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``lucifex gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-LUCIFEX_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The LUCIFEX_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own LUCIFEX_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{LUCIFEX_HOME:=/opt/data}}"\n'
            f'log_dir="$LUCIFEX_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
