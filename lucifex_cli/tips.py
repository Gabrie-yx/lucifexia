...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or lucifex -c.",
    ...
    "Ctrl+Z suspends Lucifex to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "lucifex -c resumes your most recent CLI session. lucifex -c \"project name\" resumes by title.",
    "lucifex -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "lucifex -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "lucifex chat -t web,terminal enables only specific toolsets for a focused session.",
    "lucifex chat -s github-pr-workflow preloads a skill at launch.",
    "lucifex chat -q \"query\" runs a single non-interactive query and exits.",
    "lucifex chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "lucifex chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "lucifex --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lucifex chat --source telegram tags the session for filtering in lucifex sessions list.",
    "lucifex -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lucifex doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lucifex dump outputs a compact setup summary — great for bug reports.",
    ...
    "lucifex sessions browse opens an interactive session picker with search.",
    "lucifex sessions stats shows session counts by platform and database size.",
    "lucifex sessions prune --older-than 30 cleans up old sessions.",
    "lucifex skills search react --source skills-sh searches the skills.sh public directory.",
    "lucifex skills check scans installed hub skills for upstream updates.",
    "lucifex skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lucifex skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lucifex mcp add github --command npx adds MCP servers from the command line.",
    "lucifex mcp serve runs Lucifex itself as an MCP server for other agents.",
    "lucifex auth add lets you add multiple API keys for credential pool rotation.",
    "lucifex completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lucifex logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lucifex backup creates a zip backup of your entire Lucifex home directory.",
    "lucifex profile create coder creates an isolated profile that becomes its own command.",
    "lucifex profile create work --clone copies your current config and keys to a new profile.",
    "lucifex update syncs new bundled skills to ALL profiles automatically.",
    "lucifex gateway install sets up Lucifex as a system service (systemd/launchd).",
    "lucifex memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "lucifex webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: lucifex tools disables unused tools, lucifex skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "lucifex models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as lucifex -w).",
    ...
    "LUCIFEX_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Lucifex tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'lucifex profile create coder' creates the 'coder' command.",
    "lucifex profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Lucifex prints a resume command with session ID and stats.",
    "lucifex sessions export backup.jsonl exports all sessions for backup or analysis.",
    "lucifex -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "lucifex skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .lucifex/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lucifex cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Lucifex runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "lucifex gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Lucifex via a local macOS server.",
    ...
    "Context files (.lucifex.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Lucifex loads project context from .lucifex.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "lucifex mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "lucifex mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "lucifex mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — lucifex tools can toggle them per platform.",
    ...
    "lucifex chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "lucifex plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lucifex acp runs Lucifex as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "LUCIFEX_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "lucifex auth add nous or lucifex auth add openai-codex sets up OAuth-based providers.",
    ...
    "lucifex status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Lucifex your own.",
    ...
    "Before a gateway session reset, Lucifex auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Lucifex state via LUCIFEX_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lucifex profile create ls' would be rejected.",
    "lucifex profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "lucifex claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "LUCIFEX_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lucifex tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Lucifex auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Lucifex config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Lucifex is working.',
    ...
    "LUCIFEX_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    ...
    # --- Curator ---
    'lucifex curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lucifex curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lucifex curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lucifex auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'LUCIFEX_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "LUCIFEX_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'lucifex dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lucifex dashboard embeds the full Lucifex TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'LUCIFEX_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'LUCIFEX_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'LUCIFEX_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'LUCIFEX_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'lucifex -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'lucifex chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'lucifex chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'lucifex chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "lucifex chat --source tool tags programmatic chats so they don't clutter lucifex sessions list.",
    'lucifex dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lucifex sessions rename <ID> "new title" renames any past session; lucifex sessions delete <ID> removes one.',
    'lucifex import restores a full Lucifex backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'lucifex fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lucifex pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lucifex setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lucifex status --deep runs the full health sweep across every component; plain lucifex status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'LUCIFEX_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'LUCIFEX_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.lucifex/plugins/ — trust-gated by design.',
    "LUCIFEX_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'LUCIFEX_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'LUCIFEX_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'LUCIFEX_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'LUCIFEX_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'LUCIFEX_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'LUCIFEX_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'LUCIFEX_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'LUCIFEX_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'LUCIFEX_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'LUCIFEX_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Lucifex block commands when the tirith scanner itself errors out.',
