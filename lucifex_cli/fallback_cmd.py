"""
lucifex fallback — manage the fallback provider chain.
...
Subcommands:
  lucifex fallback [list]   Show the current fallback chain (default when no subcommand)
  lucifex fallback add      Pick provider + model via the same picker as `lucifex model`,
                           then append the selection to the chain
  lucifex fallback remove   Pick an entry to delete from the chain
  lucifex fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from lucifex_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `lucifex model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  lucifex fallback add")
        print()
        return
    ...
    print("  Docs: https://lucifex-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `lucifex model`, then append the selection to the chain."""
    from lucifex_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `lucifex model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `lucifex fallback list` to view, or `lucifex fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``lucifex fallback [subcommand]``."""
