#!/bin/sh
# shellcheck shell=sh
# /opt/lucifex/bin/lucifex — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `lucifex` user (UID 10000). When an operator runs `docker exec <c> lucifex ...`
# the default UID is root (0), and any file the command writes under
# $LUCIFEX_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> lucifex login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Lucifex is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> lucifex
...
# Fix
# ---
# This shim sits at /opt/lucifex/bin/lucifex and is placed earliest on PATH.
# When invoked as root, it drops to the lucifex user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $LUCIFEX_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user lucifex`, kanban subagents, etc. — it short-circuits
...
# (/opt/lucifex/.venv/bin/lucifex), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set LUCIFEX_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the lucifex CLI. Default is to drop.

set -e

REAL=/opt/lucifex/.venv/bin/lucifex

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "lucifex-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user lucifex`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${LUCIFEX_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the lucifex user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "lucifex-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "lucifex-shim: re-run with --user lucifex or set LUCIFEX_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the lucifex user's home before dropping privileges. Without
