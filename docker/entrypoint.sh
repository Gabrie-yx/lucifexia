...
# by /etc/cont-init.d/01-lucifex-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs lucifex"
...
echo "[lucifex] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/lucifex/docker/stage2-hook.sh "$@"
