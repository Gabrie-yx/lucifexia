...
#
# Wired into the image as /etc/cont-init.d/01-lucifex-setup by the
...
LUCIFEX_HOME="${LUCIFEX_HOME:-/opt/data}"
INSTALL_DIR="/opt/lucifex"

# Drop to lucifex via s6-setuidgid, but skip it when already non-root.
as_lucifex() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid lucifex "$@"; }
...
# starts non-root. The baked install tree under /opt/lucifex is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $LUCIFEX_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass LUCIFEX_UID/LUCIFEX_GID — or the PUID/PGID aliases — which the
...
# `id -u lucifex` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# lucifex UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lucifex)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-lucifex UID).
...
and the baked /opt/lucifex install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e LUCIFEX_UID=\$(id -u) -e LUCIFEX_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the lucifex user to that UID/GID at boot and chowns the data
...
# --- Bootstrap LUCIFEX_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid lucifex mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom LUCIFEX_HOME paths whose parents only
# root can create (e.g. `LUCIFEX_HOME=/home/lucifex/.lucifex` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for LUCIFEX_UID/LUCIFEX_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  LUCIFEX_UID/
# LUCIFEX_GID still win when both are set.  See #15290, salvages #25872.
LUCIFEX_UID="${LUCIFEX_UID:-${PUID:-}}"
LUCIFEX_GID="${LUCIFEX_GID:-${PGID:-}}"

if [ -n "${LUCIFEX_UID:-}" ] && validate_uid_gid "$LUCIFEX_UID" && [ "$LUCIFEX_UID" != "$(id -u lucifex)" ]; then
    echo "[stage2] Changing lucifex UID to $LUCIFEX_UID"
    usermod -u "$LUCIFEX_UID" lucifex
fi
if [ -n "${LUCIFEX_GID:-}" ] && validate_uid_gid "$LUCIFEX_GID" && [ "$LUCIFEX_GID" != "$(id -g lucifex)" ]; then
    echo "[stage2] Changing lucifex GID to $LUCIFEX_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$LUCIFEX_GID" lucifex 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised lucifex user (UID 10000)
...
# lucifex process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes lucifex. Idempotent across container
...
#   - lucifex is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G lucifex 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] lucifex already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" lucifex 2>/dev/null; then
        echo "[stage2] Added lucifex to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group lucifex failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When LUCIFEX_UID is remapped or the top-level $LUCIFEX_HOME isn't owned by
# the runtime lucifex UID, restore ownership to lucifex — but ONLY for the
# directories lucifex actually writes to. The full $LUCIFEX_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of lucifex-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_lucifex_uid=$(id -u lucifex)
...
chown_lucifex_tree() {
    ...
    chown -R lucifex:lucifex "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$LUCIFEX_HOME" 2>/dev/null)" != "$actual_lucifex_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $LUCIFEX_HOME (targeted) to lucifex ($actual_lucifex_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $LUCIFEX_HOME: chown the directory itself (not its contents)
    # so lucifex can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$LUCIFEX_HOME"; then
        :
    else
        chown lucifex:lucifex "$LUCIFEX_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $LUCIFEX_HOME failed (rootless container?) — continuing"
    fi
    # Lucifex-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by lucifex (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$LUCIFEX_HOME/$sub" ]; then
            chown_lucifex_tree "$LUCIFEX_HOME/$sub"
        fi
    done
fi
...
# lucifex user. Hosted/container instances keep mutable state under
# $LUCIFEX_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# LUCIFEX_DISABLE_LAZY_INSTALLS=1. Keeping /opt/lucifex root-owned and
...
# $LUCIFEX_HOME/lazy-packages dir on the data volume (Dockerfile sets
# LUCIFEX_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to lucifex
...
# Always reset ownership of $LUCIFEX_HOME/profiles to lucifex on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> lucifex …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as lucifex and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$LUCIFEX_HOME/profiles" ]; then
    chown_lucifex_tree "$LUCIFEX_HOME/profiles"
fi

# Always reset ownership of $LUCIFEX_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged lucifex runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$LUCIFEX_HOME/cron" ]; then
    chown_lucifex_tree "$LUCIFEX_HOME/cron"
fi
...
# lucifex pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged lucifex gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $LUCIFEX_HOME is
...
# is negligible.
if [ -d "$LUCIFEX_HOME/platforms/pairing" ]; then
    chown_lucifex_tree "$LUCIFEX_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$LUCIFEX_HOME/pairing" ]; then
    chown_lucifex_tree "$LUCIFEX_HOME/pairing"
fi

# Reset ownership of lucifex-owned top-level state files on every boot.
# The targeted data-volume chown above only covers lucifex-owned
# *subdirectories*; loose state files living directly under $LUCIFEX_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> lucifex …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged lucifex runtime then hits
...
# sweep so host-owned files in a bind-mounted $LUCIFEX_HOME are never
...
# entries of lucifex_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$LUCIFEX_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$LUCIFEX_HOME/$f"; then
            :
        else
            chown lucifex:lucifex "$LUCIFEX_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the lucifex runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$LUCIFEX_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$LUCIFEX_HOME/config.yaml"; then
        :
    else
        chown lucifex:lucifex "$LUCIFEX_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$LUCIFEX_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as lucifex user ---
# Run as lucifex via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $LUCIFEX_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_lucifex mkdir -p \
    "$LUCIFEX_HOME/backups" \
    "$LUCIFEX_HOME/cron" \
    "$LUCIFEX_HOME/sessions" \
    "$LUCIFEX_HOME/logs" \
    "$LUCIFEX_HOME/logs/gateways" \
    "$LUCIFEX_HOME/hooks" \
    "$LUCIFEX_HOME/memories" \
    "$LUCIFEX_HOME/skills" \
    "$LUCIFEX_HOME/skins" \
    "$LUCIFEX_HOME/plans" \
    "$LUCIFEX_HOME/workspace" \
    "$LUCIFEX_HOME/home" \
    "$LUCIFEX_HOME/pairing" \
    "$LUCIFEX_HOME/platforms/pairing" \
    "$LUCIFEX_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/lucifex/.install_method (see Dockerfile), NOT written here into
# $LUCIFEX_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $LUCIFEX_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.lucifex:/opt/data) and sometimes shared with a
...
# 'lucifex update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $LUCIFEX_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/lucifex stamp, so deleting the data-dir copy is safe).
if [ -f "$LUCIFEX_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$LUCIFEX_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$LUCIFEX_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$LUCIFEX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$LUCIFEX_HOME/$dest"; then
            :
        else
            as_lucifex cp "$INSTALL_DIR/$src" "$LUCIFEX_HOME/$dest"
        fi
    fi
}
...
if [ -f "$LUCIFEX_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$LUCIFEX_HOME/.env"; then
        :
    else
        chown lucifex:lucifex "$LUCIFEX_HOME/.env" 2>/dev/null || true
        chmod 600 "$LUCIFEX_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $LUCIFEX_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `lucifex update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set LUCIFEX_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$LUCIFEX_HOME/config.yaml" ]; then
    s6-setuidgid lucifex "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$LUCIFEX_HOME/auth.json" ] && [ -n "${LUCIFEX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$LUCIFEX_HOME/auth.json"; then
        :
    else
        printf '%s' "$LUCIFEX_AUTH_JSON_BOOTSTRAP" > "$LUCIFEX_HOME/auth.json"
        chown lucifex:lucifex "$LUCIFEX_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$LUCIFEX_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via LUCIFEX_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$LUCIFEX_HOME/auth.json" ] && [ -n "${LUCIFEX_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$LUCIFEX_HOME/auth.json"; then
        :
    else
        s6-setuidgid lucifex "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$LUCIFEX_HOME/auth.json" \
...
# can set LUCIFEX_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# LUCIFEX_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$LUCIFEX_HOME/gateway_state.json" ] && \
        [ "${LUCIFEX_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$LUCIFEX_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$LUCIFEX_HOME/gateway_state.json"
        chown lucifex:lucifex "$LUCIFEX_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$LUCIFEX_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_lucifex "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/lucifex/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Lucifex spawns for the
...
# main-wrapper.sh propagates it into the supervised ``lucifex`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-lucifex, dashboard, etc.).
