...
def _sidecar_deps_stale() -> bool:
    ...
    `lucifex update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``lucifex photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        ...
        # A `lucifex update` that bumps the spectrum-ts pin rewrites
