...
Credential storage mirrors every other Lucifex channel:
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a lucifex dependency
...
# registers Lucifex as its own client_id.
...
# Default name of the project Lucifex provisions for the operator.
DEFAULT_PROJECT_NAME = "Lucifex Agent"
...
def _auth_json_path() -> Path:
    """Resolve ``~/.lucifex/auth.json`` honouring the active Lucifex profile."""
    try:
        from lucifex_constants import get_lucifex_home
        return Path(get_lucifex_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.lucifex")) / "auth.json"
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    ...
    try:
        ...
        logger.warning("photon: lucifex_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `lucifex photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `lucifex photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `lucifex photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `lucifex photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `lucifex photon setup`)"
