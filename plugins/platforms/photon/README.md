...
This plugin connects Lucifex Agent to iMessage (and other Spectrum
...
TypeScript-only, Lucifex runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Lucifex-Sidecar-Token`.
...
There is no separate `login` command; like every other Lucifex channel,
...
Run `lucifex photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.lucifex/.env` (the same place every other
...
Management metadata lives in `~/.lucifex/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `lucifex photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `lucifex photon status`, a DM and a group roundtrip,
