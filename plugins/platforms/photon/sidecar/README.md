...
Small Node helper that bridges Lucifex Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Lucifex is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Lucifex-Sidecar-Token`)
...
The Lucifex plugin's `lucifex photon setup` command runs `npm install`
here automatically.
