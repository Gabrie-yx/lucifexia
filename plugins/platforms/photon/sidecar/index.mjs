// LUCIFEX Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> LUCIFEX): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (LUCIFEX -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-LUCIFEX-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `LUCIFEX photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so LUCIFEX can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply LUCIFEX'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local LUCIFEX allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of LUCIFEX, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-LUCIFEX-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when LUCIFEX supplied them so a known-good
