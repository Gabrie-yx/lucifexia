"""ntfy platform adapter (Lucifex plugin).
...
a Lucifex dependency.

This adapter ships as a Lucifex platform plugin under
``plugins/platforms/ntfy/``. The Lucifex plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "lucifex-in"              # subscribe topic (incoming)
          publish_topic: "lucifex-out"     # optional — defaults to topic
...
_ECHO_TAG = "lucifex-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``lucifex cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Lucifex dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `lucifex gateway status` without
