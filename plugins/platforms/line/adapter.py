"""
LINE Messaging API platform adapter for Lucifex Agent.
...
to Lucifex Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``LUCIFEX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            lucifex_home = Path(get_lucifex_home()).resolve()
        except Exception:
            lucifex_home = Path.home().joinpath(".lucifex").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``lucifex status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``lucifex status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lucifex setup line``.
