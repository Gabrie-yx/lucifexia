...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = env_float(
            "LUCIFEX_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "LUCIFEX_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("LUCIFEX_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["LUCIFEX_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("LUCIFEX_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["LUCIFEX_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
