"""SimpleX Chat platform adapter (Lucifex plugin).
...
This adapter ships as a Lucifex platform plugin under
``plugins/platforms/simplex/``. The Lucifex plugin loader scans the
...
Optional environment variables:
    ...
    LUCIFEX_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``lucifex setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Lucifex — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("LUCIFEX_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lucifex setup gateway`` → SimpleX.
