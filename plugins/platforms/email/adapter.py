"""
Email platform adapter for the Lucifex gateway.

Allows users to interact with Lucifex by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "lucifex-agent" "version" "{_lucifex_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `lucifex config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `lucifex gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Lucifex Agent")
        ...
        msg_id = f"<lucifex-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# lucifex_cli/gateway.py, and the _send_email dispatch in
