...
# ``model_tools`` import time, so ``lucifex status``, ``lucifex chat``, etc.
...
# ``lucifex_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Lucifex is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a lucifex MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "lucifex-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LUCIFEX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _lucifex_home = _get_lucifex_home()
        except (ModuleNotFoundError, ImportError):
            _lucifex_home = _Path.home() / ".lucifex"
        self._thread_count_store = _ThreadCountStore(
            _lucifex_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Lucifex is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LUCIFEX_HOME", str(_Path.home() / ".lucifex"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LUCIFEX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a lucifex MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Lucifex is thinking…' marker message.
        ...
        "Lucifex is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Lucifex is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Lucifex is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Lucifex is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``lucifex setup``.
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. lucifex-chat-events) and a Pull subscription.")
    ...
    print_info("Restart the gateway: lucifex gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``lucifex cron`` running as a
    separate process from ``lucifex gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Lucifex plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lucifex-agent[google_chat]'",
        ...
        # the "Lucifex is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Lucifex is thinking…' marker message "
