...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `lucifex slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_lucifex_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Lucifex does not currently need to route them into the agent loop.
            ...
            # /model work directly without /lucifex prefix). A single regex
            ...
            # manifest (see `lucifex slack manifest`). In Socket Mode, Slack
            ...
            from lucifex_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/lucifex$")

            @self._app.command(_slash_pattern)
            async def handle_lucifex_command(ack, command):
            ...
            for _action_id in (
                "lucifex_approve_once",
                "lucifex_approve_session",
                "lucifex_approve_always",
                "lucifex_deny",
            ):
            ...
            for _action_id in (
                "lucifex_confirm_once",
                "lucifex_confirm_always",
                "lucifex_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("lucifex_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Lucifex handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Lucifex session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Lucifex owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@lucifex`` still resolve.
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "lucifex_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "lucifex_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "lucifex_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "lucifex_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "lucifex_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "lucifex_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lucifex_confirm_once": "once",
            "lucifex_confirm_always": "always",
            "lucifex_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lucifex_approve_once": "once",
            "lucifex_approve_session": "session",
            "lucifex_approve_always": "always",
            "lucifex_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/lucifex <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/lucifex
        ...
        if slash_name in {"lucifex", ""}:
            # Legacy /lucifex <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from lucifex_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/lucifex   ``).
        ...
        # "Running /cmd…" ack shown by handle_lucifex_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/lucifex <question>" must produce public replies so
...
# ``lucifex_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``lucifex_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under LUCIFEX_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from lucifex_cli.slack_cli import _build_full_manifest
            from lucifex_constants import get_lucifex_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Lucifex",
                bot_description="Your Lucifex agent on Slack",
            )
            ...
            print_info(
                "   Re-run `lucifex slack manifest --write` anytime to refresh after "
                "Lucifex adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lucifex update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://lucifex-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``lucifex_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lucifex-agent[slack]'",
        # Interactive setup wizard — replaces lucifex_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in lucifex_cli/gateway.py.
