# Lucifex Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: lucifex-achievements plugin
...
### `GET /api/plugins/lucifex-achievements/achievements`
...
### `POST /api/plugins/lucifex-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/lucifex-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/lucifex-achievements/overview`
...
- Persist plugin data under:
  - `~/.lucifex/plugins/lucifex-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.lucifex/plugins/lucifex-achievements/scan_checkpoint.json`
...
- `~/.lucifex/plugins/lucifex-achievements/`
