...
lucifex meet install                 # pip deps + Chromium (transcribe only)
lucifex meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
lucifex meet auth                    # optional; skips guest-lobby wait
lucifex meet setup                   # preflight checks
```

`lucifex meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or LUCIFEX_MEET_REALTIME_KEY in ~/.lucifex/.env
```
...
pip install playwright websockets && python -m playwright install chromium
lucifex plugins enable google_meet
lucifex meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
lucifex meet node approve my-mac ws://<mac-ip>:18789 <token>
lucifex meet node ping my-mac                   # confirm reachable
```

Run `lucifex meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Lucifex agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `lucifex meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `LUCIFEX_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `lucifex_meet_src`). |
...
Local:
```
$LUCIFEX_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$LUCIFEX_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `lucifex meet node approve`.
