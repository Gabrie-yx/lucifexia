...
session-scoped state file under ``$LUCIFEX_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $LUCIFEX_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_lucifex_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this lucifex install, leave it first —
    ...
    env = os.environ.copy()
    env["LUCIFEX_MEET_URL"] = url
    env["LUCIFEX_MEET_OUT_DIR"] = str(out)
    env["LUCIFEX_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["LUCIFEX_MEET_HEADED"] = "1"
    if auth_state:
        env["LUCIFEX_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["LUCIFEX_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if LUCIFEX_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["LUCIFEX_MEET_MODE"] = mode
    if realtime_model:
        env["LUCIFEX_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["LUCIFEX_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["LUCIFEX_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["LUCIFEX_MEET_REALTIME_KEY"] = realtime_api_key
