"""`lucifex meet node ...` subcommand tree.

Wired into the existing ``lucifex meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``lucifex meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="lucifex-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``lucifex meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             lucifex meet node approve <name> ws://<host>:{args.port} {token}")
        try:
