...
Launched by ``lucifex meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$LUCIFEX_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``lucifex meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_lucifex_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "lucifex-meet-node",
    ) -> None:
        self.host = host
        self.port = port
