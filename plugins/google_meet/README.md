...
Let the lucifex agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lucifex runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lucifex meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lucifex meet` CLI |
...
| `cli.py` | `lucifex meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LUCIFEX_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `lucifex meet node {run,list,approve,remove,status,ping}` |
...
lucifex meet install                                      # pip + Chromium
lucifex meet setup                                        # preflight
lucifex meet auth                                         # optional
lucifex meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lucifex meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.lucifex/.env
lucifex meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lucifex meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lucifex meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.lucifex/.env
lucifex meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lucifex will **not** switch your system audio input automatically — the
...
lucifex plugins enable google_meet
lucifex meet node run --display-name my-mac --host 0.0.0.0 --port 18789
