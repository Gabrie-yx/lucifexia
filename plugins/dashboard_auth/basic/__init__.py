...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or LUCIFEX_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "LUCIFEX_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "LUCIFEX_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "LUCIFEX_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # LUCIFEX_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
