...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that Lucifex' read deny-list blocks.
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `lucifex auth add xai-oauth` "
    ...
    *,
    ...
    try:
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="lucifex-xai-video",
            extension="mp4",
        )
