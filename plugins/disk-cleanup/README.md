...
Auto-tracks and cleans up ephemeral files created during Lucifex Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LUCIFEX_HOME` and `/tmp/lucifex-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LUCIFEX_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LUCIFEX_HOME` or `/tmp/lucifex-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LUCIFEX_HOME/disk-cleanup/` is itself excluded
- `$LUCIFEX_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
