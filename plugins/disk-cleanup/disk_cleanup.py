"""disk_cleanup — ephemeral file cleanup for Lucifex Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under LUCIFEX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly LUCIFEX_HOME and /tmp/lucifex-*
...
try:
    ...
    def get_lucifex_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("LUCIFEX_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".lucifex").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$LUCIFEX_HOME/logs/``."""
    return get_lucifex_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$LUCIFEX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under LUCIFEX_HOME or ``/tmp/lucifex-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    lucifex_home = get_lucifex_home()
    try:
        path.resolve().relative_to(lucifex_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/lucifex-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("lucifex-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "lucifex-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $LUCIFEX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so LUCIFEX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        lucifex_home = get_lucifex_home()
        for parent in ("cron", "cronjobs"):
            base = lucifex_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside LUCIFEX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under LUCIFEX_HOME, but never recurse into known
    # durable state trees.  Some installs place the Lucifex checkout, venv,
    # and desktop build under LUCIFEX_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    lucifex_home = get_lucifex_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in lucifex_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    lucifex_home = get_lucifex_home()
    try:
        rel = path.resolve().relative_to(lucifex_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "lucifex-agent",
        }:
            return None
        ...
        # Path isn't under LUCIFEX_HOME (e.g. /tmp/lucifex-*) — fall through.
        pass
