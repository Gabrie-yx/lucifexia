...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=lucifex``
...
models show up in ``lucifex tools`` automatically. No model ids are
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `lucifex tools` → Image "
                    "Generation → DeepInfra to configure, or `lucifex setup` "
                    "to add the key."
                ),
        ...
        from lucifex_cli.models import deepinfra_base_url
