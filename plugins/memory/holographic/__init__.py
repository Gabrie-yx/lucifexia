"""lucifex-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $LUCIFEX_HOME/config.yaml (profile-scoped):
  plugins:
    lucifex-memory-store:
      db_path: $LUCIFEX_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "lucifex-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, lucifex_home):
        """Write config to config.yaml under plugins.lucifex-memory-store."""
        from pathlib import Path
        config_path = Path(lucifex_home) / "config.yaml"
        try:
            import yaml
            existing = {}
            ...
            existing["plugins"]["lucifex-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_lucifex_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from lucifex_constants import get_lucifex_home
        _lucifex_home = str(get_lucifex_home())
