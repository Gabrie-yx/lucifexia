...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Lucifex peer ID in OpenViking (default: lucifex)
...
_DEFAULT_AGENT = "lucifex"
_AGENT_PROMPT_LABEL = "Lucifex peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "LUCIFEX_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Lucifex slash-skill scaffolding before sending content to OpenViking.
...
def _load_lucifex_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("LUCIFEX_HOME", "")).expanduser() if os.environ.get("LUCIFEX_HOME") else Path.home() / ".lucifex"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Lucifex session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Lucifex only", "write values only to Lucifex .env"),
    ...
    _save_lucifex_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Lucifex .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_lucifex_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Lucifex peer ID in OpenViking, sent as the actor peer and "
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_lucifex_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Lucifex run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Lucifex' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Lucifex canonical messages into OpenViking batch payloads."""
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/lucifex/.overview.md).
