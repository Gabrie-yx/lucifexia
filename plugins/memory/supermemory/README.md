...
- Hosted: API key from [app.supermemory.ai/integrations?connect=lucifex](http://app.supermemory.ai/integrations?connect=lucifex)
...
Before running `lucifex memory setup`, add the local endpoint to
...
Then run `lucifex memory setup` and enter the local server's API key. Configuring
...
`https://api.supermemory.ai`. Lucifex resolves it once and uses the same endpoint
