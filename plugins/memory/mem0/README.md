...
```bash
lucifex memory setup    # select "mem0"
```
...
Behavioral settings live in `$LUCIFEX_HOME/mem0.json` (set them via `lucifex memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.lucifex/.env`.
...
| `user_id` | `lucifex-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Lucifex session and call `mem0_search` — it connects to your server.
...
```bash
lucifex memory setup mem0 --mode oss \
...
```bash
lucifex memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$LUCIFEX_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.lucifex/mem0_qdrant"}}
  }
}
```
...
```bash
lucifex memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$LUCIFEX_HOME/mem0.json`).
