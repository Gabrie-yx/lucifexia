...
React + Ink terminal UI for Lucifex. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LUCIFEX_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `lucifex setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.lucifex/.lucifex_history` or under `LUCIFEX_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `lucifex setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/lucifex-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external lucifex setup, suspends Ink while it runs
    ...
    types/
      lucifex-ink.d.ts            type declarations for @lucifex/ink
