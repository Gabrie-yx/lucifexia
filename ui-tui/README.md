...
React + Ink terminal UI for lucifexex. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `lucifexex_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `lucifexex setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.lucifex/.lucifexex_history` or under `LUCIFEX_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `lucifexex setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/lucifexex-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external lucifexex setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.lucifex/tui-parent.log
    ...
    types/
      lucifexex-ink.d.ts            type declarations forlucifexifex/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent LucifexCLI subprocess for slash commands
```
