...
// Terminal. Explicit lucifexex_TUI_THEME / COLORFGBG signals above still win,
...
// `lucifexex_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `lucifexex_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `lucifexex_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `lucifexex_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default lucifexex palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.lucifexex_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.lucifexex_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.lucifexex_TUI_BACKGROUND ?? '')
