...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LUCIFEX_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Lucifex on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LUCIFEX_TUI_TRUECOLOR=1.
