...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.lucifexex_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep lucifexex on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via lucifexex_TUI_TRUECOLOR=1.
