...
const logDir = join(process.env.LUCIFEX_HOME?.trim() || join(homedir(), '.lucifexex'), 'logs')
...
// a real ~/.lucifexex (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('lucifexex-tui: parent lifecycle log unavailable\n')
    }
  }
}
