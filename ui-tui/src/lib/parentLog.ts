...
const logDir = join(process.env.LUCIFEX_HOME?.trim() || join(homedir(), '.lucifex'), 'logs')
...
// a real ~/.lucifex (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('lucifex-tui: parent lifecycle log unavailable\n')
    }
  }
}
