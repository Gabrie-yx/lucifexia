...
_UNSET = object()
_LUCIFEX_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_LUCIFEX_HOME_OVERRIDE", default=_UNSET
)


def set_lucifex_home_override(path: str | Path | None) -> Token:
    """Set a context-local Lucifex home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _LUCIFEX_HOME_OVERRIDE.set(value)


def reset_lucifex_home_override(token: Token) -> None:
    """Restore the previous context-local Lucifex home override."""
    _LUCIFEX_HOME_OVERRIDE.reset(token)


def get_lucifex_home_override() -> str | None:
    """Return the active context-local Lucifex home override, if any."""
    override = _LUCIFEX_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_lucifex_home() -> Path:
    """Return the platform-native default Lucifex home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "lucifex"
    return Path.home() / ".lucifex"


def _lucifex_home_from_env() -> Path:
    """Resolve LUCIFEX_HOME from the process environment only.

    Reads the ``LUCIFEX_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_lucifex_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_lucifex_home`
    and :func:`get_process_lucifex_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``LUCIFEX_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_lucifex_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[LUCIFEX_HOME fallback] LUCIFEX_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass LUCIFEX_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_lucifex_home() -> Path:
    """Return the Lucifex home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_lucifex_home_override`) → ``LUCIFEX_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``LUCIFEX_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``LUCIFEX_HOME`` explicitly (see the systemd
    template in ``lucifex_cli/gateway.py`` and the kanban dispatcher in
    ``lucifex_cli/kanban_db.py``).  See https://github.com/NousResearch/lucifex-agent/issues/18594.
    """
    override = get_lucifex_home_override()
    if override:
        return Path(override)

    if not os.environ.get("LUCIFEX_HOME", "").strip():
        _warn_profile_fallback_once()

    return _lucifex_home_from_env()


def get_process_lucifex_home() -> Path:
    """Return the Lucifex home for the running process, ignoring task overrides.

    Unlike :func:`get_lucifex_home`, this never follows the context-local
    override set by :func:`set_lucifex_home_override`.  It resolves only the
    process ``LUCIFEX_HOME`` env var (falling back to the platform default),
...
def get_default_lucifex_root() -> Path:
    """Return the root Lucifex directory for profile-level operations.

    In standard deployments this is the platform-native Lucifex home
    (``~/.lucifex`` on POSIX, ``%LOCALAPPDATA%\\lucifex`` on native Windows).

    In Docker or custom deployments where ``LUCIFEX_HOME`` points outside
    ``~/.lucifex`` (e.g. ``/opt/data``), returns ``LUCIFEX_HOME`` directly
    — that IS the root.

    In profile mode where ``LUCIFEX_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.lucifex/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_lucifex_home()
    env_home = os.environ.get("LUCIFEX_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # LUCIFEX_HOME is under ~/.lucifex (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — LUCIFEX_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Lucifex is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``LUCIFEX_OPTIONAL_SKILLS``.
    """
    override = os.getenv("LUCIFEX_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lucifex_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``LUCIFEX_OPTIONAL_MCPS``.
    """
    override = os.getenv("LUCIFEX_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lucifex_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``LUCIFEX_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<LUCIFEX_HOME>/skills`` last-resort
    """
    override = os.getenv("LUCIFEX_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lucifex_home() / "skills"


def get_lucifex_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Lucifex subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to LUCIFEX_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to LUCIFEX_HOME (e.g. ``"image_cache"``).
...
def iter_lucifex_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Lucifex-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\lucifex\\node``. POSIX installs use
    ``$LUCIFEX_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_lucifex_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with lucifexManagedNodePathEntries() in
...
_LUCIFEX_NODE_TARGET_MAJOR = int(os.environ.get("LUCIFEX_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Lucifex-managed Node trees live under ``$LUCIFEX_HOME/node`` (or a profile's
    ``LUCIFEX_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_lucifex_node_executable``
    used to trust file presence alone, so ``lucifex update`` would pick that
...
def lucifex_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Lucifex-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_lucifex_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%LUCIFEX_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_lucifex_home()
    index_url = f"https://nodejs.org/dist/latest-v{_LUCIFEX_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_LUCIFEX_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_lucifex_managed_node() -> bool:
    """Redownload Lucifex-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not lucifex_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LUCIFEX_HOME": str(get_lucifex_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_lucifex_node_executable(command: str) -> str | None:
    """Return a Lucifex-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_lucifex_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_lucifex_managed_node():
        for directory in iter_lucifex_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Lucifex-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Lucifex-managed installs.

    This is for Lucifex-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_lucifex_node_executable(command)
    if managed:
        return managed
    if lucifex_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_lucifex_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Lucifex-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_lucifex_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``lucifex update`` — leaving a **dangling symlink** that ``which`` still
...
def display_lucifex_home() -> str:
    """Return a user-friendly display string for the current LUCIFEX_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.lucifex``
        profile:  ``~/.lucifex/profiles/coder``
        custom:   ``/opt/lucifex-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.lucifex``.  For code that needs a real ``Path``, use
    :func:`get_lucifex_home` instead.
    """
    home = get_lucifex_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``LUCIFEX_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/lucifex-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{LUCIFEX_HOME}/home`` when the profile-home directory exists."""
    lucifex_home = get_lucifex_home_override() or (env or {}).get("LUCIFEX_HOME") or os.getenv("LUCIFEX_HOME")
    if not lucifex_home:
        return None
    profile_home = os.path.join(lucifex_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("LUCIFEX_REAL_HOME") or os.getenv("LUCIFEX_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Lucifex profile HOME.

    ``LUCIFEX_HOME`` scopes Lucifex state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={LUCIFEX_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{LUCIFEX_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{LUCIFEX_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Lucifex' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["LUCIFEX_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Lucifex itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/lucifex-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under LUCIFEX_HOME.

    Replaces the ``get_lucifex_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, lucifex_logging.py, lucifex_time.py, etc.).
    """
    return get_lucifex_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under LUCIFEX_HOME."""
    return get_lucifex_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under LUCIFEX_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_lucifex_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._lucifex_ipv4_patched = True  # type: ignore[attr-defined]
