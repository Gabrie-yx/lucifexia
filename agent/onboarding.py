...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Lucifex starts and finds ``~/.openclaw/``.

    Points users at ``lucifex claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``lucifex claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Lucifex, run "
        "`lucifex claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `lucifex claw cleanup` (renames it to ~/.openclaw.pre-migration — "
