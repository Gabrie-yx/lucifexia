"""Upload a Lucifex session transcript to Hugging Face as an agent trace.

Lucifex stores sessions in its own SQLite store (``lucifex_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``lucifex trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Lucifex' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "lucifex-traces"
_LUCIFEX_VERSION = "lucifex-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Lucifex OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Lucifex' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Lucifex conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "3. Run /upload-trace again (or `lucifex trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/lucifex-traces`` dataset. Returns a
