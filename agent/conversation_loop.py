...
from lucifex_constants import PARTIAL_STREAM_STUB_ID
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Lucifex tool use: "
    ...
    return (
        ...
        f"context, but Lucifex needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Lucifex config, set `model.ollama_num_ctx: 65536` "
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `lucifex fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from lucifex_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default Lucifex path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Lucifex internals.
        #
        # Lucifex invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Lucifex tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from lucifex_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("LUCIFEX_DUMP_REQUESTS"):
                ...
                from lucifex_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Lucifex/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: lucifex auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `lucifex doctor` for credential-chain diagnostics, or")
                    ...
                    from lucifex_constants import display_lucifex_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Lucifex-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: lucifex config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: lucifex config set ANTHROPIC_API_KEY \"\"")
                ...
                # which Lucifex' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Lucifex' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`lucifex",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Lucifex may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `lucifex auth` to re-authenticate.", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: lucifex setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        lucifex fallback add   (interactive picker — same as `lucifex model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"timeout. (Lucifex's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Lucifex mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from lucifex_cli.plugins import get_pre_verify_continue_message, has_hook
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("LUCIFEX_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
