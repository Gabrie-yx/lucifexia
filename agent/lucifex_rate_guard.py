"""Cross-session rate limit guard for Lucifex portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Lucifex portal is currently rate-limited
...
(3 SDK retries x 3 Lucifex retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = get_lucifex_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".lucifex")
...
def record_lucifex_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Lucifex portal is rate-limited.
...
def lucifex_rate_limit_remaining() -> Optional[float]:
    """Check if Lucifex portal is currently rate-limited.
...
def clear_lucifex_rate_limit() -> None:
...
def is_genuine_lucifex_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Lucifex portal is a real account rate limit.

    Lucifex portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Lucifex, ...) behind one endpoint.  A 429 can mean either:
