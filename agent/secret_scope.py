...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the LUCIFEX_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Lucifex runtime / deployment
    "LUCIFEX_HOME", "LUCIFEX_PROFILE", "LUCIFEX_GATEWAY_LOCK_DIR",
    "LUCIFEX_MAX_ITERATIONS", "LUCIFEX_MAX_TOKENS", "LUCIFEX_API_TIMEOUT",
    "LUCIFEX_REDACT_SECRETS", "LUCIFEX_NOUS_TIMEOUT_SECONDS",
    "_LUCIFEX_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "LUCIFEX_KANBAN_DB", "LUCIFEX_KANBAN_WORKSPACES_ROOT", "LUCIFEX_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "LUCIFEX_KANBAN_",
    "LUCIFEX_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(lucifex_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(lucifex_home) / ".env")
