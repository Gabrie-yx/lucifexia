...
row taxonomy is inferred from the atlas shape so Lucifex can render both legacy
...
# rows (1536x1872), while older Lucifex/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Lucifex' activity state names. They are not always identical to the
...
# Legacy Lucifex/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Lucifex activity names -> accepted row-name aliases in descending
