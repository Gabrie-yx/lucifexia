...
Plugins built on Lucifex Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Lucifex
...
:class:`~lucifex_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Lucifex' main config
...
handles every provider, fallback chain, and per-task override Lucifex
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`lucifex_cli.plugins.PluginContext`
