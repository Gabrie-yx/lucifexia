    ...
    if _ttfb_timeout <= 0:
        ...
        if (
            ...
            logger.info(
                ...
                "Set LUCIFEX_CODEX_TTFB_STRICT=1 to force early reconnects.",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    try:
        ...
        _ft = _fixed_temperature_for_model(model_name, agent.base_url)
    ...
    try:
        ...
        _model_norm = (model_name or "").lower().replace(".", "-")
        if any(key in _model_norm for key in _ANTHROPIC_OUTPUT_LIMITS):
            _ant_max = _get_anthropic_max_output(model_name)
    except Exception:
        pass
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    # the user explicitly set LUCIFEX_STREAM_STALE_TIMEOUT.
