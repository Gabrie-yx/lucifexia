...
def _state_file() -> Path:
    return get_lucifex_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.lucifex/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `lucifex update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``lucifex curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``lucifex update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``lucifex curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `lucifex curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Lucifex' background skill CURATOR. This is an "
    ...
    "into ~/.lucifex/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: lucifex-config-*, lucifex-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.lucifex/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.lucifex/logs/curator/``)
    ...
    authored skill data in ``~/.lucifex/skills/``.

    ``ensure_lucifex_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_lucifex_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every lucifex install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: lucifex curator status
        keep an umbrella stable: lucifex curator pin document-tools
    ...
    lines.append("full report: lucifex curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: lucifex curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.lucifex/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.lucifex/skills/.archive/` for "
            "safety and can be restored via `lucifex curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.lucifex/skills/.archive/`. "
            "Restore any via `lucifex curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `lucifex curator restore <name>`")
    lines.append("- All archives live under `~/.lucifex/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``lucifex curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `lucifex curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `lucifex update` only if "
        ...
        # recorded in state so `lucifex curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``lucifex model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `lucifex model` → auxiliary
