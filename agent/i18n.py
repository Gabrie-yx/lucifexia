"""Lightweight internationalization (i18n) for Lucifex static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Lucifex itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``LUCIFEX_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``LUCIFEX_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("LUCIFEX_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "LUCIFEX_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a lucifex dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`lucifex_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("LUCIFEX_LANGUAGE")
    if env_lang:
