...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Lucifex never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Lucifex continues with whatever credentials ``.env`` already had.
...
so back-to-back short-lived ``lucifex`` invocations don't re-shell ``op`` for
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Lucifex to source it from, so normalize to that name here.
    if token_value:
