...
MoA-enabled; the normal Lucifex agent loop still owns tool calling and turn
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, lucifex portal tags — still fire. Those branches
...
def _run_references_parallel(
    ...
    *,
    ...
    # callbacks + the Lucifex portal conversation tag) into each worker so
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Lucifex boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Lucifex agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Lucifex agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Lucifex agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
