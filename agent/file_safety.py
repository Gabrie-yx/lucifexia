...
def _lucifex_home_path() -> Path:
    """Resolve the active LUCIFEX_HOME (profile-aware) without circular imports."""
    try:
        from lucifex_constants import get_lucifex_home  # local import to avoid cycles
        return get_lucifex_home()
    except Exception:
        return Path(os.path.expanduser("~/.lucifex"))


def _lucifex_root_path() -> Path:
    """Resolve the Lucifex root dir (always the parent of any profile, never per-profile)."""
    try:
        from lucifex_constants import get_default_lucifex_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    lucifex_home = _lucifex_home_path()
    lucifex_root = _lucifex_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(lucifex_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(lucifex_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(lucifex_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(lucifex_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved LUCIFEX_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("LUCIFEX_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    lucifex_dirs = []
    for base in (_lucifex_home_path(), _lucifex_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in lucifex_dirs:
                lucifex_dirs.append(real)
        except Exception:
            continue

    for base_real in lucifex_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside LUCIFEX_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Lucifex path.

    Three categories are blocked:

      * Internal Lucifex cache files under ``LUCIFEX_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under LUCIFEX_HOME and the global Lucifex
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Resolve BOTH the active LUCIFEX_HOME (profile-aware) AND the global
    # Lucifex root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (LUCIFEX_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    lucifex_dirs: list[Path] = []
    for base in (_lucifex_home_path(), _lucifex_root_path()):
        try:
            real = base.resolve()
            if real not in lucifex_dirs:
    ...
    for hd in lucifex_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Lucifex credential store "
    ...
    for hd in lucifex_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Lucifex MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Lucifex MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Lucifex read (see
...
#
# Lucifex profiles are separate LUCIFEX_HOME dirs under
...
#
# Reference: May 2026 incident where a lucifex-security profile session
# edited skills under both ``~/.lucifex/profiles/lucifex-security/skills/``
# AND ``~/.lucifex/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under LUCIFEX_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _lucifex_home_path().resolve()
        root_real = _lucifex_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Lucifex scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Lucifex entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Lucifex profile {info['target_profile']!r}, but the "
...
#
#   <LUCIFEX_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.lucifex/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.lucifex/…`` segment and warns
# regardless of which Lucifex profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.lucifex/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.lucifex`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.lucifex/…`` and returns the
    index where the inner Lucifex-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .lucifex / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".lucifex":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Lucifex state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.lucifex``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.lucifex`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Lucifex resolver to
    succeed, so it works correctly even when called from contexts where
    LUCIFEX_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Lucifex process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real LUCIFEX_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.lucifex/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.lucifex/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host LUCIFEX_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Lucifex state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Lucifex "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real LUCIFEX_HOME. Use the "
