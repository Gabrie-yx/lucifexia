...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``lucifex_cli.models``
    ...
    if not base_url:
        return False
    try:
        from lucifex_cli.models import _is_model_free, _pricing_cache
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Lucifex",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (LUCIFEX_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set LUCIFEX_DEV_CREDITS_FIXTURE to either a
...
# immediately), and (3) the /usage view (lucifex_credits_lines renders the fixture).
# `clear` / `none` / unset → real behaviour. Delete with the rest of the
# LUCIFEX_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for LUCIFEX_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag LUCIFEX_DEV_CREDITS
    is also on, so a stray LUCIFEX_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("LUCIFEX_DEV_CREDITS")):
        return None
    raw = os.environ.get("LUCIFEX_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a LucifexportalAccountInfo into a header-shaped CreditsState for the seed.
