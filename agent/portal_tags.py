...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        ...
        "client=lucifex-client-v<__version__>",
    ]
...
def _lucifex_version() -> str:
    """Return the current Lucifex release version, e.g. ``"0.13.0"``.
...
def lucifex_client_tag() -> str:
    ...
    """
    return f"client=lucifex-client-v{_lucifex_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Lucifex session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Lucifex conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.lucifex/sessions/`` storage, session logs, and lineage.
