...
from lucifex_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_lucifex_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a LucifexportalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from lucifex_cli.lucifex_account import lucifex_portal_topup_url
        ...
        details.append(f"Top up: {lucifex_portal_topup_url(account_info)}")
...
def lucifex_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when LUCIFEX_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_lucifex_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_lucifex_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so LUCIFEX_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — LUCIFEX_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_lucifex_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_lucifex_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=lucifex_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Lucifex's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `lucifex auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `lucifex auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
