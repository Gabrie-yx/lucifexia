...
_SENSITIVE_LUCIFEX_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    lucifex_home = get_lucifex_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(lucifex_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(lucifex_home / rel for rel in _SENSITIVE_LUCIFEX_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Lucifex path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.lucifex/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Lucifex path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
