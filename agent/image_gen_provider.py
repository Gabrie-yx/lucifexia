...
as ``kind: backend``) or ``~/.lucifex/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``lucifex tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``lucifex tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``lucifex tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``lucifex tools``
...
def _images_cache_dir() -> Path:
    """Return ``$LUCIFEX_HOME/cache/images/``, creating parents as needed."""
    from lucifex_constants import get_lucifex_home

    path = get_lucifex_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$LUCIFEX_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$LUCIFEX_HOME/cache/images/``.
