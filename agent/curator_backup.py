...
files under ``~/.lucifex/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.lucifex/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_lucifex_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.lucifex/cron/jobs.json``)."""
    return get_lucifex_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.lucifex/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.lucifex/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.lucifex/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.lucifex/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `lucifex curator rollback --list` to see available snapshots)",
            None,
        )
