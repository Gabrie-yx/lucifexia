...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Lucifex with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.lucifex/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Lucifex
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``lucifex tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``lucifex setup`` for
