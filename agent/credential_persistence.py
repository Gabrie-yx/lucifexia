...
``auth.json``.  They intentionally have no dependency on ``lucifex_cli.auth`` so
...
# Sources Lucifex owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Lucifex-owned OAuth/device-code state)
