...
projection into Lucifex' display, approval bridging, transcript projection into
...
"codex_app_server"`. Lucifex' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Lucifex secret — gateway
        ...
        spawn_env = lucifex_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("LUCIFEX_KANBAN_TASK"):
            kanban_db = spawn_env.get("LUCIFEX_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "LUCIFEX_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("LUCIFEX_HOME", os.path.expanduser("~/.lucifex")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "lucifex",
        client_title: str = "Lucifex Agent",
