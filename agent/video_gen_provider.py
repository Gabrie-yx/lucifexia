...
as ``kind: backend``) or ``~/.lucifex/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``lucifex tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$LUCIFEX_HOME/cache/videos/``, creating parents as needed."""
    from lucifex_constants import get_lucifex_home

    path = get_lucifex_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$LUCIFEX_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$LUCIFEX_HOME/cache/videos/``.
