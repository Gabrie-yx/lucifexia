...
```bash
# throwaway LUCIFEX_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Lucifex Agent runtime into `LUCIFEX_HOME`
(`~/.lucifex`, or `%LOCALAPPDATA%\lucifex` on Windows), using the same layout as a
CLI install.
...
- **Lucifex Agent** runs as a headless `lucifex serve` process and exposes the
...
1. `LUCIFEX_DESKTOP_LUCIFEX_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `LUCIFEX_DESKTOP_LUCIFEX`, or `lucifex` on `PATH`
5. a system Python that can import the Lucifex runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Lucifex
