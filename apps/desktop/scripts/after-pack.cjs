 ...
 *
 * Stamps the LUCIFEX icon + identity onto the packed Windows LUCIFEX.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `LUCIFEX desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'LUCIFEX')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'LUCIFEX'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); LUCIFEX.exe keeps the stock Electron icon`)
  }
}
