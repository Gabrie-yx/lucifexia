 ...
 * product name (`LUCIFEX`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/LUCIFEX'
 *
 * This is a hard failure with no obvious cause for the user — `LUCIFEX desktop`
 ...
 * (the mac-unpacked LUCIFEX.app bundle) and Windows (win-unpacked), so we
