#!/usr/bin/env node
// set-exe-identity.cjs — stamp the LUCIFEX icon + version metadata onto the
// built LUCIFEX.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked LUCIFEX.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `LUCIFEX desktop`, the installer's
...
// path (which rebuilds via `LUCIFEX desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-LUCIFEX.exe>
//
...
// Stamp the LUCIFEX icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — LUCIFEX icon + identity stamped')
}
