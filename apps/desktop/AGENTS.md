...
How to build Lucifex Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Lucifex surface can also
...
- A **runtime home change** (switching the underlying `LUCIFEX_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Lucifex — do not assume
