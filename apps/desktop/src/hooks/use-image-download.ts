...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'lucifex:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.lucifexDesktop?.saveImageFromUrl) {
        if (await window.lucifexDesktop.saveImageFromUrl(src)) {
