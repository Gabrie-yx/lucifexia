...
import { formatRefValue, lucifexDirectiveFormatter } from './directive-text'
...
describe('lucifexDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = lucifexDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = lucifexDirectiveFormatter.parse('@file:src/main.tsx the entry point')
