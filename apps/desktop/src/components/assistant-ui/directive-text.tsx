...
const LUCIFEX_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type LucifexRefType = (typeof LUCIFEX_REF_TYPES)[number]
...
const ICON_PATHS: Record<LucifexRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as LucifexRefType] ?? ICON_FALLBACK
...
const LUCIFEX_DIRECTIVE_RE = new RegExp(
...
export const lucifexDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(LUCIFEX_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as LucifexRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: LucifexRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...lucifexDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Lucifex directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.lucifexDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.lucifexDesktop?.readFileDataUrl(id)
