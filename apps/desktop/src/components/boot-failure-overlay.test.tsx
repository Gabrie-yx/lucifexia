...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Lucifex gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.lucifexDesktop
  Object.defineProperty(window, 'lucifexDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'lucifexDesktop', { configurable: true, value: original })
}
