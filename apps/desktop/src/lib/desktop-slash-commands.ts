...
/** Why a known Lucifex command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Lucifex profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Lucifex Agent version', surface: exec() },
...
function isKnownLucifexSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Lucifex' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownLucifexSlashCommand(normalized)
}
