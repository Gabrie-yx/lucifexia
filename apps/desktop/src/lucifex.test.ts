...
describe('Lucifex REST helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'lucifexDesktop')
  })
    ...
    api.mockImplementation(({ path }: { path: string }) => {
      if (path.startsWith('/api/profiles/sessions/sidebar')) {
        ...
        return Promise.reject(
          new Error(
            'Error invoking remote method \'lucifex:api\': Error: 404: {"detail":"No such API endpoint: /api/profiles/sessions/sidebar"}'
          )
        )
      }
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getLucifexConfig, '/api/config'],
      [getLucifexConfigDefaults, '/api/config/defaults'],
