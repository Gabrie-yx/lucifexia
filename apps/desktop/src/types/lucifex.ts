...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Lucifex doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in lucifex_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `lucifex model` uses — instead of
...
export interface LucifexConfig {
  agent?: {
...
export type LucifexConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `lucifex model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors lucifex_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Lucifex, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `lucifex tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Lucifex); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/lucifex/update/check` — the backend's own update state.
