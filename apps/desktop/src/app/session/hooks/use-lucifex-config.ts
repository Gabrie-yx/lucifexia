...
interface LucifexConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useLucifexConfig({ activeSessionIdRef, refreshProjectBranch }: LucifexConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshLucifexConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getLucifexConfig(), getLucifexConfigDefaults().catch(() => ({}))])
  ...
  return { refreshLucifexConfig, sttEnabled, voiceMaxRecordingSeconds }
}
