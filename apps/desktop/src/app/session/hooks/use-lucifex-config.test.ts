...
vi.mock('@/lucifex', () => ({
  getLucifexConfig: vi.fn(),
  getLucifexConfigDefaults: vi.fn().mockResolvedValue({})
}))
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getLucifexConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getLucifexConfig>>)

describe('useLucifexConfig refreshLucifexConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshLucifexConfig()
    })
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getLucifexConfig>>>()
    vi.mocked(getLucifexConfig).mockReturnValueOnce(profileConfig.promise)
    ...
    act(() => {
      pendingRefresh = result.current.refreshLucifexConfig(true)
    })
    expect(getLucifexConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getLucifexConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getLucifexConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getLucifexConfig>>>()
    vi.mocked(getLucifexConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshLucifexConfig(true)
      refreshC = result.current.refreshLucifexConfig(true)
    })
