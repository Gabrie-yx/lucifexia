...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<LucifexGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.lucifexDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<LucifexGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as LucifexGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('Lucifex gateway unavailable')
      }
