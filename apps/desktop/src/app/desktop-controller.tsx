...
export function DesktopController() {
  ...
  useEffect(() => {
    window.lucifexDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.lucifexDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.lucifexDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.lucifexDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // lucifex:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.lucifexDesktop?.onDeepLink?.(payload => {
    ...
    void window.lucifexDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.lucifexDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshLucifexConfig])
