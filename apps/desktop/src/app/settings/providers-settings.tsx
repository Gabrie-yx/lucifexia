...
import { disconnectOAuthProvider, listOAuthProviders } from '@/lucifex'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/lucifex'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.lucifexDesktop?.terminal)
...
//      in lucifex_cli/provider_catalog.py) — the SAME provider identity
//      `lucifex model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Lucifex can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Lucifex can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Lucifex never deletes creds another tool owns behind a silent API call.
