...
// Lucifex Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- Lucifex Cloud (cloud mode) state ---
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.lucifexDesktop.applyConnectionConfig(payload())
        : await window.lucifexDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.lucifexDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.lucifexDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.lucifexDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.lucifexDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.lucifexDesktop.getConnectionConfig(scope)
      acceptSavedConfig(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.lucifexDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.lucifexDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Lucifex Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/lucifex"
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.lucifexDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
