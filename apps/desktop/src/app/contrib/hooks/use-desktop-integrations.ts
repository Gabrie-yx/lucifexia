...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.lucifexDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  // lucifex:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
  ...
  useEffect(() => {
    const unsubscribe = window.lucifexDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
