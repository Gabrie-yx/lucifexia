...
const STORAGE_KEY = 'lucifex:native-notifications'
...
// "Backgrounded" = the user isn't on Lucifex. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.lucifexDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.lucifexDesktop
