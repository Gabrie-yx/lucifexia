 ...
 * app's bounds and stays visible while Lucifex is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'lucifex.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'lucifex.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.lucifexDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.lucifexDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.lucifexDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.lucifexDesktop?.petOverlay?.close()
}
