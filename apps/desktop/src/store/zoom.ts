...
export function setZoomPercent(percent: number): void {
  window.lucifexDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.lucifexDesktop?.zoom) {
  void window.lucifexDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.lucifexDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
