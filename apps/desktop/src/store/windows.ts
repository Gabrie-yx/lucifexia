...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.lucifexDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.lucifexDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.lucifexDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.lucifexDesktop.openNewSessionWindow(), 'Could not open new session window')
}
