...
import type { LucifexReviewFile, LucifexReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.lucifexDesktop.git,
...
function file(path: string, over: Partial<LucifexReviewFile> = {}): LucifexReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as LucifexReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.lucifexDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { lucifexDesktop?: unknown }).lucifexDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as LucifexReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.lucifexDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: LucifexReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as LucifexReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { lucifexDesktop?: unknown }).lucifexDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as LucifexReviewShipInfo)
