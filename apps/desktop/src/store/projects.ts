...
import type { LucifexGitBaseBranch, LucifexGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/lucifex'
...
const PROJECT_SCOPE_KEY = 'lucifex.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Lucifex gateway is not connected')
  }
...
// repo, returning where Lucifex should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<LucifexGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<LucifexGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.lucifexDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.lucifexDesktop?.writeClipboard?.(path)
  }
}
