 ...
 *   (`@lucifex/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default LucifexPlugin -> register(ctx)
 *
 ...
 * and `<lucifex home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/lucifex'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type LucifexPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@lucifex/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @lucifex/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: LucifexPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default LucifexPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<lucifex home>/desktop-plugins/<name>/plugin.js`
