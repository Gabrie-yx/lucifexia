 ...
 * `LucifexPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/lucifex'
...
export type { PluginRestOptions } from '@/lucifex'
 ...
 *  under `lucifex.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface LucifexPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `lucifex.plugin.${pluginId}.${key}`
