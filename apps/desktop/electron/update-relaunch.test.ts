 ...
 *      internals; LUCIFEX_HOME / LUCIFEX_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.lucifex/lucifex-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'lucifex')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Lucifex12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/lucifex/lucifex', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Lucifex/lucifex', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.lucifex/lucifex-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'lucifex'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'lucifex')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'lucifex://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'lucifex://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves LUCIFEX_HOME + LUCIFEX_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    LUCIFEX_HOME: '/home/u/.lucifex',
    LUCIFEX_DESKTOP_REMOTE_URL: 'http://box:9119',
    LUCIFEX_DESKTOP_REMOTE_TOKEN: 'secret',
    LUCIFEX_DESKTOP_LUCIFEX_ROOT: '/home/u/dev/lucifex',
    LUCIFEX_DESKTOP_APP_NAME: 'LucifexSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.lucifex/lucifex-agent/apps/desktop/release/linux-unpacked/Lucifex',
    args: ['lucifex://open/agent/42', "--note=it's fine"],
    env: { LUCIFEX_HOME: '/home/u/.lucifex', LUCIFEX_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export LUCIFEX_HOME='\/home\/u\/\.lucifex'/)
  assert.match(script, /export LUCIFEX_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Lucifex' 'lucifex:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `lucifex-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Lucifex/Lucifex',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `lucifex-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Lucifex\/Lucifex'\n/)
})
