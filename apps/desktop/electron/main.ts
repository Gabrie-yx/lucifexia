...
import { buildDesktopBackendEnv, normalizeLucifexHomeRoot } from './backend-env'
import { canImportLucifexCli, verifyLucifexCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.LUCIFEX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.LUCIFEX_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.LUCIFEX_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with LUCIFEX_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[lucifex] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[lucifex] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `lucifex desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Lucifex sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[lucifex] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[lucifex] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[lucifex] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[lucifex] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[lucifex] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('lucifex:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[lucifex] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[lucifex] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[lucifex] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[lucifex] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// LUCIFEX_HOME — the user-facing root for everything Lucifex-related. Mirrors
// scripts/install.ps1's $LucifexHome and scripts/install.sh's $LUCIFEX_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\lucifex (matches install.ps1)
//   macOS / Linux: ~/.lucifex (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.lucifex directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\lucifex yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// LUCIFEX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// LUCIFEX_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.lucifex / %LOCALAPPDATA%\lucifex.
function resolveLucifexHome() {
  if (process.env.LUCIFEX_HOME) {
    return normalizeLucifexHomeRoot(process.env.LUCIFEX_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'lucifex-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a LUCIFEX_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\lucifex and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('LUCIFEX_HOME')

    if (fromRegistry) {
      return normalizeLucifexHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'lucifex')
    const legacy = path.join(app.getPath('home'), '.lucifex')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.lucifex setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.lucifex')
}

const LUCIFEX_HOME = resolveLucifexHome()

function lucifexManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_lucifex_node_dirs() in
  // lucifex_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(LUCIFEX_HOME, 'node')
...
function pathWithLucifexManagedNode(...entries) {
  return [...lucifexManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_LUCIFEX_ROOT — the canonical mutable Lucifex install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_LUCIFEX_ROOT = path.join(LUCIFEX_HOME, 'lucifex-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_LUCIFEX_ROOT, 'venv')
...
// boots, going straight to `resolveLucifexBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_LUCIFEX_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_LUCIFEX_ROOT, '.lucifex-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Lucifex profile the desktop launches its
// local backend as. When set, startLucifex() passes `lucifex --profile <name>
...
// lucifexDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under LUCIFEX_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by lucifex_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(LUCIFEX_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (lucifex_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.LUCIFEX_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.LUCIFEX_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'lucifex:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.lucifex) —
...
// an in-place `lucifex update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'lucifex-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(lucifex-media://…)`.
...
// startLucifex(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.LUCIFEX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.LUCIFEX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startLucifex() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning lucifex serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Lucifex backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[lucifex] ${line}`)
  lucifexLog.push(...lines)

  if (lucifexLog.length > 300) {
    lucifexLog.splice(0, lucifexLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-lucifex-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('lucifex:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the lucifex:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('lucifex:bootstrap:event', ev)
}
...
// The Tauri updater writes LUCIFEX_HOME/.lucifex-update-in-progress for the whole
...
// (`force_kill_other_lucifex`, taskkill /IM lucifex.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(LUCIFEX_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Lucifex will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(LUCIFEX_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `lucifex` — must not shadow `lucifex.cmd`/`lucifex.exe`.
...
function unwrapWindowsVenvLucifexCommand(command, backendArgs) {
  return resolveVenvLucifexCommand(command, backendArgs, {
...
// spawns `lucifex serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `lucifex` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'lucifex_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, LUCIFEX_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isLucifexSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'lucifex_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.LUCIFEX_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Lucifex deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Lucifex' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\lucifex\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'lucifex', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'lucifex', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal LUCIFEX_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\lucifex\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'lucifex', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'lucifex', 'git', 'bin', 'git.exe'))
  }
...
function recentLucifexLog() {
  return lucifexLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's lucifex root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_LUCIFEX_ROOT.
// LUCIFEX_DESKTOP_LUCIFEX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.LUCIFEX_DESKTOP_LUCIFEX_ROOT && path.resolve(process.env.LUCIFEX_DESKTOP_LUCIFEX_ROOT),
    !IS_PACKAGED && isLucifexSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isLucifexSourceRoot(ACTIVE_LUCIFEX_ROOT) ? ACTIVE_LUCIFEX_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_LUCIFEX_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('lucifex:updates:progress', merged)
  }
}
...
// LUCIFEX_HOME/lucifex-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_lucifex_home). That binary owns
// ALL repo mutation — running `lucifex update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'lucifex-setup.exe' : 'lucifex-setup'
  const candidate = path.join(LUCIFEX_HOME, name)
...
// Path to the venv shim whose lock decides whether `lucifex update` can write
// fresh entry points. On Windows this is the file the running backend
// `lucifex.exe` holds open; on POSIX it's never mandatory-locked.
function venvLucifexShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'lucifex.exe')
    : path.join(updateRoot, 'venv', 'bin', 'lucifex')
}
...
// only signals the direct child, so on Windows a backend `lucifex.exe` that
// spawned its own grandchildren (a `lucifex` REPL, a pty terminal session, the
...
// `lucifexProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `lucifex.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a lucifex
// REPL, a pty terminal, the gateway) keep `lucifex.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const lucifexProcess = backendConnectionState.getProcess()

  if (lucifexProcess && Number.isInteger(lucifexProcess.pid)) {
    pids.push(lucifexProcess.pid)
  }
  ...
  if (lucifexProcess && !lucifexProcess.killed) {
    try {
      lucifexProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvLucifexShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentLucifexProcess = backendConnectionState.getProcess()

    if (currentLucifexProcess && Number.isInteger(currentLucifexProcess.pid)) {
      stragglers.push(currentLucifexProcess.pid)
    }
...
// `lucifex update`). Instead we spawn the staged Lucifex-Setup binary with
// --update and quit, so it can run `lucifex update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri lucifex-setup. Unlike Windows
      ...
      // whole update itself: `lucifex update` (backend) + `lucifex desktop
    ...
    if (!updater) {
      ...
      // `lucifex desktop`, never the Tauri installer that self-copies
      // lucifex-setup.exe into LUCIFEX_HOME). They DO have a working `lucifex`
      ...
      // checkout they're on — bare `lucifex update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `lucifex update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `lucifex update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, lucifexRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Lucifex — this window will close and the updater will open. Don’t reopen Lucifex yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // lucifex.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running lucifex, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Lucifex install open ' +
        '(a second Lucifex window or a terminal running lucifex?). Close it and retry.'
    ...
    // `lucifex update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: LUCIFEX_HOME,
      env: {
        ...process.env,
        LUCIFEX_HOME,
        PATH: pathWithLucifexManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(LUCIFEX_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvLucifex = path.join(venvBin, IS_WINDOWS ? 'lucifex.exe' : 'lucifex')
  ...
  // not just the `lucifex.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvLucifex) || fileExists(path.join(updateRoot, '.lucifex-bootstrap-complete'))
...
// Resolve the lucifex CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `lucifex` on PATH.
function resolveLucifexCliBinary(updateRoot) {
  const venvLucifex = path.join(updateRoot, 'venv', 'bin', 'lucifex')

  if (fileExists(venvLucifex)) {
    return venvLucifex
  }

  return findOnPath('lucifex') || null
}
...
// macOS/Linux in-app update: backend (`lucifex update`) + OS-aware GUI rebuild
// (`lucifex desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const lucifex = resolveLucifexCliBinary(updateRoot)

  if (!lucifex) {
    emitUpdateProgress({ stage: 'manual', message: 'lucifex update', percent: null })

    return { ok: true, manual: true, command: 'lucifex update', lucifexRoot: updateRoot }
  }

  // Put the Lucifex-managed Node and the venv on PATH so `lucifex desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\lucifex\node, not node\bin.
  // PYTHONUNBUFFERED: `lucifex update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    LUCIFEX_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithLucifexManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `lucifex update` reaps stale `lucifex serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads LUCIFEX_DESKTOP_CHILD_PID and excludes
  ...
  if (lucifexProcess && Number.isInteger(lucifexProcess.pid)) {
    desktopChildPids.push(lucifexProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.LUCIFEX_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Lucifex (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(lucifex, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'lucifex update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'lucifex update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(lucifex, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Lucifex to retry.',
  ...
  // Linux in-app update terminal state (#45205). `lucifex desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Lucifex…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `lucifex-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Lucifex to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Lucifex desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Lucifex to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Lucifex.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Lucifex.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Lucifex to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.lucifex-update-new"; then
    rm -rf "$DST.lucifex-update-old" 2>/dev/null || true
    mv "$DST" "$DST.lucifex-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.lucifex-update-new" "$DST"
    rm -rf "$DST.lucifex-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Lucifex to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_LUCIFEX_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isLucifexSourceRoot(ACTIVE_LUCIFEX_ROOT) &&
    fileExists(venvPython) &&
    canImportLucifexCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_LUCIFEX_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `lucifex
...
function resolveWebDist() {
  const override = process.env.LUCIFEX_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set LUCIFEX_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: lucifex desktop --force-build`
  )
...
function resolveLucifexCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Lucifex.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.LUCIFEX_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveLucifexCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_LUCIFEX_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Lucifex at ${ACTIVE_LUCIFEX_ROOT}`,
    command,
    args: ['-m', 'lucifex_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      lucifexHome: LUCIFEX_HOME,
      pythonPathEntries: [ACTIVE_LUCIFEX_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_LUCIFEX_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveLucifexBackend(backendArgs) {
  // 1. Explicit override -- LUCIFEX_DESKTOP_LUCIFEX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.LUCIFEX_DESKTOP_LUCIFEX_ROOT && path.resolve(process.env.LUCIFEX_DESKTOP_LUCIFEX_ROOT)

  if (overrideRoot && isLucifexSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Lucifex source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `lucifex` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isLucifexSourceRoot.)
  if (!IS_PACKAGED && isLucifexSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Lucifex source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_LUCIFEX_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\lucifex\lucifex-agent (Windows) or ~/.lucifex/lucifex-agent.
  ...
  //    to spawning lucifex. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `lucifex update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `lucifex` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    LUCIFEX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.LUCIFEX_DESKTOP_IGNORE_EXISTING !== '1') {
    let lucifexCommand = null
    const lucifexOverride = process.env.LUCIFEX_DESKTOP_LUCIFEX

    if (lucifexOverride) {
      const resolvedOverride = findOnPath(lucifexOverride)

      if (resolvedOverride) {
        lucifexCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(lucifexOverride, { isWsl: IS_WSL })) {
        lucifexCommand = lucifexOverride
      } else {
        rememberLog(`Ignoring Windows Lucifex override under WSL: ${lucifexOverride}`)
      }
    } else {
      lucifexCommand = findOnPath('lucifex')
    }

    if (lucifexCommand) {
      if (looksLikeDesktopAppBinary(lucifexCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Lucifex CLI: ${lucifexCommand}`)
        lucifexCommand = null
      }
    }

    if (lucifexCommand) {
      const unwrapped = unwrapWindowsVenvLucifexCommand(lucifexCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `lucifex` shim
      ...
      const shellForProbe = isCommandScript(lucifexCommand)

      if (verifyLucifexCli(lucifexCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvLucifexCommand(lucifexCommand, backendArgs) || {
            label: `existing Lucifex CLI at ${lucifexCommand}`,
            command: lucifexCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Lucifex CLI at ${lucifexCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed lucifex_cli module via system Python.
  ...
  if (python) {
    ...
    // having lucifex_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\lucifex\lucifex-agent\venv.
    if (canImportLucifexCli(python)) {
      return {
        kind: 'python',
        label: `installed lucifex_cli module via ${python}`,
        command: python,
        args: ['-m', 'lucifex_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: lucifex_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveLucifexBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Lucifex Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_LUCIFEX_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveLucifexBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Lucifex install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Lucifex recovery was handed off to Lucifex Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      lucifexHome: LUCIFEX_HOME,
      logRoot: path.join(LUCIFEX_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Lucifex install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Lucifex bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(LUCIFEX_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startLucifex() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // lucifex:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveLucifexBackend(backend.args))
  }
  ...
  if (!isLucifexSourceRoot(ACTIVE_LUCIFEX_ROOT)) {
    throw new Error(
      `Lucifex install at ${ACTIVE_LUCIFEX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Lucifex' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\lucifex\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `lucifex` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Lucifex on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Lucifex.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isLucifexSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Lucifex venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Lucifex at ${ACTIVE_LUCIFEX_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Lucifex runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----lucifex-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Lucifex backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Lucifex backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Lucifex backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Lucifex-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('lucifex:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveLucifexCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('lucifex:preview-file-changed', payload)
}
...
async function waitForLucifex(baseUrl, token) {
  ...
  throw new Error(`Lucifex backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('lucifex:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('lucifex:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Lucifex...".
function sendPowerResume() {
  ...
  webContents.send('lucifex:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('lucifex:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('lucifex:window-state-changed', state)
}
...
//
// Hosted Lucifex gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``lucifex_session_at``),
...
const OAUTH_SESSION_PARTITION = 'persist:lucifex-remote-oauth'
...
// "not signed in", which on the initial boot path (startLucifex → the renderer's
// single-shot boot() with no retry) surfaced as the "Lucifex couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Lucifex
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Lucifex Cloud agent…' : 'Sign in to Lucifex gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Lucifex backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// hasLiveOauthSession, which looks for lucifex_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Lucifex Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Lucifex Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Lucifex Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Lucifex Cloud. Open Settings → Gateway, choose Lucifex Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Lucifex Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Lucifex Cloud session has expired. Sign in to Lucifex Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Lucifex Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root LUCIFEX_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.LUCIFEX_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.LUCIFEX_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Lucifex Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Lucifex Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by LUCIFEX_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Lucifex Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Lucifex Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (lucifex #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Lucifex gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Lucifex gateway is selected, but no session token is saved. ' +
...
//   2. env override (LUCIFEX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.LUCIFEX_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.LUCIFEX_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'LUCIFEX_DESKTOP_REMOTE_URL is set but LUCIFEX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Lucifex backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.LUCIFEX_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startLucifex() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Lucifex
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startLucifex())
  ...
  // connect to Lucifex gateway". Mirror the renderer's connect here so the test
...
function resetLucifexConnection({ soft = false } = {}) {
  backendStartFailure = null
  const lucifexProcess = backendConnectionState.invalidate()
  stopBackendChild(lucifexProcess)
...
// startLucifex() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetLucifexConnection() invalidates it.
  const lucifexProcess = backendConnectionState.getProcess()
  const dying = lucifexProcess && !lucifexProcess.killed ? lucifexProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('lucifex:connection:applied')
}
...
// profile to startLucifex() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startLucifex() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForLucifex(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: lucifexLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited LUCIFEX_HOME env (see _apply_profile_override
  // step 3 in lucifex_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveLucifexBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const lucifexCwd = resolveLucifexCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Lucifex backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: lucifexCwd,
        LUCIFEX_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        LUCIFEX_DESKTOP: '1',
        LUCIFEX_WEB_DIST: webDist,
        ...(readyFile ? { LUCIFEX_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Lucifex backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Lucifex backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Lucifex backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForLucifex(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Lucifex backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_lucifex_home() recreates the deleted profile directory.
//
...
async function startLucifex() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startLucifex() call re-throws the same error
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Lucifex backend', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Lucifex backend at ${remote.baseUrl}`, 24)
      await waitForLucifex(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Lucifex backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // resolves LUCIFEX_HOME the same way `lucifex -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Lucifex runtime', 28)
    const backend = await ensureRuntime(resolveLucifexBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const lucifexCwd = resolveLucifexCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Lucifex backend via ${backend.label}`, 84)
    rememberLog(`Starting Lucifex backend via ${backend.label}`)

    const lucifexProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: lucifexCwd,
        env: {
          ...process.env,
          // Explicitly pin LUCIFEX_HOME for the child so Python's get_lucifex_home()
          // resolves to the SAME location our resolveLucifexHome() picked. Without
          // this pin, Python falls back to ~/.lucifex on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\lucifex, which differs from C:\Users\<u>\.lucifex.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets LUCIFEX_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, lucifexProcess)

    if (!processOwner) {
      stopBackendChild(lucifexProcess)
      throw new Error('Lucifex backend start was superseded by a newer connection attempt.')
    }

    lucifexProcess.stdout.on('data', rememberLog)
    lucifexProcess.stderr.on('data', rememberLog)
    ...
    lucifexProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Lucifex backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Lucifex backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Lucifex backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Lucifex backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    lucifexProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Lucifex backend exit (${signal || code})`)
      ...
      rememberLog(`Lucifex backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Lucifex backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Lucifex backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentLucifexLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Lucifex backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(lucifexProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Lucifex backend to become ready', 90)
    await Promise.race([waitForLucifex(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => lucifexProcess.exitCode === null && !lucifexProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Lucifex backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Lucifex is
...
// pushes pet state over IPC (lucifex:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via lucifex:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Lucifex app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Lucifex icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see lucifex:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Lucifex in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('lucifex:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startLucifex().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('lucifex:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Lucifex…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('lucifex:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetLucifexConnection only
    // clears the connection promise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Lucifex backend failed liveness probe; dropping stale connection.')
    resetLucifexConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('lucifex:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('lucifex:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('lucifex:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('lucifex:window:openNewSession', async () => {
...
ipcMain.handle('lucifex:zoom:get', event => {
...
ipcMain.on('lucifex:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('lucifex:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('lucifex:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('lucifex:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('lucifex:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('lucifex:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('lucifex:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('lucifex:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('lucifex:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('lucifex:pet-overlay:control', payload)
})
ipcMain.handle('lucifex:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startLucifex() call restarts the
...
ipcMain.handle('lucifex:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startLucifex() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('lucifex:bootstrap:cancel', async () => {
...
ipcMain.handle('lucifex:boot-progress:get', async () => bootProgressState)
ipcMain.handle('lucifex:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('lucifex:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('lucifex:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('lucifex:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('lucifex:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('lucifex:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Lucifex Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('lucifex:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('lucifex:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('lucifex:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on lucifex:connection:applied.
...
ipcMain.handle('lucifex:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('lucifex:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new LUCIFEX_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('lucifex:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('lucifex:requestMicrophoneAccess', async () => {
...
ipcMain.handle('lucifex:api', async (_event, request) => {
  ...
  // backend calls ensure_lucifex_home() which recreates the profile directory,
...
ipcMain.handle('lucifex:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('lucifex:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('lucifex:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('lucifex:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('lucifex:readFileText', async (_event, filePath) => {
...
ipcMain.handle('lucifex:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('lucifex:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('lucifex:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('lucifex:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('lucifex:saveClipboardImage', async () => {
...
ipcMain.handle('lucifex:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('lucifex:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('lucifex:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('lucifex:titlebar-theme', (_event, payload) => {
...
ipcMain.on('lucifex:native-theme', (_event, mode) => {
...
ipcMain.on('lucifex:translucency', (_event, payload) => {
...
ipcMain.handle('lucifex:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('lucifex:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveLucifexCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('lucifex:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveLucifexCwd()
}))

ipcMain.handle('lucifex:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('lucifex:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('lucifex:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('lucifex:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('lucifex:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('lucifex:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: lucifexLog.slice(-200) }))
...
function terminalShellCommand() {
  // LUCIFEX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.LUCIFEX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Lucifex' TUI into
  ...
  env.TERM_PROGRAM = 'Lucifex'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a lucifex/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from LUCIFEX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.LUCIFEX_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `lucifex:terminal:${id}:${suffix}`
}
...
ipcMain.handle('lucifex:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('lucifex:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('lucifex:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('lucifex:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('lucifex:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('lucifex:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('lucifex:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('lucifex:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('lucifex:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('lucifex:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('lucifex:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('lucifex:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('lucifex:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('lucifex:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('lucifex:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('lucifex:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('lucifex:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('lucifex:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('lucifex:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('lucifex:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('lucifex:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('lucifex:terminal:write', (_event, id, data) => {
...
ipcMain.handle('lucifex:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('lucifex:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('lucifex:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('lucifex:updates:check', async () =>
...
ipcMain.handle('lucifex:updates:apply', async (_event, payload) =>
...
ipcMain.handle('lucifex:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('lucifex:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Lucifex version (the one `release.py` bumps in
// lucifex_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Lucifex version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveLucifexVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'lucifex_cli', '__init__.py')
...
// Re-resolve the live Lucifex version and push it into the native About panel
// just before showing it, so an in-place `lucifex update` is reflected without
...
ipcMain.handle('lucifex:version', async () => ({
  appVersion: resolveLucifexVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  lucifexRoot: resolveUpdateRoot()
}))
...
// the actual removal via `lucifex uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (lucifex_cli/uninstall.py + lucifex_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_LUCIFEX_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isLucifexSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'lucifex_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, LUCIFEX_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Lucifex agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import lucifex_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_LUCIFEX_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_LUCIFEX_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `lucifex-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `lucifex-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('lucifex:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('lucifex:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('lucifex:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('lucifex:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// lucifex:// deep links (e.g. lucifex://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const LUCIFEX_PROTOCOL = 'lucifex'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${LUCIFEX_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // lucifex://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('lucifex:deep-link', payload)
...
ipcMain.handle('lucifex:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${LUCIFEX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(LUCIFEX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(LUCIFEX_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `lucifex://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching lucifex:// URL is in our own argv.
