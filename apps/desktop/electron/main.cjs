...
const { canImportLUCIFEXCli, verifyLUCIFEXCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeLUCIFEXHomeRoot } = require('./backend-env.cjs')
...
function resolveLUCIFEXHome() {
  if (process.env.LUCIFEX_HOME) return normalizeLUCIFEXHomeRoot(process.env.LUCIFEX_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'LUCIFEX-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeLUCIFEXHomeRoot(fromRegistry)
  }
...
let mainWindow = null
let LUCIFEXProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by LUCIFEXProcess +
// connectionPromise + startLUCIFEX(); this pool only holds EXTRA profile
...
function unwrapWindowsVenvLUCIFEXCommand(command, backendArgs) {
  ...
  if (!/^LUCIFEX(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing LUCIFEX Python at ${python}`,
    command: python,
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (LUCIFEXProcess && Number.isInteger(LUCIFEXProcess.pid)) pids.push(LUCIFEXProcess.pid)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (LUCIFEXProcess && Number.isInteger(LUCIFEXProcess.pid)) stragglers.push(LUCIFEXProcess.pid)
    for (const entry of backendPool.values()) {
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `LUCIFEX update --branch ${branch}`
        }
      } catch {
...
function resolveLUCIFEXCliBinary(updateRoot) {
  ...
  if (fileExists(venvLUCIFEX)) return venvLUCIFEX
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('LUCIFEX install was cancelled.')
...
const ZOOM_STORAGE_KEY = 'LUCIFEX:desktop:zoomLevel'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to LUCIFEX gateway',
...
function resetLUCIFEXConnection() {
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetLUCIFEXConnection() nulls LUCIFEXProcess.
...
async function startLUCIFEX() {
  ...
  connectionPromise = (async () => {
    ...
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null LUCIFEXProcess when the child dies.
      childAlive: () => LUCIFEXProcess !== null && LUCIFEXProcess.exitCode === null && !LUCIFEXProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
function createWindow() {
  ...
  // On Windows and Linux, explicitly set the window icon via nativeImage
  // so the taskbar and window chrome use the .ico asset instead of the
  // Electron default. The icon= option in BrowserWindow is sometimes not
  // picked up reliably on dev builds; setIcon() is the authoritative call.
  if (!IS_MAC) {
    const icoPath = [
      process.resourcesPath ? path.join(process.resourcesPath, 'icon.ico') : null,
      path.join(APP_ROOT, 'assets', 'icon.ico')
    ].filter(Boolean).find(fs.existsSync)
    const winIcon = icoPath
      ? require('electron').nativeImage.createFromPath(icoPath)
      : (icon ? require('electron').nativeImage.createFromPath(icon) : null)
...
ipcMain.handle('LUCIFEX:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('LUCIFEX:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart LUCIFEX.')
  }
