...
export function markerPath(lucifexHome) {
  return path.join(lucifexHome, '.lucifex-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(lucifexHome)
  let raw
 ...
 *
 * The Tauri-based lucifex-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(lucifexHome, pid, { now = Date.now } = {}) {
  const file = markerPath(lucifexHome)
