 ...
 *     `X-Lucifex-Session-Token` header; WS uses `?token=`.
...
// lucifex_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - lucifex_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - lucifex_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / lucifex #37247). When the AT cookie
...
// Lucifex Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://lucifex.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a lucifex session ACCESS-token cookie
