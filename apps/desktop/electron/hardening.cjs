...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so LUCIFEX Desktop cannot save remote gateway tokens. ' +
        'Set LUCIFEX_DESKTOP_REMOTE_URL and LUCIFEX_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set LUCIFEX_DESKTOP_REMOTE_URL and LUCIFEX_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
