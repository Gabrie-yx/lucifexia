...
// Backend subcommand routing for the desktop-managed LUCIFEX process.
//
// The desktop app launches its own headless backend via `LUCIFEX serve` — it
...
// app hasn't updated yet, or an older `LUCIFEX` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional LUCIFEX profile to pin via `--profile`.
 */
 ...
 * `-m lucifex_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `lucifex_cli/subcommands/dashboard.py` source registers
