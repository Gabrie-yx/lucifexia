/**
 * windows-lucifex-path.ts
 *
 * Pure, dependency-injected pieces of Windows `lucifex` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvLucifexCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `lucifex` shim shadowed the real
 *      lucifex.cmd/lucifex.exe; the shim then failed the --version probe and
 ...
 *   3. resolveVenvLucifexCommand() — unwrapWindowsVenvLucifexCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `lucifex`) must
 * not shadow `lucifex.cmd`/`lucifex.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv lucifex shim, bootstrap-complete
...
export interface ResolveVenvLucifexCommandDeps {
  isWindows: boolean
...
/**
 * If `command` is a Windows venv `lucifex`/`lucifex.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/lucifex(.exe)`), resolve it to the underlying
 ...
 *
 * Returns null when `command` is not a venv lucifex shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvLucifexCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvLucifexCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^lucifex(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Lucifex at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
