// Profile-delete routing logic for the `lucifex:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_lucifex_home() and
...
/**
 * Parse a `lucifex:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `lucifex:api` request away from the primary/window backend
 ...
 * ensure_lucifex_home() recreates the directory the delete just removed.
 */
