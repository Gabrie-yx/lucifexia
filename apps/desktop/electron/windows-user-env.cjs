...
// even though a fresh shell — and the LUCIFEX CLI — sees it immediately. The
// desktop's LUCIFEX_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\LUCIFEX. Reading
...
//       LUCIFEX_HOME    REG_SZ    F:\LUCIFEX\data
//
