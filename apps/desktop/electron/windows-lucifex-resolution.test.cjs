...
// Regression guards for Windows `LUCIFEX` resolution in main.cjs.
//
...
//      Git-Bash `LUCIFEX` shim shadowed the real LUCIFEX.cmd/LUCIFEX.exe; the
...
//      --repair by checking ONLY venv\Scripts\LUCIFEX.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow LUCIFEX.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\LUCIFEX.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvLUCIFEX\) \?/,
    'recovery regressed to gating only on the LUCIFEX.exe shim, which forces destructive --repair'
  )
})
