...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'LUCIFEX-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/LUCIFEX-runner-test',
    sourceRepoRoot: null,
    LUCIFEXHome: '/tmp/LUCIFEX-runner-test',
    logRoot: '/tmp/LUCIFEX-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'LUCIFEX-agent', 'scripts')
...
test('resolveInstallScript falls back to branch download when commit download fails', async () => {
  const home = mkTmpHome()
  try {
    const commit = 'a'.repeat(40)
    ...
    const result = await resolveInstallScript({
      ...
      _download: async (ref, dest) => {
        ...
        // Success for the branch download fallback
        fs.mkdirSync(path.dirname(dest), { recursive: true })
        fs.writeFileSync(dest, '#!/bin/sh\necho branch-fallback\n')
      }
    })

    assert.equal(result.source, 'download-fallback')
    assert.equal(result.commit, 'dev-branch')
    assert.equal(result.path, cachedScriptPath(home, 'dev-branch'))
    assert.deepEqual(downloads, [commit, 'dev-branch'])
  } finally {
    fs.rmSync(home, { recursive: true, force: true })
  }
})
