 ...
 * Background
 * ----------
 * After `LUCIFEX update` + `LUCIFEX desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything LUCIFEX_DESKTOP_* is preserved
// plus LUCIFEX_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['LUCIFEX_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['LUCIFEX_DESKTOP_']
