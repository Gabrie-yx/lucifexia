 ...
 * the three user-facing uninstall modes to the `LUCIFEX uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m lucifex_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `lucifex_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'lucifex_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/LUCIFEX.app/Contents/MacOS/LUCIFEX  → …/LUCIFEX.app
 *   Windows: …\LUCIFEX\LUCIFEX.exe                 → …\LUCIFEX  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/LUCIFEX.app/Contents/MacOS/LUCIFEX → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/LUCIFEX.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs LUCIFEX.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]LUCIFEX$/i.test(dir) || /[\\/]LUCIFEX-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/LUCIFEX
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import lucifex_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, LUCIFEXHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export LUCIFEX_HOME=${q(LUCIFEXHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import lucifex_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but LUCIFEX install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "LUCIFEX_HOME=${String(LUCIFEXHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
