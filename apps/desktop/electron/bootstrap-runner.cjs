 ...
 *
 * Drives apps/desktop's first-launch install of LUCIFEX Agent by spawning
...
function bootstrapCacheDir(LUCIFEXHome) {
  return path.join(LUCIFEXHome, 'bootstrap-cache')
}
...
function installedAgentInstallScript(LUCIFEXHome) {
  if (!LUCIFEXHome) return null
  const candidate = path.join(LUCIFEXHome, 'LUCIFEX-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(LUCIFEXHome, commit) {
  return path.join(bootstrapCacheDir(LUCIFEXHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  // 0. Packaged resources shortcut: check if we packaged the script in resources
  if (process.resourcesPath) {
    const resourceScript = path.join(process.resourcesPath, 'scripts', installScriptName())
    try {
      fs.accessSync(resourceScript, fs.constants.R_OK)
      emit({ type: 'log', line: `[bootstrap] using packaged ${installScriptName()} at ${resourceScript}` })
      return { path: resourceScript, source: 'resources', kind: installScriptKind() }
    } catch {
      // not in resources, fall through
    }
  }
  ...
  const cached = cachedScriptPath(LUCIFEXHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(LUCIFEXHome)
    if (installed) {
      emit({
        type: 'log',
        line:
    ...
    // Try falling back to downloading from the branch or 'main' since this is likely a local build
    const fallbacks = []
    if (installStamp.branch && installStamp.branch !== 'master' && installStamp.branch !== 'main') {
      fallbacks.push(installStamp.branch)
    }
    fallbacks.push('main')
    ...
    for (const fallbackRef of fallbacks) {
      emit({
        type: 'log',
        line: `[bootstrap] GitHub fetch for commit ${installStamp.commit.slice(0, 12)} failed (${err.message}). ` +
              `Trying fallback download from ref '${fallbackRef}'...`
      })
      try {
        const fallbackCache = cachedScriptPath(LUCIFEXHome, fallbackRef)
        await _download(fallbackRef, fallbackCache)
        emit({ type: 'log', line: `[bootstrap] saved fallback to ${fallbackCache}` })
        return { path: fallbackCache, source: 'download-fallback', commit: fallbackRef, kind: installScriptKind() }
      } catch (fallbackErr) {
        emit({
          type: 'log',
          line: `[bootstrap] fallback download for ref '${fallbackRef}' failed: ${fallbackErr.message}`
        })
      }
    }
    throw err
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, LUCIFEXHome, sourceRepoRoot } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass LUCIFEX_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          LUCIFEX_HOME: LUCIFEXHome || process.env.LUCIFEX_HOME || '',
          LUCIFEX_SOURCE_REPO_ROOT: sourceRepoRoot || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, LUCIFEXHome, sourceRepoRoot } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, LUCIFEXHome }) {
  const args = ['--dir', activeRoot, '--LUCIFEX-home', LUCIFEXHome]
  const branch = installStamp && installStamp.branch
  if (branch && branch !== 'master' && branch !== 'HEAD') {
    args.push('--branch', branch)
  }
  // Do NOT pass --commit: the local SHA won't exist on the remote repo.
  return args
}

async function fetchManifest({ scriptPath, installerKind, emit, LUCIFEXHome, activeRoot, installStamp, sourceRepoRoot }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, LUCIFEXHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, LUCIFEXHome, activeRoot, abortSignal, installStamp, sourceRepoRoot }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, LUCIFEXHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(LUCIFEXHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, LUCIFEXHome, emit })
    const installerKind = scriptInfo.kind || 'powershell'

    let activeStamp = installStamp
    if (scriptInfo.source === 'download-fallback' && installStamp) {
      activeStamp = {
        ...installStamp,
        commit: null, // Clear the invalid commit SHA so install.ps1 doesn't fail checking it out
        branch: scriptInfo.commit || 'main' // Pin to the branch name we downloaded from
      }
      emit({
        type: 'log',
        line: `[bootstrap] using fallback stamp: branch=${activeStamp.branch}`
      })
    }
    ...
    const markerPayload = {
      pinnedCommit: activeStamp ? activeStamp.commit : null,
      pinnedBranch: activeStamp ? activeStamp.branch : null
    }
