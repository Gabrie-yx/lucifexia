...
//!
//! Mirrors `lucifex_constants.get_lucifex_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\LUCIFEX
//!   macOS:   ~/.lucifex
//!   Linux:   ~/.lucifex  (override via $LUCIFEX_HOME)
//!
//! NOTE (macOS): Python's get_lucifex_home(), scripts/install.sh, and the
//! Electron desktop's resolveLucifexHome() ALL use ~/.lucifex on macOS — there
...
/// Returns the canonical Lucifex home directory, respecting $LUCIFEX_HOME if set.
pub fn lucifex_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("LUCIFEX_HOME") {
    ...
    {
        // %LOCALAPPDATA%\LUCIFEX — matches scripts/install.ps1's $lucifex_home.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("LUCIFEX");
        }
    }

    // macOS + Linux + fallback: ~/.lucifex (matches Python get_lucifex_home(),
    // install.sh, and the Electron desktop's resolveLucifexHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".lucifex");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".lucifex")
}

pub fn log_dir() -> PathBuf {
    lucifex_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    lucifex_home().join("bootstrap-cache")
}
...
/// LUCIFEX_HOME so it survives repo checkout deletion (unlike anything under
/// lucifex-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\LUCIFEX\LUCIFEX-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_lucifex`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under LUCIFEX_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves LUCIFEX_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    lucifex_home().join(".LUCIFEX-update-in-progress")
}
...
pub fn copy_self_to_lucifex_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to LUCIFEX_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_LUCIFEX_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".LUCIFEX-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under LUCIFEX_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[lucifex-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("LUCIFEX_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_lucifex_home() -> String {
    lucifex_home().to_string_lossy().into_owned()
}
