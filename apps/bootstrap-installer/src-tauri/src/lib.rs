//! Lucifex Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `LUCIFEX-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under LUCIFEX_HOME/logs/ so install
    ...
    // Lucifex is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Lucifex installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Lucifex is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Lucifex" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Lucifex" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_lucifex_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::lucifex_home().join("lucifex-agent");
                if bootstrap::lucifex_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_lucifex_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "lucifex already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_lucifex_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_lucifex_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Lucifex Setup");
}
