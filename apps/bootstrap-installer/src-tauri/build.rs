...
fn main() {
    ...
    // local checkout hasn't pushed). Set LUCIFEX_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - LUCIFEX_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. LUCIFEX_BUILD_PIN_BRANCH, if set and non-empty.
    //   2. `git rev-parse --abbrev-ref HEAD` of the checkout this build.rs
    //      lives in â€” the current branch. (None on a detached HEAD.)
    //   3. Last-resort fallback handled below: if neither commit nor branch
    //      resolves, warn â€” the binary needs a runtime arg or dev-repo env.
    //
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=lucifex-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=lucifex-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=lucifex-bootstrap: following branch {b} HEAD (no commit pin; \
                 set LUCIFEX_BUILD_PIN_COMMIT for an immutable pin)"
            ),
        }
    }
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=lucifex-bootstrap: no pin resolved at build time; binary will fail at runtime without LUCIFEX_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // LUCIFEX_BUILD_PIN_COMMIT resolves a moving ref (tag/branch) to a SHA, a
    ...
    println!("cargo:rerun-if-env-changed=LUCIFEX_BUILD_PIN_COMMIT");
    println!("cargo:rerun-if-env-changed=LUCIFEX_BUILD_PIN_BRANCH");

    // -----------------------------------------------------------------
    // Tauri windows manifest. See lucifex-setup.manifest for rationale â€”
    // declares level="asInvoker" so Windows's installer-detection
    // heuristic doesn't refuse to launch us without UAC elevation.
    // -----------------------------------------------------------------
    #[cfg(target_os = "windows")]
    let attrs = {
        let manifest = include_str!("lucifex-setup.manifest");
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via LUCIFEX_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("LUCIFEX_BUILD_PIN_COMMIT").ok()?;
    ...
    panic!(
        "LUCIFEX_BUILD_PIN_COMMIT={requested:?} could not be resolved to a commit \
...
fn resolve_branch_pin() -> Option<String> {
    if let Ok(v) = std::env::var("LUCIFEX_BUILD_PIN_BRANCH") {
        if !v.trim().is_empty() {
    ...
    // "HEAD" is what you get on a detached checkout â€” no meaningful branch
