...
export const $lucifexHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, lucifexHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_lucifex_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $lucifexHome.set(lucifexHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Lucifex" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Lucifex-Setup.exe --update);
...
export async function launchLucifexDesktop(): Promise<void> {
  ...
  await invoke('launch_lucifex_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Lucifex repository' },
