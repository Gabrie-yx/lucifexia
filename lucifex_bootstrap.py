"""Windows UTF-8 bootstrap for Lucifex entry points.
...
should be imported at the very top of every Lucifex entry point
(``lucifex``, ``lucifex-agent``, ``lucifex-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Lucifex modules.

    Lucifex ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Lucifex source root
    ...
    """
    root = src_root or os.environ.get("LUCIFEX_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``LUCIFEX_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("LUCIFEX_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import lucifex_bootstrap``
# (or ``from lucifex_bootstrap import apply_windows_utf8_bootstrap``) at
