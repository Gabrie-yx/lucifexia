...
_lucifexCORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on lucifexDESKTOP via check_fn —
    ...
    # spawned as a kanban worker (lucifexKANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `lucifextools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (lucifexKANBAN_TASK env "
    ...
    # ==========================================================================
    # Full lucifextoolsets (CLI + messaging platforms)
    #
    ...
    # the `lucifexsend` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "lucifex-cli": {
        ...
        "tools": _lucifexCORE_TOOLS,
        "includes": []
    },

    "lucifexcron": {
        # Mirrors lucifexcli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `lucifextools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as lucifexcli; gated by `llucifexools`",
        "tools": _lucifexCORE_TOOLS,
        "includes": []
    },
    ...
    "lucifex-email": {
        "description": "Email bot toolset - interact with lucifexvia email (IMAP/SMTP)",
        "tools": _lucifexCORE_TOOLS,
        "includes": []
    },
    ...
    "lucifex-sms": {
        "description": "SMS bot toolset - interact with lucifexvia SMS (Twilio)",
        "tools": _lucifexCORE_TOOLS,
        "includes": []
    },
    ...
    "lucifex-webhook": {
        ...
        "tools": _lucifexWEBHOOK_SAFE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Platform bundles are defined as ``_lucifexCORE_TOOLS + [platform extras]``.
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        ...
        # Gives them _lucifexCORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("lucifex"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_lucifexCORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
