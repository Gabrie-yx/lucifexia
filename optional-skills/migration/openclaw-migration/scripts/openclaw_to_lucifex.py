#!/usr/bin/env python3
"""OpenClaw -> Lucifex migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Lucifex Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Lucifex.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Lucifex memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Lucifex memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Lucifex-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Lucifex-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Lucifex command_allowlist.",
    },
    "skills": {
        "label": "User skills",
    ...
    "discord-settings": {
        ...
        "description": "Import Discord bot token and allowlist into Lucifex .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Lucifex .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Lucifex .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Lucifex .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Lucifex .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Lucifex config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Lucifex config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Lucifex.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Lucifex MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Lucifex config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'lucifex cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Lucifex config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Lucifex session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Lucifex custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Lucifex config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Lucifex config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Lucifex config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Lucifex config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Lucifex in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Lucifex`` (prose), but lowercase matches
# like ``openclaw`` → ``lucifex`` (so filesystem paths like ``~/.openclaw``
# become ``~/.lucifex`` — the real Lucifex home — not the broken ``~/.Lucifex``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Lucifex'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Lucifex'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Lucifex'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Lucifex`` but maps ``openclaw`` → ``lucifex`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.lucifex/config.yaml`` (the real Lucifex home) instead of the broken
    ``~/.Lucifex/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Lucifex.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Lucifex Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-lucifex/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Lucifex config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-lucifex/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Lucifex session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Lucifex
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Lucifex-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Lucifex-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: lucifex config set",
                        )
                    continue
        ...
        for oc_key, lucifex_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and lucifex_key not in secret_additions:
                secret_additions[lucifex_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, lucifex_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and lucifex_key not in secret_additions:
                        secret_additions[lucifex_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lucifex_config = load_yaml_file(destination)
        current_model = lucifex_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = lucifex_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                lucifex_config["model"] = {"default": model_str}
            dump_yaml_file(destination, lucifex_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Lucifex still uses "edge"
        ...
        lucifex_config = load_yaml_file(destination)
        existing_tts = lucifex_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Lucifex destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Lucifex-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Lucifex-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lucifex_cfg_path = self.target_root / "config.yaml"
        lucifex_cfg = load_yaml_file(lucifex_cfg_path)
        existing_mcp = lucifex_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Lucifex config")
                continue

            lucifex_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                lucifex_srv["command"] = srv["command"]
                if srv.get("args"):
                    lucifex_srv["args"] = srv["args"]
                if srv.get("env"):
                    lucifex_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    lucifex_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                lucifex_srv["url"] = srv["url"]
                if srv.get("headers"):
                    lucifex_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    lucifex_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                lucifex_srv["enabled"] = False
            if srv.get("timeout"):
                lucifex_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                lucifex_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                lucifex_srv["tools"] = {}
                if tools_cfg.get("include"):
                    lucifex_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    lucifex_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                lucifex_srv["sampling"] = {
                    k: v for k, v in {
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lucifex_cfg_path)
            lucifex_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(lucifex_cfg_path, lucifex_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'lucifex cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = lucifex_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Lucifex reasoning_effort
        ...
        if compaction:
            compression = lucifex_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = lucifex_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            lucifex_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = lucifex_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'lucifex gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("LUCIFEX_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lucifex_cfg_path = self.target_root / "config.yaml"
        lucifex_cfg = load_yaml_file(lucifex_cfg_path)
        sr = lucifex_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lucifex_cfg_path = self.target_root / "config.yaml"
        lucifex_cfg = load_yaml_file(lucifex_cfg_path)
        custom_providers = lucifex_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lucifex_cfg_path)
            lucifex_cfg["custom_providers"] = custom_providers
            dump_yaml_file(lucifex_cfg_path, lucifex_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Lucifex config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            lucifex_cfg_path = self.target_root / "config.yaml"
            lucifex_cfg = load_yaml_file(lucifex_cfg_path)
            discord_lucifex = lucifex_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_lucifex["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_lucifex["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                lucifex_cfg["discord"] = discord_lucifex
                dump_yaml_file(lucifex_cfg_path, lucifex_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lucifex_cfg_path = self.target_root / "config.yaml"
        lucifex_cfg = load_yaml_file(lucifex_cfg_path)
        browser_lucifex = lucifex_cfg.get("browser") or {}
        changed = False

        # Map fields that have Lucifex equivalents
        if browser.get("cdpUrl"):
            browser_lucifex["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_lucifex["headless"] = browser["headless"]
            changed = True

        if changed:
            lucifex_cfg["browser"] = browser_lucifex
            if self.execute:
                self.maybe_backup(lucifex_cfg_path)
                dump_yaml_file(lucifex_cfg_path, lucifex_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            lucifex_mode = mode_map.get(mode, "manual")
            lucifex_cfg.setdefault("approvals", {})["mode"] = lucifex_mode
            if self.execute:
                self.maybe_backup(lucifex_cfg_path)
                dump_yaml_file(lucifex_cfg_path, lucifex_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{lucifex_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Lucifex Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Lucifex. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Lucifex Config Not Overwritten)",
                "",
                "These items already existed in your Lucifex config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Lucifex agent discovers these",
            "directories, it may read/write to them instead of the Lucifex state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `lucifex claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `lucifex claw cleanup` to fix it.",
            "",
            "## Lucifex-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `lucifex claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `lucifex setup` to configure any remaining settings",
            "- Run `lucifex mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `lucifex cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `lucifex cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `lucifex gateway install` if you need the gateway service",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Lucifex Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("LUCIFEX_HOME") or str(Path.home() / ".lucifex"), help="Lucifex home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Lucifex targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Lucifex-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Lucifex Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if args.execute:
        ...
        print("    2. Run: lucifex mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: lucifex cron")
