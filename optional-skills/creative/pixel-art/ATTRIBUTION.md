...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Lucifex uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Lucifex has `image_generate`
...
- License: MIT (inherits from lucifex-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Lucifex Agent contributors
