...
|--------|----------|--------|
| Metadata namespace | `openclaw` | `lucifex` (with `tags` + `homepage`) |
| Trigger | Slash commands / CLI flags | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Lucifex infra |
...
| File operations | Generic instructions | Lucifex file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Lucifex repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
