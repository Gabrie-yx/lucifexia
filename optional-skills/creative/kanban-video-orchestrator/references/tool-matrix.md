...
Maps each role archetype to the Lucifex skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public lucifex-agent
...
## Lucifex skills relevant to video production

### Visual / rendering skills (`lucifex-agent/skills/creative/`)
...
### Design / pre-production skills (`lucifex-agent/skills/creative/`)
...
### Audio / media skills (`lucifex-agent/skills/creative/` + `skills/media/`)
...
These are **not** Lucifex skills but external CLIs / APIs that profiles invoke.
...
## Built-in Lucifex tools for media review

These are native Lucifex tools — not invoked via terminal but through their own
...
key is present in `${LUCIFEX_HOME:-~/.lucifex}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Lucifex profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${LUCIFEX_HOME:-~/.lucifex}/.env` → environment variable.
...
When a new Lucifex-public video skill ships:
