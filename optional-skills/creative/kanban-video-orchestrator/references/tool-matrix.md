...
key is present in `${LUCIFEX_HOME:-~/.hermes}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${LUCIFEX_HOME:-~/.hermes}/.env` → environment variable.
