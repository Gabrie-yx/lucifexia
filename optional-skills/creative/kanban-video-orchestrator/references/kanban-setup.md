...
the actual `setup.sh` that creates the project workspace, configures Lucifex
...
2. **Create profiles** — `lucifex profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.lucifex/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `lucifex kanban create` assigned to the director
...
```bash
lucifex profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.lucifex/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.lucifex/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Lucifex
...
Each profile gets a `SOUL.md` at `~/.lucifex/profiles/<name>/SOUL.md` that
...
```bash
lucifex kanban create "Direct production of <video title>" \
    --assignee director \
...
the Lucifex `.env` (`${LUCIFEX_HOME:-$HOME/.lucifex}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _lucifex_env="${LUCIFEX_HOME:-$HOME/.lucifex}/.env"
    if grep -q "^${var}=" "$_lucifex_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_lucifex_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_lucifex_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY lucifex ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY lucifex OPENROUTER_API_KEY || exit 1
# ...
```
