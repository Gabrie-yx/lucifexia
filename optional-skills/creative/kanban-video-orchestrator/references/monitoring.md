...
lucifex kanban watch --tenant <project-slug>

# Snapshot of the board
lucifex kanban list --tenant <project-slug>
lucifex kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lucifex kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lucifex dashboard

# Inspect a specific task (includes comments + events)
lucifex kanban show <task-id>

# Follow a single task's event stream
lucifex kanban tail <task-id>
```

Verify available subcommands with `lucifex kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lucifex kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lucifex kanban block <id>` then `lucifex kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lucifex kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lucifex kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lucifex profile list` (profile exists?), `lucifex status` (gateway/dispatcher up?), and verify tenant. |
...
lucifex kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lucifex kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(lucifex kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
lucifex kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lucifex kanban block <task-id>
lucifex kanban archive <task-id>

# Diagnose what happened
lucifex kanban show <task-id>      # task body, comments, recent events
lucifex kanban tail <task-id>      # follow the live event stream
lucifex kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lucifex kanban create` for the director
...
while true; do
    clear
    lucifex kanban list --tenant <slug>
    echo "---"
    lucifex kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lucifex kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lucifex profile list` and start
