#!/usr/bin/env python3
"""Canvas LMS API CLI for Lucifex Agent.
...
def _check_config():
    ...
    if missing:
        lucifex_env = os.path.join(
            os.environ.get("LUCIFEX_HOME", os.path.expanduser("~/.lucifex")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {lucifex_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Lucifex Agent"
    )
