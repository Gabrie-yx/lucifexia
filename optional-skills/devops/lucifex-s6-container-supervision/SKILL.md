---
name: lucifex-s6-container-supervision
description: Modify, debug, or extend the s6-overlay supervision tree inside the Lucifex Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
version: 1.0.0
...
metadata:
  lucifex:
    ...
    related_skills: [lucifex-agent, lucifex-agent-dev]
---
...
│   └── dashboard/run                  ← if LUCIFEX_DASHBOARD=1, runs `lucifex dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid lucifex lucifex -p coder gateway run")
...
| `docker/s6-rc.d/main-lucifex/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main lucifex runs as the CMD, not as a supervised service. |
...
| `lucifex_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `lucifex gateway start/stop/restart` and routes to s6 when running in a container. |
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/lucifex/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to lucifex via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.
...
2. Drop to lucifex via `s6-setuidgid lucifex` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `lucifex_cli/service_manager.py`. The function is also called by `lucifex_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/lucifex_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
...
`/command/` (where s6-overlay puts its binaries) is on PATH only for processes spawned by the supervision tree — services, cont-init.d, main-wrapper.sh. `docker exec <c> s6-svstat …` will fail with "command not found"; always use the absolute path `/command/s6-svstat`. The `lucifex` binary works because the Dockerfile adds `/opt/lucifex/.venv/bin` to the runtime `ENV PATH`.

### Profile directory ownership

The cont-init reconciler runs as lucifex (`s6-setuidgid lucifex` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> lucifex profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$LUCIFEX_HOME/profiles` to lucifex on **every** boot, idempotently. Don't remove that block.
...
- `lucifex-tool-quirks`: Specific Lucifex-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with lucifex built-in tools.
