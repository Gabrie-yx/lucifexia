...
Usage (via cron with --no-agent):
    ...
    lucifex cron create lucifex-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$LUCIFEX_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name lucifex-issues --repo NousResearch/lucifex-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Lucifex .env file
(``${LUCIFEX_HOME:-~/.lucifex}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Lucifex-Watcher/1.0",
    }
