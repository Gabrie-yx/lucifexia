...
Usage (via cron with --no-agent):
    ...
    lucifex cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LUCIFEX_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Lucifex-Watcher/1.0"})
