...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${LUCIFEX_HOME:-~/.hermes}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $LUCIFEX_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name hermes-issues --repo NousResearch/lucifex-agent --scope issues
```
