...
The design is **Lucifex-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
lucifex skills install official/security/unbroker
```

Then start a new Lucifex session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Lucifex session:
...
- **PII handling.** Dossiers live under the Lucifex home directory (`0600`, optionally
