...
which is the same trust boundary Lucifex uses for .env and OAuth tokens.
"""
...
def lucifex_home() -> Path:
    return Path(os.environ.get("LUCIFEX_HOME") or (Path.home() / ".lucifex"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else lucifex_home() / "unbroker"
