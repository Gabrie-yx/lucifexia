...
The Lucifex agent orchestrates scanning and opt-out submission with native tools
...
Run it through the `terminal` tool (it can read PII files under LUCIFEX_HOME);
...
def cmd_setup(args) -> None:
    if getattr(args, "auto", False):
        ...
        # asking anyone. Read creds from $LUCIFEX_HOME/.env too (the terminal shell doesn't export
...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in $LUCIFEX_HOME/.env too
    ...
    if engaged:
        L.append(f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
                 "exposure, NOT a full-LUCIFEX_HOME read")
    ...
    if cfg["email_mode"] == "draft_only":
        ...
        L.append("  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
                 "links via the operator's logged-in webmail. This needs Lucifex pointed at the "
    ...
    if not crypto.is_engaged():
        L.append("  Storage: dossiers are PLAINTEXT JSON (0600 under LUCIFEX_HOME). "
...
def cmd_cdp(args) -> None:
    ...
    Lucifex at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Lucifex's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Lucifex's browser tools at 127.0.0.1:{port} (CDP)",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--profile",
                   help="user-data-dir (default: $LUCIFEX_HOME/chrome-debug, a dedicated debug profile)")
