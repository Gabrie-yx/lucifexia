...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LUCIFEX_HOME", os.path.expanduser("~/.lucifex")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LUCIFEX_HOME", Path.home() / ".lucifex")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Lucifex models — already uncensored, just needs clean prompt
    "lucifex": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "lucifex" in model_lower or "nous" in model_lower:
        return "lucifex"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lucifex config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.lucifex/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lucifex for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lucifex for changes to take effect.")
