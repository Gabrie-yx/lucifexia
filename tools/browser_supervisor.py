...
One ``CDPSupervisor`` runs per Lucifex ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Lucifex logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "lucifex-dialog-bridge.invalid"
...
(() => {
  if (window.__lucifexDialogBridgeInstalled) return;
  window.__lucifexDialogBridgeInstalled = true;
  const ENDPOINT = "http://lucifex-dialog-bridge.invalid/";
