...
One ``CDPSupervisor`` runs per lucifexex ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach lucifexex logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "lucifexex-dialog-bridge.invalid"
...
(() => {
  if (window.__lucifexexDialogBridgeInstalled) return;
  window.__lucifexexDialogBridgeInstalled = true;
  const ENDPOINT = "http://lucifexex-dialog-bridge.invalid/";
