"""
Cron job management tools for Lucifex Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("LUCIFEX_SESSION_PLATFORM")
    origin_chat_id = get_session_env("LUCIFEX_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            # send_message, which passes LUCIFEX_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``LUCIFEX_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via lucifex model.
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("LUCIFEX_INTERACTIVE")
        or env_var_enabled("LUCIFEX_GATEWAY_SESSION")
        or env_var_enabled("LUCIFEX_EXEC_ASK")
    )
