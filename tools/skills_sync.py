...
# Ensure repo root is in sys.path when invoked directly as a standalone script
...
# Marker file written by `lucifex profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.lucifex profile).
# When present in LUCIFEX_HOME, sync_skills() is a no-op so neither the
# installer, `lucifex update`, nor a direct sync re-injects bundled skills.
...
def _get_bundled_dir() -> Path:
    ...
    Checks LUCIFEX_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    """
    # Opt-out: a profile (named or the default ~/.lucifex) that wrote the
    ...
    for skill_name, skill_src, origin_dir in bundled_skills:
        ...
        # this skip, every `lucifex update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `lucifex skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``shutil.rmtree(~/.lucifex)`` into a loud, recoverable ``ValueError``
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `lucifex skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `lucifex update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``lucifex update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``lucifex update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``lucifex skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `lucifex skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``lucifex update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``lucifex skills opt-out`` /
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `lucifex update` (or `lucifex skills "
