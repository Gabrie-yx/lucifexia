...
from lucifex_constants import get_bundled_skills_dir, get_lucifex_home, get_optional_skills_dir
...
# Marker file written by `lucifexex profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.lucifexex profile).
# When present in LUCIFEX_HOME, sync_skills() is a no-op so neither the
# installer, `lucifexex update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# lucifex_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks lucifexex_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    """
    # Opt-out: a profile (named or the default ~/.lucifexex) that wrote the
    ...
    if (LUCIFEX_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # this skip, every `lucifexex update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `lucifexex skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``LUCIFEX_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `lucifexex skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `lucifexex update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``lucifexex update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``lucifexex update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``lucifexex skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `lucifexex skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes LUCIFEX_HOME/.no-bundled-skills so the
    installer, ``lucifexex update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``lucifexex skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = LUCIFEX_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `lucifexex update` (orlucifexifex skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (LUCIFEX_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
