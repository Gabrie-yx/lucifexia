...
Tracks per-skill usage metadata in a sidecar JSON file (~/.lucifex/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.lucifex/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.lucifex/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.lucifex/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.lucifex/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.lucifex/skills/<skill>/SKILL.md) and category-nested
    (~/.lucifex/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
