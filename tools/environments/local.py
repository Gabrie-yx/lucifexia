...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Lucifex was already launched from (for example ``lucifex-agent``
    ...
    # Common recovery for config values like ``lucifex-agent`` when Lucifex was
...
_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX = "_LUCIFEX_FORCE_"

# Lucifex-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Lucifex inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    blocked.update({
        ...
        "LUCIFEX_DASHBOARD_SESSION_TOKEN",
    ...
    # Lucifex-managed inference credential — Claude subscription auth is not a
    # working Lucifex provider path.  Stripping it broke agent-spawned
...
_LUCIFEX_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Lucifex venv path instead of the project's own
# ``.venv`` — silently clobbering the Lucifex environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Lucifex venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_lucifex_internal_secret(key: str) -> bool:
    """Return True for Lucifex-internal secrets injected under *dynamic* names.

    ``_LUCIFEX_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Lucifex-internal dynamic secret"
...
def _inject_context_lucifex_home(env: dict) -> None:
    """Bridge the context-local Lucifex home override into subprocess env."""
    try:
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (LUCIFEX_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Lucifex-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_lucifex_internal_secret(key):
            continue
        if key not in _LUCIFEX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_lucifex_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_lucifex_internal_secret(key):
            continue
        elif key not in _LUCIFEX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
# legitimate child Lucifex spawns needs them, and they are the highest-value
...
# narrow subset of _LUCIFEX_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_lucifex_internal_secret, but _ID has no secret suffix, so it must be
...
def lucifex_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_LUCIFEX_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Lucifex spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_LUCIFEX_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Lucifex-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_lucifex_internal_secret`.
    for key in list(env):
        if key.startswith(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_lucifex_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _LUCIFEX_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)
    ...
    # copies os.environ, whose LUCIFEX_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("LUCIFEX_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale LUCIFEX_GIT_BASH_PATH pointing at one) must not
    ...
    if _lucifex_portable_git:
        for candidate in (
            ...
            os.path.join(_lucifex_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # LUCIFEX_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\lucifex\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "LUCIFEX_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Lucifex Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set LUCIFEX_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Lucifex. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``lucifex`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_LUCIFEX_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_lucifex_bin_dir() -> str | None:
    """Return the directory holding the ``lucifex`` console-script, or None.
    ...
    lucifex install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``lucifex``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``lucifex`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``lucifex`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("lucifex")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``lucifex`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _LUCIFEX_BIN_DIR
    if _LUCIFEX_BIN_DIR is not _SENTINEL:
        return _LUCIFEX_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("lucifex")
    if which:
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "lucifex.exe" if _IS_WINDOWS else "lucifex"
    ...
    _LUCIFEX_BIN_DIR = candidate
    return candidate


def _prepend_lucifex_bin_dir(existing_path: str) -> str:
    """Prepend the lucifex install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_lucifex_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Lucifex
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_LUCIFEX_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_lucifex_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_lucifex_internal_secret(k):
            continue
        elif k not in _LUCIFEX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    if path_key is not None:
        ...
        # Ensure the lucifex install dir is reachable so plugins can shell out
        # to bare ``lucifex`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_lucifex_bin_dir(new_path)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Lucifex trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = Path(tempfile.gettempdir()) / "lucifex_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._lucifex_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_lucifex_pgid", None)
                    if pgid is None:
                        raise
