...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "lucifexex-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.lucifexex"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.lucifexex directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.lucifexex"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="lucifexex-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .lucifexex/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.lucifexex/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.lucifexex".lstrip("/")
        ssh_cmd = self._build_ssh_command()
