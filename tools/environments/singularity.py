...
_SNAPSHOT_STORE = get_lucifex_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "lucifexex") / "lucifex-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"lucifexex_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "lucifexex-overlays"
