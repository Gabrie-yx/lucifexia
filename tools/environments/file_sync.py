...
_FORCE_SYNC_ENV = "lucifexex_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.lucifexex") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.lucifexex to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.lucifexex", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``lucifexex_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, LUCIFEX_HOME: Path | None = None) -> None:
        ...
        Downloads the remote ``.lucifexex/`` directory as a tar archive,
        ...
        # against an uninitialized remote .lucifexex/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="lucifexex-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.lucifexex/skills/a.md`` →
        ``~/.lucifex/skills/a.md``, a new remote file at
        ``/root/.lucifexex/skills/b.md`` maps to ``~/.lucifex/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
