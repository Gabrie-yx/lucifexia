"""Base class for all lucifexex execution environment backends.
...
# lucifexex_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("lucifexex_DEBUG_INTERRUPT"))
...
def _cwd_marker(session_id: str) -> str:
    return f"__lucifexex_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all lucifexex backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/lucifexex-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/lucifexex-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"__lucifexex_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__lucifexex_fns\" ] && declare -f $lucifexifex_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("__lucifexex_ec=$?")
        # Restrict lucifexex metadata files without changing the user's command
        ...
        parts.append("exit $__lucifexex_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"lucifexex_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via lucifexex_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __lucifexex_CWD_{session}__ marker from stdout output.
