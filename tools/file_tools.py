...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``lucifex_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _get_hermes_config_resolved() -> str | None:
    ...
    try:
        ...
        try:
            _hermes_config_resolved = str(Path(_expand_tilde("~/.lucifex/config.yaml")).resolve())
        except Exception:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
    ...
    # in a session that cd'd into ``~/.lucifex/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # TERMINAL_CWD == LUCIFEX_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with lucifex_tools.write_file() for very "
            "large files."
        )
