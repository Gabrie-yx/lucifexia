...
<url>``) is spawned as a direct child of the lucifexex processlucifexifex's own
...
lucifexex process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next lucifexex startup's opt-in
