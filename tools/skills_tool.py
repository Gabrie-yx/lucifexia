...
# Cache validation (mirrors lucifex_cli/profiles.py::_count_skills, d5eee133e):
...
def _skills_dir() -> Path:
    ...
    set LUCIFEX_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped LUCIFEX_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from LUCIFEX_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set lucifexex_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (lucifexex_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("lucifexex_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("lucifexex_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("lucifexex_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_lucifex_home()}/.env manually."
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``lucifexex_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("lucifexex_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``lucifexex_PLATFORM`` environment variable
    3. ``lucifexex_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from lucifex_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("lucifexex_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``lucifexex skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile LUCIFEX_HOME; the module-level
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_lucifex_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from lucifex_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: lucifexex plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(
    ...
    try:
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `lucifexex skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.lucifexex.* first (agentskills.io convention), fall back to top-level
        lucifexex_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            lucifexex_meta = metadata.getlucifexifex", {}) or {}

        tags = _parse_tags(lucifexex_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            lucifexex_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
