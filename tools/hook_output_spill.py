...
* Never raises. Any I/O error (disk full, permission denied, missing
  LUCIFEX_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            # Last-resort fallback: LUCIFEX_HOME env var, then ~/.lucifex
            home = os.environ.get("LUCIFEX_HOME") or os.path.expanduser("~/.lucifex")
