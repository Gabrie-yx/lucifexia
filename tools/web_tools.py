...
Backend is selected during ``lucifex tools`` setup (web.backend in config.yaml).
When available, Lucifex can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``lucifex tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `lucifex tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the Lucifex portal on every CLI startup (invoked at
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `lucifex plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `lucifex tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`lucifex plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
