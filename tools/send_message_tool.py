...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``lucifex send`` (and the send_message
    ...
    ``lucifex send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: lucifex config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("lucifex_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("lucifex_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("lucifex_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("lucifex_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``lucifex send`` CLI).
    """
    media_files = media_files or []
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``lucifex_KANBAN_TASK``
...
#   - the ``lucifex send`` CLI command (lucifex_cli/send_cmd.py)
