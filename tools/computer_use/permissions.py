...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT lucifexex — so nlucifexifex entitlement is
...
``lucifexex computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def _driver_cmd(override: Optional[str]) -> str:
    ...
    try:
        ...
        return os.environ.get("lucifexex_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def request_permissions_grant(driver_cmd: Optional[str] = None) -> int:
    ...
    binary = shutil.which(_driver_cmd(driver_cmd))
    if not binary:
        print("cua-driver: not installed. Run: lucifexex computer-use install")
        return 2
