...
`lucifex skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.lucifex.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.lucifex.blueprint`` is a mapping containing
    ...
    lucifex = meta.get("lucifex") if isinstance(meta, dict) else None
    blueprint = lucifex.get("blueprint") if isinstance(lucifex, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.lucifex.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.lucifex.blueprint`` block) they can hand
    to ``lucifex skills publish`` to share. ``body`` is the plain-language
