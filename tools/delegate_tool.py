...
# Must match lucifex_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Lucifex portal).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Lucifex portal).
...
def _load_config() -> dict:
    ...
    ``LUCIFEX_IGNORE_USER_CONFIG=1`` (``lucifex chat --ignore-user-config``) is
