#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the lucifexex desktop GUI.
...
``read_terminal`` it is gated on ``lucifexex_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — lucifexex_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("lucifexex_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the lucifexex desktop GUI (the tabs mirroring terminal(background=true) runs). "
