...
Used by lucifex_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _lucifex_home() / "skills"
...
def _load_lucifex_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    lucifex_index_cache_file = _lucifex_index_cache_file()
    ...
    try:
        resp = httpx.get(LUCIFEX_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
