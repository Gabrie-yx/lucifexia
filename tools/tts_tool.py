...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.lucifex/config.yaml``. Hermes
...
Configuration is loaded from ~/.lucifex/config.yaml under the 'tts:' key.
...
# Base URL now resolved via lucifex_cli.models.deepinfra_base_url (shared).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.lucifex/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.lucifex/config.yaml.
    ...
    try:
        ...
        logger.debug("lucifex_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# file, so retired models disappear from lucifex the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``lucifex_cli.models`` helpers so every DeepInfra surface
    ...
    from lucifex_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.lucifex/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'lucifex-agent[mistral]'"
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_lucifex_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
