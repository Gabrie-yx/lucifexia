"""
Lazy dependency installer for opt-in Lucifex Agent backends.

Many Lucifex features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
remediation hint pointing at ``lucifex tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``LUCIFEX_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/lucifex`` read-only), setting
  ``LUCIFEX_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Lucifex, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (LUCIFEX_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (lucifex trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "LUCIFEX_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``LUCIFEX_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("LUCIFEX_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``lucifex update`` lazy-refresh pass.
    """
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``lucifex update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="lucifex-core-constraints-", suffix=".txt")
...
def active_features() -> list[str]:
    ...
    Used by ``lucifex update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``lucifex update``. Never raises; lazy-install failures
