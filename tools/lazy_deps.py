"""
Lazy dependency installer for opt-in lucifexex Agent backends.

Many lucifexex features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``lucifex-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``lucifexex tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``lucifexex_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/lucifexex`` read-only), setting
  ``lucifexex_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick lucifexex, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (lucifexex_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (lucifexex trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "lucifexex_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``lucifexex_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("lucifexex_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``lucifexex update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Lucifex under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``lucifexex update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="lucifexex-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``lucifex_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = lucifex_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``lucifexex update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``lucifexex update``. Never raises; lazy-install failures
