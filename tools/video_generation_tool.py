...
provider** — turn on a backend by enabling a plugin (``lucifex plugins
enable video_gen/<name>``) and selecting it in ``lucifex tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `lucifex tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `lucifex tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `lucifex tools` → "
...
# `lucifex tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`lucifex tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `lucifex tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
