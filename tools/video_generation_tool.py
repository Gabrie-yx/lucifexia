...
provider** — turn on a backend by enabling a plugin (``lucifexex plugins
enable video_gen/<name>``) and selecting it in ``lucifexex tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/lucifex-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `lucifexex tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `lucifexex plugins list` to see "
            f"installed video gen backends, or `lucifexex tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `lucifexex tools` → "
...
# `lucifexex tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`lucifexex tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `lucifexex tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
