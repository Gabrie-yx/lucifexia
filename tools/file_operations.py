...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__lucifexex_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__lucifexex_FENCE_" in line or "\x1b]" in line
...
class ShellFileOperations(FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.lucifexex-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".lucifexex-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.lucifexex-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.lucifexex-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.lucifexex-tmp``
