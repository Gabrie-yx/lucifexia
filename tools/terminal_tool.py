...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or lucifexex exit
...
# display_lucifex_home imported lazily at call site (stale-module safety during lucifexex update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of lucifexex directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "lucifexex-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("lucifexex_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("lucifexex_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("lucifexex_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (lucifexex_INTERACTIVE=1).
    ...
    try:
        os.environ["lucifexex_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "lucifexex_SPINNER_PAUSE" in os.environ:
            del os.environ["lucifexex_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (lucifexex_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive lucifexex password prompt or the sudo -S password-pipe path.
    ...
    # silently without lucifexex prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    has_sudo_prompt_callback = _get_sudo_password_callback() is not None
    should_prompt_for_sudo = (
        env_var_enabled("lucifexex_INTERACTIVE") or has_sudo_prompt_callback
    )
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so lucifexex can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    by lucifexex processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      lucifexex processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling lucifexex processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, lucifexexSweEnv, ...)
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the lucifexex host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    lucifexex host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``lucifexex
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for lucifexex-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior lucifexex processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so lucifexex can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/lucifexex
        ...
        # never restart. This mirrors the `lucifexex gateway restart` guard in
        # lucifex_cli/gateway.py and the cron-path guard in lucifex_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_lucifexex_GATEWAY") == "1":
            ...
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `lucifexex gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `lucifexex -z` or a "
                        ...
                        _gw_platform = _gse("lucifexex_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("lucifexex_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("lucifexex_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("lucifexex_SESSION_USER_ID", "")
                            _gw_user_name = _gse("lucifexex_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("lucifexex_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("lucifexex_INTERACTIVE"):
                    output += (
