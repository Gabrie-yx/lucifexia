...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("LUCIFEX_YOLO_MODE", ""))
...
# os.environ["LUCIFEX_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export LUCIFEX_INTERACTIVE.
_lucifex_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "lucifex_interactive",
    default=None,
)


def set_lucifex_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["LUCIFEX_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``LUCIFEX_INTERACTIVE`` env var for legacy callers.
    """
    return _lucifex_interactive_ctx.set("1" if interactive else "")


def reset_lucifex_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_lucifex_interactive_context`."""
    _lucifex_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``LUCIFEX_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _lucifex_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("LUCIFEX_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("LUCIFEX_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("LUCIFEX_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("LUCIFEX_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set LUCIFEX_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind LUCIFEX_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds LUCIFEX_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("LUCIFEX_CRON_SESSION"):
        return False
    if env_var_enabled("LUCIFEX_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $LUCIFEX_HOME, or by the resolved absolute
# active profile home path such as /home/lucifex/.lucifex/config.yaml. The
...
# theater. Mirrors _LUCIFEX_ENV_PATH; matches the LUCIFEX_HOME override form as
# well as ~/.lucifex/.
_LUCIFEX_CONFIG_PATH = (
    r'(?:~\/\.lucifex/|'
    r'(?:\$home|\$\{home\})/\.lucifex/|'
    r'(?:\$lucifex_home|\$\{lucifex_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_LUCIFEX_ENV_PATH}|'
    rf'{_LUCIFEX_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `lucifex` and `gateway` (e.g. `lucifex -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\blucifex\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart lucifex gateway (kills running agents)"),
    (r'\blucifex\s+update\b', "lucifex update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `lucifex gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart lucifex-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart lucifex-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(lucifex|gateway|cli\.py)\b', "kill lucifex/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill lucifex` but not
    # `kill -9 $(pgrep -f lucifex)` because the substitution is opaque
    ...
    # the `lucifex gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.lucifex.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(lucifex|ai\.lucifex)\b', "stop/restart lucifex launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Lucifex file.
    ...
    # In-place edit of a Lucifex-managed security file (~/.lucifex/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_LUCIFEX_CONFIG_PATH}|{_LUCIFEX_ENV_PATH})', "in-place edit of Lucifex config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_LUCIFEX_CONFIG_PATH}|{_LUCIFEX_ENV_PATH})', "in-place edit of Lucifex config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_LUCIFEX_CONFIG_PATH}|{_LUCIFEX_ENV_PATH})', "in-place edit of Lucifex config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    #
    # Fold the (more specific) Lucifex home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\lucifex), so folding the user home
    # first would eat the prefix the Lucifex-home fold needs.
    command = _rewrite_resolved_lucifex_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / LUCIFEX_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.lucifex``); the matched
...
def _rewrite_resolved_lucifex_home(command: str) -> str:
    """Rewrite the resolved absolute Lucifex home prefix to ``~/.lucifex/``.
    ...
    ``~/.lucifex/`` so the static ``_LUCIFEX_CONFIG_PATH`` / ``_LUCIFEX_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/lucifex/.lucifex/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$LUCIFEX_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from lucifex_constants import get_lucifex_home
        home = get_lucifex_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.lucifex")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Lucifex ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"lucifex-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Lucifex approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``LUCIFEX_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            LUCIFEX_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("LUCIFEX_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set LUCIFEX_INTERACTIVE or "
                "LUCIFEX_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set LUCIFEX_INTERACTIVE or "
            "LUCIFEX_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("LUCIFEX_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``LUCIFEX_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("LUCIFEX_EXEC_ASK")
