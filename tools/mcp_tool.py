...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach lucifex' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            LUCIFEX_HOME = os.path.expanduser(
                os.getenv(
                    "LUCIFEX_HOME", os.path.join(os.path.expanduser("~"), ".lucifex")
                )
            )
            candidates = [
                ...
                # image (which the lucifex Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → lucifex MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that lucifex gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the lucifex document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through lucifex' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (lucifex_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit lucifex_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into lucifex' logging (agent.log via lucifex_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this lucifex process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through lucifex' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until lucifex is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `lucifex mcp login {server_name}` (or delete the tokens "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the lucifex config file.
    ...
    try:
        ...
        if _env_enabled("lucifex_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through lucifex' MEDIA: tag convention and out to messaging
            ...
            # lucifex' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves lucifex's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. lucifex uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the lucifex registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``lucifex tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the lucifex registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple lucifex
